...
-- Ejemplo de Ejecucion:
        SELECT * FROM [FERCO].[OP_WMS_VIEW_CONSOLIDATE_QA]
*/
-- =============================================
CREATE VIEW [FERCO].[OP_WMS_VIEW_CONSOLIDATE_QA01042022]
AS
(SELECT
		[IL].[MATERIAL_ID]
		,[M].[MATERIAL_NAME]
		...
		,[L].[CURRENT_WAREHOUSE]
		,[W].[ERP_WAREHOUSE]
		,[L].[WAVE_PICKING_ID] AS WAVE_PICKING
		--,[L].[LICENSE_ID]
	FROM
		...
		LEFT JOIN [FERCO].[OP_WMS_TASK_LIST] [#TL] ON ([L].[WAVE_PICKING_ID] = [#TL].[WAVE_PICKING_ID] )
	...
	WHERE
		...
		AND S.SPOT_TYPE <> 'PUERTA'
		--AND [L].[WAVE_PICKING_ID] NOT IN (SELECT DISTINCT WAVE_PICKING_ID FROM FERCO.OP_WMS_PASS_DETAIL)
