...
-- Ejemplo de Ejecucion:
		--
		SELECT * FROM [FERCO].[OP_WMS_VIEW_INVENTORY_DETAIL_WHITH_SERIES]
		--      
*/
-- =============================================
CREATE VIEW [FERCO].[OP_WMS_VIEW_INVENTORY_DETAIL_WHITH_SERIES_GG]
AS
SELECT [C].[CLIENT_NAME],
       ...
       [M].[MATERIAL_NAME],
       CASE
           WHEN [SN].[SERIAL] IS NOT NULL THEN
               1
           ELSE
               [I].[QTY]
       END [QTY],
       ...
       [SN].[SERIAL_PREFIX] + CAST([SN].[SERIAL_CORRELATIVE] AS VARCHAR(50)) [SERIAL_NUMBER],
       [L].[CURRENT_WAREHOUSE],
       [PH].[DOC_ID],
       [L].[USED_MT2],
       [I].[VIN],
       ...
       [L].[WAVE_PICKING_ID],
       [M].[ERP_AVERAGE_PRICE]
FROM [FERCO].[OP_WMS_INV_X_LICENSE] AS [I]
    INNER JOIN [FERCO].[OP_WMS_LICENSES] AS [L]
        ON [I].[LICENSE_ID] = [L].[LICENSE_ID]
    INNER JOIN [FERCO].[OP_WMS_MATERIALS] AS [M]
        ON (
               [I].[MATERIAL_ID] = [M].[MATERIAL_ID]
               AND [M].[MATERIAL_ID] > ''
           )
    INNER JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] AS [C]
    ...
    LEFT JOIN [FERCO].[OP_WMS_SHELF_SPOTS] [SH]
        ON (
               [L].[CURRENT_LOCATION] = [SH].[LOCATION_SPOT]
               AND [SH].[LOCATION_SPOT] > ''
           )
    LEFT JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [SN]
        ON (
    ...
    LEFT JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [S]
        ON (
    ...
    LEFT JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TC]
        ON (
		   ...
		   WHERE [I].[LICENSE_ID]=2000436;

		   --SELECT * FROM [FERCO].[OP_WMS_VIEW_INVENTORY_DETAIL_WHITH_SERIES_gg]
