...
*/
-- =============================================
CREATE VIEW [FERCO].[OP_WMS_VIEW_GET_CONSOLIDATE_WMS_INVENTORY_STATUS_BY_WH]
AS
(SELECT 
MATERIAL_ID 
,MATERIAL_NAME
...
FROM (
	SELECT 
		...
		,SUM(CASE 
				...
				WHEN [IL].[LOCKED_BY_INTERFACES] = 1 AND TL.TASK_TYPE = 'TAREA_RECEPCION' AND M.IS_MASTER_PACK = 1 THEN (MD.QTY * [IL].[QTY])
				ELSE 0
		...
		,ISNULL(MAX([PWT].[RESERVADO]),0) [PICKED_PENDING_ERP_WT]
		,[L].[CURRENT_WAREHOUSE]
		,[W].[ERP_WAREHOUSE]
	FROM
