...
-- alberto.ruiz
-- Se agregaron los ajuste de Rudi en nave

/*
-- Ejemplo de Ejecucion:
			SELECT * FROM [FERCO].OP_WMS_VIEW_INVENTORY_DETAIL_ALMGEN
*/
-- =============================================
CREATE VIEW FERCO.OP_WMS_VIEW_INVENTORY_DETAIL_ALMGEN
AS
/*SELECT     (SELECT     CLIENT_NAME
                      ...
                      B.CURRENT_LOCATION,
                          (SELECT     TOP (1) DOC_ID
                            FROM          [FERCO].OP_WMS_POLIZA_HEADER AS Z
...
FROM         [FERCO].OP_WMS_INV_X_LICENSE AS A INNER JOINñ
                      [FERCO].OP_WMS_LICENSES AS B ON A.LICENSE_ID = B.LICENSE_ID INNER JOIN
                      [FERCO].OP_WMS_MATERIALS AS C ON A.MATERIAL_ID = C.MATERIAL_ID
...
INNER JOIN [FERCO].[OP_WMS_MATERIALS] AS [M] WITH (INDEX (IN_OP_WMS_MATERIALS_MATERIAL_ID))
