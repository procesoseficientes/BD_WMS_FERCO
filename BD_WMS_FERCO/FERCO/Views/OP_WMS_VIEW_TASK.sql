...
/*
	Ejemplo Ejecucion: 
    SELECT * FROM [FERCO].[OP_WMS_VIEW_TASK] where wave_picking_id = 4386
 */
-- =============================================
CREATE VIEW [FERCO].[OP_WMS_VIEW_TASK]
AS
SELECT
	[WAVE_PICKING_ID]
	,[A].[CLIENT_NAME]
...
LEFT JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PHS] ON [PHS].[DOC_ID] = [A].[DOC_ID_SOURCE]
											AND [PHS].[WAREHOUSE_REGIMEN] = [A].[REGIMEN]
LEFT JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PHT] ON [PHT].[DOC_ID] = [A].[DOC_ID_TARGET]
