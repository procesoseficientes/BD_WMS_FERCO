...
-- Ejemplo de Ejecucion:
		SELECT * FROM [FERCO].OP_WMS_VIEW_INVENTORY_GENERAL
*/
-- =============================================
CREATE VIEW [FERCO].[OP_WMS_VIEW_INVENTORY_GENERAL]
AS
SELECT
	[inv].[MATERIAL_ID]
...
FROM
	[FERCO].[OP_WMS_INV_X_LICENSE] AS [inv]
INNER JOIN [FERCO].[OP_WMS_MATERIALS] AS [mt] ON [inv].[MATERIAL_ID] = [mt].[MATERIAL_ID]
INNER JOIN [FERCO].[OP_WMS_LICENSES] AS [li] ON [inv].[LICENSE_ID] = [li].[LICENSE_ID]
INNER JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] AS [cl] ON [li].[CLIENT_OWNER] = [cl].[CLIENT_CODE] COLLATE DATABASE_DEFAULT
LEFT JOIN [FERCO].[OP_WMS_FN_GET_COMMITED_INVENTORY_BY_LICENCE]() [CI] ON [CI].[LICENCE_ID] = [inv].[LICENSE_ID]
											AND [CI].[MATERIAL_ID] = [inv].[MATERIAL_ID]
											AND [li].[CLIENT_OWNER] = [CI].[CLIENT_OWNER]
INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [owph] ON [li].[CODIGO_POLIZA] = [owph].[CODIGO_POLIZA]
INNER JOIN [FERCO].[OP_WMS_TARIFICADOR_HEADER] [owth] ON [owph].[ACUERDO_COMERCIAL] = [owth].[ACUERDO_COMERCIAL_ID]
INNER JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [SML] ON [SML].[STATUS_ID] = [inv].[STATUS_ID]
WHERE
	([li].[REGIMEN] = 'GENERAL')
