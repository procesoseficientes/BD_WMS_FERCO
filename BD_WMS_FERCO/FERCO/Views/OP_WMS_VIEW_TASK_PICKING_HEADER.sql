...
/*
	Ejemplo Ejecucion: 
    SELECT * FROM [FERCO].OP_WMS_VIEW_TASK_PICKING_HEADER
 */
-- =============================================
CREATE VIEW [FERCO].[OP_WMS_VIEW_TASK_PICKING_HEADER]
AS
SELECT
...
LEFT OUTER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PHS] ON [PHS].[DOC_ID] = [A].[DOC_ID_SOURCE]
											AND [PHS].[WAREHOUSE_REGIMEN] = [A].[REGIMEN]
LEFT OUTER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PHT] ON [PHT].[DOC_ID] = [A].[DOC_ID_TARGET]
