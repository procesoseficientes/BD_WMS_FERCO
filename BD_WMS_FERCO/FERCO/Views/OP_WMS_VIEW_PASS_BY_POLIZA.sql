...
CREATE VIEW FERCO.OP_WMS_VIEW_PASS_BY_POLIZA
AS
SELECT
  P.CODIGO_POLIZA
 ,M.MATERIAL_ID
 ...
 ,(SELECT TOP 1
      [VUA].[VALOR_UNITARIO]
    FROM [FERCO].[OP_WMS_FUNC_GET_SKU_VALOR_UNITARIO_ALMGEN]([PH].[DOC_ID], '%' + [M].[BARCODE_ID] + '%') VUA)
  AS UNIT_VALUE
 ,(SELECT TOP 1
      [VUA].[VALOR_UNITARIO] * (T.QUANTITY_UNITS * -1)
    FROM [FERCO].[OP_WMS_FUNC_GET_SKU_VALOR_UNITARIO_ALMGEN]([PH].[DOC_ID], '%' + [M].[BARCODE_ID] + '%') VUA)
  AS TOTAL_LINE
...
INNER JOIN [FERCO].OP_WMS3PL_POLIZA_X_PASS PS
  ON (
