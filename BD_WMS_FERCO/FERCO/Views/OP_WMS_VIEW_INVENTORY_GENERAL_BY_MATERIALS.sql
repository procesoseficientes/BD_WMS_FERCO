...
-- Ejemplo de Ejecucion:
		SELECT * FROM [FERCO].OP_WMS_VIEW_INVENTORY_GENERAL_BY_MATERIALS 
*/
-- =============================================
CREATE VIEW [FERCO].[OP_WMS_VIEW_INVENTORY_GENERAL_BY_MATERIALS]
AS
SELECT
	[li].[LICENSE_ID]
	,[inv].[MATERIAL_ID]
...
LEFT JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MS] ON [inv].[LICENSE_ID] = [MS].[LICENSE_ID]
											AND [inv].[MATERIAL_ID] = [MS].[MATERIAL_ID]
											AND [MS].[STATUS] > 0
LEFT JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCM] ON ([TCM].[TONE_AND_CALIBER_ID] = [inv].[TONE_AND_CALIBER_ID])
WHERE
	([li].[REGIMEN] = 'GENERAL')
	AND [inv].[QTY] > 0
