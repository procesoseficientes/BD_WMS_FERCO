...
WHERE     (emp = 1)*/
CREATE VIEW FERCO.OP_WMS_VIEW_CUSTOMER_TERMS_OF_TRADE
AS
--SELECT	(SELECT TOP (1)CLIENT_CODE
...
SELECT
  (SELECT TOP (1)
    ...
    WHERE (CLIENT_CODE = a.CLIENT_ID COLLATE DATABASE_DEFAULT))
  AS CLIENT_CODE
...
FROM [FERCO].OP_WMS_ACUERDOS_X_CLIENTE AS a
INNER JOIN [FERCO].OP_WMS_TARIFICADOR_HEADER t
  ON a.ACUERDO_COMERCIAL = t.ACUERDO_COMERCIAL_ID
