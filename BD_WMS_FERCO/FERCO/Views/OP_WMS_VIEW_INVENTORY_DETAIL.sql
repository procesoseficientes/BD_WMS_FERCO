...
-- Ejemplo de Ejecucion:
			select count(*) from [FERCO].OP_WMS_VIEW_INVENTORY_DETAIL
*/
-- =============================================

CREATE VIEW [FERCO].[OP_WMS_VIEW_INVENTORY_DETAIL]
AS
SELECT
	(SELECT TOP 1
			[CLIENT_NAME]
		FROM
			[FERCO].[OP_WMS_VIEW_CLIENTS]
		WHERE
	...
	,ISNULL((SELECT TOP (1)
					[NUMERO_ORDEN]
				FROM
					[FERCO].[OP_WMS_POLIZA_HEADER] AS [C]
				WHERE
  ...
  --Cambio
	,(SELECT TOP (1)
			[DOC_ID]
		FROM
			[FERCO].[OP_WMS_POLIZA_HEADER] AS [Z]
		WHERE
...
FROM
	[FERCO].[OP_WMS_INV_X_LICENSE] AS [A]
INNER JOIN [FERCO].[OP_WMS_LICENSES] AS [B] ON [A].[LICENSE_ID] = [B].[LICENSE_ID]
INNER JOIN [FERCO].[OP_WMS_MATERIALS] AS [C] ON [A].[MATERIAL_ID] = [C].[MATERIAL_ID]
...
LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RDH] ON (
...
LEFT JOIN [FERCO].[OP_WMS_PROJECT] [P] ON [A].[PROJECT_ID] = [P].[ID];										;