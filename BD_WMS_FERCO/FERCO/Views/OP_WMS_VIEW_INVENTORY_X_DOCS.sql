...
-- Ejemplo de Ejecucion:
        SELECT * FROM [FERCO].[OP_WMS_VIEW_INVENTORY_X_DOCS]
*/
-- =============================================
CREATE VIEW FERCO.OP_WMS_VIEW_INVENTORY_X_DOCS
AS
SELECT
  [PH].[NUMERO_ORDEN]
...
LEFT JOIN [FERCO].[OP_WMS_POLIZA_DETAIL] AS [PD]
  ON ([PH].[DOC_ID] = [PD].[DOC_ID])
LEFT JOIN [FERCO].[fn_saldo_linea_poliza]() AS [FN]
  ON (
      [FN].[DOC_ID] = [PD].[DOC_ID]
      AND [FN].[LINE_NUMBER] = [PD].[LINE_NUMBER]
    )
LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] AS [VC]
  ON ([VC].[CLIENT_CODE] = [PD].[CLIENT_CODE])
