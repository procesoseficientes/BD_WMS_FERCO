...
-- Ejemplo de Ejecucion:
        SELECT * FROM [FERCO].OP_WMS_VIEW_PICKING_TASK_BY_POLIZA
*/
-- =============================================
CREATE VIEW FERCO.OP_WMS_VIEW_PICKING_TASK_BY_POLIZA
AS
SELECT
  A.WAVE_PICKING_ID
 ,TASK_SUBTYPE
 ...
 ,(SELECT TOP (1)
    ...
    WHERE (CODIGO_POLIZA = (A.CODIGO_POLIZA_SOURCE))
    AND (WAREHOUSE_REGIMEN = (A.REGIMEN)))
  AS NUMERO_ORDEN_SOURCE
 ,(SELECT TOP (1)
      NUMERO_ORDEN
    FROM [FERCO].OP_WMS_POLIZA_HEADER AS B
    WHERE (CODIGO_POLIZA = (A.CODIGO_POLIZA_TARGET))
    AND (WAREHOUSE_REGIMEN = (A.REGIMEN)))
...
INNER JOIN [FERCO].[OP_WMS_TRANS] T
  ON (A.CODIGO_POLIZA_TARGET = T.CODIGO_POLIZA
