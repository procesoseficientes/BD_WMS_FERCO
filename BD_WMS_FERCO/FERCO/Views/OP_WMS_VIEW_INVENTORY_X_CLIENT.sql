...
-- Ejemplo de Ejecucion:
			SELECT COUNT(*) FROM [FERCO].OP_WMS_VIEW_INVENTORY_X_CLIENT
*/
-- =============================================

CREATE VIEW [FERCO].[OP_WMS_VIEW_INVENTORY_X_CLIENT]
AS
SELECT
	[B].[CLIENT_OWNER]
	,(SELECT
		...
		WHERE
			([CLIENT_CODE] = [B].[CLIENT_OWNER])) AS [CLIENT_NAME]
	,[B].[REGIMEN]
	,[B].[CURRENT_WAREHOUSE]
	,[C].[MATERIAL_ID]
	,[C].[BARCODE_ID]
...
INNER JOIN [FERCO].[OP_WMS_LICENSES] AS [B] ON (
