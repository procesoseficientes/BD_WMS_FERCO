...
*/
-- =============================================
CREATE VIEW FERCO.OP_WMS_VIEW_PICKING_TASK
AS
SELECT
  [WAVE_PICKING_ID]
 ,[TASK_TYPE]
 ...
 ,(SELECT TOP (1)
      [NUMERO_ORDEN]
    FROM [FERCO].[OP_WMS_POLIZA_HEADER] AS [B]
    WHERE ([CODIGO_POLIZA] = MAX([A].[CODIGO_POLIZA_SOURCE]))
    AND ([WAREHOUSE_REGIMEN] = MAX([A].[REGIMEN])))
  AS [NUMERO_ORDEN_SOURCE]
 ,(SELECT TOP (1)
      [NUMERO_ORDEN]
    FROM [FERCO].[OP_WMS_POLIZA_HEADER] AS [B]
    WHERE ([CODIGO_POLIZA] = MAX([A].[CODIGO_POLIZA_TARGET]))
    AND ([WAREHOUSE_REGIMEN] = MAX([A].[REGIMEN])))
...
FROM [FERCO].[OP_WMS_TASK_LIST] AS [A]
