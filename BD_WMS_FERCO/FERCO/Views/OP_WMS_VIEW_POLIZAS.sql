...
CREATE VIEW FERCO.OP_WMS_VIEW_POLIZAS
AS
SELECT
  PH.DOC_ID
 ,NUMERO_ORDEN
...
INNER JOIN [FERCO].OP_WMS_SAP_CLIENTS SP
  ON (PH.CLIENT_CODE = SP.CardCode COLLATE DATABASE_DEFAULT)
LEFT JOIN [FERCO].OP_WMS_TARIFICADOR_HEADER TH
  ON ((CASE
      WHEN PH.ACUERDO_COMERCIAL = '' THEN 0
      ELSE PH.ACUERDO_COMERCIAL
    END) = TH.ACUERDO_COMERCIAL_ID)
LEFT JOIN [FERCO].OP_WMS_VIEW_INSURANCE_DOC ID
  ON (PH.POLIZA_ASEGURADA = (CASE
      WHEN ID.DOC_ID = 0 THEN ID.POLIZA_INSURANCE
