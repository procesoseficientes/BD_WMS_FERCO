...
-- Ejemplo de Ejecucion:
	SELECT COUNT(*) FROM 		[FERCO].[OP_WMS_VIEW_VALORIZACION_FISCAL]
*/
-- =============================================

CREATE VIEW FERCO.OP_WMS_VIEW_VALORIZACION_FISCAL
AS
SELECT
  CLIENT_NAME
 ,CLIENT_OWNER
 ...
 ,ISNULL
  ((SELECT
      VALOR_UNITARIO
    FROM FERCO.OP_WMS_FUNC_GET_SKU_VALOR_UNITARIO(A.CODIGO_POLIZA, '%' + A.BARCODE_ID + '%')
 ...
 ,[A].[TONE]
 ,[A].[CALIBER]
FROM FERCO.OP_WMS_VIEW_INVENTORY_DETAIL_FISCAL AS A
WHERE (QTY > 0)
