...
-- descripcion:			obtiene el inventario listo para picking por material, lote y fecha expiracion y estado

/*
-- Ejemplo de Ejecucion:
			SELECT  * FROM [FERCO].[OP_WMS_VIEW_INVENTORY_FOR_PICKING_BY_STATUS_MATERIAL_BY_BATCH]  
*/
-- =============================================
CREATE VIEW [FERCO].[OP_WMS_VIEW_INVENTORY_FOR_PICKING_BY_STATUS_MATERIAL_BY_BATCH]
AS
SELECT
	[M].[MATERIAL_ID]
	,[M].[MATERIAL_NAME]
...
INNER JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [CL] ON CAST([L].[CLIENT_OWNER] COLLATE DATABASE_DEFAULT AS NVARCHAR(15)) = [CL].[CLIENT_CODE]
...
INNER JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [C] ON [C].[PARAM_TYPE] = 'PRODUCTOS'
											AND [C].[PARAM_GROUP] = 'BLOQUEO_EXPIRACION'
											AND [C].[PARAM_NAME] = 'BLOQUEO_DIAS_PRONTA_EXPIRACION'
LEFT OUTER JOIN [FERCO].[OP_WMS_FN_GET_COMMITED_INVENTORY_BY_LICENCE]() [CI] ON [CI].[LICENCE_ID] = [IL].[LICENSE_ID]
