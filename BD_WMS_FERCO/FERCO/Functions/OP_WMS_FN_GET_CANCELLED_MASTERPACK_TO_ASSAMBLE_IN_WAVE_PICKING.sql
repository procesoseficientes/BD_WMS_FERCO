...
-- Ejemplo de Ejecucion:
        SELECT * FROM  [FERCO].[OP_WMS_FN_GET_CANCELLED_MASTERPACK_TO_ASSAMBLE_IN_WAVE_PICKING]('17190') 
		SELECT * FROM [FERCO].[OP_WMS_TASK_LIST] WHERE [WAVE_PICKING_ID] = 17190
*/
-- =============================================
CREATE FUNCTION [FERCO].[OP_WMS_FN_GET_CANCELLED_MASTERPACK_TO_ASSAMBLE_IN_WAVE_PICKING] (
...
BEGIN
	...
	INNER JOIN  ( SELECT [D].[MATERIAL_ID] , MAX([H].[WAVE_PICKING_ID]) [WAVE_PICKING_ID] FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [H] 
	...
	INNER JOIN [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [CXMP]  ON [T].[WAVE_PICKING_ID] = [HD].[WAVE_PICKING_ID]
