...
-- =============================================
CREATE FUNCTION [FERCO].[OP_WMS_FUNC_GET_INSURANCE_DOC]
(	
...
BEGIN
	...
	SELECT [CLIENT_NAME]
			...
			,[TOTAL_VOLUMEN]
			,[TERMS_OF_TRADE]
			,[CODIGO_POLIZA] FROM [FERCO].OP_WMS_VIEW_VALORIZACION;
	...
	SELECT DOC_ID
			...
			, ( SELECT ISNULL(SUM(TOTAL_VALOR),0) FROM @TB_VALORIZACION V 
					INNER JOIN [FERCO].OP_WMS_POLIZA_HEADER P ON V.CODIGO_POLIZA = P.CODIGO_POLIZA
		...
		FROM [FERCO].OP_WMS_INSURANCE_DOCS isd
			INNER JOIN [FERCO].OP_WMS_INSURANCE_COMPANIES isc ON isd.COMPANY_ID = isc.COMPANY_ID
			INNER JOIN [FERCO].OP_WMS_VIEW_CLIENTS vc ON  isd.CLIENT_CODE = vc.CLIENT_CODE COLLATE DATABASE_DEFAULT
		UNION
		SELECT 0 AS DOC_ID 
