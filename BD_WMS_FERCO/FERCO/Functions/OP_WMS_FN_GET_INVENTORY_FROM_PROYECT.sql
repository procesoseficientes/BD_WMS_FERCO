...
-- Ejemplo de Ejecucion:
				SELECT * FROM [FERCO].[OP_WMS_FN_GET_INVENTORY_FROM_PROYECT](@PROYECT_ID)
					
*/
-- =============================================
CREATE FUNCTION [FERCO].[OP_WMS_FN_GET_INVENTORY_FROM_PROYECT] (
...
RETURNS TABLE
  ...
  RETURN
	(SELECT
		...
		INNER JOIN [FERCO].[OP_WMS_PROJECT] [P] ON ([IRP].[PROJECT_ID] = [P].[ID])
		INNER JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [SML] ON (
											[IL].[STATUS_ID] = [SML].[STATUS_ID]
											AND [SML].[STATUS_CODE] <> ''
											AND [SML].[STATUS_CODE] IS NOT NULL
											)
		LEFT OUTER JOIN [FERCO].[OP_WMS_FN_GET_COMMITED_INVENTORY_BY_LICENCE]() [CI] ON (
