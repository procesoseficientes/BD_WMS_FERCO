...
-- Ejemplo de Ejecucion:
        SELECT [FERCO].[OP_WMS_FN_GET_AVAILABLE_INVENTORY_FOR_MASTERPACK_ON_DISPATCH_DEMAND]('FERCO/SKUPRUEBA','BODEGA_01')
*/
-- =============================================
CREATE FUNCTION [FERCO].OP_WMS_FN_GET_AVAILABLE_INVENTORY_FOR_MASTERPACK_ON_DISPATCH_DEMAND (@MASTER_PACK_CODE VARCHAR(50)
...
BEGIN
  ...
  FROM [FERCO].[OP_WMS_VIEW_INVENTORY_FOR_PICKING]
  ...
  BEGIN
    ...
    , [REAL_QTY])
      ...
      LEFT JOIN [FERCO].[OP_WMS_VIEW_INVENTORY_FOR_PICKING] [IXW]
  ...
  BEGIN
    ...
    , [REAL_QTY])
      SELECT
       ...
       ,CAST(SUM(ISNULL([IXW].[AVAILABLE_QTY], 0)) / [CXMP].[QTY] AS INT) REAL_QTY
      FROM [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [CXMP]
      LEFT JOIN [FERCO].[OP_WMS_VIEW_INVENTORY_FOR_PICKING_BY_STATUS_MATERIAL] [IXW]
        ON ([IXW].[MATERIAL_ID] = [CXMP].[COMPONENT_MATERIAL]
        AND [IXW].[CURRENT_WAREHOUSE] = @WAREHOUSE_ID
