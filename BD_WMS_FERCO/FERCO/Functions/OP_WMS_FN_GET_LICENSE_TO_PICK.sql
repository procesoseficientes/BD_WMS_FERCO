...
-- Ejemplo de Ejecucion:
        SELECT * FROM [FERCO].[OP_WMS_FN_GET_LICENSE_TO_PICK]('FERCO/I00000002','BODEGA_02',255,1,NULL,10)
		SELECT * FROM [FERCO].[OP_WMS_FN_GET_LICENSE_TO_PICK]('autovanguard/VAA1001','BODEGA_01',225,0,NULL)
		SELECT * FROM [FERCO].OP_WMS_INV_X_LICENSE IL
		INNER JOIN [FERCO].OP_WMS_LICENSES L ON IL.LICENSE_ID = L.LICENSE_ID WHERE MATERIAL_ID = 'viscosa/VWD1002'
		SELECT * FROM [FERCO].OP_WMS_MATERIALS WHERE MATERIAL_ID = 'FERCO/I00000002'
		SELECT * FROM [FERCO].OP_WMS_TASK_LIST WHERE MATERIAL_ID = 'FERCO/I00000002'
		
*/
-- =============================================
CREATE FUNCTION [FERCO].[OP_WMS_FN_GET_LICENSE_TO_PICK] (
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SELECT
				...
				,[ALLOW_FAST_PICKING]
			FROM
				[FERCO].[OP_WMS_VIEW_PICKING_AVAILABLE_GENERAL]
			WHERE
		...
		BEGIN
			...
			FROM
				[FERCO].[OP_WMS_VIEW_PICKING_AVAILABLE_GENERAL] [PAG]
			INNER JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [SML] ON ([SML].[STATUS_ID] = [PAG].[STATUS_ID])
			WHERE
	...
	BEGIN
		...
		BEGIN
			...
			FROM
				[FERCO].[OP_WMS_VIEW_PICKING_AVAILABLE_GENERAL_BATCH] [PAGB]
			WHERE
