...
-- Modificacion 10/5/2017 @ NEXUS-Team Sprint enave
-- rodrigo.gomez
...
-- Ejemplo de Ejecucion:
				SELECT * FROM [FERCO].OP_WMS_FUNC_GET_MY_PICKING_ALMGEN_HEADER('ACAMACHO')
*/
-- =============================================
CREATE FUNCTION FERCO.OP_WMS_FUNC_GET_MY_PICKING_ALMGEN_HEADER (@pLOGIN_ID VARCHAR(25))
RETURNS TABLE
AS
  RETURN
  (SELECT
  ...
  LEFT JOIN [FERCO].[OP_WMS_WAREHOUSES] [W]
    ON [W].[WAREHOUSE_ID] = [T].[WAREHOUSE_SOURCE]
  LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH]
    ON ([PDH].[WAVE_PICKING_ID] = [T].[WAVE_PICKING_ID])
  ...
  AND ( [T].[IN_PICKING_LINE] = 0
      OR ( [T].[IS_FROM_SONDA] = 0
          AND [T].[IS_FROM_ERP] = 0
        )
      )  
