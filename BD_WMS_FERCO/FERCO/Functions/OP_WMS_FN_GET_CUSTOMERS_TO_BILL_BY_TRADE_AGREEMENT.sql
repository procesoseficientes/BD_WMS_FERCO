...
-- Ejemplo de Ejecucion:
			SELECT * FROM [FERCO].[OP_WMS_FN_GET_CUSTOMERS_TO_BILL_BY_TRADE_AGREEMENT]( 1
					, 'ON_DEMAND'
					, '20170110 00:00:00.000'
					, 'PP', 0)
*/
-- =============================================    
CREATE FUNCTION [FERCO].[OP_WMS_FN_GET_CUSTOMERS_TO_BILL_BY_TRADE_AGREEMENT] (@USE_FRECUENCY INT = 0
, @TYPE VARCHAR(25)
  ...
  BEGIN
    ...
    BEGIN
      ...
      WHERE [BILLING_FRECUENCY] = 30 --FRECUENCIA MENSUAL
        AND (DATEPART(DAY, [FERCO].[OP_WMS_FN_GET_LAST_DAY_OF_MONTH](@PROCESS_DATE)) <> DATEPART(DAY, @PROCESS_DATE))
      --
      DELETE @CLIENT_TO_BILL
      WHERE [BILLING_FRECUENCY] = 15 --FRECUENCIA QUINCENAL
        AND NOT (
        DATEPART(DAY, @PROCESS_DATE) = 15
        OR DATEPART(DAY, [FERCO].[OP_WMS_FN_GET_LAST_DAY_OF_MONTH](@PROCESS_DATE)) = DATEPART(DAY, @PROCESS_DATE)
        )
      --
      DELETE @CLIENT_TO_BILL
