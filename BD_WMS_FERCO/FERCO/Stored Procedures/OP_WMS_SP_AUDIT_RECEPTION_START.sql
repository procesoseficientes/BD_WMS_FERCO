...
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_AUDIT_RECEPTION_START] 
...
BEGIN
	...
	IF EXISTS(SELECT 1 FROM 
				[FERCO].OP_WMS_AUDIT_RECEPTION_CONTROL 
	...
	ELSE
		BEGIN
			...
			INSERT INTO [FERCO].[OP_WMS_AUDIT_RECEPTION_CONTROL]
				   ([CODIGO_POLIZA]
				   ,[CONTENEDOR]
				   ,[COUNTING_METHOD]
				   ,[STATUS]
			...
			SELECT @pCREATED_AUDIT_ID = IDENT_CURRENT('FERCO.OP_WMS_AUDIT_RECEPTION_CONTROL')
			SELECT @pCREATED_AUDIT_ID = MAX(AUDIT_ID) FROM [FERCO].[OP_WMS_AUDIT_RECEPTION_CONTROL]
			IF @@ERROR = 0 BEGIN
				SELECT	@pResult	= 'OK'
			END
			ELSE
				BEGIN
					SELECT	@pResult	= ERROR_MESSAGE()
					INSERT INTO [FERCO].OP_LOG
					VALUES(CURRENT_TIMESTAMP,ERROR_MESSAGE(),'INSERT INTO [FERCO].[OP_WMS_AUDIT_RECEPTION_CONTROL]')
				END
			END
		END
	END TRY
