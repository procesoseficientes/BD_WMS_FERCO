...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_CANCEL_PREPARED_DISPATCH]
					@PICKING_DEMAND_HEADER_ID = 1
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_CANCEL_PREPARED_DISPATCH](
...
BEGIN
	...
	BEGIN TRY
		...
		-- ------------------------------------------------------------------------------------    
		IF EXISTS (SELECT TOP 1 1 FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] WHERE [PICKING_DEMAND_HEADER_ID] = @PICKING_DEMAND_HEADER_ID AND [IS_FOR_DELIVERY_IMMEDIATE] = 1)
		BEGIN
		...
		IF EXISTS (SELECT TOP 1 1 FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] WHERE [DOC_NUM] = @DOC_NUM AND [IS_FOR_DELIVERY_IMMEDIATE] = 1)
		BEGIN
		...
		-- ------------------------------------------------------------------------------------
		DELETE FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] 
		WHERE [PICKING_DEMAND_HEADER_ID] = @PICKING_DEMAND_HEADER_ID

		DELETE FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER]
		WHERE [PICKING_DEMAND_HEADER_ID] = @PICKING_DEMAND_HEADER_ID
