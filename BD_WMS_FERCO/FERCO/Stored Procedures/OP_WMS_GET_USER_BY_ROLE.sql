...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].OP_WMS_GET_USER_BY_ROLE  @USER_ROLE = 'OPERADOR', @USER_CODE= 'OPER2'
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_GET_USER_BY_ROLE (@USER_ROLE VARCHAR(25) = 'PILOTO'
...
BEGIN
  ...
  LEFT JOIN [FERCO].[OP_WMS_USER_X_PILOT] [UXP]
    ON ([L].[LOGIN_ID] = [UXP].[USER_CODE])
  WHERE [L].[ROLE_ID] = @USER_ROLE
  AND ([UXP].[PILOT_CODE] IS NULL
  OR ([UXP].[USER_CODE] = @USER_CODE))
  AND [L].[LOGIN_ID] > ''
