...
-- Modificacion 		6/02/2023  
-- Autor: 				PABLO.REYES
-- Descripcion: 		6/02/2023 - Se agregaron campos para guardad trasabilidad de las licencias.

/*
-- Ejemplo de Ejecucion: 
			SELECT * FROM [FERCO].[OP_WMS_LICENSES] WHERE [LICENSE_ID] =  23434
  EXEC [FERCO].[OP_WMS_SP_REALLOC_LICENSE] @pLICENCIA_ID = 23434
										   ...
										   ,@pTOTAL_POSITION = 3
  SELECT * FROM [FERCO].[OP_WMS_LICENSES] WHERE [LICENSE_ID] =  23434
  SELECT * FROM [FERCO].[OP_WMS_TRANS] [OWT] WHERE [OWT]. = 23434
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REALLOC_LICENSE]
	...
	BEGIN TRY
		...
		BEGIN
			...
			SELECT
				...
				,[PRIORITY]
			FROM
				[FERCO].[OP_WMS_FN_GET_CLASSES_BY_LICENSE](@pLICENCIA_ID);
			...
			SELECT
				...
				,[PRIORITY]
			FROM
				[FERCO].[OP_WMS_FN_GET_CLASSES_BY_LOCATION](@pNEW_LOCATION_SPOT);
		...
		SELECT TOP 1
			...
			,(SELECT TOP 1
					[LOGIN_NAME]
				FROM
					[FERCO].[OP_WMS_FUNC_GETLOGIN_NAME](@pLOGIN_ID)) [LOGIN_NAME]
		...
		INNER JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [C] ON [L].[CLIENT_OWNER] = [C].[CLIENT_CODE]
		INNER JOIN FERCO.OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE SML ON (IL.LICENSE_ID = SML.LICENSE_ID AND IL.STATUS_ID = SML.STATUS_ID)
		WHERE
		...
		BEGIN
			EXEC [FERCO].[OP_WMS_SP_UPDATE_LICENSE_TOTAL_REALLOC_FAST_PICKING] @LOGIN_ID = @pLOGIN_ID, -- varchar(50)
