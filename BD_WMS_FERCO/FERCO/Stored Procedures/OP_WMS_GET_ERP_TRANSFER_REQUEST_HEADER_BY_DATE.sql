...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_GET_ERP_TRANSFER_REQUEST_HEADER_BY_DATE]
					@WAREHOUSE_ID = 'C001'
					,@START_DATETIME = '2016-12-01 00:00:00.000'
					,@END_DATETIME = '2016-12-18 00:00:00.000'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_ERP_TRANSFER_REQUEST_HEADER_BY_DATE] (
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			SELECT
				...
				+ ''' [SOURCE_NAME]
					...
					FROM ' + @DATA_BASE_NAME + '.'
				+ @SCHEMA_NAME 
				+ '.[ERP_VW_TRANSFER_REQUEST_HEADER] [TRH] WITH (NOLOCK)
					INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W1] WITH (NOLOCK) ON ([W1].[ERP_WAREHOUSE] = [TRH].[FROM_WAREHOUSE_CODE] COLLATE DATABASE_DEFAULT)
					INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W2]  WITH (NOLOCK) ON ([W2].[ERP_WAREHOUSE] = [TRH].[TO_WAREHOUSE_CODE] COLLATE DATABASE_DEFAULT)
		...
		FROM
			[#PICKING_DOCUMENT] [PD]
		LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [P]  WITH (NOLOCK) ON (
