...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].OP_WMS_SP_CANCEL_MANIFEST @MANIFEST_HEADER_ID =1, @STATUS = 'CANCELED'
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_CANCEL_MANIFEST (@MANIFEST_HEADER_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  BEGIN TRY
    ...
    SELECT
      @IS_STATUS_CREATED = 1
    FROM [FERCO].[OP_WMS_MANIFEST_HEADER] [M]
    ...
    --    INSERT INTO [FERCO].[OP_WMS_CANCELED_MANIFEST_HEADER]
    --      SELECT
    ...
    --
    --    INSERT INTO [FERCO].[OP_WMS_CANCELED_MANIFEST_DETAIL]
    --      SELECT
    ...
    --      FROM [FERCO].[OP_WMS_CANCELED_MANIFEST_DETAIL] [MD]
    --      WHERE [MD].[MANIFEST_HEADER_ID] = @MANIFEST_HEADER_ID

    DELETE [L]
      FROM [FERCO].[OP_WMS_PICKING_LABEL_BY_MANIFEST] [L]
    ...
    --    DELETE [FERCO].[OP_WMS_MANIFEST_DETAIL]
    --    WHERE [MANIFEST_HEADER_ID] = @MANIFEST_HEADER_ID
    --
    --    DELETE [FERCO].[OP_WMS_MANIFEST_HEADER]
    --    WHERE [MANIFEST_HEADER_ID] = @MANIFEST_HEADER_ID
