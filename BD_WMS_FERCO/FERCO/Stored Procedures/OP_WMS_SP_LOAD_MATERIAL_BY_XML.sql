...
-- Modificacion 10/3/2017 @ NEXUS-Team Sprint eFERCO
					-- rodrigo.gomez
...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_LOAD_MATERIAL_BY_XML]
					@MATERIALS = N'<?xml version="1.0"?>
<Data>
  <Material>
    <CantidadMaxPorBin>1</CantidadMaxPorBin>
    <Clase>S</Clase>
    <Cliente>FERCO</Cliente>
    <Codigo>p1</Codigo>
    <CodigoBarras>FERCO/p1</CodigoBarras>
    <CodigoBarrasAlterno>FERCO/p1</CodigoBarrasAlterno>
  ...
  <Material>
    ...
    <Cliente>FERCO</Cliente>
    <Codigo>p2</Codigo>
    <CodigoBarras>FERCO/p2</CodigoBarras>
    <CodigoBarrasAlterno>FERCO/p2</CodigoBarrasAlterno>
  ...
  <Material>
    ...
    <Cliente>FERCO</Cliente>
    <Codigo>p3</Codigo>
    <CodigoBarras>FERCO/p3</CodigoBarras>
    <CodigoBarrasAlterno>FERCO/p3</CodigoBarrasAlterno>
  ...
  <Material>
    ...
    <Cliente>FERCO</Cliente>
    <Codigo>p4</Codigo>
    <CodigoBarras>FERCO/p4</CodigoBarras>
    <CodigoBarrasAlterno>FERCO/p4</CodigoBarrasAlterno>
  ...
  <Material>
    ...
    <Cliente>FERCO</Cliente>
    <Codigo>p5</Codigo>
    <CodigoBarras>FERCO/p5</CodigoBarras>
    <CodigoBarrasAlterno>FERCO/p5</CodigoBarrasAlterno>
...
<Data>
  ...
  <MasterPack>
    ...
    <Cantidad>7</Cantidad>
				...
				SELECT * FROM 
*/
CREATE PROCEDURE [FERCO].[OP_WMS_SP_LOAD_MATERIAL_BY_XML] (
		@MATERIALS XML = NULL
		,@LOCATIONS XML = NULL
	...
	BEGIN TRY
		...
		BEGIN
			BEGIN TRY
				...
				FROM
					@MATERIAL [M]
				LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [C] ON ([M].[CLIENT_OWNER] = [C].[CLIENT_CODE])
				WHERE
				...
				FROM
					@MATERIAL [M]
				LEFT JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [C] ON (
				...
				IF @HAS_ERROR = 0
				BEGIN
					MERGE [FERCO].[OP_WMS_MATERIALS] AS [M]
					USING
						(SELECT
								[LINE_NUM]
								,[CLIENT_OWNER]
		...
		BEGIN
			BEGIN TRY
				...
				FROM
					@LOCATION [L]
				LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [C] ON ([L].[CLIENT_CODE] = [C].[CLIENT_CODE])
				WHERE
					[C].[CLIENT_NAME] IS NULL;
				--
				IF @@ROWCOUNT > 0
				...
				FROM
					@LOCATION [L]
				LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([M].[MATERIAL_ID] = [L].[MATERIAL_ID])
				WHERE
				...
				FROM
					@LOCATION [L]
				LEFT JOIN [FERCO].[OP_WMS_SHELF_SPOTS] [S] ON ([S].[LOCATION_SPOT] = [L].[LOCATION_SPOT])
				WHERE
				...
				IF @HAS_ERROR = 0
				BEGIN
					MERGE [FERCO].[OP_WMS_MATERIALS_JOIN_SPOTS]
						AS [S]
					USING
						(SELECT
								[L].[LINE_NUM]
							...
							FROM
								@LOCATION [L]
							INNER JOIN [FERCO].[OP_WMS_SHELF_SPOTS] [S] ON ([S].[LOCATION_SPOT] = [L].[LOCATION_SPOT]))
						AS [TL]
		...
		BEGIN
			BEGIN TRY
				...
				LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [C] ON ([C].[CLIENT_CODE] = [UMBM].[CLIENT_ID])
				WHERE
				...
				FROM
					@UNIT_MEASURE_BY_MATERIAL [UMBM]
				LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([M].[MATERIAL_ID] = [UMBM].[MATERIAL_ID])
				WHERE
				...
				FROM
					@UNIT_MEASURE_BY_MATERIAL [UMBM]
				JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [UMBM].[MATERIAL_ID] = [M].[MATERIAL_ID]
				WHERE
					(
				...
				FROM
					@UNIT_MEASURE_BY_MATERIAL [UMBM]
				JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UM] ON (
				...
				IF @HAS_ERROR = 0
				BEGIN
					MERGE [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL]
						AS [TABLEBDD]
					USING
						(SELECT
								[CLIENT_ID]
		...
		BEGIN
			BEGIN TRY
				...
				FROM
					@MASTER_PACK [M]
				LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [C] ON ([M].[CLIENT_CODE] = [C].[CLIENT_CODE])
				WHERE
					[C].[CLIENT_NAME] IS NULL;
				--
				IF @@ROWCOUNT > 0
				...
				FROM
					@MASTER_PACK [M]
				LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [MA] ON (
				...
				FROM
					@MASTER_PACK [M]
				LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [MA] ON ([MA].[MATERIAL_ID] = [M].[COMPONENT_MATERIAL])
				WHERE
					[MA].[CLIENT_OWNER] IS NULL;
				--
				IF @@ROWCOUNT > 0
				...
				IF @HAS_ERROR = 0
				BEGIN
					MERGE [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK]
						AS [C]
					USING
						(SELECT
								[M].[LINE_NUM]
		...
		BEGIN 
			...
			FROM
				...
				AS [x] ([Rec])
			LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY] [P] ON [P].[DESCRIPTION] = [x].[Rec].[query]('./Propiedad').[value]('.',
											'varchar(250)')
			LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY_OPTION] [O] ON [O].[MATERIAL_PROPERTY_ID] = [O].[MATERIAL_PROPERTY_ID]
		...
		-- ------------------------------------------------------------------------------------
			...
			LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY] [MP] ON ([MP].[MATERIAL_PROPERTY_ID] = [M].[MATERIAL_PROPERTY_ID])
			WHERE
		...
		-- ------------------------------------------------------------------------------------
			...
			FROM
				@MATERIAL_PROPERTY_BY_WAREHOUSE [M]
			LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY_OPTION] [O] ON (
		...
		-- ------------------------------------------------------------------------------------
			...
			FROM
				@MATERIAL_PROPERTY_BY_WAREHOUSE [M]
			LEFT JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] ON ([W].[WAREHOUSE_ID] = [M].[WAREHOUSE_ID])
			WHERE
		...
		-- ------------------------------------------------------------------------------------
			...
			FROM
				@MATERIAL_PROPERTY_BY_WAREHOUSE [M]
			LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [W] ON ([W].[MATERIAL_ID] = [M].[MATERIAL_ID])
			WHERE
			...
			-- Realiza el merge
			-- ------------------------------------------------------------------------------------
			MERGE [FERCO].[OP_WMS_MATERIAL_PROPERTY_BY_WAREHOUSE]
				AS [MPW]
			USING
				(SELECT
