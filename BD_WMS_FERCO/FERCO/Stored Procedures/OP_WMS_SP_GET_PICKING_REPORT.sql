...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_PICKING_REPORT]
					@START_DATE = '20180725',
					@END_DATE = '20180726'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_PICKING_REPORT] (
		@START_DATE DATE
		,@END_DATE DATE
	)
AS
BEGIN
	...
	LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [h] ON [h].[WAVE_PICKING_ID] = [T].[WAVE_PICKING_ID]
											AND [h].[IS_CONSOLIDATED] = 0
	INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [M].[MATERIAL_ID] = [T].[MATERIAL_ID]
	INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] ON [T].[WAREHOUSE_ID] = [W].[WAREHOUSE_ID]
	ORDER BY
		[T].[WAVE_PICKING_ID]
		,[T].[TaskOrder];
END;