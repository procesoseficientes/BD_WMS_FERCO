...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_INSERT_NEXT_DEMAND_PICKING_DETAIL] @PICKING_DEMAND_HEADER_ID = 4131, 
                                                                    @MATERIAL_ID = 'FERCO/100019',
                                                                    @QTY = 2.5000,
                                                                    @LINE_NUM = 1,
                                                                    @ERP_OBJECT_TYPE  = 0,
                                                                    @PRICE  = 10.000000,
																	...
																	@MATERIAL_OWNER = 'FERCO'
																	,@TONE = '1412'
																	, @CALIBER = '2013'
  SELECT * FROM BEGIN
                    [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL]
                END [OWNPDD]  where [OWNPDD].[MATERIAL_ID] = 'FERCO/100018'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_INSERT_NEXT_DEMAND_PICKING_DETAIL] (
...
BEGIN
	...
	BEGIN TRY
		...
		INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [T].[WAVE_PICKING_ID] = [H].[WAVE_PICKING_ID]
