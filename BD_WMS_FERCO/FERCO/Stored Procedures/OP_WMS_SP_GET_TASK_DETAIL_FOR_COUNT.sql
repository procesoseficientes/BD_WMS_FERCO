...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_GET_TASK_DETAIL_FOR_COUNT] @TASK_ID = 8, @LOGIN = 'ADMIN'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TASK_DETAIL_FOR_COUNT] (@TASK_ID INT
, @LOGIN VARCHAR(25))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  SELECT
   ...
   ,[CE].[QTY_SCANNED]
   ,[CE].[QTY_EXPECTED]
   ,[CE].[QTY_EXPECTED] - [CE].[QTY_SCANNED] [DIFFERENCE]
  FROM [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [CD]
  ...
  INNER JOIN [FERCO].[OP_WMS_TASK] [T]
    ON [CH].[TASK_ID] = [T].[TASK_ID]
  LEFT JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_EXECUTION] [CE] 
