...
-- Description:			obtiene las series que se deben enviar para una recepcion general que se enviará a al erp

/*
-- Ejemplo de Ejecucion:
         EXEC [FERCO].[OP_WMS_SP_GET_GENERAL_RECEPTION_DETAILS_SERIES_BY_SAP_R3] @RECEPTION_HEADER = 108
*/
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_GENERAL_RECEPTION_DETAILS_SERIES_BY_SAP_R3] (
...
BEGIN
	...
	SELECT
		...
		,[H].[DOC_ID_POLIZA]
		,[M].[MATERIAL_ID]
		,DENSE_RANK() OVER (ORDER BY [D].[ERP_RECEPTION_DOCUMENT_DETAIL_ID]) AS [ROW_NUMBER]
	FROM
	...
	LEFT JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MS] ON [MS].[RECEPTION_HEADER_ID] = [H].[ERP_RECEPTION_DOCUMENT_HEADER_ID]
	...
	ORDER BY
		[D].[ERP_RECEPTION_DOCUMENT_DETAIL_ID] ASC;
	...
	BEGIN
		...
		BEGIN
			...
			INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [L].[CODIGO_POLIZA] = CAST(@DOC_ID_POLIZA AS VARCHAR(25))
			...
			FROM
				[#RECEPTION_SERIES] [R]
			INNER JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MS] ON [MS].[CORRELATIVE] = @SERIAL_NUMBER_ID
			WHERE
				[EBELP] = @LINE_NUM;
