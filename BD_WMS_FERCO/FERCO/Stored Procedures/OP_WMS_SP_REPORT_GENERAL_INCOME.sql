...
BEGIN
  ...
  INNER JOIN [FERCO].[OP_WMS_TRANS] AS [T]
    ON (
  ...
  LEFT JOIN [FERCO].[OP_WMS_VIEW_VALORIZACION_ALMGEN] AS [VA]
    ON (
    [VA].[CODIGO_POLIZA] = [T].[CODIGO_POLIZA]
    AND [VA].[LICENSE_ID] = [T].[LICENSE_ID]
    AND [VA].[MATERIAL_ID] = [T].[MATERIAL_CODE]
    )
  LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RH]
