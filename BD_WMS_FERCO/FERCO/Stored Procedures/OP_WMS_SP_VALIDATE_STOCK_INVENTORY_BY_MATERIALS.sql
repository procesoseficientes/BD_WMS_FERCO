...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_VALIDATE_STOCK_INVENTORY_BY_MATERIALS]
					@EXTERNAL_SOURCE_ID = '1|2'
					,@MATERIAL_ID = 'FERCO/100002|confitesa/000000000000000002'
					,@QTY = '1000|10'
          ,@CODE_WAREHOUSE = 'BODEGA_01'
				--
				EXEC [FERCO].[OP_WMS_SP_VALIDATE_STOCK_INVENTORY_BY_MATERIALS]
					@EXTERNAL_SOURCE_ID = '1|1'
					,@MATERIAL_ID = 'FERCO/100002|FERCO/100002'
					,@QTY = '1500|100'
          ,@CODE_WAREHOUSE = 'BODEGA_01'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_VALIDATE_STOCK_INVENTORY_BY_MATERIALS] (@EXTERNAL_SOURCE_ID VARCHAR(MAX)
...
BEGIN
  ...
  FROM [FERCO].OP_WMS_FUNC_SPLIT_3(@EXTERNAL_SOURCE_ID, @DELIMITER) [E]
  INNER JOIN [FERCO].OP_WMS_FUNC_SPLIT_3(@MATERIAL_ID, @DELIMITER) [M]
    ON (
    [E].[ID] = [M].[ID]
    )
  INNER JOIN [FERCO].OP_WMS_FUNC_SPLIT_3(@QTY, @DELIMITER) [Q]
    ON (
    [M].[ID] = [Q].[ID]
    )
...
--    ON [TM].[MATERIAL_ID] = [M].[MATERIAL_ID]
  LEFT JOIN [FERCO].[OP_WMS_VIEW_INVENTORY_FOR_PICKING] AS [IP]
    ON (
