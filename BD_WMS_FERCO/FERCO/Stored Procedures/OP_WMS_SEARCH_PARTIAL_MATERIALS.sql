...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SEARCH_PARTIAL_MATERIALS]
					@MATERIAL_ID = '', -- varchar(50)
					@MATERIAL_NAME = '', -- varchar(200)
					@SHORT_NAME = '' -- varchar(200)
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SEARCH_PARTIAL_MATERIALS] (
		@MATERIAL_ID VARCHAR(50)
		,@MATERIAL_NAME VARCHAR(200)
		,@SHORT_NAME VARCHAR(200)
	)
...
/*DECLARE @MATERIAL_ID VARCHAR(50)= '', -- varchar(50)
	...
	FROM
		...
		INNER JOIN ferco.OP_WMS_VIEW_CLIENTS CL ON M.CLIENT_OWNER=CL.CLIENT_CODE
	LEFT JOIN (SELECT
					[MM].[MATERIAL_ID]
					,COUNT([MM].[MATERIAL_ID]) [QTY_MEASURE]
				FROM
					[FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [MM]
				GROUP BY
					[MM].[MATERIAL_ID]) [MM] ON [MM].[MATERIAL_ID] = [M].[MATERIAL_ID]
	LEFT JOIN [FERCO].[OP_WMS_CLASS] [C] ON [M].[MATERIAL_CLASS] = [C].[CLASS_ID]
	LEFT JOIN [FERCO].[OP_WMS_SUB_CLASS] [SC] ON [M].[MATERIAL_SUB_CLASS] = [SC].[SUB_CLASS_ID]
	WHERE
