...
-- Ejemplo de Ejecucion:
  EXEC	[FERCO].[OP_WMS_SP_VALIDATE_SCANNED_LOCATION_FOR_COUNT] @LOGIN = 'ACAMACHO' , @TASK_ID = 7 , @LOCATION = 'B02-P01-F01-NU'
  SELECT * FROM [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [OWPCD]
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_VALIDATE_SCANNED_LOCATION_FOR_COUNT (@LOGIN VARCHAR(25)
, @TASK_ID INT
, @LOCATION VARCHAR(50))
AS
BEGIN
  ...
  TOP 1
   ...
   ,@CODIGO =
    CASE
      ...
      ELSE 2
    END
  FROM [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [D]
