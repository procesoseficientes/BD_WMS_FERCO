...
-- Modificacion 10/16/2017 @ NEXUS-Team Sprint eFERCO
-- rodrigo.gomez
...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_GET_PICKING_DOCUMENT] @PICKING_DEMAND_HEADER_ID = 5228, @IS_INVOICE = 0, @OWNER = 'motorganica'
			--
			EXEC  [FERCO].[OP_WMS_GET_PICKING_DOCUMENT] @PICKING_DEMAND_HEADER_ID = 5228, @IS_INVOICE = 1, @OWNER = 'motorganica'
			--
			EXEC  [FERCO].[OP_WMS_GET_PICKING_DOCUMENT] @PICKING_DEMAND_HEADER_ID = 5230, @IS_INVOICE = 0, @OWNER = 'motorganica'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_PICKING_DOCUMENT]
    (
...
BEGIN
  ...
  --------------------------------------------------------------------------------------------
    ...
    FROM
        [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PH]
    INNER JOIN [FERCO].[OP_SETUP_EXTERNAL_SOURCE] [ES] ON ([PH].[EXTERNAL_SOURCE_ID] = [ES].[EXTERNAL_SOURCE_ID])
    INNER JOIN [FERCO].[OP_WMS_COMPANY] [C] ON (
  ...
  --------------------------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------------------
        SELECT
			...
			LEFT JOIN [FERCO].[OP_WMS_CUSTOMER_INTERCOMPANY] [CI] ON ([SOH].[CLIENT_ID] = [CI].[MASTER_ID] AND [CI].[SOURCE] = '''
            + @OWNER
            + ''')
			INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PH] ON ([PH].DOC_NUM = [SOH].[SALES_ORDER_ID])
			WHERE [SOH].[IS_VOID] = 0
				AND [SOH].[IS_DRAFT] = 0
