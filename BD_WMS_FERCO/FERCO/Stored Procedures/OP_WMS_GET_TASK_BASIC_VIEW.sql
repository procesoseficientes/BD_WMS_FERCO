...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_TASK_BASIC_VIEW] (
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
	...
	INTO
		[#USERS]
	FROM
		[FERCO].[OP_WMS_FN_SPLIT](@USERS, '|') [T]
	LEFT JOIN [FERCO].[OP_WMS_LOGINS] [L] ON [L].[LOGIN_ID] = [T].[VALUE];
  ...
  -- ------------------------------------------------------------------------------------
	...
	SELECT
		...
		,[W].[USE_PICKING_LINE]
	FROM
		[FERCO].[OP_WMS_WAREHOUSE_BY_USER] [WU] WITH (NOLOCK)
	LEFT JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] WITH (NOLOCK) ON [WU].[WAREHOUSE_ID] = [W].[WAREHOUSE_ID]
	WHERE
		[WU].[LOGIN_ID] = @LOGIN;
  -- ------------------------------------------------------------------------------------
  ...
  -- ------------------------------------------------------------------------------------
	...
	FROM
		[FERCO].[OP_WMS_TASK_LIST] [TL] WITH (NOLOCK)
	LEFT JOIN [FERCO].[OP_WMS_TRANS] [T] WITH (NOLOCK) ON [TL].[SERIAL_NUMBER] = [T].[TASK_ID]
  ...
  -- ------------------------------------------------------------------------------------
	...
	FROM
		[FERCO].[OP_WMS_TASK_LIST] [TL] WITH (NOLOCK)
	LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH] WITH (NOLOCK) ON [DH].[WAVE_PICKING_ID] = [TL].[WAVE_PICKING_ID]
	LEFT JOIN [FERCO].[OP_WMS_LICENSES] [L] WITH (NOLOCK) ON [DH].[PICKING_DEMAND_HEADER_ID] = [L].[PICKING_DEMAND_HEADER_ID]
	WHERE
  ...
  -- ------------------------------------------------------------------------------------
	...
	LEFT JOIN [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER] [TH] WITH (NOLOCK) ON [TH].[TRANSFER_REQUEST_ID] = [TL].[TRANSFER_REQUEST_ID]
	WHERE
  ...
  -- ------------------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------------------
			...
			FROM
				[FERCO].[OP_WMS_VIEW_TASK] [VT] WITH (NOLOCK)
			INNER JOIN [#LOCATION_TARGET_RECEPTION] [LTR]  ON [LTR].[SERIAL_NUMBER] = [VT].[SERIAL_NUMBER]
			LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RDH]  WITH (NOLOCK) ON ([VT].[SERIAL_NUMBER] = [RDH].[TASK_ID])
    ...
    -- ------------------------------------------------------------------------------------
			SELECT DISTINCT
				...
				,[VT].[PROJECT_SHORT_NAME]
			FROM
				[FERCO].[OP_WMS_VIEW_TASK_PICKING_HEADER] [VT] WITH (NOLOCK)
			INNER JOIN @TB_WAREHOUSE [W] ON ([W].[WAREHOUSE_ID] = [VT].[WAREHOUSE_SOURCE])
			INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] WITH (NOLOCK) ON [T].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
			INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M]  WITH (NOLOCK)  ON [M].[MATERIAL_ID] = [T].[MATERIAL_ID]
			...
			LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] WITH (NOLOCK) ON ([VT].[WAVE_PICKING_ID] = [PDH].[WAVE_PICKING_ID])
			LEFT JOIN [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER] [TRH] WITH (NOLOCK) ON ([PDH].[TRANSFER_REQUEST_ID] = [TRH].[TRANSFER_REQUEST_ID])
			LEFT JOIN [FERCO].[OP_WMS_PICKING_ERP_DOCUMENT] [PD] WITH (NOLOCK) ON [PD].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
			LEFT JOIN [#USERS] [U]  ON [U].[login] = [VT].[TASK_ASSIGNEDTO]
    ...
    -- ------------------------------------------------------------------------------------
			...
			FROM
				[FERCO].[OP_WMS_TASK] [T] WITH (NOLOCK)
			INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [CH] WITH (NOLOCK) ON [T].[TASK_ID] = [CH].[TASK_ID]
			INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [CD] WITH (NOLOCK)  ON [CH].[PHYSICAL_COUNT_HEADER_ID] = [CD].[PHYSICAL_COUNT_HEADER_ID]
    ...
    -- ------------------------------------------------------------------------------------
			SELECT DISTINCT
				...
				,[VT].[PROJECT_SHORT_NAME]
			FROM
				[FERCO].[OP_WMS_VIEW_TASK_REALLOC_HEADER] [VT]  WITH (NOLOCK)
			INNER JOIN @TB_WAREHOUSE [W] ON ([W].[WAREHOUSE_ID] = [VT].[WAREHOUSE_SOURCE])
			INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] WITH (NOLOCK) ON [T].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
			INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] WITH (NOLOCK) ON [M].[MATERIAL_ID] = [T].[MATERIAL_ID]
			INNER JOIN [#CLASS] [C] ON [C].[CLASS_ID] = CAST([M].[MATERIAL_CLASS] AS INT)
  ...
  -- ------------------------------------------------------------------------------------
	...
	FROM
		[#RESULT] [R] WITH (NOLOCK)
	LEFT JOIN [#TYPES] [T] WITH (NOLOCK) ON [R].[TASK_TYPE] = [T].[TYPE]
	WHERE
		(
			@TYPES IS NULL
