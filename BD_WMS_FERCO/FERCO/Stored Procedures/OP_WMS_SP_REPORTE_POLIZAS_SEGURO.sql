...
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_REPORTE_POLIZAS_SEGURO
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @AmountTotal = (SELECT
        SUM(AMOUNT)
      FROM [FERCO].OP_WMS_INSURANCE_DOCS)
    SET @TotalInventory = (SELECT
        SUM(TOTAL_INVENTORY)
      FROM [FERCO].OP_WMS_INVENTORY iv
      INNER JOIN [FERCO].OP_WMS_INSURANCE_DOCS ds
        ON iv.CLIENT_CODE = ds.CLIENT_CODE)

    SELECT
      POLIZA_INSURANCE AS NumPoliza
    ...
    FROM [FERCO].OP_WMS_INSURANCE_DOCS d
    INNER JOIN [FERCO].OP_WMS_VIEW_CLIENTS c
      ON d.CLIENT_CODE = c.CLIENT_CODE COLLATE DATABASE_DEFAULT
    INNER JOIN [FERCO].OP_WMS_INVENTORY i
      ON d.CLIENT_CODE = i.CLIENT_CODE
    ORDER BY VALIN_TO
