...
-- Ejemplo de Ejecucion:
				SELECT * FROM [SWIFT_EXPRESS].[FERCO].[SONDA_SALES_ORDER_HEADER] WHERE SALES_ORDER_ID = 33687
				--
				EXEC [FERCO].[OP_WMS_SP_SET_SALE_ORDER_MAKE_PICKING_FROM_EXTERNAL_SOURCE]
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_SET_SALE_ORDER_MAKE_PICKING_FROM_EXTERNAL_SOURCE](
...
BEGIN
	...
	BEGIN TRY
		...
		-- ------------------------------------------------------------------------------------
		SELECT @QUERY = N'UPDATE ' + @DATA_BASE_NAME + '.' + @SCHEMA_NAME + '.[SONDA_SALES_ORDER_HEADER]
		SET [HAVE_PICKING] = 1
		WHERE [SALES_ORDER_ID] = ' + CAST(@SALES_ORDER_ID AS VARCHAR)
		--
		PRINT '--> @QUERY: ' + @QUERY
		--
		EXEC (@QUERY)
