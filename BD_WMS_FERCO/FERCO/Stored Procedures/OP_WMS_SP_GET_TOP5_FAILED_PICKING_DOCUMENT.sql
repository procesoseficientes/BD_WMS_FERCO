...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_GET_TOP5_FAILED_PICKING_DOCUMENT] @OWNER = 'Arium'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TOP5_FAILED_PICKING_DOCUMENT] (@OWNER VARCHAR(50), @IS_INVOICE INT = 0)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  SELECT
    @SENT_STATUS = [FERCO].[OP_WMS_FN_GET_PARAMETER_VALUE]('PICKING_DETAIL_STATUS', 'SENT');

  --
  SELECT
    @MAX_ATTEMPTS = [owc].[NUMERIC_VALUE]
  FROM [FERCO].[OP_WMS_CONFIGURATIONS] [owc]
  ...
  LEFT JOIN [FERCO].[OP_WMS_FUNC_SPLIT](@INTERNAL_SALE_COMPANIES,
  '|') [ISC]
  ...
  LEFT JOIN [FERCO].[OP_WMS_CUSTOMER_INTERCOMPANY] [CI]
    ON [CI].[MASTER_ID] = [PDH].[CLIENT_CODE]
      AND [CI].[SOURCE] = @OWNER
  LEFT JOIN [FERCO].[OP_WMS_SELLER_INTERCOMPANY] [SI]
    ON [SI].[MASTER_ID] = [PDH].[MASTER_ID_SELLER]
      AND [SI].[SOURCE] = @OWNER
  LEFT JOIN [FERCO].[OP_WMS_SELLER_INTERCOMPANY] [SI2]
