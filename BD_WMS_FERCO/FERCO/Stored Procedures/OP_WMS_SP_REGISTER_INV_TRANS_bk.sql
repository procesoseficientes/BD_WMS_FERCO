...
EXEC [FERCO].OP_WMS_SP_REGISTER_INV_TRANS @pTRADE_AGREEMENT = ''
                                         ,@pLOGIN_ID = ''
                                         ,@pTRANS_TYPE = ''
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_INV_TRANS_bk] (
...
--
		...
		LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY_BY_WAREHOUSE] [MW] ON [MW].[MATERIAL_ID] = [M].[MATERIAL_ID]
											AND @WAREHOUSE = [MW].[WAREHOUSE_ID]
		LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY] [MP] ON [MP].[MATERIAL_PROPERTY_ID] = [MW].[MATERIAL_PROPERTY_ID]
		...
		BEGIN
			...
			INNER JOIN [FERCO].[OP_WMS_ACUERDOS_X_CLIENTE] [AC] ON [PH].[CLIENT_CODE] = [AC].[CLIENT_ID]
			WHERE
				[PH].[DOC_ID] = @DOC_ID;
    ...
    ---------------------------------------------------------------------------------  
		...
		INNER JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IL] ON ([TCM].[TONE_AND_CALIBER_ID] = [IL].[TONE_AND_CALIBER_ID])
		WHERE
		...
		VALUES
				(
					...
					,(SELECT
							*
						FROM
							[FERCO].[OP_WMS_FUNC_GETLOGIN_NAME](@pLOGIN_ID))
					,@pTRANS_TYPE
					,ISNULL((SELECT
									*
								FROM
									[FERCO].[OP_WMS_FUNC_GETTRANS_DESC](@pTRANS_TYPE)),
							@pTRANS_TYPE)
					,@pTRANS_EXTRA_COMMENTS
					,@pMATERIAL_BARCODE
					,@pMATERIAL_ID
					,(SELECT
						...
						FROM
							[FERCO].[OP_WMS_FUNC_GETMATERIAL_DESC](@pMATERIAL_BARCODE,
											@pCLIENT_OWNER))
					,NULL
					,[FERCO].[OP_WMS_FN_GET_MATERIAL_COST_BY_MATERIAL](@pMATERIAL_ID,
											@pCLIENT_OWNER)
					,@pSOURCE_LICENSE
					,@pTARGET_LICENSE
					,@pSOURCE_LOCATION
					...
					,(SELECT
							*
						FROM
							[FERCO].[OP_WMS_FUNC_GETCLIENT_NAME](@pCLIENT_OWNER))
					,@pQUANTITY_UNITS
					,ISNULL((SELECT
    ...
    --
      ...
      ---------------------------------------------------------------------------------  
			EXEC [FERCO].[OP_WMS_SP_INSERT_MASTER_PACK] @MATERIAL_ID_MASTER_PACK = @pMATERIAL_CODE,
				@LICENSE_ID = @pLICENSE_ID,
      ...
      ---------------------------------------------------------------------------------  
        ...
        ---------------------------------------------------------------------------------  
				...
				BEGIN
					EXEC [FERCO].[OP_WMS_SP_EXPLODE_CASCADE_IN_RECEPTION] @LICENSE_ID = @pLICENSE_ID,
						@LOGIN_ID = @pLOGIN_ID,
						@MATERIAL_ID = @pMATERIAL_CODE;
				END;
				ELSE
				BEGIN
					EXEC [FERCO].[OP_WMS_EXPLODE_MASTER_PACK] @LICENSE_ID = @pLICENSE_ID,
						@MATERIAL_ID = @pMATERIAL_CODE,
						@LAST_UPDATE_BY = @pLOGIN_ID,
						@MANUAL_EXPLOTION = 0;
				END;
		...
		EXEC [FERCO].[OP_WMS_SP_REGISTER_RECEPTION_STATUS] @pTRANS_TYPE = @pTRANS_TYPE,
			@pLOGIN_ID = @pLOGIN_ID,
