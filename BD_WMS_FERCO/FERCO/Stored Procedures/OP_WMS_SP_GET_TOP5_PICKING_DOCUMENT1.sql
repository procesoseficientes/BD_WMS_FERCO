...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_GET_TOP5_PICKING_DOCUMENT] @OWNER = 'MOTORGANICA', @IS_INVOICE = 0
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TOP5_PICKING_DOCUMENT1]
    (
     @OWNER VARCHAR(50)
    ,@IS_INVOICE INT = 0
    )
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
    ...
    LEFT JOIN [FERCO].[OP_WMS_CUSTOMER_INTERCOMPANY] [CI] ON [CI].[MASTER_ID] = [PDH].[CLIENT_CODE]
                                                           AND [CI].[SOURCE] = @OWNER
    LEFT JOIN [FERCO].[OP_WMS_SELLER_INTERCOMPANY] [SI] ON [SI].[MASTER_ID] = [PDH].[MASTER_ID_SELLER]
                                                         AND [SI].[SOURCE] = @OWNER
    LEFT JOIN [FERCO].[OP_WMS_SELLER_INTERCOMPANY] [SI2] ON [SI2].[SLP_CODE] = [PDH].[CODE_SELLER]
                                                          AND [SI2].[SOURCE] = @OWNER
    INNER JOIN [#PERFORMS_INTERNAL_SALE] [PIS] ON [PIS].[PICKING_DEMAND_HEADER_ID] = [PDH].[PICKING_DEMAND_HEADER_ID]
    INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] ON [W].[WAREHOUSE_ID] = [PDH].[CODE_WAREHOUSE]
    WHERE
        (
