...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_VALIDATE_INVENTORY_FOR_MASTERPACK_IMPLOSION] 
				@MATERIAL_ID = 'autovanguard/VAD1001', -- varchar(50)
				@WAREHOUSE = 'BODEGA_01', -- varchar(50)
				@QUANTITY = 4 -- decimal
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_VALIDATE_INVENTORY_FOR_MASTERPACK_IMPLOSION] (
...
BEGIN
	...
	BEGIN TRY
		...
		INTO [#MATERIALS]
		FROM [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [CMP]
		LEFT JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IXL] ON [CMP].[COMPONENT_MATERIAL] = [IXL].[MATERIAL_ID]
		LEFT JOIN [FERCO].[OP_WMS_LICENSES] [L] ON (
			[L].[LICENSE_ID] = [IXL].[LICENSE_ID]
			AND [L].[CURRENT_WAREHOUSE] = @WAREHOUSE
		)
		LEFT JOIN [FERCO].[OP_WMS_FN_GET_COMMITED_INVENTORY_BY_LICENCE]() [IR] ON (
