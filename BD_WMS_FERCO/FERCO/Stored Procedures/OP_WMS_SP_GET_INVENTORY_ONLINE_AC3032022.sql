...
-- Descripcion: 		1/25/2020 - Se agrego el campo TOTAL_POSITION de la tabla [FERCO].[OP_WMS_INV_X_LICENSE]

/*
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_GET_INVENTORY_ONLINE_AC3032022] @LOGIN = 'ADMIN'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_INVENTORY_ONLINE_AC3032022]
(@LOGIN VARCHAR(25))
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT [V].[LICENSE_ID],
           ...
           [V].[TOTAL_VALOR],
           [V].[MATERIAL_ID]
    FROM [FERCO].[OP_WMS_VIEW_VALORIZACION] [V]
    WHERE [V].[QTY] > 0;
    ...
    SELECT [ID].[PK_LINE],
           ...
           [ID].[ZONE],
           CASE
               WHEN ISNULL([ID].[LOCKED_BY_INTERFACES], 0) = 1 THEN  0
				WHEN ISNULL([ID].IS_DISPATCH,0) = 1 THEN 0
               ELSE
           ...
           [PH].[LAST_UPDATED] AS FECHA_DOCUMENTO,
           CASE [PH].[WAREHOUSE_REGIMEN]
               WHEN @WAREHOUSE_REGIMEN THEN
                   [FERCO].[OP_WMS_FN_GET_DAYS_BY_REGIMEN]([PH].[REGIMEN])
               ELSE
                   NULL
           END [DIAS_REGIMEN],
           CASE [PH].[WAREHOUSE_REGIMEN]
               WHEN @WAREHOUSE_REGIMEN THEN
                   DATEDIFF(DAY, GETDATE(), [FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA]))
               ELSE
                   NULL
           END [DIAS_PARA_VENCER],
           CASE [PH].[WAREHOUSE_REGIMEN]
               WHEN @WAREHOUSE_REGIMEN THEN
                   [FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA])
               ELSE
                   NULL
           ...
           (
               SELECT TOP (1)
                      IXL.TOTAL_POSITION
               FROM [FERCO].OP_WMS_INV_X_LICENSE AS IXL
               WHERE IXL.LICENSE_ID = ID.LICENSE_ID
               ORDER BY IXL.LICENSE_ID
           ),
           1
                 ) [TOTAL_POSITION]
    FROM [FERCO].[OP_WMS_VIEW_INVENTORY_DETAIL_WHITH_SERIES_QAPASE30032022] [ID]
        left JOIN [FERCO].[OP_WMS_TARIFICADOR_HEADER] [TH]
        ...
        LEFT JOIN [FERCO].[OP_WMS_LICENSES] AS [L]
            ON ([L].[LICENSE_ID] = [ID].[LICENSE_ID])
        LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] AS [DH]
            ON ([DH].[PICKING_DEMAND_HEADER_ID] = [L].[PICKING_DEMAND_HEADER_ID])
    WHERE [ID].[QTY] > 0 AND [ID].[CURRENT_WAREHOUSE] = 'CEDI_ZONA_5' and id.MATERIAL_ID = 'ferco/032370';
END;
