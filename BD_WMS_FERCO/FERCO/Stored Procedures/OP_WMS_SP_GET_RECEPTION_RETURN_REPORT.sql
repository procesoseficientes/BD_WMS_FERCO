...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_RECEPTION_RETURN_REPORT] 
...
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_RECEPTION_RETURN_REPORT (@START_DATE DATETIME
, @END_DATE DATETIME
, @LOGIN VARCHAR(50)
...
BEGIN
  ...
  INSERT INTO @WAREHOUSES
    ...
    FROM [FERCO].[OP_WMS_FN_SPLIT](@WAREHOUSE, '|')
  ...
  INSERT INTO @DETAIL
    SELECT
      [RH].[ERP_RECEPTION_DOCUMENT_HEADER_ID]
     ,[RH].[TASK_ID]
     ,[IL].[MATERIAL_ID]
     ,[IL].[MATERIAL_NAME]
    ...
    INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T]
      ON [T].[SERIAL_NUMBER] = [RH].[TASK_ID]
    INNER JOIN [FERCO].[OP_WMS_LICENSES] [L]
      ON [T].[CODIGO_POLIZA_SOURCE] = [L].[CODIGO_POLIZA]
  ...
  INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W]
    ON [RH].[ERP_WAREHOUSE_CODE] = [W].[ERP_WAREHOUSE]
  INNER JOIN @WAREHOUSES [PW]
    ON [PW].[WAREHOUSE_CODE] = [W].[WAREHOUSE_ID]
  INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M]
    ON [M].[MATERIAL_ID] = [RD].[MATERIAL_ID]
  LEFT JOIN @DETAIL [D]
    ON [D].[MATERIAL_ID] = [RD].[MATERIAL_ID]
      AND [RH].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [D].[RECEPTION_DOCUMENT_HEADER_ID]
  WHERE [RH].[SOURCE] = 'INVOICE'
  AND [RH].[ERP_DATE] BETWEEN @START_DATE AND @END_DATE
END