...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_DISTRIBUTION_CENTER_ASSOCIATED_TO_USER]
					@LOGIN = 'ADMIN'
				--
				EXEC [FERCO].[OP_WMS_SP_GET_DISTRIBUTION_CENTER_ASSOCIATED_TO_USER]
					@LOGIN = 'AREYES'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_DISTRIBUTION_CENTER_ASSOCIATED_TO_USER](
	@LOGIN VARCHAR(25)
)
AS
BEGIN
	...
	+ CASE 
		WHEN @HAS_PERMISSION = 0 THEN ' WITH(INDEX ([IN_OP_WMS_CONFIGURATIONS_PARAM_GROUP])) INNER JOIN [FERCO].[OP_WMS_LOGINS] [L] ON ([L].[DISTRIBUTION_CENTER_ID] = [C].[PARAM_NAME])'
		ELSE ''
	END
