...
-- Ejemplo de Ejecucion:
	EXECUTE [FERCO].[OP_WMS_SP_DELETE_INVENTORY_RESERVED_BY_XML] @LOGIN = 'MARVIN', @PROJECT_ID = '5D9B419A-96D3-4EA1-A2F9-06CE6AB83EED', 
...
CREATE PROCEDURE [FERCO].[OP_WMS_SP_DELETE_INVENTORY_RESERVED_BY_XML] (
		@LOGIN AS VARCHAR(25)
...
BEGIN TRY
	...
	FROM
		...
		AS [x] ([data])
	INNER JOIN [FERCO].[OP_WMS_INVENTORY_RESERVED_BY_PROJECT] [IRP] ON (
											[x].[data].[query]('./ID').[value]('.',
											'INTEGER') = [IRP].[ID]
											AND [x].[data].[query]('./PK_LINE').[value]('.',
											'INTEGER') = [IRP].[PK_LINE]
											)
	LEFT JOIN [FERCO].[OP_WMS_FN_GET_COMMITED_INVENTORY_BY_LICENCE]() [FNCIL] ON (
	...
	SELECT
		...
		,[IRP].[QTY_DISPATCHED]
	FROM
		[FERCO].[OP_WMS_INVENTORY_RESERVED_BY_PROJECT] [IRP]
	...
	DELETE
		[IRBP]
	FROM
		[FERCO].[OP_WMS_INVENTORY_RESERVED_BY_PROJECT] [IRBP]
