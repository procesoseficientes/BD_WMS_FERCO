...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_TRANSFER_REQUEST_TRACEABILITY]
					@TRANSFER_REQUEST_ID = 75
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TRANSFER_REQUEST_TRACEABILITY](
...
BEGIN
	...
	FROM [FERCO].[OP_WMS_MANIFEST_HEADER] [MH]
		...
		INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RH] ON [RH].[DOC_NUM] = [MH].[MANIFEST_HEADER_ID] AND [RH].[IS_FROM_WAREHOUSE_TRANSFER] = 1
		INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [TL] ON [TL].[SERIAL_NUMBER] = [RH].[TASK_ID]
		INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [L].[CODIGO_POLIZA] = [TL].[CODIGO_POLIZA_SOURCE]
