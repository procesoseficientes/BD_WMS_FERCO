...
-- Description:			SP que mueve la licencia creada en recepcion a una licencia de la ubicacion cuando la ubicacion es de fast picking
...
CREATE PROCEDURE [FERCO].[OP_WMS_SP_UPDATE_LICENSE_FAST_PICKING] (
		@LOGIN_ID VARCHAR(50)
		,@LICENSE_ID INT
		,@LOCATION_ID VARCHAR(25)
...
BEGIN
	...
	SET	
		...
		,[ORIGINAL_LICENSE] = @ORIGIN_LOCATION_LICENSE
	WHERE
		[TASK_ID] = @TASK_ID;
	...
	-- ------------------------------------------------------------------------------------
	-- ELIMINAMOS LA LICENCIA ORIGINAL LUEGO DE TRASLADAR EL INVENTARIO
	-- ------------------------------------------------------------------------------------
	DELETE FROM
		[FERCO].[OP_WMS_INV_X_LICENSE]
	WHERE
