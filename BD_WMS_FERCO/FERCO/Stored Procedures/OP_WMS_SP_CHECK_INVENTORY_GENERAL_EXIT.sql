...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_CHECK_INVENTORY_GENERAL_EXIT]
					@XML = N'
<EGRESO>
  <MATERIAL>
    ...
    <MATERIAL_ID>FERCO/SKUPRUEBA</MATERIAL_ID>
    <BARCODE_ID>FERCO/SKUPRUEBA</BARCODE_ID>
    <ALTERNATE_BARCODE>FERCO/SKUPRUEBA</ALTERNATE_BARCODE>
    <MATERIAL_NAME>Prueba</MATERIAL_NAME>
    <CLIENT_OWNER>FERCO</CLIENT_OWNER>
    <CLIENT_NAME>Ferco Guatemala</CLIENT_NAME>
  ...
  <MATERIAL>
    ...
    <CODIGO_POLIZA_TARGET>0</CODIGO_POLIZA_TARGET>
    <MATERIAL_ID>FERCO/RD001</MATERIAL_ID>
    <BARCODE_ID>FERCO/RD001</BARCODE_ID>
    <ALTERNATE_BARCODE>FERCO/RD001</ALTERNATE_BARCODE>
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_CHECK_INVENTORY_GENERAL_EXIT] (
		@XML XML
	...
	BEGIN TRY
			...
			BEGIN
				SELECT
					@CURRENTLY_AVAILABLE = ISNULL(SUM([ORP].[QTY_RESERVED]
											...
											0)
				FROM
					[FERCO].[OP_WMS_INVENTORY_RESERVED_BY_PROJECT] [ORP]
		...
		BEGIN
			...
			SELECT
				@CURRENTLY_AVAILABLE = (SUM([VPAG].[QTY])
										- (ISNULL([RD].[QTY],
											...
											0)))
			FROM
				[FERCO].[OP_WMS_VIEW_PICKING_AVAILABLE_GENERAL] [VPAG]
			...
			BEGIN
				...
				FROM
					[FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [CXMP]
				LEFT JOIN [FERCO].[OP_WMS_VIEW_PICKING_AVAILABLE_GENERAL] [IXW] ON [IXW].[MATERIAL_ID] = [CXMP].[COMPONENT_MATERIAL]
