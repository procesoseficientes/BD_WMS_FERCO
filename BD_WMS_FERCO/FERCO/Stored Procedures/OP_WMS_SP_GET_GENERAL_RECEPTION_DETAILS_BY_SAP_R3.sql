...
-- Ejemplo de Ejecucion:
         EXEC [FERCO].[OP_WMS_SP_GET_GENERAL_RECEPTION_DETAILS_BY_SAP_R3] @RECEPTION_HEADER = 109
*/
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_GENERAL_RECEPTION_DETAILS_BY_SAP_R3] (
...
BEGIN
	...
	INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [h] ON [h].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [D].[ERP_RECEPTION_DOCUMENT_HEADER_ID]
	INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [tl] ON [tl].[SERIAL_NUMBER] = [h].[TASK_ID]
	INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [M].[MATERIAL_ID] = [D].[MATERIAL_ID]
	INNER JOIN [FERCO].[OP_WMS_COMPANY] [CP] ON [CP].[EXTERNAL_SOURCE_ID] = [h].[EXTERNAL_SOURCE_ID]
											AND [CP].[CLIENT_CODE] = [h].[OWNER]
	LEFT JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [C] ON [C].[PARAM_GROUP] = 'TIPO_RECEPCION_RFC'
											AND [C].[PARAM_NAME] = @PARAM_NAME
	LEFT JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [CM] ON [CM].[PARAM_NAME] = [M].[BASE_MEASUREMENT_UNIT]
