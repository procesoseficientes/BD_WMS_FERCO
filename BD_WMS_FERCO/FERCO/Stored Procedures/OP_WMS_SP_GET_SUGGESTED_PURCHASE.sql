...
-- Ejemplo de Ejecucion:
        EXEC [FERCO].OP_WMS_SP_GET_SUGGESTED_PURCHASE
*/
-- =============================================

CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_SUGGESTED_PURCHASE] (
		@LOGIN VARCHAR(25)
		,@WAREHOUSE_XML XML
		,@MATERIAL_XML XML
	)
...
BEGIN
  ...
  -- -----------------------------------------------------------------
    ...
    -- -----------------------------------------------------------------
		...
		INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] ON ([W].[WAREHOUSE_ID] = [WU].[WAREHOUSE_ID])
		WHERE
			[WU].[LOGIN_ID] = @LOGIN;
	END;
  ...
  -- ------------------------------------------------------------------------------------
	...
	FROM
		[FERCO].[OP_WMS_WAREHOUSE_INDICES];
-- ------------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------------
	...
	FROM
		[FERCO].[OP_WMS_WAREHOUSE_INDICES] [WI]
	INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [WI].[MATERIAL_CODE] = [M].[MATERIAL_ID]
