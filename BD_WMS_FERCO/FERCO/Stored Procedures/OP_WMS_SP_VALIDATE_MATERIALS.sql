-- =============================================
-- Autor:				alberto.ruiz
-- Fecha de Creacion: 	16-Oct-17 @ Nexus Team Sprint eFERCO 
-- Description:			SP que valida la configuracion de materiales
/*
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_VALIDATE_MATERIALS]
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_VALIDATE_MATERIALS]
AS
BEGIN
	SET NOCOUNT ON;
	--
	...
	SELECT
		...
		,[SS].[LINE_ID]
	FROM [FERCO].[OP_WMS_MATERIALS] [M]
	INNER JOIN [FERCO].[OP_WMS_MATERIALS_JOIN_SPOTS] [MJS] ON ([MJS].[MATERIAL_ID] = [M].[MATERIAL_ID])
	INNER JOIN [FERCO].[OP_WMS_SHELF_SPOTS] [SS] ON ([SS].[LOCATION_SPOT] = [MJS].[LOCATION_SPOT])
	...
	FROM [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [M]
	...
	FROM [FERCO].[OP_WMS_MATERIALS] [M]
	LEFT JOIN [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [CMP] ON ([M].[MATERIAL_ID] = [CMP].[MASTER_PACK_CODE])
	...
	FROM [FERCO].[OP_WMS_MATERIALS] [M1]
	INNER JOIN [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [CMP1] ON ([M1].[MATERIAL_ID] = [CMP1].[MASTER_PACK_CODE])
	INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M2] ON ([M2].[MATERIAL_ID] = [CMP1].[COMPONENT_MATERIAL])
	INNER JOIN [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [CMP2] ON ([M2].[MATERIAL_ID] = [CMP2].[MASTER_PACK_CODE])
