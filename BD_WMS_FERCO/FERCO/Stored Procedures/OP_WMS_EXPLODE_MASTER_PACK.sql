...
-- Modificacion 2/21/2018 @LOGISTICA_FERCO
-- alejandro.ochoa
...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_EXPLODE_MASTER_PACK] @LICENSE_ID = 4 , @MATERIAL_ID = 'MELLEGA/C00000493'
                                                ,@LAST_UPDATE_BY = 'PABS'
      SELECT * FROM [FERCO].[OP_WMS_MASTER_PACK_HEADER] [H]
      SELECT * FROM [FERCO].[OP_WMS_MASTER_PACK_DETAIL] [D] 
      SELECT * FROM [FERCO].[OP_WMS_INV_X_LICENSE]  WHERE [LICENSE_ID] = 177679
  SELECT * FROM [FERCO].OP_WMS_TRANS WHERE [LICENSE_ID] = 177679
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_EXPLODE_MASTER_PACK] (
		@LICENSE_ID INT
...
BEGIN
  ...
  ---Obtiene la propiedad de Exposion en Recepcion tomando en cuenta las excepciones por bodega
	...
	LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY_BY_WAREHOUSE] [PW] ON (
											[PW].[MATERIAL_ID] = [M].[MATERIAL_ID]
											AND [PW].[WAREHOUSE_ID] = @LICENCE_WAREHOUSE
											)
	LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY] [MP] ON (
  ...
  ---------------------------------------------------------------------------------  
	IF NOT EXISTS ( SELECT TOP 1
						1
					FROM
						[FERCO].[OP_WMS_MASTER_PACK_HEADER] [PH]
					WHERE
	...
	IF EXISTS ( SELECT TOP 1
				...
				LEFT JOIN [FERCO].[OP_WMS_MASTER_PACK_DETAIL] [D] ON [H].[MASTER_PACK_HEADER_ID] = [D].[MASTER_PACK_HEADER_ID]
				WHERE
	...
	--- se comenta para que no haga la validacion y permita realizar la explocion manual del kit 
  ...
  --
    ...
    -- Agregar inventario de componentes de masterpack
		MERGE [FERCO].[OP_WMS_INV_X_LICENSE] [IL]
		USING
			(SELECT
					[H].[LICENSE_ID] [LICENSE_ID]
				...
				FROM
					[FERCO].[OP_WMS_MASTER_PACK_HEADER] [H]
				INNER JOIN [FERCO].[OP_WMS_MASTER_PACK_DETAIL] [D] ON [H].[MASTER_PACK_HEADER_ID] = [D].[MASTER_PACK_HEADER_ID]
				INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [D].[MATERIAL_ID] = [M].[MATERIAL_ID]
				INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [P] ON [H].[POLICY_HEADER_ID] = [P].[DOC_ID]
				INNER JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IXL] ON (
    ...
    -- Transaccion de salida
		...
		FROM
			[FERCO].[OP_WMS_MASTER_PACK_HEADER] [H]
		INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [P] ON [H].[POLICY_HEADER_ID] = [P].[DOC_ID]
		INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [H].[MATERIAL_ID] = [M].[MATERIAL_ID]
		INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [H].[LICENSE_ID] = [L].[LICENSE_ID]
		INNER JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [C] ON [P].[CLIENT_CODE] = [C].[CLIENT_CODE]
		INNER JOIN [FERCO].[OP_WMS_SHELF_SPOTS] [S] ON [S].[LOCATION_SPOT] = [L].[CURRENT_LOCATION]
		INNER JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [I] ON [I].[LICENSE_ID] = [L].[LICENSE_ID]
    ...
    --transacción de entrada
		...
		INNER JOIN [FERCO].[OP_WMS_MASTER_PACK_HEADER] [H] ON [D].[MASTER_PACK_HEADER_ID] = [H].[MASTER_PACK_HEADER_ID]
		INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [P] ON [H].[POLICY_HEADER_ID] = [P].[DOC_ID]
		INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [D].[MATERIAL_ID] = [M].[MATERIAL_ID]
		INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [H].[LICENSE_ID] = [L].[LICENSE_ID]
		INNER JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [C] ON [P].[CLIENT_CODE] = [C].[CLIENT_CODE]
    ...
    ---------------------------------------------------------------------------------  
		...
		BEGIN
			...
			EXEC [FERCO].[OP_WMS_SP_INSERT_MASTER_PACK] @MATERIAL_ID_MASTER_PACK = @M_MATERIAL_ID,
