...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_PICKING_HAS_BEEN_COMPLETED]
					@WAVE_PICKING_ID = 1
					,@LOGIN = 'admin'
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_PICKING_HAS_BEEN_COMPLETED (@WAVE_PICKING_ID INT
, @LOGIN VARCHAR(50))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  BEGIN
    SELECT
      @IS_COMPLETED = MIN([IS_COMPLETED])
    FROM [FERCO].[OP_WMS_TASK_LIST]
    WHERE [WAVE_PICKING_ID] = @WAVE_PICKING_ID
    --
    IF (@IS_COMPLETED > 0)
    BEGIN
      EXEC [FERCO].[OP_WMS_SP_REGENERATE_CARGO_MANIFEST_FROM_PICKING_DEMAND] @WAVE_PICKING_ID = @WAVE_PICKING_ID
                                                                            , -- int
