...
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_COPY_INVENTORY_TO_HIST @pRESULT VARCHAR(300) OUTPUT
AS
BEGIN
  ...
  BEGIN TRY
    ...
    INSERT INTO [FERCO].OP_WMS_INV_HISTORY (REGIMEN
    , CLIENT_NAME
    , CLIENT_OWNER
    , NUMERO_ORDEN
    , LICENSE_ID
    ...
    , VIN)
      SELECT
       ...
       ,A.DATE_EXPIRATION
       ,A.VIN
      FROM [FERCO].OP_WMS_VIEW_VALORIZACION_FISCAL A
      WHERE [A].[QTY] > 0
    ...
    , VIN)
      SELECT
       ...
       ,A.DATE_EXPIRATION
       ,A.VIN
      FROM [FERCO].OP_WMS_VIEW_VALORIZACION_ALMGEN A
      WHERE [A].[QTY] > 0
