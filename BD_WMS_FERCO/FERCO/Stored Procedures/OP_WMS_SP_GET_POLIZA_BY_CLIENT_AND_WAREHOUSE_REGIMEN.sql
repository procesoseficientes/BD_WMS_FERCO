...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_GET_POLIZA_BY_CLIENT_AND_WAREHOUSE_REGIMEN]
				@CLIENT_CODE = 'C01096'
                ,@WHAREHOUSE_REGIMEN = 'GENERAL'
				,@LOGIN = 'ADMIN'
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_POLIZA_BY_CLIENT_AND_WAREHOUSE_REGIMEN (@CLIENT_CODE VARCHAR(25)
...
BEGIN
  ...
  SELECT
   ...
   ,CASE [P].[WAREHOUSE_REGIMEN]
      WHEN 'FISCAL' THEN [FERCO].[OP_WMS_FN_GET_DAYS_BY_REGIMEN]([P].[REGIMEN])
      ELSE NULL
    END [DIAS_REGIMEN]
   ,CASE [P].[WAREHOUSE_REGIMEN]
      WHEN 'FISCAL' THEN DATEDIFF(DAY, GETDATE(), [FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([P].[CODIGO_POLIZA]))
      ELSE NULL
    END [DIAS_PARA_VENCER]
   ,CASE [P].[WAREHOUSE_REGIMEN]
      WHEN 'FISCAL' THEN [FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([P].[CODIGO_POLIZA])
      ELSE NULL
    END [FECHA_VENCIMIENTO]
   ,CASE
      WHEN [P].[WAREHOUSE_REGIMEN] = 'FISCAL' AND
        DATEDIFF(DAY, GETDATE(), [FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([P].[CODIGO_POLIZA])) < 1 THEN 'Bloqueado'
      ELSE 'Libre'
    END [ESTADO_REGIMEN]
