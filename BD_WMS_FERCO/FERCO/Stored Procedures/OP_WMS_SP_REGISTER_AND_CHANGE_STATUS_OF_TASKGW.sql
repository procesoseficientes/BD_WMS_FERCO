...
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_AND_CHANGE_STATUS_OF_TASKGW]
(
                ...
                WHERE [SERIAL_NUMBER] = @pTASK_ID
					...
					BEGIN
						...
						UPDATE
							[D]
						SET	
							[D].[POSTED_RESPONSE] = 'Completada sin movimientos'
							,[D].[POSTED_STATUS] = 3
							,[D].[IS_POSTED_ERP] = 1
						FROM
						...
						INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [TL] ON [TL].[WAVE_PICKING_ID] = [H].[WAVE_PICKING_ID]
						WHERE
							[TL].[SERIAL_NUMBER] = @pTASK_ID;
--						select * from [FERCO].[OP_WMS_TASK_LIST]  where WAVE_PICKING_ID = 103144
--						select * from [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] where PICKING_DEMAND_HEADER_ID=199995
--						select * from [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] where WAVE_PICKING_ID = 103144
--						select * from [FERCO].[OP_WMS_TRANS] order by 1 descwhere [TASK_ID]=5140297
--5140295
--5140294
--5140297
					...
					ELSE 
            BEGIN
			PRINT 'STATUS COMPLETED TRANSACTION'
                -- ------------------------------------------------------------------------------------
			   ...
			   IF @pSTATUS = 'ACCEPTED'
                BEGIN PRINT 'STATUS ACCEPTED BEGIN IF'
