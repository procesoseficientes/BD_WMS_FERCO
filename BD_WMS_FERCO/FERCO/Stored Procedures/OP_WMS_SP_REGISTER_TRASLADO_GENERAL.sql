...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_TRASLADO_GENERAL]
...
BEGIN
    ...
    BEGIN TRY
        ...
        (   ISNULL(
            ...
            (
                SELECT *
                FROM [FERCO].OP_WMS_FUNC_GETMATERIAL_DESC(@pMATERIAL_BARCODE, @pCLIENT_OWNER_LOCAL)
            ), ISNULL(
               (
               ...
               'N/A'
                     ), [FERCO].[OP_WMS_FN_GET_MATERIAL_COST_BY_MATERIAL](@pMATERIAL_ID_LOCAL, @pCLIENT_OWNER_LOCAL),
            @pSOURCE_LICENSE, NULL, @pSOURCE_LOCATION, 'PUERTA_1', @pCLIENT_OWNER_LOCAL,
            (
                SELECT * FROM [FERCO].OP_WMS_FUNC_GETCLIENT_NAME(@pCLIENT_OWNER_LOCAL)
            ), (@pQUANTITY_UNITS * -1),
            ISNULL(
            (
        ...
        BEGIN
            ...
            DELETE FROM [FERCO].OP_WMS_TASK_LIST
            WHERE WAVE_PICKING_ID = @pWAVE_PICKING_ID
                  AND MATERIAL_ID = @pMATERIAL_ID_LOCAL
                  AND CODIGO_POLIZA_TARGET = @pCODIGO_POLIZA
                  AND IS_COMPLETED = 0;
