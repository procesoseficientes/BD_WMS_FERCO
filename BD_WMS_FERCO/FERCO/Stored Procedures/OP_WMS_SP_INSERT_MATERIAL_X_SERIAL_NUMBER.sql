...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_INSERT_MATERIAL_X_SERIAL_NUMBER]
					@LICENSE_ID = 5
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_INSERT_MATERIAL_X_SERIAL_NUMBER] (
...
BEGIN
	...
	-- ------------------------------------------------------------------------------------
		...
		IF EXISTS ( SELECT TOP 1
					...
					INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RH] ON [RH].[DOC_ID_POLIZA] = [PH].[DOC_ID]
					WHERE
        ...
        -- ------------------------------------------------------------------------------------		
			...
			INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [RD] ON [RD].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [RH].[ERP_RECEPTION_DOCUMENT_HEADER_ID]
			LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [UMM].[MATERIAL_ID] = [RD].[MATERIAL_ID]
        ...
        -- ------------------------------------------------------------------------------------
			...
			FROM
				[FERCO].[OP_WMS_LICENSES] [L]
			INNER JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [IL] ON [IL].[LICENSE_ID] = [L].[LICENSE_ID]
											AND [IL].[STATUS] > 0
			WHERE
		...
		INSERT	INTO [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER]
				(
					[LICENSE_ID]
					,[MATERIAL_ID]
					,[SERIAL]
