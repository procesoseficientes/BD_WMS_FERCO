...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_BOX_BY_ID]
					@BOX_ID = 'PC-4905-9/13'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_BOX_BY_ID](
	@BOX_ID VARCHAR(50)
)
AS
BEGIN
	...
	-- Obtiene la ola
	-- ------------------------------------------------------------------------------------
	SELECT @WAVE_PICKING_ID = [FERCO].[OP_WMS_FN_SPLIT_COLUMNS](@ERP_DOC,2,'-')
	--
	...
	INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PH] ON ([PH].[PICKING_DEMAND_HEADER_ID] = [PDD].[PICKING_DEMAND_HEADER_ID])
	INNER JOIN [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [CMP] ON ([CMP].[MASTER_PACK_CODE] = [PDD].[MATERIAL_ID])
	...
	FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PD]
