...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_INSERT_CERTIFICATION_DETAIL] @CERTIFICATION_HEADER_ID = 1                                                
*/
-- =============================================
CREATE PROCEDURE [FERCO].OP_WMS_SP_INSERT_CERTIFICATION_DETAIL
    (
...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM
            [FERCO].[OP_WMS_CERTIFICATION_HEADER] [CH]
        WHERE
        ...
        FROM
            [FERCO].[OP_WMS_CERTIFICATION_DETAIL] [CD]
        WHERE
		...
		BEGIN
            ...
            IF NOT EXISTS ( SELECT TOP 1 1
							FROM
								[FERCO].[OP_WMS_PICKING_LABEL_BY_MANIFEST] [PLM]
							INNER JOIN [FERCO].[OP_WMS_MANIFEST_DETAIL] [MD] ON ([MD].[MANIFEST_DETAIL_ID] = [PLM].[MANIFEST_DETAIL_ID])
							INNER JOIN [FERCO].[OP_WMS_PICKING_LABELS] [PL] ON ([PL].[LABEL_ID] = [PLM].[LABEL_ID])
							WHERE
			...
			BEGIN
				IF EXISTS ( SELECT TOP 1 1 
							FROM 
								[FERCO].[OP_WMS_CERTIFICATION_DETAIL] [CD]
							INNER JOIN [FERCO].[OP_WMS_CERTIFICATION_HEADER] [CH] ON [CH].[CERTIFICATION_HEADER_ID] = [CD].[CERTIFICATION_HEADER_ID]
				...
				BEGIN
					...
					INSERT INTO [FERCO].[OP_WMS_CERTIFICATION_DETAIL]
					        (
					...
					SELECT DISTINCT @CERTIFICATION_HEADER_ID
						...
						,@LAST_UPDATE
						,[BOX_ID]
					FROM [FERCO].[OP_WMS_CERTIFICATION_HEADER] [CH]
					INNER JOIN [FERCO].[OP_WMS_MANIFEST_HEADER] [MH] ON [MH].[MANIFEST_HEADER_ID] = [CH].[MANIFEST_HEADER_ID]
					INNER JOIN [FERCO].[OP_WMS_MANIFEST_DETAIL] [MD] ON [MD].[MANIFEST_HEADER_ID] = [MH].[MANIFEST_HEADER_ID]
					INNER JOIN [FERCO].[OP_WMS_DISTRIBUTED_TASK] [DT] ON [FERCO].[OP_WMS_FN_SPLIT_COLUMNS]([ERP_DOC], 2,'-') = [MD].[WAVE_PICKING_ID]
        ...
        BEGIN
			...
			-- ------------------------------------------------------------
                ...
                FROM
                    [FERCO].[OP_WMS_PICKING_LABELS] [PL]
                WHERE
		...
		-- ------------------------------------------------------------
            UPDATE
                [FERCO].[OP_WMS_CERTIFICATION_DETAIL]
            SET
                [QTY] += @QTY
            WHERE
		...
		-- ------------------------------------------------------------
			...
			-- ------------------------------------------------------------
                DELETE
                    [CSN]
                FROM
                    [FERCO].[OP_WMS_CERTIFICATION_BY_SERIAL_NUMBER] CSN
                INNER JOIN [FERCO].[OP_WMS_PICKING_LABELS_BY_SERIAL_NUMBER] [PLS] ON ([CSN].[SERIAL_NUMBER] = [PLS].[SERIAL_NUMBER])
                WHERE
                ...
                SET @QUERY = 'INSERT INTO [FERCO].[OP_WMS_CERTIFICATION_BY_SERIAL_NUMBER] ([CERTIFICATION_HEADER_ID], [MATERIAL_ID], [SERIAL_NUMBER])
						SELECT TOP ' + CAST(CAST(@QTY AS INT) AS VARCHAR) + '
						  ' + CAST(@CERTIFICATION_HEADER_ID AS VARCHAR) + '
						  ,''' + @MATERIAL_ID + ''' 
						  ,[PLS].[SERIAL_NUMBER]
						FROM  [FERCO].[OP_WMS_PICKING_LABELS_BY_SERIAL_NUMBER] [PLS]
						LEFT JOIN [FERCO].[OP_WMS_CERTIFICATION_BY_SERIAL_NUMBER] [CSN] ON(
