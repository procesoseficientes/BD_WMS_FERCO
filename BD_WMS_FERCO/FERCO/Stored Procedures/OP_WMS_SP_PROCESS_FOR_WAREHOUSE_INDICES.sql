...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_PROCESS_FOR_WAREHOUSE_INDICES]
				
*/
-- =============================================  
CREATE PROCEDURE [FERCO].[OP_WMS_SP_PROCESS_FOR_WAREHOUSE_INDICES]
AS
BEGIN
	BEGIN TRAN;
	BEGIN TRY
    ...
    -- --------------------------------------------------
		INSERT	INTO [FERCO].[OP_WMS_WAREHOUSE_INDICES]
				(
					[CODE_WAREHOUSE]
					,[MATERIAL_CODE]
					,[BARCODE_ID]
		...
		INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON ([MDW].[CODE_WAREHOUSE] = [L].[CURRENT_WAREHOUSE])
