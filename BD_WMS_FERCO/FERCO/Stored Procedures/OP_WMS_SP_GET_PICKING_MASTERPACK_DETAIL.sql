...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_PICKING_MASTERPACK_DETAIL]
					@PICKING_DEMAND_HEADER_ID = 4190
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_PICKING_MASTERPACK_DETAIL (@PICKING_DEMAND_HEADER_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  SELECT
   ...
   ,[FERCO].[OP_WMS_GET_STRING_FROM_CHAR]([C].[COMPONENT_MATERIAL], '/') AS [ItemCode]
  ...
  INNER JOIN [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [C]
    ON ([C].[MASTER_PACK_CODE] = [D].[MATERIAL_ID])
