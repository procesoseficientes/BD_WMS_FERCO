...
-- Description:			SP que unifica las licencias de una ubicación con material opcional. 

-- Autor:				rudi.garcia
-- Fecha de Creacion: 	20-Feb-2018 @ Reborn Team Sprint Ulrich
-- Description:			Se eliminaron la agrupacion por "[MATERIAL_NAME]" y "[BARCODE_ID]" donde se inserta "[#NEW_LICENCES_DETAIL]"
...
-- Ejemplo de Ejecucion:
				 EXEC [FERCO].[OP_WMS_SP_MERGE_LICENSE_IN_LOCATION]  
						@LOCATION  = 'PISO1'
				 		,@MATERIAL_ID  = 'me_llega/U00000539'
						,@LOGIN  = 'PABS'

		SELECT [L].[CURRENT_LOCATION], IL.* FROM [FERCO].[OP_WMS_INV_X_LICENSE] IL INNER JOIN [FERCO].[OP_WMS_LICENSES] L ON [L].[LICENSE_ID] = [IL].[LICENSE_ID] WHERE [QTY] > 0 AND L.CURRENT_LOCATION = 'PISO1' ORDER BY MATERIAL_ID

		SELECT * FROM [FERCO].OP_WMS_TRANS where license_id IN (14418,14419)

		select * from [FERCO].OP_WMS_INV_X_LICENSE WHERE MATERIAL_ID = 'me_llega/U00000539'
		

*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_MERGE_LICENSE_IN_LOCATION_TRANS_OFF]
(
    @LOCATION VARCHAR(25),
...
BEGIN
    ...
    DECLARE @LICENSE_CORRELATIVE INT,
    ...
    -- ------------------------------------------------------------------------------------
    SELECT @IS_WASTE_LOCATION = [IS_WASTE]
    FROM [FERCO].[OP_WMS_SHELF_SPOTS]
    WHERE [LOCATION_SPOT] = @LOCATION;
    ...
    SELECT [IL].[PK_LINE],
           ...
           [L].[CURRENT_LOCATION],
           CASE
               WHEN [L].[PICKING_DEMAND_HEADER_ID] IS NULL THEN
                   ISNULL([C].[COMMITED_QTY], 0)
               ELSE
                   0
           END [NEW_QTY],
           CASE
           ...
           CASE
               WHEN @IS_WASTE_LOCATION = 0 THEN
                   RANK() OVER (PARTITION BY [L].[PICKING_DEMAND_HEADER_ID],
                                             [IL].[MATERIAL_ID]
                                ORDER BY [IL].[BATCH],
                                         [IL].[DATE_EXPIRATION],
                                         [TC].[TONE],
    ...
    FROM [FERCO].[OP_WMS_LICENSES] [L]
        ...
        INNER JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [S]
            ON [S].[STATUS_ID] = [IL].[STATUS_ID]
        LEFT JOIN [FERCO].[OP_WMS_FN_GET_COMMITED_INVENTORY_BY_LICENCE]() [C]
            ON [C].[MATERIAL_ID] = [IL].[MATERIAL_ID]
    ...
    INTO [#POSSIBLE_LICENSES]
    FROM [#OLD_LICENSE];

    WHILE EXISTS (SELECT TOP 1 1 FROM [#POSSIBLE_LICENSES])
    BEGIN
        SELECT TOP 1
               @CLIENT_OWNER = [CLIENT_OWNER],
               @HEADER_ID = ISNULL([PICKING_DEMAND_HEADER_ID], 0),
               @ORDER = [CORRELATIVE_LICENSE_ID]
        FROM [#POSSIBLE_LICENSES];
        --
        ...
        SELECT [IL].[MATERIAL_ID],
               ...
               SUM([IL].[QTY]) - SUM(ISNULL([IL].[COMMITED_QTY], 0)) [QTY],
        ...
        FROM [#OLD_LICENSE] [IL]
            ...
            LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH]
                ON [PDH].[PICKING_DEMAND_HEADER_ID] = [IL].[PICKING_DEMAND_HEADER_ID]
        WHERE [IL].[CLIENT_OWNER] = @CLIENT_OWNER
              AND ISNULL([IL].[PICKING_DEMAND_HEADER_ID], 0) = @HEADER_ID
              AND [IL].[CORRELATIVE_LICENSE_ID] = @ORDER
        GROUP BY [IL].[CLIENT_OWNER],
                 [IL].[MATERIAL_ID],
                 --,[IL].[MATERIAL_NAME]
                 [IL].[BATCH],
                 [IL].[DATE_EXPIRATION],
                 ...
                 [IL].[PICKING_DEMAND_HEADER_ID],
                 [PDH].[DOC_NUM]
        HAVING SUM([IL].[QTY]) - SUM(ISNULL([IL].[COMMITED_QTY], 0)) > 0;
        --
        SELECT @CORRELATIVE_ID = @CORRELATIVE_ID + 1;
        --
        DELETE FROM [#POSSIBLE_LICENSES]
        WHERE [CLIENT_OWNER] = @CLIENT_OWNER
              AND ISNULL([PICKING_DEMAND_HEADER_ID], 0) = @HEADER_ID
              AND [CORRELATIVE_LICENSE_ID] = @ORDER;
    END;

    SELECT @COUNT_ACTUAL_LICENSE = COUNT([LICENSE_ID])
    FROM [#OLD_LICENSE];
    PRINT 'Licencias actuales ' + CAST(@COUNT_ACTUAL_LICENSE AS VARCHAR);


    SELECT @COUNT_NEW_LICENSE = COUNT([LICENSE_ID])
    FROM
    (
        SELECT DISTINCT
               [LICENSE_ID] [LICENSE_ID]
        FROM [#NEW_LICENCES_DETAIL]
    ) [LD];
    PRINT 'Licencias nuevas ' + CAST(@COUNT_NEW_LICENSE AS VARCHAR);
    ...
    BEGIN TRY
        ...
        -- ------------------------------------------------------------------------------------
        EXEC [FERCO].[OP_WMS_SP_CREATE_MATERIAL_STATUS] 'ESTADO_DEFAULT',
                                                        @STATUS_ID OUT,
                                                        @DEFAULT = 1;
        ...
        -- ------------------------------------------------------------------------------------
        WHILE EXISTS (SELECT TOP 1 1 FROM [#NEW_LICENCES_DETAIL] WHERE [USED] = 0)
        BEGIN
            SELECT @LICENSE_CORRELATIVE = [LICENSE_ID],
                   @CLIENT_CODE = [CLIENT_OWNER],
                   @HEADER_ID = [PICKING_DEMAND_HEADER_ID]
            FROM [#NEW_LICENCES_DETAIL]
            WHERE [USED] = 0;

            PRINT 'INICIA CORRELATIVO  ' + CAST(@LICENSE_CORRELATIVE AS VARCHAR);

            SELECT TOP 1
                   @ACUERDO_COMERCIAL = [AC].[ACUERDO_COMERCIAL]
            FROM [FERCO].[OP_WMS_ACUERDOS_X_CLIENTE] [AC]
                LEFT JOIN [FERCO].[OP_WMS_TARIFICADOR_HEADER] [T]
                    ON [T].[ACUERDO_COMERCIAL_ID] = [AC].[ACUERDO_COMERCIAL]
            WHERE [AC].[CLIENT_ID] = @CLIENT_CODE
            ORDER BY [T].[DEFAULT] DESC;
            ...
            EXEC [FERCO].[OP_WMS_SP_INSERT_POLIZA_HEADER] @DOC_ID = 0,                             -- int
            ...
            PRINT 'Crea Poliza ' + CAST(@CODIGO_POLIZA AS VARCHAR);
            -- ------------------------------------------------------------------------------------
            -- Inserta la licencia
            -- ------------------------------------------------------------------------------------
            INSERT INTO @OPERACION
            ...
            EXEC [FERCO].[OP_WMS_SP_CREA_LICENCIA] @pCODIGO_POLIZA = @CODIGO_POLIZA, -- varchar(25)
            ...
            DELETE @OPERACION;
            PRINT 'Crea Licencia ' + CAST(@LICENSE_ID AS VARCHAR) + ' - Orden de preparado: '
                  + CAST(ISNULL(@HEADER_ID, '') AS VARCHAR);


            UPDATE [FERCO].[OP_WMS_LICENSES]
            SET [PICKING_DEMAND_HEADER_ID] = @HEADER_ID
            ...
            SELECT @LICENSE_ID,
                   ...
                   [TONE_AND_CALIBER_ID],
                   CASE
                       WHEN [PICKING_DEMAND_HEADER_ID] IS NULL THEN
                           0
                       ELSE
                           1
                   END,
                   'Unificacion de licencias ubicación ' + @LOCATION
            FROM [#NEW_LICENCES_DETAIL]
            WHERE [LICENSE_ID] = @LICENSE_CORRELATIVE
                  AND [USED] = 0;
            ...
            SELECT @ACUERDO_COMERCIAL,
                   ...
                   [IL].[LICENSE_ID]
            FROM [#OLD_LICENSE] [IL]
            WHERE [IL].[CORRELATIVE_LICENSE_ID] = @LICENSE_CORRELATIVE;
            ...
            -- ------------------------------------------------------------------------------------
            UPDATE [MS]
            SET [MS].[LICENSE_ID] = @LICENSE_ID
            FROM [#OLD_LICENSE] [IL]
                INNER JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MS]
                    ON [MS].[LICENSE_ID] = [IL].[LICENSE_ID]
            ...
            FROM [#OLD_LICENSE] [L]
                INNER JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IL]
                    ON [IL].[PK_LINE] = [L].[PK_LINE]
            WHERE [L].[CORRELATIVE_LICENSE_ID] = @LICENSE_CORRELATIVE;
            ...
            -- ------------------------------------------------------------------------------------
            UPDATE [#NEW_LICENCES_DETAIL]
            SET [USED] = 1,
                [LICENSE_ID] = @LICENSE_ID
            WHERE [LICENSE_ID] = @LICENSE_CORRELATIVE;

            DELETE [#OLD_LICENSE]
            WHERE [CORRELATIVE_LICENSE_ID] = @LICENSE_CORRELATIVE;

            PRINT 'TERMINO NUEVA LICENCIA ' + CAST(@LICENSE_ID AS VARCHAR);
            PRINT 'TERMINA CORRELATIVO  ' + CAST(@LICENSE_CORRELATIVE AS VARCHAR);
        ...
        SELECT DISTINCT
               [LICENSE_ID],
               CAST([LICENSE_ID] AS VARCHAR) + '-' + [CLIENT_OWNER] [DESCRIPTION_LICENSE]
        FROM [#NEW_LICENCES_DETAIL];
