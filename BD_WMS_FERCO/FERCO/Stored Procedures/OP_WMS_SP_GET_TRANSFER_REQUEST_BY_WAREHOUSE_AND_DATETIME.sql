...
-- Ejemplo de Ejecucion:
				...
				--
				EXEC [FERCO].[OP_WMS_SP_GET_TRANSFER_REQUEST_BY_WAREHOUSE_AND_DATETIME] 
...
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_TRANSFER_REQUEST_BY_WAREHOUSE_AND_DATETIME (@START_DATETIME DATETIME
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
  SELECT
    [VALUE] [WAREHOUSE_ID] INTO [#WAREHOUSES]
  FROM [FERCO].[OP_WMS_FN_SPLIT](@WAREHOUSES, '|');
  -- ------------------------------------------------------------------------------------
  -- Inserta los centros de distribucion en una tabla temporal
  -- ------------------------------------------------------------------------------------
  SELECT
    [PARAM_CAPTION] [DISTRIBUTION_CENTER_NAME]
   ,[TEXT_VALUE] [DISTRIBUTION_CENTER_ID] INTO [#DISTRIBUTION_CENTERS]
  ...
  INNER JOIN [FERCO].[OP_WMS_TRANSFER_REQUEST_DETAIL] [TRD]
    ON [TRD].[TRANSFER_REQUEST_ID] = [TRH].[TRANSFER_REQUEST_ID]
  INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W]
    ON [W].[WAREHOUSE_ID] = [TRH].[WAREHOUSE_TO]
  INNER JOIN [#DISTRIBUTION_CENTERS] [DC]
    ON [DC].[DISTRIBUTION_CENTER_ID] = [W].[DISTRIBUTION_CENTER_ID]
  INNER JOIN [#WAREHOUSES] [WF]
    ON [TRH].[WAREHOUSE_TO] = [WF].[WAREHOUSE_ID]
  LEFT JOIN [FERCO].[OP_WMS_MANIFEST_HEADER] [MH]
    ON [MH].[TRANSFER_REQUEST_ID] = [TRH].[TRANSFER_REQUEST_ID]
  LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RDH]
    ON [RDH].[DOC_NUM] = [MH].[MANIFEST_HEADER_ID]
      AND [RDH].[IS_FROM_WAREHOUSE_TRANSFER] = 1
  WHERE (@STATUS <> 'OPEN'
  OR [TRD].[STATUS] = @STATUS)
