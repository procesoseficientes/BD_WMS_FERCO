...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_WAREHOUSE_BY_DISTRIBUTION_CENTER_AND_USER] @DISTRIBUTION_CENTER = 'CTR_SUR', @LOGIN_ID = 'ADMIN', @IS_WAREHOUSE_FROM = 1
			EXEC [FERCO].[OP_WMS_SP_GET_WAREHOUSE_BY_DISTRIBUTION_CENTER_AND_USER] @DISTRIBUTION_CENTER = 'CTR_SUR', @LOGIN_ID = 'BASC_OPER', @IS_WAREHOUSE_FROM = 1
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_WAREHOUSE_BY_DISTRIBUTION_CENTER_AND_USER] (
...
BEGIN
	...
	FROM [FERCO].[OP_WMS_FN_SPLIT](@DISTRIBUTION_CENTER,'|')
	-- ------------------------------------------------------------------------------------
	-- Obtiene el permiso
	-- ------------------------------------------------------------------------------------
	SELECT TOP 1 @HAS_PERMISSION = 1
		FROM [FERCO].[OP_WMS_ROLES_JOIN_CHECKPOINTS] [C]
		INNER JOIN [FERCO].[OP_WMS_LOGINS] [L] ON ([L].[ROLE_ID] = [C].[ROLE_ID])
	...
	SELECT @QUERY = N'
		...
		' + 
	CASE 
		WHEN @HAS_PERMISSION = 0 AND @IS_WAREHOUSE_FROM = 0 THEN '  INNER JOIN [FERCO].[OP_WMS_WAREHOUSE_BY_USER] [WU] ON [W].[WAREHOUSE_ID] = [WU].[WAREHOUSE_ID]
