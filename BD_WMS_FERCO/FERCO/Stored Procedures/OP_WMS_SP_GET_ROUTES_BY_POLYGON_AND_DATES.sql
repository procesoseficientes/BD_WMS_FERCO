...
-- Ejemplo de Ejecucion:
				EXEC  [FERCO].[OP_WMS_SP_GET_ROUTES_BY_POLYGON_AND_DATES]
					@POLYGON = '9315|9319'
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_ROUTES_BY_POLYGON_AND_DATES](
	@POLYGON VARCHAR(MAX)
...
BEGIN
	...
	BEGIN TRY
		...
		INTO [#POLYGON]
		FROM [FERCO].[OP_WMS_FN_SPLIT](@POLYGON,'|') [P]
			INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](@EXTERNAL_SOURCE_POLYGON, '|') [ES] ON [ES].[ID] = [P].[ID]
		-- ------------------------------------------------------------------------------------
