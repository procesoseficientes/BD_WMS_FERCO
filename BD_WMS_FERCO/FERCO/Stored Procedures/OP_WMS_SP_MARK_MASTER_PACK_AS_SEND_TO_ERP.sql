...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_MARK_MASTER_PACK_AS_SEND_TO_ERP]
				  ...
				  ,@IS_IMPLOSION = 0
			--
			SELECT * FROM [FERCO].OP_WMS_MASTER_PACK_HEADER WHERE MASTER_PACK_HEADER_ID = 3
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_MARK_MASTER_PACK_AS_SEND_TO_ERP] (
...
BEGIN
	...
	BEGIN TRY
		...
		-- ------------------------------------------------------------------------------------
		SELECT TOP 1 
			@DOC_ENTRY_IN = CAST([FERCO].[OP_WMS_FN_GET_NUMERIC_VALUE_FROM_STRING]([VALUE]) AS INT)
		FROM [FERCO].[OP_WMS_FN_SPLIT](@ERP_REFERENCE, ';')
		WHERE [VALUE] <> ''
		...
		-- ------------------------------------------------------------------------------------
		SELECT TOP 1 
			@DOC_ENTRY_OUT = CAST([FERCO].[OP_WMS_FN_GET_NUMERIC_VALUE_FROM_STRING]([VALUE]) AS INT)
		FROM [FERCO].[OP_WMS_FN_SPLIT](@ERP_REFERENCE, ';')
		WHERE [VALUE] <> ''
		...
		INNER JOIN [FERCO].[OP_WMS_MATERIAL_INTERCOMPANY] [MI] ON (
  ...
  --
		...
		BEGIN
			...
			SELECT @LICENSE_ID = [LICENSE_ID]
					...
					,@QTY = [QTY]
			FROM [FERCO].[OP_WMS_MASTER_PACK_HEADER]
			WHERE [MASTER_PACK_HEADER_ID] = @MASTER_PACK_HEADER_ID
			-- ------------------------------------------------------------------------------------
