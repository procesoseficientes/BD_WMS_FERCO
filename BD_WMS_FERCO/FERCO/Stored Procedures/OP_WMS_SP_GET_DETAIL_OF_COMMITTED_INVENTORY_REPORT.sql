...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_DETAIL_OF_COMMITTED_INVENTORY_REPORT]
...
<ArrayOfInventarioComprometidoEncabezado xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  <InventarioComprometidoEncabezado>
    ...
    <CLIENT_CODE>FERCO_ALMACENADORA</CLIENT_CODE>
    <CLIENT_NAME>Almacenes y Servicios, S.A.  -FERCO-</CLIENT_NAME>
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_DETAIL_OF_COMMITTED_INVENTORY_REPORT]
    (
...
BEGIN
    ...
    FROM
        @DEMAND_HEADER [DH]
    INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] ON [PDH].[WAVE_PICKING_ID] = [DH].[WAVE_PICKING_ID]
    INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [L].[PICKING_DEMAND_HEADER_ID] = [PDH].[PICKING_DEMAND_HEADER_ID]
    ...
    FROM
        @DEMAND_HEADER [DHX]
    INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH] ON [DH].[WAVE_PICKING_ID] = [DHX].[WAVE_PICKING_ID]
    INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [DD] ON [DD].[PICKING_DEMAND_HEADER_ID] = [DH].[PICKING_DEMAND_HEADER_ID]
    INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [M].[MATERIAL_ID] = [DD].[MATERIAL_ID]
    INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [TL] ON [TL].[WAVE_PICKING_ID] = [DH].[WAVE_PICKING_ID]
