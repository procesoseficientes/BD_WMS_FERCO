...
-- Ejemplo de Ejecucion:
		EXEC [FERCO].[OP_WMS_GET_SONDA_SALES_ORDER_FOR_REPORT]
			@WAREHOUSE = 'BODEGA_01',
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_SONDA_SALES_ORDER_FOR_REPORT] (
...
BEGIN
    ...
    -- ------------------------------------------------------------------------------------
		SELECT
			...
			LEFT JOIN [FERCO].OP_WMS_MATERIALS M ON (M.MATERIAL_ID = (SKU.OWNER + ''/'' + SKU.OWNER_ID))
			...
			LEFT JOIN [FERCO].OP_WMS_VEHICLE V ON (V.VEHICLE_CODE = MH.VEHICLE)
