...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_SALES_INVOICE_HEADER_FOR_INTERNAL_SALE]
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_SALES_INVOICE_HEADER_FOR_INTERNAL_SALE](
...
BEGIN
	...
	BEGIN
		SELECT	
			...
			,GETDATE() [LAST_UPDATE]
		FROM [FERCO].[OP_WMS_COMPANY] [C]
			INNER JOIN [FERCO].[OP_WMS_CUSTOMER_INTERCOMPANY] [CI] ON [CI].[MASTER_ID] = [C].[MASTER_ID_CLIENT_CODE] AND [CI].[SOURCE] = @OWNER
	...
	BEGIN
		...
		SELECT DISTINCT	
			...
			[MATERIAL_OWNER],
			CAST([FERCO].[OP_WMS_FN_SPLIT_COLUMNS](REPLACE([INNER_SALE_RESPONSE],';',' '),5,' ') AS INT)
		FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL]
		...
		FROM [FERCO].[OP_WMS_COMPANY] [C]
			INNER JOIN [FERCO].[OP_WMS_CUSTOMER_INTERCOMPANY] [CI] ON [CI].[MASTER_ID] = [C].[MASTER_ID_SUPPLIER] AND [CI].[SOURCE] = @OWNER
