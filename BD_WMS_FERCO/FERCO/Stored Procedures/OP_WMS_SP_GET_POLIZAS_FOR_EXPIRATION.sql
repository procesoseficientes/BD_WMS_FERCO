...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_POLIZAS_FOR_EXPIRATION]
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_POLIZAS_FOR_EXPIRATION] (
...
BEGIN
	...
	SELECT
		...
		,[P].[VALUE]
		,[P].[LABEL]
	FROM [FERCO].[OP_WMS_FN_GET_PARAMETER_BY_GROUP]('STATUS') [P]
	...
	SELECT
		...
		,[FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA]) [EXPIRATION_DATE]
		,MAX([S].[VALUE]) [BLOCKED_STATUS]
		,MAX([S].[LABEL]) [DESCRIPTION_STATUS]
		,CASE 
			WHEN DATEDIFF(DAY,[FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA]),GETDATE()) > 0
			THEN DATEDIFF(DAY,[FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA]),GETDATE())
			ELSE 0 
		END [TIME_BLOCKED]
		,CASE 
			WHEN DATEDIFF(DAY,GETDATE(),[FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA])) > 0
			THEN DATEDIFF(DAY,GETDATE(),[FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA])) 
			ELSE 0
	...
	INNER JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [VW] ON ([VW].[CLIENT_CODE] = [C].[CLIENT_CODE])
	LEFT JOIN @PARAM [S] ON ([S].[VALUE] = [PH].[BLOCKED_STATUS])
	INNER JOIN [FERCO].[OP_WMS_VIEW_VALORIZACION_FISCAL] [VF] ON ([VF].[CODIGO_POLIZA] = [PH].[CODIGO_POLIZA])
	INNER JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [CO] ON (
