...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[GetExternalManifest] @ManifestHeaderId = 1108
				
*/
-- =============================================  
CREATE PROCEDURE [FERCO].[GetExternalManifest] (
		@ManifestHeaderId INT
	)
AS
BEGIN
	...
	BEGIN
		...
		BEGIN
			SELECT
				...
				+ @INTERFACE_DATA_BASE_NAME + '.'
				+ @SCHEMA_NAME
				+ '.SWIFT_SP_GET_CUSTOMERS_FOR_NEXT @DATABASE ='''
				+ @ERP_DATABASE + ''',@OWNER ='''
				+ @CLIENT_CODE + '''';
	--
	...
	*/
	...
	SELECT DISTINCT [CUSTOMER_CODE]
			...
			,[LONGITUDE]
			,[IMG_FACADE]
			,[EMAIL] FROM @TEMP_CUSTOMER_NEXT

	SELECT
		ROW_NUMBER() OVER (ORDER BY [MH].[MANIFEST_HEADER_ID] ASC) [PrimaryKey]
		,
	--ENCABEZADO MANIFIESTO
		[MH].[MANIFEST_HEADER_ID] [ExternalId]
		,[MH].[DRIVER] [DriverId]
		,MAX(ISNULL([P].[NAME], '') + ISNULL([P].[LAST_NAME],
											'')) [DriverName]
		,[MH].[VEHICLE] [VehicleId]
		,MAX(ISNULL([V].[BRAND], '') + ISNULL([V].[LINE], '')
				+ ISNULL([V].[MODEL], '')
				+ ISNULL([V].[COLOR], '')) [VehicleDescription]
		...
		,'pablo.aguilar@mobilityscm.com' [Email]
		--DETALLE MANIFIESTO
		...
		,MAX([PH].[OWNER]) [Owner]
		--detalle documento
		,MAX([MD].[MANIFEST_DETAIL_ID]) [DetailId] --MAX([PD].[PICKING_DEMAND_DETAIL_ID]) [DetailId]
		,MAX([MD].[PICKING_DEMAND_HEADER_ID]) [DetailDocumentId] --MAX([PD].[PICKING_DEMAND_HEADER_ID]) [DetailDocumentId]
		,[MD].[MATERIAL_ID] [DetailMaterialId] --MAX([PD].[MATERIAL_ID]) [DetailMaterialId]
		,MAX([M].[MATERIAL_NAME]) [DetailMaterialName]
		,SUM([MD].[QTY]) [DetailQty] -- MAX([PD].[QTY]) [DetailQty]
		,MAX([MD].[LINE_NUM]) [DetailLineNum] --MAX([PD].[LINE_NUM]) [DetailLineNum]
		,MAX([MD].[PRICE]) [DetailPrice] --MAX([PD].[PRICE]) [DetailPrice]
		,CAST('' AS VARCHAR(20)) [Tone] -- MAX([PD].[TONE]) [Tone]
		,CAST('' AS VARCHAR(20)) [Caliber] --MAX([PD].[CALIBER]) [Caliber]
		,CAST(0 AS [NUMERIC](18, 6)) [DetailDiscount] --MAX([PD].[DISCOUNT]) [DetailDiscount]
		,CAST('' AS VARCHAR(50)) [DiscountType] --MAX([PD].[DISCOUNT_TYPE]) [DiscountType]
	FROM
		[FERCO].[OP_WMS_MANIFEST_HEADER] [MH]
	INNER JOIN [FERCO].[OP_WMS_MANIFEST_DETAIL] [MD] ON [MH].[MANIFEST_HEADER_ID] = [MD].[MANIFEST_HEADER_ID]
	INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PH] ON [MD].[PICKING_DEMAND_HEADER_ID] = [PH].[PICKING_DEMAND_HEADER_ID]
	--INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [PD] ON [PD].[PICKING_DEMAND_HEADER_ID] = [PH].[PICKING_DEMAND_HEADER_ID]
	LEFT OUTER JOIN [FERCO].[OP_WMS_PILOT] [P] ON [P].[PILOT_CODE] = [MH].[DRIVER]
	LEFT OUTER JOIN [FERCO].[OP_WMS_VEHICLE] [V] ON [V].[VEHICLE_CODE] = [MH].[VEHICLE]
	LEFT JOIN @TEMP_CUSTOMER_NEXT_NOT_REPEATED [TCN] ON ([MD].[CLIENT_CODE] = [TCN].[CUSTOMER_CODE])
	INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([MD].[MATERIAL_ID] = [M].[MATERIAL_ID])
	WHERE
		[MH].[MANIFEST_HEADER_ID] = @ManifestHeaderId
	GROUP BY
		[MD].[CLIENT_CODE]
		,[MD].[MATERIAL_ID]
		,[MD].[WAVE_PICKING_ID]
