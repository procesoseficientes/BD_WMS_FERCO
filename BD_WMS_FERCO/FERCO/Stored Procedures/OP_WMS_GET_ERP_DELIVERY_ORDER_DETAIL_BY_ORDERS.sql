...
-- Fecha de Creación: 2020-06-25 Junio4
-- Description:	 Se crea para que reciba varios documentos y fuentes de externas separados por | para eficientar el proceso de consultar detalles de ordenes 
/*
-- Ejemplo de Ejecucion:
		EXEC  [FERCO].[OP_WMS_GET_ERP_DELIVERY_ORDER_DETAIL_BY_ORDERS] 
			@DOC_NUMS = '99780|', 
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_ERP_DELIVERY_ORDER_DETAIL_BY_ORDERS]
    (
  ...
  --
		...
		-- ------------------------------------------------------------------------------------
            INSERT  INTO [#SALES_ORDERS]
                    SELECT  [ES].[ID] [ORDER] ,
                            ...
                            [SES].[SCHEMA_NAME]
                    FROM    [FERCO].[OP_WMS_FN_SPLIT](@EXTERNAL_SOURCE_IDS,
                                                      @DELIMITER) [ES]
                            INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](@DOC_NUMS,
                                                              @DELIMITER) [SO] ON ( [SO].[ID] = [ES].[ID] )
                            INNER JOIN [FERCO].[OP_SETUP_EXTERNAL_SOURCE] [SES] ON [ES].[VALUE] = [SES].[EXTERNAL_SOURCE_ID];
        END;

	--	ALTER TABLE [#SALES_ORDERS]
	--	ADD CONSTRAINT PK_TEMP_SALES_ORDERS PRIMARY KEY([SALE_ORDER_ID])
        SELECT  @ERP_WAREHOUSE = [ERP_WAREHOUSE]
        FROM    [FERCO].[OP_WMS_WAREHOUSES]
	...
	--
		...
		--
                ...
                INSERT  [#PICKING_DEMAND_DETAIL]
                        SELECT  [H].[DOC_NUM] ,
                                ...
                                [H].[OWNER] ,
                                [D].[LINE_NUM] [LINE_SEQ]
                        FROM    [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [D]
		...
		EXEC ' + @DATA_BASE_NAME + '.' + @SCHEMA_NAME
                        + '.ERP_SP_INSERT_DELIVERY_ORDER_DETAIL 
      ...
      ,SOD.[statusOfMaterial] 
		...
		INNER JOIN [FERCO].OP_WMS_COMPANY OWC WITH(NOLOCK) ON (OWC.CLIENT_CODE COLLATE DATABASE_DEFAULT = SOD.U_OwnerSKU  COLLATE DATABASE_DEFAULT AND OWC.EXTERNAL_SOURCE_ID = '
                        + CAST(@EXTERNAL_SOURCE_ID AS VARCHAR)
                        + ')
		INNER JOIN [FERCO].OP_WMS_MATERIALS M WITH(NOLOCK) ON (SOD.ItemCode COLLATE DATABASE_DEFAULT = [M].[ITEM_CODE_ERP] COLLATE DATABASE_DEFAULT AND SOD.U_OwnerSKU COLLATE DATABASE_DEFAULT= M.CLIENT_OWNER COLLATE DATABASE_DEFAULT )
		INNER JOIN #SALES_ORDERS SOTMP WITH(NOLOCK) ON (SOD.DOCNUM = SOTMP.SALE_ORDER_ID AND SOTMP.EXTERNAL_SOURCE_ID = '
                        + CAST(@EXTERNAL_SOURCE_ID AS VARCHAR)
                        + ' )
		LEFT JOIN #PICKING_DEMAND_DETAIL DD ON (SOD.DOCNUM  = DD.DOC_NUM 
		...
		LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [M].[MATERIAL_ID] = [UMM].[MATERIAL_ID] 
        ...
        SELECT DISTINCT
                ...
                [R].[IS_BONUS] ,
                [R].[MATERIAL_WEIGHT] ,
				[R].[MATERIAL_WEIGHT]*[R].[QTY] AS TOTAL_WEIGHT,
                [R].[MATERIAL_VOLUME] ,
                [R].[USE_PICKING_LINE] ,
        ...
        FROM    [#RESULTADO] [R]
                LEFT JOIN [FERCO].[OP_WMS_VIEW_INVENTORY_FOR_PICKING] [A] ON [R].[SKU] COLLATE DATABASE_DEFAULT = [A].[MATERIAL_ID] COLLATE DATABASE_DEFAULT
                                                              AND [A].[CURRENT_WAREHOUSE]COLLATE DATABASE_DEFAULT= @CODE_WAREHOUSE COLLATE DATABASE_DEFAULT
                LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [UMM].[MATERIAL_ID]COLLATE DATABASE_DEFAULT = [R].[SKU]COLLATE DATABASE_DEFAULT
