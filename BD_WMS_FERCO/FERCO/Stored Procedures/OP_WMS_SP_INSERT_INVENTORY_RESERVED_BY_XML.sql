...
-- Ejemplo de Ejecucion:
	EXECUTE [FERCO].[OP_WMS_SP_INSERT_INVENTORY_RESERVED_BY_XML] @LOGIN = 'MARVIN', @XML ='<ArrayOfInventoryReserved>
																		<Inventory>
...
CREATE PROCEDURE [FERCO].[OP_WMS_SP_INSERT_INVENTORY_RESERVED_BY_XML] (
		@LOGIN AS VARCHAR(25)
		,@XML AS XML
	)
AS
BEGIN TRY
	...
	BEGIN 
		...
		BEGIN
			...
			INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [L].[LICENSE_ID] = [T].[LICENSE_ID]
			LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [VC] ON [L].[CLIENT_OWNER] = [VC].[CLIENT_CODE];
		END;
		ELSE
		BEGIN
			-- ------------------------------------------------------------------------------------
			...
			SET	
				...
				+ [IAR].[QTY_AVAILABLE]
			FROM
				[FERCO].[OP_WMS_INVENTORY_RESERVED_BY_PROJECT] [IAP]
