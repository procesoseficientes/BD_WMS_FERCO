...
-- Ejemplo de Ejecucion:
				-- 
				exec [FERCO].[OP_WMS_SP_PROCESS_DISPATCH_INVENTORY_EXTERNAL]
			
				

*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_PROCESS_DISPATCH_INVENTORY_EXTERNAL (@DOCUMENT_DATE DATE)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  SELECT
    EXT.MATERIAL_ID
   ,EXT.QTY_REQUESTED
   ,[EXT].[LOGIN_ID] INTO #INVEXT
  FROM [FERCO].[OP_WMS_DISPATCH_FROM_EXTERNAL] EXT
  ...
  WHERE [M].[MATERIAL_ID] = (SELECT TOP 1
      E.[MATERIAL_ID]
    FROM #INVEXT E)

  DECLARE @DOC_ID NUMERIC --= 'EXT-' + convert(VARCHAR(21),GETDATE())
  INSERT INTO [FERCO].[OP_WMS_POLIZA_HEADER] ([CODIGO_POLIZA]
  , [FECHA_LLEGADA]
  , [LAST_UPDATED_BY]
  , [LAST_UPDATED]
  , [STATUS]
  ...
  , [IS_EXTERNAL_INVENTORY])
    VALUES ('', @DOCUMENT_DATE, (SELECT TOP 1 E.[LOGIN_ID] FROM #INVEXT E), GETDATE(), 'CREATED', @CLIENT_OWNER, 'GENERAL', @DOCUMENT_DATE, 'EGRESO', '', 'NO', '', 1)
  SELECT
    @DOC_ID = SCOPE_IDENTITY()

  UPDATE [FERCO].[OP_WMS_POLIZA_HEADER]
  SET [NUMERO_ORDEN] = CONVERT(VARCHAR(25), @DOC_ID)
  ...
  BEGIN TRY
    ...
    EXEC [FERCO].[OP_WMS_SP_UPDATE_INV_X_LICENSE_EXT] @QTY = @QTY
                                                     ,@MATERIAL_ID = @SKU
                                                     ,@LOGIN_ID = @LOGIN
  ...
  --			SET @SP=N'
  --			EXEC [FERCO].[OP_WMS_SP_UPDATE_INV_X_LICENSE_EXT] 
  ...
  UPDATE [FERCO].[OP_WMS_DISPATCH_FROM_EXTERNAL]
