...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_GET_PROJET_VALUE_BASED_ON_LICENSE] @LICENSE_ID = 22732
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_PROJECT_VALUE_BASED_ON_LICENSE] (@LICENSE_ID INT)
AS
BEGIN
	SET NOCOUNT ON;
	--	
	SELECT
		...
		END AS [PROJECT]
	FROM
		[FERCO].[OP_WMS_LICENSES] [IL]
	LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] ON [IL].[PICKING_DEMAND_HEADER_ID] = [PDH].[PICKING_DEMAND_HEADER_ID]
	WHERE
