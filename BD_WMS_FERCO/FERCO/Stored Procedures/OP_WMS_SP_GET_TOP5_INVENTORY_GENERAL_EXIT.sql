...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_TOP5_INVENTORY_GENERAL_ENTRY]
					@OWNER = 'me_llega'
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_TOP5_INVENTORY_GENERAL_EXIT (@OWNER VARCHAR(50))
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  SELECT DISTINCT
   ...
   ,@SERIE [SeriesSalida]
  FROM [FERCO].[OP_WMS_PICKING_ERP_DOCUMENT] [PED]
  INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [TL]
    ON [TL].[WAVE_PICKING_ID] = [PED].[WAVE_PICKING_ID]
  WHERE [PED].[IS_POSTED_ERP] <> 1
