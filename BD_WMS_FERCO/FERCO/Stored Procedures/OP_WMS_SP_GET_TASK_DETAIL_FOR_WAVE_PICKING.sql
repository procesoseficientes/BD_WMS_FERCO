...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TASK_DETAIL_FOR_WAVE_PICKING] (
...
INNER JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IL] ON [TL].[LICENSE_ID_SOURCE] = [IL].[LICENSE_ID]
											AND [TL].[MATERIAL_ID] = [IL].[MATERIAL_ID]
LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] ON ([TL].[WAVE_PICKING_ID] = [PDH].[WAVE_PICKING_ID])
LEFT JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [SML] ON [IL].[STATUS_ID] = [SML].[STATUS_ID]
LEFT JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCL] ON [TCL].[TONE_AND_CALIBER_ID] = [IL].[TONE_AND_CALIBER_ID]
WHERE
