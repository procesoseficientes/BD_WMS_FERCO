...
-- Modificacion 18-Apr-18 @ G-FORCE Team Sprint bUHO
					-- pablo.aguilar
					-- Se agrega el campo PROJECT a la transferencia.

/*
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_ERP_TRANSFER_REQUEST_HEADER_BY_DATE]
					@WAREHOUSE_ID = 'BODEGA_C002'
					,@START_DATETIME = '2016-08-23 00:00:00.000'
					,@END_DATETIME = '2018-08-23 00:00:00.000'
*/
-- =============================================
CREATE PROCEDURE [FERCO].OP_WMS_SP_GET_ERP_TRANSFER_REQUEST_HEADER_BY_DATE1 (@WAREHOUSE_ID VARCHAR(25)
...
BEGIN
  ...
  CREATE TABLE [#PICKING_DOCUMENT] (
   ...
   ,[IS_FROM_SONDA] INT
		,[PROJECT] VARCHAR(25)
   --,PRIMARY KEY ([SALES_ORDER_ID], [EXTERNAL_SOURCE_ID], [OWNER])
  );
  --
  ...
  BEGIN TRY
    ...
    -- ------------------------------------------------------------------------------------
    SELECT
      @ONLY_ROWS_WITH_SPECIFIC_DATA_ON_COLUMN = ISNULL([P].[VALUE], '0')
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO @EXTERNAL_SOURCE ([EXTERNAL_SOURCE_ID]
    , [SOURCE_NAME]
    , [DATA_BASE_NAME]
    , [SCHEMA_NAME]
      ...
      SELECT
        @QUERY = N'INSERT INTO [#PICKING_DOCUMENT]
					SELECT
						...
						,0
						, [TRH].[PROJECT]
					FROM ' + @DATA_BASE_NAME + '.' + @SCHEMA_NAME + '.[ERP_VW_TRANSFER_REQUEST_HEADER] [TRH]
					INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W1] ON ([W1].[ERP_WAREHOUSE] COLLATE DATABASE_DEFAULT = [TRH].[FROM_WAREHOUSE_CODE] COLLATE DATABASE_DEFAULT)
					INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W2] ON ([W2].[ERP_WAREHOUSE] COLLATE DATABASE_DEFAULT = [TRH].[TO_WAREHOUSE_CODE] COLLATE DATABASE_DEFAULT)
          INNER JOIN [FERCO].[OP_WMS_COMPANY] [C] ON [TRH].[SOURCE] COLLATE DATABASE_DEFAULT = [C].[CLIENT_CODE] COLLATE DATABASE_DEFAULT AND [C].[EXTERNAL_SOURCE_ID] = ' + CAST(@EXTERNAL_SOURCE_ID AS VARCHAR) + '
					WHERE [W1].[WAREHOUSE_ID] = ''' + @WAREHOUSE_ID + '''
						AND [TRH].[DOC_DATE] BETWEEN ''' + CONVERT(VARCHAR, @START_DATETIME, 121) + ''' AND ''' + CONVERT(VARCHAR, @END_DATETIME, 121) + ''''
        +
         CASE @ONLY_ROWS_WITH_SPECIFIC_DATA_ON_COLUMN
           WHEN '1' THEN ' AND [TRH].[U_ESTADO] = ''03'' '
		   ELSE '' 
         END
      ;
      --
    ...
    SELECT DISTINCT
     ...
     ,[PD].[DELIVERY_DATE]
     ,'WT - ERP' [SOURCE]
     ,[PD].[FROM_WAREHOUSE_ID] [WAREHOUSE_FROM]
     ,[PD].[TO_WAREHOUSE_ID] [WAREHOUSE_TO]
     ,[PD].[SALES_ORDER_ID] [DOC_ENTRY]
     ,1 [IS_FROM_ERP]
	 ,[PD].[PROJECT]
    FROM [#PICKING_DOCUMENT] [PD]
    LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [P]
      ON (
      [PD].[SALES_ORDER_ID] = [P].[DOC_ENTRY]
      AND [PD].[EXTERNAL_SOURCE_ID] = [P].[EXTERNAL_SOURCE_ID]
      AND [PD].[OWNER] COLLATE DATABASE_DEFAULT = [P].[OWNER] COLLATE DATABASE_DEFAULT
      )
    GROUP BY [PD].[DOC_NUM]
            ,[PD].[SALES_ORDER_ID]
            ,[PD].[POSTED_DATETIME]
