...
-- Ejemplo de Ejecucion:
        EXEC [FERCO].[OP_WMS_SP_GET_TASK_DETAIL_CONFIRMATION_FOR_RECEPTION]
          @SERIAL_NUMBER = 557849          
		EXEC [FERCO].[OP_WMS_SP_GET_TASK_DETAIL_CONFIRMATION_FOR_RECEPTION]
          @SERIAL_NUMBER = 13386
		  SELECT * FROM [FERCO].OP_WMS_TASK_LIST where serial_number = 557849
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TASK_DETAIL_CONFIRMATION_FOR_RECEPTION] (@SERIAL_NUMBER INT)
AS
...
INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [TR].[MATERIAL_CODE] = [M].[MATERIAL_ID]
WHERE
	[TR].[TASK_ID] = @SERIAL_NUMBER
