...
EXEC [FERCO].[[OP_WMS_INSERT_CERTIFICATION_BY_SERIAL_NUMBER]] @CERTIFICATION_HEADER_ID = 1
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_INSERT_CERTIFICATION_BY_SERIAL_NUMBER]
...
BEGIN
    ...
    -- ------------------------------------------------------------
        IF NOT EXISTS ( SELECT TOP 1
                        ...
                        INNER JOIN [FERCO].[OP_WMS_PICKING_LABELS_BY_SERIAL_NUMBER] [PLSN] ON ([PLSN].[LABEL_ID] = [PLM].[LABEL_ID])
                        WHERE
    ...
    -- ------------------------------------------------------------
        IF EXISTS ( SELECT TOP 1
                    ...
                    FROM
                        [FERCO].[OP_WMS_PICKING_LABEL_BY_MANIFEST] [PLM]
                    INNER JOIN [FERCO].[OP_WMS_MANIFEST_DETAIL] [MD] ON ([MD].[MANIFEST_DETAIL_ID] = [PLM].[MANIFEST_DETAIL_ID])
                    INNER JOIN [FERCO].[OP_WMS_PICKING_LABELS_BY_SERIAL_NUMBER] [PLSN] ON ([PLM].[LABEL_ID] = [PLM].[LABEL_ID])
                    INNER JOIN [FERCO].[OP_WMS_CERTIFICATION_BY_SERIAL_NUMBER] [CSN] ON ([CSN].[SERIAL_NUMBER] = [PLSN].[SERIAL_NUMBER])
                    WHERE
