...
-- Ejemplo de Ejecucion:
	EXEC [FERCO].[OP_WMS_GET_TASK_BASIC_VIEW] 
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_TASK_BASIC_VIEW1]
(
...
BEGIN
    ...
    -- ------------------------------------------------------------------------------------
    SELECT @MAX_ATTEMPTS = [C].[NUMERIC_VALUE]
    ...
    INTO [#TYPES]
    FROM [FERCO].[OP_WMS_FN_SPLIT](@TYPES, '|') [T];

    -- ------------------------------------------------------------------------------------
    -- Arma una tabla temporal con los usuarios
    -- ------------------------------------------------------------------------------------
    SELECT [T].[VALUE] AS [login],
           ISNULL([L].[LOGIN_NAME], [T].[VALUE]) [LOGIN_NAME]
    INTO [#USERS]
    FROM [FERCO].[OP_WMS_FN_SPLIT](@USERS, '|') [T]
        LEFT JOIN [FERCO].[OP_WMS_LOGINS] [L]
    ...
    BEGIN
        INSERT INTO [#CLASS]
        SELECT [C].[VALUE] AS [CLASS_ID]
        FROM [FERCO].[OP_WMS_FN_SPLIT](@CLASS, '|') [C];
    END;
    ...
    FROM [FERCO].[OP_WMS_WAREHOUSE_BY_USER] [WU]
        LEFT JOIN [FERCO].[OP_WMS_WAREHOUSES] [W]
    ...
    WHERE [PARAM_GROUP] = 'PRIORITY'
          AND [PARAM_TYPE] = 'SISTEMA';
    -- ------------------------------------------------------------------------------------
    -- Obtiene las ubicaciones de destino de las tareas de recepcion
    -- ------------------------------------------------------------------------------------
    SELECT [TL].[SERIAL_NUMBER],
           CASE
               WHEN MAX([T].[TARGET_LOCATION]) IS NULL THEN
                   ''
               WHEN MAX([T].[TARGET_LOCATION]) <> MIN([T].[TARGET_LOCATION]) THEN
                   'Multiple'
               WHEN MAX([T].[TARGET_LOCATION]) = MIN([T].[TARGET_LOCATION]) THEN
                   MAX([T].[TARGET_LOCATION])
           END [LOCATION_SPOT_TARGET]
    INTO [#LOCATION_TARGET_RECEPTION]
    FROM [FERCO].[OP_WMS_TASK_LIST] [TL]
        LEFT JOIN [FERCO].[OP_WMS_TRANS] [T]
            ON [TL].[SERIAL_NUMBER] = [T].[TASK_ID]
               AND [T].[STATUS] = 'PROCESSED'
               AND [T].[TRANS_TYPE] = 'INGRESO_GENERAL'
               AND [T].[TRANS_DATE]
               BETWEEN GETDATE() - 15 AND GETDATE()
    WHERE [TL].[ASSIGNED_DATE]
    ...
    --LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH] ON [DH].[WAVE_PICKING_ID] = [TL].[WAVE_PICKING_ID]
    --LEFT JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [DH].[PICKING_DEMAND_HEADER_ID] = [L].[PICKING_DEMAND_HEADER_ID]
    --WHERE
    ...
    SELECT [WAVE_PICKING_ID],
           CASE
               WHEN ISNULL(MAX([TH].[TRANSFER_REQUEST_ID]), 0) <> 0 THEN
                   MAX([TH].[WAREHOUSE_TO])
               WHEN MAX([TL].[LOCATION_SPOT_TARGET]) <> MIN([TL].[LOCATION_SPOT_TARGET]) THEN
                   'Multiple'
               WHEN MAX([TL].[LOCATION_SPOT_TARGET]) = MIN([TL].[LOCATION_SPOT_TARGET]) THEN
                   MAX([TL].[LOCATION_SPOT_TARGET])
           END [LOCATION_SPOT_TARGET]
    INTO [#LOCATION_TARGET_PICKING]
    FROM [FERCO].[OP_WMS_TASK_LIST] [TL]
        LEFT JOIN [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER] [TH]
            ON [TH].[TRANSFER_REQUEST_ID] = [TL].[TRANSFER_REQUEST_ID]
    ...
    (
        ...
        --       CASE
        --           WHEN ISNULL([RDH].[IS_POSTED_ERP], 0) = -1 THEN
        --               'Fallido'
        --           WHEN ISNULL([RDH].[IS_POSTED_ERP], 0) = 0
        --                AND ISNULL([RDH].[IS_AUTHORIZED], 0) = 1 THEN
        --               'Autorizada'
        --           WHEN ISNULL([RDH].[IS_POSTED_ERP], 0) = 0
        --                AND ISNULL([RDH].[IS_AUTHORIZED], 0) = 0 THEN
        ...
        --       NULL AS [SOURCE_TYPE],
        --       [VT].[CREATE_BY]
        --FROM [FERCO].[OP_WMS_VIEW_TASK] [VT]
        --    INNER JOIN [#LOCATION_TARGET_RECEPTION] [LTR]
        --        ON [LTR].[SERIAL_NUMBER] = [VT].[SERIAL_NUMBER]
        --    LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RDH]
        --        ON ([VT].[SERIAL_NUMBER] = [RDH].[TASK_ID])
        --    LEFT JOIN [#USERS] [U]
        --        ON [U].[login] = [VT].[TASK_ASSIGNEDTO]
        --    LEFT JOIN @TB_PRIORITY [P]
        --        ON [VT].[PRIORITY] = [P].[PRIORITY]
        --WHERE [VT].[ASSIGNED_DATE]
        --      BETWEEN @START_DATETIME AND @END_DATETIME
        --      AND
        --      (
        --          @USERS IS NULL
        ...
        --       [VT].[TASK_TYPE],
        --       CASE
        --           WHEN [VT].[TASK_SUBTYPE] = 'DESPACHO_WT' THEN
        --               'PICKING_WT'
        --           WHEN [VT].[TASK_SUBTYPE] = 'DESPACHO_GENERAL'
        --                AND [PDH].[PICKING_DEMAND_HEADER_ID] IS NOT NULL THEN
        --               'PICKING' + ISNULL(' - ' + [PDH].[TYPE_DEMAND_NAME], '')
        --           WHEN [VT].[TASK_SUBTYPE] = 'DESPACHO_CONSOLIDADO' THEN
        --               'PICKING' + ISNULL(' - ' + [PDH].[TYPE_DEMAND_NAME], '')
        --           ELSE
        --               [VT].[TASK_SUBTYPE] + ISNULL(' - ' + [PDH].[TYPE_DEMAND_NAME], '')
        --       END [TASK_SUBTYPE],
        --       [VT].[TASK_ASSIGNEDTO],
        --       [VT].[TASK_COMMENTS],
        --       [VT].[REGIMEN],
        ...
        --       ISNULL([VT].[IS_FROM_ERP], 0) [IS_FROM_ERP],
        --       ISNULL([VT].[IS_FROM_SONDA], 0) [IS_FROM_SONDA],
        --       ISNULL([PDH].[DOC_NUM], [PD].[WAVE_PICKING_ID]) AS [NO_DOC],
        --       ISNULL([PDH].[PICKING_DEMAND_HEADER_ID], [PD].[PICKING_ERP_DOCUMENT_ID]) [WMS_DOCUMENT_HEADER_ID],
        --       ISNULL(   CASE
        --                     WHEN [PDH].[SOURCE_TYPE] = 'WT - ERP' THEN
        ...
        --                 [PD].[WAVE_PICKING_ID]
        --             ) [DOC_ID],
        --       ISNULL([PDH].[ATTEMPTED_WITH_ERROR], [PD].[ATTEMPTED_WITH_ERROR]) [ATTEMPTED_WITH_ERROR],
        --       COALESCE([PDH].[IS_POSTED_ERP], [PD].[IS_POSTED_ERP], 0) [IS_POSTED_ERP],
        --       CASE
        --           WHEN COALESCE([PDH].[IS_POSTED_ERP], [PD].[IS_POSTED_ERP], 0) = -1 THEN
        --               'Fallido'
        --           WHEN COALESCE([PDH].[IS_POSTED_ERP], [PD].[IS_POSTED_ERP], 0) = 0
        --                AND ISNULL([PDH].[IS_AUTHORIZED], 0) = 1 THEN
        --               'Autorizada'
        --           WHEN COALESCE([PDH].[IS_POSTED_ERP], [PD].[IS_POSTED_ERP], 0) = 0
        --                AND COALESCE([PDH].[IS_AUTHORIZED], [PD].[IS_AUTHORIZED], 0) = 0 THEN
        --               'Pendiente de Autorización'
        --           WHEN COALESCE([PDH].[IS_POSTED_ERP], [PD].[IS_POSTED_ERP], 0) = 1 THEN
        --               'Enviado'
        --       END [STATUS_POSTED_ERP],
        --       ISNULL([PDH].[POSTED_ERP], [PD].[POSTED_ERP]) [POSTED_ERP],
        --       ISNULL([PDH].[POSTED_RESPONSE], [PD].[POSTED_RESPONSE]) [POSTED_RESPONSE],
        --       ISNULL([PDH].[ERP_REFERENCE_DOC_NUM], [PD].[ERP_REFERENCE_DOC_NUM]) AS [ERP_REFERENCE],
        --       COALESCE([PDH].[IS_AUTHORIZED], [PD].[IS_AUTHORIZED], 0) [IS_AUTHORIZED],
        --       @MAX_ATTEMPTS AS [MAX_ATTEMPTS],
        ...
        --       [VT].[CREATE_BY]
        --FROM [FERCO].[OP_WMS_VIEW_TASK_PICKING_HEADER] [VT]
        --    INNER JOIN @TB_WAREHOUSE [W]
        --        ON ([W].[WAREHOUSE_ID] = [VT].[WAREHOUSE_SOURCE])
        --    INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T]
        --        ON [T].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
        --    --INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M]
        --    --    ON [M].[MATERIAL_ID] = [T].[MATERIAL_ID]
        --    --INNER JOIN [#CLASS] [C]
        --    --    ON [C].[CLASS_ID] = CAST([M].[MATERIAL_CLASS] AS INT)
        --    INNER JOIN [#LOCATION_TARGET_PICKING] [LTP]
        --        ON [LTP].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
        --    LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH]
        --        ON ([VT].[WAVE_PICKING_ID] = [PDH].[WAVE_PICKING_ID])
        --    LEFT JOIN [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER] [TRH]
        --        ON ([PDH].[TRANSFER_REQUEST_ID] = [TRH].[TRANSFER_REQUEST_ID])
        --    LEFT JOIN [FERCO].[OP_WMS_PICKING_ERP_DOCUMENT] [PD]
        --        ON [PD].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
        --    LEFT JOIN [#USERS] [U]
        --        ON [U].[login] = [VT].[TASK_ASSIGNEDTO]
        --    LEFT JOIN @TB_PRIORITY [P]
        --        ON [P].[PRIORITY] = [VT].[PRIORITY]
        --WHERE [VT].[ASSIGNED_DATE]
        --      BETWEEN @START_DATETIME AND @END_DATETIME
        --      AND
        --      (
        ...
        SELECT DISTINCT
               ...
               'TAREA DE CONTEO #' + CAST([T].[TASK_ID] AS VARCHAR) AS [TASK_COMMENTS],
               [T].[REGIMEN],
               [T].[IS_PAUSED],
               ...
               NULL AS [SOURCE_TYPE],
               [T].[CREATE_BY]
        FROM [FERCO].[OP_WMS_TASK] [T]
            INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [CH]
                ON [T].[TASK_ID] = [CH].[TASK_ID]
            INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [CD]
                ON [CH].[PHYSICAL_COUNT_HEADER_ID] = [CD].[PHYSICAL_COUNT_HEADER_ID]
            INNER JOIN [#USERS] [U]
                ON ([U].[login] = [CD].[ASSIGNED_TO])
            LEFT JOIN @TB_PRIORITY [P]
                ON [P].[PRIORITY] = [T].[PRIORITY]
        WHERE [T].[ASSIGNED_DATE]
              BETWEEN @START_DATETIME AND @END_DATETIME
              AND [T].[IS_CANCELED] = 0
    ...
    --		,[VT].[CREATE_BY]
    --	FROM
    --		[FERCO].[OP_WMS_VIEW_TASK_REALLOC_HEADER] [VT]
    --	INNER JOIN @TB_WAREHOUSE [W] ON ([W].[WAREHOUSE_ID] = [VT].[WAREHOUSE_SOURCE])
    --	INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [T].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
    ...
    SELECT [R].[TASK_ID],
           ...
           [R].[IS_AUTHORIZED],
           CASE
               WHEN [R].[IS_POSTED_ERP] = 1 THEN
                   'ENVIADA'
               WHEN [R].[IS_AUTHORIZED] = 0 THEN
                   'NO'
               WHEN [R].[IS_AUTHORIZED] = 1 THEN
                   'SI'
