...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_GET_POLIZA_FISCAL_ENTRY]
					@TYPE = 'INGRESO'
				--
				EXEC [FERCO].[OP_WMS_GET_POLIZA_FISCAL_ENTRY]
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_POLIZA_FISCAL_ENTRY] (
	@TYPE VARCHAR(50)
) AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT 
		...
		,@QUERY = CASE @TYPE
			WHEN 'INGRESO' THEN N'SELECT
									...
									,[PH].[STATUS] [ESTADO]
									,[FERCO].[OP_WMS_FN_GET_DAYS_BY_REGIMEN]([PH].[REGIMEN]) [DIAS_REGIMEN]
									,DATEDIFF(DAY,GETDATE(),[FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA])) [DIAS_PARA_VENCER]
									,[FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA]) [FECHA_VENCIMIENTO]
									,CASE 
										WHEN DATEDIFF(DAY,GETDATE(),[FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA])) > 0 THEN ''Libre''
										ELSE ''Bloqueado''
									END [ESTADO_REGIMEN]
								FROM [FERCO].[OP_WMS_POLIZA_HEADER] [PH]
								LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [CL] ON ([CL].[CLIENT_CODE] = [PH].[CLIENT_CODE])
								WHERE [PH].[DOC_ID] > 0
									AND [PH].[TIPO] = @TIPO
			...
			WHEN 'DETALLE_INGRESO' THEN N'SELECT
										...
										INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PH] ON ([PH].[DOC_ID] = [PD].[DOC_ID])
										LEFT JOIN [FERCO].[OP_WMS_VIEW_CUSTOMER_TERMS_OF_TRADE] [TT] ON (
			...
			WHEN 'EGRESO' THEN N'SELECT DISTINCT
									...
									END [ESTADO_REGIMEN]
								FROM [FERCO].[OP_WMS_POLIZA_HEADER] [PH]
								LEFT JOIN [FERCO].[OP_WMS_POLIZA_DETAIL] [PD] ON [PH].[DOC_ID] = [PD].[DOC_ID]
								WHERE [PH].[DOC_ID] > 0 
									AND [PH].[TIPO] = @TIPO
									AND [PH].[WAREHOUSE_REGIMEN] = @REGIMEN
