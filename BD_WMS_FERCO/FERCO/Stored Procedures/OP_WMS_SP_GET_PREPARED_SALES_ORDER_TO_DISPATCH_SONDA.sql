...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_PREPARED_SALES_ORDER_TO_DISPATCH_SONDA]
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_PREPARED_SALES_ORDER_TO_DISPATCH_SONDA]
    (
...
BEGIN
    ...
    INTO
        [#ROUTE]
    FROM
        [FERCO].[OP_WMS_FN_SPLIT](@SOURCE_CODE_ROUTE, @DELIMITER) [SCR]
    INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](@CODE_ROUTE, @DELIMITER) [CR] ON ([CR].[ID] = [SCR].[ID]);
    ...
    INNER JOIN [FERCO].[OP_SETUP_EXTERNAL_SOURCE] [ES] ON [ES].[EXTERNAL_SOURCE_ID] = [PDH].[EXTERNAL_SOURCE_ID]
