...
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_DISPATCH_BY_REGIME_GENERAL] (
...
BEGIN
    ...
    ---------------------------------------------------------------------------------
		...
		FROM
			[FERCO].[OP_WMS_FUNC_GETLOGIN_NAME](@pLOGIN_ID) [D];
		...
		INNER JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [C] ON [C].[CLIENT_CODE] = [L].[CLIENT_OWNER]
		WHERE
		...
		LEFT JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [SML] ON (
    ...
    ---------------------------------------------------------------------------------  
      ...
      ---------------------------------------------------------------------------------  
		...
		---------------------------------------------------------------------------------
        -- Validamos si el material es de masterpack
        ---------------------------------------------------------------------------------
			SELECT top 1 @FROM_MASTERPACK = [FROM_MASTERPACK] FROM [FERCO].[OP_WMS_TASK_LIST] WHERE WAVE_PICKING_ID = @pWAVE_PICKING_ID AND MATERIAL_ID = @pMATERIAL_ID
        ---------------------------------------------------------------------------------
        ...
        ---------------------------------------------------------------------------------  
				...
				BEGIN
					IF NOT EXISTS ( SELECT TOP 1
										1
									FROM
										[FERCO].[OP_WMS_INVENTORY_RESERVED_BY_PROJECT]
									WHERE
										[LICENSE_ID] = @pSOURCE_LICENSE
										AND [MATERIAL_ID] = @pMATERIAL_ID_LOCAL
		...
		BEGIN
			...
			SELECT
				...
				,ISNULL((SELECT TOP 1
								[PARAM_CAPTION]
							FROM
								[FERCO].[OP_WMS_FUNC_GETTRANS_DESC]('DESPACHO_GENERAL')),
						'DESPACHO GENERAL')
				,NULL
				,@pMATERIAL_BARCODE
				,@pMATERIAL_ID_LOCAL
				,(SELECT TOP 1
						*
					FROM
						[FERCO].[OP_WMS_FUNC_GETMATERIAL_DESC](@pMATERIAL_BARCODE,
											@pCLIENT_ID_LOCAL))
				,NULL
				,[FERCO].[OP_WMS_FN_GET_MATERIAL_COST_BY_MATERIAL](@pMATERIAL_ID_LOCAL,
											@pCLIENT_ID_LOCAL)
				,@pSOURCE_LICENSE
				,NULL
				,@pSOURCE_LOCATION
				...
				,(SELECT TOP 1
						*
					FROM
						[FERCO].[OP_WMS_FUNC_GETCLIENT_NAME](@pCLIENT_ID_LOCAL))
				,-1
				,@WAREHOUSE_PARENT
				,NULL
				,@TRANS_SUBTYPE
				...
				,@PROJECT_SHORT_NAME
			FROM
				[FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [S]
			INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [T].[WAVE_PICKING_ID] = [S].[WAVE_PICKING_ID]
      ...
      ---------------------------------------------------------------------------------    
        ...
        ----
				SELECT
					...
					,@CALIBER = [TCM].[CALIBER]
				FROM
					[FERCO].[OP_WMS_INV_X_LICENSE] [L]
				INNER JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCM] ON ([L].[TONE_AND_CALIBER_ID] = [TCM].[TONE_AND_CALIBER_ID])
				WHERE
					[L].[LICENSE_ID] = @pSOURCE_LICENSE
        ...
        ---------------------------------------------------------------------------------    
            ...
            ---------------------------------------------------------------------------------    
						IF EXISTS ( SELECT TOP 1
									...
									FROM
										[FERCO].[OP_WMS_INV_X_LICENSE] [IL]
									INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([IL].[MATERIAL_ID] = [M].[MATERIAL_ID])
									WHERE
										[IL].[LICENSE_ID] = @LICENSE_DISPATCH_ID
										AND [IL].[MATERIAL_ID] = @pMATERIAL_ID
										AND (
            ...
            ---------------------------------------------------------------------------------    
						IF EXISTS ( SELECT TOP 1
									...
									FROM
										[FERCO].[OP_WMS_INV_X_LICENSE] [IL]
									INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([IL].[MATERIAL_ID] = [M].[MATERIAL_ID])
									INNER JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCM] ON ([IL].[TONE_AND_CALIBER_ID] = [TCM].[TONE_AND_CALIBER_ID])
									WHERE
										[IL].[LICENSE_ID] = @LICENSE_DISPATCH_ID
										AND [IL].[MATERIAL_ID] = @pMATERIAL_ID
										AND (
          ...
          ---------------------------------------------------------------------------------    
            ...
            ---------------------------------------------------------------------------------    
						...
						FROM
							[FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCM]
						WHERE
        ...
        ---------------------------------------------------------------------------------    
          ...
          ---------------------------------------------------------------------------------    
					...
					FROM
						[FERCO].[OP_WMS_MASTER_PACK_HEADER] [MPH]
					WHERE
          ...
          ---------------------------------------------------------------------------------    
            ...
            ---------------------------------------------------------------------------------    
						...
						FROM
							[FERCO].[OP_WMS_MASTER_PACK_DETAIL] [MPDSS]
						WHERE
