...
-- Ejemplo de Ejecucion:
	exec [FERCO].OP_WMS_SP_AUTHORIZE_ERP_PICKING_DOCUMENT @WAVE_PICKING_ID=192858,@LAST_UPDATE_BY=N'ADMIN'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_AUTHORIZE_ERP_PICKING_DOCUMENT] (
		@WAVE_PICKING_ID INT
  ...
  --
	BEGIN TRY
      ...
      -- ----------------------------------------------------------------------------------
			...
			FROM
				[FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] AS [DH]
			WHERE
      ...
      -- ----------------------------------------------------------------------------------
        ...
        --
				...
				FROM
					[FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [DD] WITH (NOLOCK)
				INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH] WITH (NOLOCK) ON [DH].[PICKING_DEMAND_HEADER_ID] = [DD].[PICKING_DEMAND_HEADER_ID]
				WHERE
					[DH].[WAVE_PICKING_ID] = @WAVE_PICKING_ID
      ...
      -- ----------------------------------------------------------------------------------
        ...
        -- ----------------------------------------------------------------------------------
				...
				FROM
					[FERCO].[OP_WMS_DELIVERED_DISPATCH_HEADER]
					AS [DDH]
				WHERE
					[WAVE_PICKING_ID] = @WAVE_PICKING_ID;
