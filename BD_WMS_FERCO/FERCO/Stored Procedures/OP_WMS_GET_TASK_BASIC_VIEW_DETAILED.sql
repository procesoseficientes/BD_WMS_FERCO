...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_GET_TASK_BASIC_VIEW_DETAILED]
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_TASK_BASIC_VIEW_DETAILED]
    (
...
BEGIN
		...
		-- ------------------------------------------------------------------------------------
         SELECT
           ...
           ,ISNULL(DATEDIFF(MINUTE, [VT].[ACCEPTED_DATE],
                            [VT].[PICKING_FINISHED_DATE]), 0) AS [TIME]
           ...
           ,CASE WHEN ISNULL([RDH].[IS_POSTED_ERP], 0) = -1 THEN 'Fallido'
                 WHEN ISNULL([RDH].[IS_POSTED_ERP], 0) = 0
                      AND ISNULL([RDH].[IS_AUTHORIZED], 0) = 1
                 THEN 'Autorizada'
                 WHEN ISNULL([RDH].[IS_POSTED_ERP], 0) = 0
                      AND ISNULL([RDH].[IS_AUTHORIZED], 0) = 0
                 THEN 'Pendiente de Autorización'
                 WHEN ISNULL([RDH].[IS_POSTED_ERP], 0) = 1 THEN 'Enviado'
         ...
         SELECT DISTINCT
           ...
           ,ISNULL([VT].[IS_FROM_ERP], 0) [IS_FROM_ERP]
           ,ISNULL([VT].[IS_FROM_SONDA], 0) [IS_FROM_SONDA]
           ,CAST([VT].[NO_DOC] AS VARCHAR) AS NO_DOC
           ,[VT].[WMS_DOCUMENT_HEADER_ID]
           ,CAST([VT].[DOC_ID] AS VARCHAR) AS DOC_ID
         ...
         FROM
            [FERCO].[OP_WMS_VIEW_TASK_PICKING_HEADER_DETAILED] [VT]
         INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [TL] ON [TL].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
                                                       AND [TL].[MATERIAL_ID] = [VT].[MATERIAL_ID]
         INNER JOIN @TB_WAREHOUSE [W] ON ([W].[WAREHOUSE_ID] = [VT].[WAREHOUSE_SOURCE])
         INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [M].[MATERIAL_ID] = [VT].[MATERIAL_ID]
         ...
         SELECT DISTINCT
           ...
           ,[T].[IS_ACCEPTED]
           ,ISNULL(DATEDIFF(MINUTE, [T].[ACCEPTED_DATE], [T].[COMPLETED_DATE]),
                   0) AS [TIME]
           ,NULL AS [IS_FROM_ERP]
         ...
         FROM
            [FERCO].[OP_WMS_VIEW_TASK_REALLOC_HEADER_DETAILED] [VT]
         INNER JOIN @TB_WAREHOUSE [W] ON ([W].[WAREHOUSE_ID] = [VT].[WAREHOUSE_SOURCE])
         INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [M].[MATERIAL_ID] = [VT].[MATERIAL_ID]
         INNER JOIN [#CLASS] [C] ON [C].[CLASS_ID] = CAST([M].[MATERIAL_CLASS] AS INT)
    ...
    SELECT
       ...
       ,CAST([R].[DOC_ID] AS VARCHAR) DOC_ID
       ...
       ,CAST([R].[NO_DOC] AS VARCHAR) NO_DOC
    FROM
        [#RESULT] [R]
