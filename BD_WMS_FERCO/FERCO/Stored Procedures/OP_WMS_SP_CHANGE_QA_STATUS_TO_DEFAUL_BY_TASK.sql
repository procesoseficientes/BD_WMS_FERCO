...
-- Ejemplo de Ejecucion:
        EXEC [FERCO].[OP_WMS_SP_CHANGE_QA_STATUS_TO_DEFAUL_BY_TASK] @TASK_ID = 718
	 , @LOGIN = 'PABS'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_CHANGE_QA_STATUS_TO_DEFAUL_BY_TASK] (
		@TASK_ID INT
		,@LOGIN VARCHAR(100)
	)
AS
BEGIN
	...
	BEGIN TRY
		...
		INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [R] ON [L].[CODIGO_POLIZA] = CAST([R].[DOC_ID_POLIZA] AS VARCHAR)
		WHERE
			[R].[TASK_ID] = @TASK_ID;
	...
	-- ------------------------------------------------------------------------------------
		...
		-- ------------------------------------------------------------------------------------
			...
			INNER JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [CQM] ON [CQM].[PARAM_GROUP] = 'ESTADOS'
			...
			INNER JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [CDEF] ON [CDEF].[PARAM_GROUP] = 'ESTADOS'
