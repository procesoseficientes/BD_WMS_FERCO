...
-- Description:			SP que mueve la licencia de reubicacion total cuando la ubicacion es de fast picking

-- Autor:				marvin.solares
-- Fecha de Creacion: 	20182910 GForce@Mamba
-- Description:			se modifica para que el inventario quede liberado al mover en ubicacion de fast picking

-- Autor:				marvin.solares
-- Fecha de Creacion: 	20180730 GForce@Dublin
-- Description:			se cambia delete de lineas de inventario de licencia trasladada a fast picking por update con qty = 0, 
--						se actualiza la licencia de fast picking en las lineas de inventario reservadas a proyecto que fueron movidas a la licencia ya existente de fast picking

/*
-- Ejemplo de Ejecucion:
EXEC [FERCO].[OP_WMS_SP_UPDATE_LICENSE_FAST_PICKING] @LOGIN_ID = '', -- varchar(50)
	@LICENSE_ID = 0, -- int
...
CREATE PROCEDURE [FERCO].[OP_WMS_SP_UPDATE_LICENSE_TOTAL_REALLOC_FAST_PICKING] (
		@LOGIN_ID VARCHAR(50)
		,@LICENSE_ID INT
		,@LOCATION_ID VARCHAR(25)
...
BEGIN
	...
	FROM
		[FERCO].[OP_WMS_LICENSES] [L]
	INNER JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [INVL] ON [L].[LICENSE_ID] = [INVL].[LICENSE_ID]
	WHERE
	...
	SET	
		[LICENSE_ID_SOURCE] = @ORIGIN_LOCATION_LICENSE
	WHERE
		[LICENSE_ID_SOURCE] = @LICENSE_ID
		AND [TASK_TYPE] IN ('TAREA_PICKING',
							'TAREA_REUBICACION',
							'IMPLOSION_INVENTARIO')
		AND ([IS_COMPLETED] <> 1)
		AND [IS_CANCELED] <> 1
	...
	SET	
		...
		,[SOURCE_LICENSE] = @ORIGIN_LOCATION_LICENSE
	WHERE
		[LICENSE_ID] = @LICENSE_ID;
	...
	-- ------------------------------------------------------------------------------------
	UPDATE
		[FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER]
	SET	
		[LICENSE_ID] = @ORIGIN_LOCATION_LICENSE
	WHERE
		[LICENSE_ID] = @LICENSE_ID
	...
	-- ------------------------------------------------------------------------------------
	-- actualizamos [OP_WMS_REALLOCS_X_LICENSE]
	-- ------------------------------------------------------------------------------------

	UPDATE
		[FERCO].[OP_WMS_REALLOCS_X_LICENSE]
	SET	
		[LICENSE_ID] = @ORIGIN_LOCATION_LICENSE
	WHERE
		[LICENSE_ID] = @LICENSE_ID
		AND [TARGET_LOCATION] = @LOCATION_ID
	...
	INNER JOIN [FERCO].[OP_WMS_MASTER_PACK_HEADER] [MPH] ON [MPH].[LICENSE_ID] = [IL].[LICENSE_ID]
	...
	BEGIN
		...
		BEGIN
			...
			DELETE FROM
				[FERCO].[OP_WMS_MASTER_PACK_DETAIL]
			WHERE
				[MASTER_PACK_HEADER_ID] = @MASTER_PACK_HEADER_ID;
	...
	FROM
		[FERCO].[OP_WMS_INV_X_LICENSE]
	WHERE
		[LICENSE_ID] = @LICENSE_ID;--INVENTARIO LICENCIA CREADA EN LA RECEPCION
	...
	BEGIN
		...
		MERGE [FERCO].[OP_WMS_INV_X_LICENSE] AS [OLD]
		USING
			(SELECT
					[PK_LINE]
					,[LICENSE_ID]
		...
		DECLARE
			@PK_LINE_OLD_LICENSE [NUMERIC](18, 0)
			,@QTY_OLD_LICENSE NUMERIC(18, 4);
		...
		WHERE
			[LICENSE_ID] = @LICENSE_ID
			AND [MATERIAL_ID] = @MATERIAL_ID_NEW_LICENSE;
		...
		BEGIN
			...
			-- ------------------------------------------------------------------------------------
				...
				DELETE
					[FERCO].[OP_WMS_LOG_INVENTORY_RESERVED_BY_PROJECT]
				WHERE
					[MATERIAL_ID] = @MATERIAL_ID_NEW_LICENSE
					AND [PROJECT_ID] = @PROJECT_ID
					AND [LICENSE_ID] = @LICENSE_ID;
	...
	-- ------------------------------------------------------------------------------------
	-- dejamos sin inventario la licencia original pero no eliminamos las filas para que quede historico
	-- ------------------------------------------------------------------------------------
	UPDATE
		[FERCO].[OP_WMS_INV_X_LICENSE]
	SET	
