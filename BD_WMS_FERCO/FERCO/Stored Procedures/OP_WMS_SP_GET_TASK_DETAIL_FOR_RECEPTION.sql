...
-- Ejemplo de Ejecucion:
		...
		EXEC [FERCO].[OP_WMS_SP_GET_TASK_DETAIL_FOR_RECEPTION]
          @SERIAL_NUMBER = 13386
		  SELECT * FROM [FERCO].OP_WMS_TASK_LIST
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TASK_DETAIL_FOR_RECEPTION] (@SERIAL_NUMBER INT)
AS
...
BEGIN
    ...
    LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [UMM].[MATERIAL_ID] = [RDD].[MATERIAL_ID]
    ...
    (SELECT
     ...
     SELECT
       ...
       ,CAST(SUM([RDD].[QTY]) * MIN(ISNULL([UMM].[QTY], 1)) AS NUMERIC(18, 4)) AS [QTY_DOC]
       ,0 [QTY_DIFFERENCE]
     ...
     FROM
        [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RDH]
     INNER JOIN [FERCO].[OP_WMS_TRANS] [T] ON [RDH].[TASK_ID] = [T].[TASK_ID]
                                                AND [T].[TRANS_TYPE] = 'INGRESO_GENERAL'
                                                AND [T].[STATUS] = 'PROCESSED'
     LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [RDD] ON [RDD].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [RDH].[ERP_RECEPTION_DOCUMENT_HEADER_ID]
    ...
    SELECT
       ...
       ,CASE WHEN SUM([QTY_DOC]) - MAX([QTY]) = 0 THEN 0
             ...
             WHEN SUM([QTY_DOC]) - MAX([QTY]) < 0
             THEN (SUM([QTY_DOC]) - MAX([QTY])) * -1
             WHEN SUM([QTY_DOC]) IS NULL THEN (0 - SUM([QTY_DOC])) * -1
        END [QTY_DIFFERENCE]
