...
EXEC [FERCO].[OP_WMS_SP_GET_DEMAND_DETAIL_FOR_PASS1] @PICKING_DEMAND_HEADER_ID = '107725', @DEMAND_TYPE = 'VENTA'
...
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_DEMAND_DETAIL_FOR_PASS1]
(
...
BEGIN
    ...
    BEGIN
        ...
        SELECT [PD].[PICKING_DEMAND_HEADER_ID],
               ...
               [PD].[MATERIAL_ID],
               SUM([PD].[QTY]) [QTY]
        FROM [FERCO].[OP_WMS3PL_PASSES] [PH]
            INNER JOIN [FERCO].[OP_WMS_PASS_DETAIL] [PD]
        ...
        FROM [FERCO].[OP_WMS_TASK_LIST] [TL]
            INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](@PICKING_DEMAND_HEADER_ID, '|') [PHID]
        ...
        FROM [FERCO].[OP_WMS_TASK_LIST] [TL]
            ...
            --      INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](@PICKING_DEMAND_HEADER_ID, '|') [PHID] ON (
        ...
        SELECT [TL].[MATERIAL_ID],
               ...
               [TL].[CODE_WAREHOUSE]
        FROM @TASK_LIST [TL] --    INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](@PICKING_DEMAND_HEADER_ID, '|') [PHID]
            --      ON (
            --      [PHID].[VALUE] = [TL].[WAVE_PICKING_ID]
            --      )
