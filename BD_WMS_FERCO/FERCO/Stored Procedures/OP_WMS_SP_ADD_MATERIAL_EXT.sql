...
-- Ejemplo de Ejecucion:
				--
				EXEC [FERCO].[OP_WMS_SP_ADD_MATERIAL_EXT]
				 @CLIENTE ='C00300'
                ,@USUARIO ='ADMIN'
				
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_ADD_MATERIAL_EXT (@CLIENTE VARCHAR(50)
, @USUARIO VARCHAR(50))
AS
BEGIN
  ...
  SELECT
   ...
   ,0 [IS_CAR]
   ,0 [BATCH_REQUESTED] INTO #TEMP
  FROM [FERCO].OP_WMS_CHARGE_EXTERNAL_INVENTORY CEI
  LEFT JOIN [FERCO].[OP_WMS_MATERIALS] MT
  ...
  BEGIN TRY
    MERGE [FERCO].[OP_WMS_MATERIALS] MT
    USING (SELECT
        *
      FROM #TEMP) AS ME
    ...
    WHEN NOT MATCHED
      THEN INSERT ([CLIENT_OWNER]
        ...
        , [IS_CAR]
        , [BATCH_REQUESTED])
          VALUES (ME.[CLIENT_OWNER], ME.[MATERIAL_ID], ME.[BARCODE_ID], ME.[MATERIAL_NAME], ME.[SHORT_NAME], ME.[VOLUME_FACTOR], ME.[MATERIAL_CLASS], ME.[HIGH], ME.[LENGTH], ME.[WIDTH], ME.[MAX_X_BIN], ME.[SCAN_BY_ONE], ME.[REQUIRES_LOGISTICS_INFO], ME.[WEIGTH], ME.[LAST_UPDATED], ME.[LAST_UPDATED_BY], ME.[IS_CAR], ME.[BATCH_REQUESTED]);
