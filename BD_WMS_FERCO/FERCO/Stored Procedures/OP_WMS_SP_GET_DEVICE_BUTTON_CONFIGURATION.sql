...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_DEVICE_BUTTON_CONFIGURATION] @MANUFACTURER = 'Motorola Solutions, Inc.'
              , @MODEL  = 'MC67'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_DEVICE_BUTTON_CONFIGURATION]
(
...
BEGIN
    ...
    SELECT [OWHBC].[BUTTON_ACTION_ID],
           [OWHBA].[CODE_ACTION],
           [OWHBC].[ASCCI_VALUE]
    FROM [FERCO].[OP_WMS_HH_BUTTON_CONFIG] [OWHBC]
        INNER JOIN [FERCO].[OP_WMS_DEVICE] [OWD]
            ON [OWHBC].[DEVICE_ID] = [OWD].[DEVICE_ID]
        INNER JOIN [FERCO].[OP_WMS_HH_BUTTON_ACTION] [OWHBA]
