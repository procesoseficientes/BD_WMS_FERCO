...
-- Modificacion 10/25/2017 @ NEXUS-Team Sprint eFERCO
-- rodrigo.gomez
-- Se agrega el docnum cuando es factura de venta intenra

/*
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_SET_PICKING_INTERNAL_SALE_STATUS]
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_SET_PICKING_INTERNAL_SALE_STATUS] (
...
BEGIN
  ...
  --
    ...
    -- ------------------------------------------------------------------------------------
		...
		FROM
			[FERCO].[OP_WMS_FUNC_SPLIT](@INTERNAL_SALE_COMPANIES,
											'|') [FNS]
		WHERE
			@OWNER = [FNS].[VALUE];
    -- ------------------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------------------
		...
		INNER JOIN [FERCO].[OP_WMS_FUNC_SPLIT](@INTERNAL_SALE_COMPANIES,
    ...
    -- ------------------------------------------------------------------------------------
		...
		BEGIN
			SELECT
				...
				,@TABLE = 'OINV'
				,@ERP_REFENRECE_DOC_ENTRY = [FERCO].[OP_WMS_FN_SPLIT_COLUMNS](REPLACE(@ERP_REFERENCE,
											';', ' '), 5,
											' ')
			FROM
