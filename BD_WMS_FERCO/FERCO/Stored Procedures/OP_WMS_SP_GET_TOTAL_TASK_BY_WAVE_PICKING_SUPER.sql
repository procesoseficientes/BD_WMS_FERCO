...
-- Ejemplo de Ejecucion:
	EXEC [FERCO].[OP_WMS_SP_GET_TOTAL_TASK_BY_WAVE_PICKING_SUPER]
	@WAVE_PICKING_ID = 19932,
	@LOGIN_ID = 'Marvin'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TOTAL_TASK_BY_WAVE_PICKING_SUPER]
(
...
BEGIN
    ...
    SELECT [WAREHOUSE_ID]
    FROM [FERCO].[OP_WMS_WAREHOUSE_BY_USER]
    ...
    SELECT [WBU].[WAREHOUSE_ID],
           [WBU].[LOGIN_ID]
    FROM [FERCO].[OP_WMS_WAREHOUSE_BY_USER] [WBU]
    ...
    FROM [FERCO].[OP_WMS_TASK_LIST] AS [TL]
        ...
        LEFT JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PHR]
    ...
    FROM [FERCO].[OP_WMS_TASK_LIST] AS [TL]
		LEFT JOIN @USERS_X_WAREHOUSE [UW] 
			ON ([TL].[TASK_ASSIGNEDTO] = [UW].[LOGIN_ID])
        LEFT OUTER JOIN [FERCO].[OP_WMS_MATERIALS] AS [M]
            ON [TL].[MATERIAL_ID] = [M].[MATERIAL_ID]
        LEFT OUTER JOIN [FERCO].[OP_WMS_LICENSES] AS [L]
            ON [TL].[LICENSE_ID_SOURCE] = [L].[LICENSE_ID]
        LEFT OUTER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PH]
            ON [TL].[CODIGO_POLIZA_TARGET] = [PH].[CODIGO_POLIZA]
        LEFT OUTER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PHR]
            ON [TL].[CODIGO_POLIZA_SOURCE] = [PHR].[CODIGO_POLIZA]
		LEFT OUTER JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IL]
