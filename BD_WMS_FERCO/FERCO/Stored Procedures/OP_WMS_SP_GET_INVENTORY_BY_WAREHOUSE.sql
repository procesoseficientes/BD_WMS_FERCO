...
-- Ejemplo de Ejecucion:
		exec [FERCO].OP_WMS_SP_GET_INVENTORY_BY_WAREHOUSE @CODE_CLIENT='FERCO'
                                                      ,@CODE_WAREHOUSE='BODEGA_01'  
                                                      ,@IS_DISCRETIONAL=0
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_INVENTORY_BY_WAREHOUSE] (
...
BEGIN
	...
	FROM
		[FERCO].[OP_WMS_VIEW_INVENTORY_GENERAL] AS [IG]
	WHERE
...
BEGIN
	...
	SELECT
		...
		,MAX(CASE	WHEN [M].[IS_MASTER_PACK] = 1
					THEN [FERCO].[OP_WMS_FN_GET_AVAILABLE_INVENTORY_TO_ASSAMBLE_FOR_MASTERPACK_1]([M].[MATERIAL_ID],
											@CODE_WAREHOUSE)
					ELSE 0
				END) [CANTIDAD_MP]
		,ISNULL([PAG].[STATUS_CODE], '') [STATUS_CODE]
	FROM
		[FERCO].[OP_WMS_MATERIALS] [M]
	LEFT JOIN [FERCO].[OP_WMS_VIEW_PICKING_AVAILABLE_GENERAL] [PAG] ON [PAG].[MATERIAL_ID] = [M].[MATERIAL_ID]
											AND [PAG].[CURRENT_WAREHOUSE] = @CODE_WAREHOUSE
	LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [C] ON [C].[CLIENT_CODE] = [M].[CLIENT_OWNER]
	WHERE
