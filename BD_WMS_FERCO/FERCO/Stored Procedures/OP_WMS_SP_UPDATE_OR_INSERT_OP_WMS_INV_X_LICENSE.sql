...
-- Ejemplo de Ejecucion:				
				--
				exec [FERCO].[OP_WMS_SP_UPDATE_OR_INSERT_OP_WMS_INV_X_LICENSE] 
...
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_UPDATE_OR_INSERT_OP_WMS_INV_X_LICENSE @CODE_SKU VARCHAR(50)
...
AS
  ...
  SET @ON_HAND = (SELECT
    ...
    WHERE MATERIAL_ID = '31032016')--@CODE_SKU
  --select @ON_HAND AS QTY


  DECLARE @LICENSE_ID INT;
  SET @LICENSE_ID = (SELECT TOP 1
      L.LICENSE_ID
  ...
  BEGIN TRY
    -- BEGIN
    MERGE [FERCO].[OP_WMS_INV_X_LICENSE] I
    USING (SELECT
        @MATERIAL AS MATERIAL_ID) AS M
    ...
    WHEN NOT MATCHED
      THEN INSERT ([LICENSE_ID]
        ...
        , [BATCH]
        , [ENTERED_QTY])
          VALUES (@LICENSE_ID, @CUSTOMER + '/' + @CODE_SKU, @SKU_DESCRIPTION, @QTY, 0, 0, 'N/A', 'N/A', GETDATE(), @USER, @NumeroPoliza + @LICENSE_ID, @ACUERDO_COMERCIAL, 'PROCESSED', GETDATE(), GETDATE(), @NumeroPoliza, @QTY);
    ...
    IF @@error = 0
    BEGIN
      SET @RESULTADO = 'EL CODIGO -> ' + @CUSTOMER + '/' + @CODE_SKU + ' <- OPERADO CON EXITO'
      SELECT
    ...
    BEGIN
      ...
      SET @RESULTADO = 'ERROR EN CODIGO -> ' + @CUSTOMER + '/' + @CODE_SKU + ' <- '
      SELECT
        @RESULTADO AS RESULTADO
    END
