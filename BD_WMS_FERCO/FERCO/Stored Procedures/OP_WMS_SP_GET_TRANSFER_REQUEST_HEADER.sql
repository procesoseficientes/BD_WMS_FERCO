...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_TRANSFER_REQUEST_HEADER]
					@TRANSFER_REQUEST_ID = 47
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_TRANSFER_REQUEST_HEADER (@TRANSFER_REQUEST_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  SELECT
   ...
   ,[T].[LAST_UPDATE_BY]
   ,[T].[OWNER]
  FROM [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER] [T]
  INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W1]
    ON [T].[WAREHOUSE_FROM] = [W1].[WAREHOUSE_ID]
  INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W2]
    ON [T].[WAREHOUSE_TO] = [W2].[WAREHOUSE_ID]
