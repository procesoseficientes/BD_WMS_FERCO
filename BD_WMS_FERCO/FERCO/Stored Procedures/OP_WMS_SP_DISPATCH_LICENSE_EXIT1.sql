...
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_DISPATCH_LICENSE_EXIT1] (
		@XML XML
		,@LOGIN VARCHAR(50)
		,@WAVE_PICKING_ID INT
	)
    ...
    -- ------------------------------------------------------------------------------------
      ...
      -- ------------------------------------------------------------------------------------
			...
			SELECT
				...
				,ISNULL([MP].[COMPONENT_MATERIAL], [PDD].[MATERIAL_ID]) [MATERIAL_ID]
			...
			FROM
				...
				LEFT JOIN [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] MP 
				ON [PDD].[MATERIAL_ID] = [MP].[MASTER_PACK_CODE] AND [PDD].[WAS_IMPLODED] = 1 
			WHERE
