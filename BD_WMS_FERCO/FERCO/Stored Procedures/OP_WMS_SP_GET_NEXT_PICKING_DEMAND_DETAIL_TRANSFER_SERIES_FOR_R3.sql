...
-- Description:	        Sp que trae el detalle de un Pedido de transferencia wms para enviarlo a R3

/*
-- Ejemplo de Ejecucion:
			select * from [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER]
			--
			EXEC [FERCO].[OP_WMS_SP_GET_NEXT_PICKING_DEMAND_DETAIL_TRANSFER_SERIES_FOR_R3] 
				@PICKING_DEMAND_HEADER_ID = 26
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_NEXT_PICKING_DEMAND_DETAIL_TRANSFER_SERIES_FOR_R3] (
...
BEGIN
	...
	INNER JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [CM] ON [CM].[PARAM_NAME] = [M].[BASE_MEASUREMENT_UNIT]
											AND [CM].[PARAM_GROUP] = 'TIPOS_EMPAQUE'
	INNER JOIN [FERCO].[OP_WMS_COMPANY] [C] ON [C].[EXTERNAL_SOURCE_ID] = [PDH].[EXTERNAL_SOURCE_ID]
											AND [C].[CLIENT_CODE] = [PDH].[CLIENT_CODE]
	INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [T].[WAVE_PICKING_ID] = [PDH].[WAVE_PICKING_ID]
	LEFT JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MS] ON [MS].[PICKING_HEADER_ID] = [PDH].[PICKING_DEMAND_HEADER_ID]
