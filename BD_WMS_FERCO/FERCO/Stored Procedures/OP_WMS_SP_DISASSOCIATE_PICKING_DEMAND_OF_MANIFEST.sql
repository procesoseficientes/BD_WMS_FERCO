...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].OP_WMS_SP_DISASSOCIATE_PICKING_DEMAND_OF_MANIFEST @PICKING_DEMAND_HEADER_IDS = '32|65|7|', @MANIFEST_HEADER_ID = 3
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_DISASSOCIATE_PICKING_DEMAND_OF_MANIFEST] (@PICKING_DEMAND_HEADER_IDS VARCHAR(MAX), @MANIFEST_HEADER_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  BEGIN TRY
    ...
    AND ([M].[STATUS] = 'CREATED' OR [M].[STATUS] = 'CANCELED' OR [M].[STATUS] = 'IN_PICKING')
    ...
    INSERT INTO @PICKING_DEMAND_IDS ([PICKING_DEMAND_HEADER_ID])
      SELECT DISTINCT
        P.[VALUE]
      FROM [FERCO].[OP_WMS_FUNC_SPLIT_3](@PICKING_DEMAND_HEADER_IDS, @DELIMITER) [P]

    INSERT INTO @MANIFEST_DETAIL ([MANIFEST_DETAIL_ID])
      SELECT DISTINCT
        [MD].[MANIFEST_DETAIL_ID]
      FROM [FERCO].[OP_WMS_MANIFEST_DETAIL] [MD]
      INNER JOIN @PICKING_DEMAND_IDS
        ON [MD].[PICKING_DEMAND_HEADER_ID] = [@PICKING_DEMAND_IDS].[PICKING_DEMAND_HEADER_ID]


    WHILE EXISTS (SELECT TOP 1
          1
        FROM @MANIFEST_DETAIL)
    BEGIN
