...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_CANCEL_PICKING_DETAIL_LINE] @LOGIN_ID = 'ACAMACHO'
                                                        ,@WAVE_PICKING_ID = 4417
                                                        ,@MATERIAL_ID = 'FERCO/100003'

  
      SELECT * FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [OWNPDH]
        INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [L] ON [OWNPDH].[WAVE_PICKING_ID] = [L].[WAVE_PICKING_ID]
        WHERE [L].[IS_COMPLETED] <> 1 AND [L].[WAVE_PICKING_ID]  = 4417
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_CANCEL_PICKING_DETAIL_LINE] (
		@LOGIN_ID VARCHAR(25)
		,@WAVE_PICKING_ID INT
...
BEGIN
  ...
  ---------------------------------------------------------------------------------  
	...
	INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [D] ON [D].[PICKING_DEMAND_HEADER_ID] = [H].[PICKING_DEMAND_HEADER_ID]
	WHERE
  ...
  ---------------------------------------------------------------------------------  
    ...
    ---------------------------------------------------------------------------------  
		...
		BEGIN
			...
			INTO
				[#MasterPackCancelled]
			FROM
				[FERCO].[OP_WMS_FN_GET_CANCELLED_MASTERPACK_TO_ASSAMBLE_IN_WAVE_PICKING](@WAVE_PICKING_ID)
			GROUP BY
				[MATER_PACK_CODE];
			...
			BEGIN 
			   ...
			   --
				IF (@I >= 1000)
				BEGIN
					INSERT	INTO [FERCO].[OP_WMS_SWIFT_3PL_LOG]
							(
								[PROCESS_LOG]
								,[MESSAGE_LOG]
								,[DATETIME_LOG]
					...
					VALUES
							(
								'[FERCO].[OP_WMS_SP_CANCEL_PICKING_DETAIL_LINE]'
				...
				FROM
					...
					WITH (NOLOCK)
				INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [H]
	...
	EXEC [FERCO].[OP_WMS_SP_PICKING_HAS_BEEN_COMPLETED] @WAVE_PICKING_ID = @WAVE_PICKING_ID, -- int
