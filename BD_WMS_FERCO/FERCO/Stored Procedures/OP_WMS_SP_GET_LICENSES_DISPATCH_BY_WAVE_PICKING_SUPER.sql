...
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_LICENSES_DISPATCH_BY_WAVE_PICKING_SUPER] (
		@WAVE_PICKING_ID INT
		,@LOGIN_ID VARCHAR(50)
	)
AS
BEGIN
  ...
  --
	...
	SELECT DISTINCT
		...
		,[IL].[QTY]
		,[L].[CURRENT_WAREHOUSE]
		,CASE WHEN L.CURRENT_LOCATION = (SELECT TOP 1 LOCATION_SPOT FROM FERCO.OP_WMS_SHELF_SPOTS WHERE SPOT_TYPE = 'PUERTA' AND LOCATION_SPOT = L.CURRENT_LOCATION) THEN 'green'
		ELSE 'red'
		END AS IS_ALLOCATED
		, (SELECT COUNT(IV.MATERIAL_ID) FROM FERCO.OP_WMS_INV_X_LICENSE AS IV WHERE IV.LICENSE_ID = L.LICENSE_ID) [COUNT_MATERIALS]
	FROM
		[FERCO].[OP_WMS_INV_X_LICENSE] [IL]
		INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON ([L].[LICENSE_ID] = [IL].[LICENSE_ID])
	INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [TL] ON ([TL].[WAVE_PICKING_ID] = [L].[WAVE_PICKING_ID])
	WHERE
