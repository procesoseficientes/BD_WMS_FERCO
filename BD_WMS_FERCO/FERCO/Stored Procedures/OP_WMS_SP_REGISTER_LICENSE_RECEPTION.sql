-- =============================================
-- Autor:				rudi.garcia
-- Fecha de Creacion: 	16-Feb-2018 Reborn@Ulrich
-- Description:			SP que inserta transacciones y actualiza la ubicación de la licencias.

-- Autor:				marvin.solares
-- Fecha de Creacion: 	07-Jun-2018 GForce@Dinosaurio
-- Description:			se graba las transacciones(OP_WMS_TRANS) de materiales con status 'PROCESSED'

-- Modificacion 6/13/2018 @ GFORCE-Team Sprint Dinosaurio
-- rodrigo.gomez
-- Se corrigen bugs donde no explotaba en recepcion y no guardaba todos los masterpacks de la licencia
...
-- Modificacion:			carlos.lara
-- Fecha: 					01/23/20200 GForce@Paris
-- Description:			    se adapta sp para que trabajar con nuevo campo 'TOTAL_POSITION' en tabla [OP_WMS_INV_X_LICENSE]

-- Autor:				henry.rodriguez
-- Fecha de Creacion: 	08-Abril-2020 GForce@Paris
-- Description:			Se agrega validacion al momento de obtener el documento de la poliza

/*
-- Ejemplo de Ejecucion:
EXEC [FERCO].[OP_WMS_SP_REGISTER_LICENSE_RECEPTION] @LOGIN_ID VARCHAR(50) = 'RD'
       ,@TRANS_TYPE VARCHAR(25) = 'INGRESO_GENERAL'
       ,@LICENSE_ID INT = 1
       ,@LOCATION_ID VARCHAR(25) = ''
       ,@MT2 DECIMAL(18, 2) = 0
       ,@TASK_ID INT = 123
	   ,@TOTAL_POSITION INT = 1
*/
-- =============================================

CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_LICENSE_RECEPTION]
(
    @LOGIN_ID VARCHAR(50),
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT TOP 1
               @LOGIN_ID = [VALUE]
        FROM [FERCO].[OP_WMS_FUNC_SPLIT](@LOGIN_ID, '@');
        ...
        SELECT @TRADE_AGREEMENT,
               ...
               [Il].[MATERIAL_ID],
               [Il].[MATERIAL_NAME],
               [FERCO].[OP_WMS_FN_GET_MATERIAL_COST_BY_MATERIAL]([Il].[MATERIAL_ID], [L].[CLIENT_OWNER]),
               @LICENSE_ID,
               @LOCATION_ID,
               [L].[CLIENT_OWNER],
               [V].[CLIENT_NAME],
        ...
        FROM [FERCO].[OP_WMS_INV_X_LICENSE] [Il]
            INNER JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [SML]
            ...
            INNER JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [V]
                ON ([V].[CLIENT_CODE] = [L].[CLIENT_OWNER])
            LEFT JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MXSN]
                ON (
                       [MXSN].[LICENSE_ID] = [Il].[LICENSE_ID]
                       AND [MXSN].[MATERIAL_ID] = [Il].[MATERIAL_ID]
                       AND [MXSN].[STATUS] > 0
                   )
            LEFT JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCM]
        ...
        -- RECORD THE REALLOC
        ---------------------------------------------------------------------------------  
        INSERT INTO [FERCO].[OP_WMS_REALLOCS_X_LICENSE]
        (
            [LICENSE_ID],
            [TARGET_LOCATION],
            [TRANS_TYPE],
        ...
        BEGIN
            ...
            ---------------------------------------------------------------------------------  
            EXEC [FERCO].[OP_WMS_SP_INSERT_MASTER_PACK] @MATERIAL_ID_MASTER_PACK = @MATERIAL_ID,
            ...
            FROM [FERCO].[OP_WMS_MATERIALS] [M]
                LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY_BY_WAREHOUSE] [PW]
                    ON (
                           [PW].[MATERIAL_ID] = [M].[MATERIAL_ID]
                           AND [PW].[WAREHOUSE_ID] = @WAREHOUSE_ID
                       )
                LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY] [MP]
                    ON (
        ...
        BEGIN
            EXEC [FERCO].[OP_WMS_SP_UPDATE_LICENSE_FAST_PICKING] @LOGIN_ID = @LOGIN_ID,       -- varchar(50)
