...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].OP_WMS_SP_GET_BASKET_BARCODE_BY_MANIFEST @MANIFEST_HEADER_ID= 2151
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_BASKET_BARCODE_BY_MANIFEST (@MANIFEST_HEADER_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  INNER JOIN [FERCO].[OP_WMS_PICKING_LABEL_BY_MANIFEST] [PLBM]
    ON (
  ...
  INNER JOIN [FERCO].[OP_WMS_PICKING_LABELS] [PL]
    ON [PLBM].[LABEL_ID] = [PL].[LABEL_ID]
  INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH]
    ON [MD].[PICKING_DEMAND_HEADER_ID] = [PDH].[PICKING_DEMAND_HEADER_ID]
