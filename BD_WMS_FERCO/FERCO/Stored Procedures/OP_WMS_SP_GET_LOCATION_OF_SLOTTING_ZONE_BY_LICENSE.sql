...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].OP_WMS_SP_GET_LOCATION_OF_SLOTTING_ZONE_BY_LICENSE
					@LICENSE_ID = 469790
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_LOCATION_OF_SLOTTING_ZONE_BY_LICENSE] (
		@LOGIN VARCHAR(50)
		,@LICENSE_ID INT
	)
AS
BEGIN
  ...
  -- ----------------------------------------
	...
	SELECT
		...
		,SUM([FERCO].[OP_WMS_FN_CONVERT_FROM_WEIGHT_MEASURE_UNIT_TO_DEFAULT](ISNULL([M].[WEIGTH],
											0),
	...
	FROM
		@SLOTTING_ZONE [SZ]
	INNER JOIN [FERCO].[OP_WMS_SHELF_SPOTS] [SS] ON (
											[SZ].[ZONE] = [SS].[ZONE]
  ...
  -- ------------------------------------------------------------------------------------
		...
		INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON ([L].[LICENSE_ID] = [IXL].[LICENSE_ID])
  ...
  -- ------------------------------------------------------------------------------------
	...
	SELECT DISTINCT
		[C].[CLASS_ID]
	FROM
		[FERCO].[OP_WMS_CLASS] [C]
	INNER JOIN [FERCO].[OP_WMS_CLASS_ASSOCIATION] [CA] ON ([C].[CLASS_ID] = [CA].[CLASS_ASSOCIATED_ID])
  ...
  -- ------------------------------------------------------------------------------------
		...
		INNER JOIN @LOCATION_TABLE [LT] ON ([L].[CURRENT_LOCATION] = [LT].[LOCATION])
		INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([M].[MATERIAL_ID] = [IXL].[MATERIAL_ID])
		INNER JOIN [FERCO].[OP_WMS_SUB_CLASS] [C] ON ([M].[MATERIAL_SUB_CLASS] = [C].[SUB_CLASS_ID])
		WHERE
			[IXL].[QTY] > 0;
	END
  ...
  -- ----------------------------------------
	SELECT TOP 1
		@LICENSE_WEIGHT = SUM([FERCO].[OP_WMS_FN_CONVERT_FROM_WEIGHT_MEASURE_UNIT_TO_DEFAULT](ISNULL([M].[WEIGTH],
											0),
											ISNULL([M].[WEIGHT_MEASUREMENT],
											'KG'))
								* [IL].[ENTERED_QTY])
