...
/*
	Ejemplo Ejecucion: 
	EXEC	[FERCO].[OP_WMS_SP_REPORT_FISCAL_INCOME]
 ...
 */
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REPORT_FISCAL_INCOME] (@FECHA_INICIO DATETIME
...
BEGIN
  ...
  SELECT
   ...
   ,CASE [PH].[WAREHOUSE_REGIMEN]
      WHEN 'FISCAL' THEN [FERCO].[OP_WMS_FN_GET_DAYS_BY_REGIMEN]([PH].[REGIMEN])
      ELSE NULL
    END [DIAS_REGIMEN]
   ,CASE [PH].[WAREHOUSE_REGIMEN]
      WHEN 'FISCAL' THEN DATEDIFF(DAY, GETDATE(), [FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA]))
      ELSE NULL
    END [DIAS_PARA_VENCER]
   ,CASE [PH].[WAREHOUSE_REGIMEN]
      WHEN 'FISCAL' THEN [FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA])
      ELSE NULL
    END [FECHA_VENCIMIENTO]
   ,CASE
      WHEN [PH].[WAREHOUSE_REGIMEN] = 'FISCAL' AND
        DATEDIFF(DAY, GETDATE(), [FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA])) < 1 THEN 'Bloqueado'
      ELSE 'Libre'
    END [ESTADO_REGIMEN]
   ,[VF].[STATUS_NAME]
   ,[VF].[BLOCKS_INVENTORY]
  ...
  INNER JOIN [FERCO].[OP_WMS3PL_POLIZA_TRANS_MATCH] [PTM]
    ON ([PH].[DOC_ID] = [PTM].[DOC_ID])
  INNER JOIN [FERCO].[OP_WMS_TRANS] [T]
    ON ([T].[SERIAL_NUMBER] = [PTM].[TRANS_ID])
  INNER JOIN [FERCO].[OP_WMS_VIEW_VALORIZACION_FISCAL] [VF]
    ON (
  ...
  INNER JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [VC]
