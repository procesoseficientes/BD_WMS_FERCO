...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_COMPLETED_PICKING_HEADER] 
				@INITIAL_DATE = '2017-05-01 01:02:31.062'
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_COMPLETED_PICKING_HEADER] (
  ...
  -- ------------------------------------------------------------------------------------
	...
	BEGIN
		...
		SELECT DISTINCT
			[S].[VALUE]
		FROM
			[FERCO].[OP_WMS_FN_SPLIT](@CODE_ROUTE, '|') [S];
	END;
      ...
      ,CASE MAX([PDH].[IS_CONSOLIDATED])  
	   ...
	   ,MAX(ISNULL([TL].[LOCATION_SPOT_TARGET], '''')) AS [LOCATION_SPOT_TARGET]
		...
		FROM 
			...
			INNER JOIN [FERCO].[OP_WMS_FUNC_SPLIT_3]('''
		+ @WAREHOUSE
		+ ''', ''|'') [W] ON [W].[VALUE]  = [PDH].[CODE_WAREHOUSE]  
			...
			LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [PDD] ON (
