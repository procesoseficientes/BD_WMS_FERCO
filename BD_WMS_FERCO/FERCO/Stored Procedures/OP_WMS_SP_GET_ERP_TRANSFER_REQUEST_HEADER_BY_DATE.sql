...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_ERP_TRANSFER_REQUEST_HEADER_BY_DATE] (
...
BEGIN
  ...
  --
	CREATE TABLE [#PICKING_DOCUMENT] (
		...
		,[PROJECT] VARCHAR(25)
		,PRIMARY KEY
			([SALES_ORDER_ID], [EXTERNAL_SOURCE_ID], [OWNER])
	);
  --
  ...
  --
    ...
    --
	  ...
	  PRINT 5
          ...
          INNER JOIN [FERCO].[OP_WMS_COMPANY] [C] ON [TRH].[SOURCE] COLLATE DATABASE_DEFAULT  = [C].[CLIENT_CODE] COLLATE DATABASE_DEFAULT AND [C].[EXTERNAL_SOURCE_ID] = '
				+ CAST(@EXTERNAL_SOURCE_ID AS VARCHAR) + '
	...
	PRINT 7
		...
		LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [P] ON (
											[PD].[SALES_ORDER_ID]  = [P].[DOC_ENTRY] COLLATE DATABASE_DEFAULT
