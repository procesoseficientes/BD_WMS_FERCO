...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_CLEAN_INV_X_LICENSE]
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_CLEAN_INV_X_LICENSE]
AS
BEGIN
	SET NOCOUNT ON;
	...
	-- Pasa al historico
	-- ------------------------------------------------------------------------------------
	INSERT INTO [FERCO].[OP_WMS_INV_X_LICENSE_HISTORY]
			(
				[PK_LINE]
				,[LICENSE_ID]
				,[MATERIAL_ID]
	...
	-- ------------------------------------------------------------------------------------
	DELETE FROM [FERCO].[OP_WMS_INV_X_LICENSE]
	WHERE [PK_LINE] > 0
		AND [QTY] = 0
	-- ------------------------------------------------------------------------------------
	-- Inserta al historico
	-- ------------------------------------------------------------------------------------
	INSERT INTO [FERCO].[OP_WMS_LICENSES_HISTORY]
	        (
	          [LICENSE_ID]
	        , [CLIENT_OWNER]
	        , [CODIGO_POLIZA]
	...
	FROM [FERCO].[OP_WMS_LICENSES] [L]
		LEFT JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IL] ON ([IL].[LICENSE_ID] = [L].[LICENSE_ID])
