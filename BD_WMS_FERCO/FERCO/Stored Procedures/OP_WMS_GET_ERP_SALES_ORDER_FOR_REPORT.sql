...
-- Fecha de Creacion: 	19-02-2021 @ProcesosEficientes
-- Description:			Se agrego que mande a traer las Delivery order

-- Autor:				Thomas.Pamal
-- Fecha:				16-06-2021 @ProcesosEficientes
-- Description          Se creo un filtro en ordenes de venta [SOH].[U_estado2]  = ''03''

/*
-- Ejemplo de Ejecucion:
		EXEC [FERCO].[OP_WMS_GET_ERP_SALES_ORDER_FOR_REPORT]
			@WAREHOUSE = 'BODEGA_01',
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_ERP_SALES_ORDER_FOR_REPORT] (
...
BEGIN
	...
	CREATE TABLE [#PICKING_DEMAND_DETAIL] (
		[DOC_NUM] VARCHAR(50) COLLATE DATABASE_DEFAULT
	...
	CREATE TABLE [#RESULT] (
		...
		,[U_Serie] NVARCHAR(50) COLLATE DATABASE_DEFAULT
		,[U_NoDocto] NVARCHAR(10) COLLATE DATABASE_DEFAULT
		,[CLIENT_ID] NVARCHAR(15)
		,[NAME_CUSTOMER] NVARCHAR(100)
		,[SLPNAME] NVARCHAR(155)
		,[CODE_SELLER] NVARCHAR(50)
		,[SKU] NVARCHAR(50) COLLATE DATABASE_DEFAULT
	...
	BEGIN
		...
		FROM
			[FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [D]
		INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [H] ON [D].[PICKING_DEMAND_HEADER_ID] = [H].[PICKING_DEMAND_HEADER_ID]
		WHERE
    ...
    -- ------------------------------------------------------------------------------------
		...
		INSERT INTO #RESULT
			...
			+ ' [EXTERNAL_SOURCE_ID]
			   ...
			   END [MATERIAL_IN_SWIFT]
			    ,[SOH].[U_estado2] [ESTADO_DOCUMENTO]
			   ,[SOH].[Address2] 
			...
			+ '.[ERP_SALES_ORDER_HEADER_CHANNEL_MODERN] [SOH] WITH(NOLOCK) ON (
				[SOH].[DOCNUM] = [SOD].[DOCNUM] 
				AND [SOH].[OWNER] COLLATE DATABASE_DEFAULT = [SOD].[OWNER] COLLATE DATABASE_DEFAULT
			  )
			  INNER JOIN [FERCO].[OP_WMS_COMPANY] [OWC] WITH(NOLOCK) ON (
							[OWC].CLIENT_CODE COLLATE DATABASE_DEFAULT = [SOD].[OWNER] COLLATE DATABASE_DEFAULT
							AND [OWC].[EXTERNAL_SOURCE_ID] = '
			+ CAST(@EXTERNAL_SOURCE_ID AS VARCHAR)
			+ '
			  )

              LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [M] WITH(NOLOCK) ON (
            				ISNULL([SOD].[U_OWNERSKU],'''') +'
			+ '''/'' + [SOD].[ItemCode] COLLATE DATABASE_DEFAULT = [M].[MATERIAL_ID] COLLATE DATABASE_DEFAULT
              )
              ...
              WHERE [SOD].[Sequence] = @SEQUENCE_DETAIL
				AND [SOH].[U_estado2]  = ''03''
				AND [SOH].[Sequence] = @SEQUENCE_HEADER
				AND [SOD].[WhsCode] COLLATE DATABASE_DEFAULT  = ''' + @ERP_WAREHOUSE 
		...
		EXEC ' + @DATA_BASE_NAME + '.' + @SCHEMA_NAME
			+ '.[ERP_SP_INSERT_DELIVERY_ORDER_HEADER_FOR_REPORT] 
						@START_DATE = '''
			+ CAST(@START_DATE AS VARCHAR) + ''', 
						@END_DATE = '''
			+ CAST(@END_DATE AS VARCHAR) 
			...
			+ ''',
						@SEQUENCE = @SEQUENCE_HEADER OUTPUT	-- int
		EXEC ' + @DATA_BASE_NAME + '.' + @SCHEMA_NAME
			+ '.[ERP_SP_INSERT_DELIVERY_ORDER_DETAIL_FOR_REPORT] 
						@START_DATE = '''
			+ CAST(@START_DATE AS VARCHAR) + ''', 
						@END_DATE = '''
			+ CAST(@END_DATE AS VARCHAR) 
		...
		INSERT INTO #RESULT
			...
			+ ' [EXTERNAL_SOURCE_ID]
              ...
              FROM ' + @DATA_BASE_NAME + '.' + @SCHEMA_NAME
			+ '.[ERP_DELIVERY_ORDER_DETAIL] [SOD] WITH(NOLOCK)
			  INNER JOIN ' + @DATA_BASE_NAME + '.'
			+ @SCHEMA_NAME
			+ '.[ERP_DELIVERY_ORDER_HEADER] [SOH] WITH(NOLOCK) ON (
				[SOH].[DOCNUM] = [SOD].[DOCNUM] 
				AND [SOH].[OWNER] COLLATE DATABASE_DEFAULT = [SOD].[OWNER] COLLATE DATABASE_DEFAULT
			  )
			  INNER JOIN [FERCO].[OP_WMS_COMPANY] [OWC] WITH(NOLOCK) ON (
	...
	INTO
		[#DEMANDS]
	FROM
		[FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH]
	...
	SELECT
		...
		,CASE WHEN [R].[ESTADO_DOCUMENTO] = '01'
				THEN 'ANTICIPO'
				WHEN [R].[ESTADO_DOCUMENTO] = '02'
				THEN 'RESERVA'
				WHEN [R].[ESTADO_DOCUMENTO] = '33'
				THEN 'RESERVA TEMPORAL'
				WHEN [R].[ESTADO_DOCUMENTO] = '04'
				THEN 'RESERVA PAGADA'
				WHEN [R].[ESTADO_DOCUMENTO] = '05'
				THEN 'DESPACHO'
				WHEN [R].[ESTADO_DOCUMENTO] = '06'
				THEN 'ENTREGA PARCIAL'
				WHEN [R].[ESTADO_DOCUMENTO] = '07'
				THEN 'ENTREGA COMPLETA'
				WHEN [R].[ESTADO_DOCUMENTO] = '08'
				THEN 'ANULADO'
				WHEN [R].[ESTADO_DOCUMENTO] = '03'
				THEN 'PREPARACION'
				WHEN [R].[ESTADO_DOCUMENTO] = '09'
				THEN 'REVISAR'
				ELSE 'NO DEFINIDO'
			 END [ESTADO_DOCUMENTO]
	...
	LEFT JOIN [#DEMANDS] [D] ON (
									...
									AND [D].[OWNER] COLLATE DATABASE_DEFAULT = [R].[SOURCE] COLLATE DATABASE_DEFAULT
									AND [D].[DOC_NUM] = [R].[SALES_ORDER_ID]
									AND [D].[MASTER_ID_MATERIAL] COLLATE DATABASE_DEFAULT = [R].[MASTER_ID_MATERIAL] COLLATE DATABASE_DEFAULT
									AND [D].[LINE_NUM] = [R].[LINE_SEQ] 
								)
	LEFT JOIN [FERCO].[OP_WMS_MANIFEST_DETAIL] [MD] ON (
											[MD].[PICKING_DEMAND_HEADER_ID] = [D].[PICKING_DEMAND_HEADER_ID]
											AND [MD].[LINE_NUM] = [D].[LINE_NUM]
											AND [MD].[MATERIAL_ID] COLLATE DATABASE_DEFAULT = [D].[MATERIAL_ID] COLLATE DATABASE_DEFAULT
											)
	LEFT JOIN [FERCO].[OP_WMS_MANIFEST_HEADER] [MH] ON ([MH].[MANIFEST_HEADER_ID] = [MD].[MANIFEST_HEADER_ID])
	WHERE R.ESTADO_DOCUMENTO = '03'
	GROUP BY
		[R].[SOURCE]
		,[R].[SALES_ORDER_ID]
		,[R].[SKU]
