...
--Modificación 23/11/2020
					--elder.l
					--Se ha modifiado la linea 109 del sp [FERCO].[OP_WMS_SP_GET_RECEPTION_DETAILS] para que en caso de que no se 
					--defina una bodega, obtenga la bodega por defecto de la tabla [FERCO].[OP_WMS_CONFIGURATIONS] donde PARAM_NAME 
					--es igual a ERP_WAREHOUSE_PURCHASE_ORDER y tambien se ha modificado la linea 124 donde se agrego ERP_WAREHOUSE_COUDE al Group By


/*
-- Ejemplo de Ejecucion:
         EXEC [FERCO].[OP_WMS_SP_GET_RECEPTION_DETAILS] @RECEPTION_HEADER = 2006
*/
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_RECEPTION_DETAILS]
    (
...
BEGIN
    ...
    INNER JOIN [FERCO].[OP_SETUP_EXTERNAL_SOURCE] [ES] ON [RDH].[EXTERNAL_SOURCE_ID] = [ES].[EXTERNAL_SOURCE_ID]
    WHERE
    ...
    FROM
        [FERCO].[OP_WMS_CONFIGURATIONS] AS [C]
    WHERE
    ...
    SELECT
       ...
       ,[FERCO].[OP_WMS_GET_STRING_FROM_CHAR]([D].[MATERIAL_ID], '/') [ItemCode]
       ...
       ,CASE WHEN @SERIE = @COST_SERIE THEN @COST_WAREHOUSE_PARAMETER
             ELSE (h.ERP_WAREHOUSE_CODE)
        END AS [WarehouseCode]
    FROM
        [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [D]
