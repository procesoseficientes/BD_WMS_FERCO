...
-- =============================================
CREATE PROCEDURE [FERCO].OP_WMS_SP_AUDIT_RECEPTION_COUNT 
...
BEGIN
	...
	BEGIN
		...
		SELECT @pCLIENT_ID = (SELECT TOP 1 CLIENT_CODE FROM OP_WMS_POLIZA_HEADER 
								WHERE NUMERO_ORDEN = (SELECT NUMERO_ORDEN 
														FROM [FERCO].OP_WMS_AUDIT_RECEPTION_CONTROL
														WHERE AUDIT_ID = @pAUDIT_ID)
								AND CODIGO_POLIZA = (SELECT CODIGO_POLIZA FROM [FERCO].OP_WMS_AUDIT_RECEPTION_CONTROL
														WHERE AUDIT_ID = @pAUDIT_ID))
		
		SELECT @pMATERIAL_ID = ISNULL((SELECT MATERIAL_ID FROM [FERCO].OP_WMS_MATERIALS 
		...
		IF EXISTS(SELECT 1 FROM [FERCO].OP_WMS_AUDIT_RECEPTION_SERIES
		...
		INSERT INTO [FERCO].[OP_WMS_AUDIT_RECEPTION_SKUS]
			   ([AUDIT_ID]
			   ,[CODIGO_POLIZA]
			   ,[MATERIAL_ID]
			   ,[BARCODE_ID]
		 ...
		 VALUES
			   ...
			   ,@pMATERIAL_ID
			   ,@pBARCODE_ID
			   ,(SELECT MATERIAL_NAME FROM [FERCO].OP_WMS_MATERIALS WHERE MATERIAL_ID = @pMATERIAL_ID)
			   ,@pSCANNED
			   ,@pINPUTED
			   ,CURRENT_TIMESTAMP
			   ,@pLOGIN_ID)
		...
		IF(@pMETHOD = 'SCANNING') BEGIN
			BEGIN TRY
				BEGIN
					INSERT INTO [FERCO].[OP_WMS_AUDIT_RECEPTION_SERIES]
						   ([AUDIT_ID]
						   ,[SERIAL_NUMBER]
						   ,[MATERIAL_ID]
						   ,[LAST_UPDATED])
	...
	BEGIN CATCH
		...
		IF(@pMETHOD = 'INPUT') BEGIN
			UPDATE [FERCO].[OP_WMS_AUDIT_RECEPTION_SKUS]
