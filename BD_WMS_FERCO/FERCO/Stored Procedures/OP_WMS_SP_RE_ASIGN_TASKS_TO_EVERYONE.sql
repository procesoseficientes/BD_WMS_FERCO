...
-- Description:			SP que asigna las tareas pendientas de forma equitativa a todos los usuarios de tipo operador y activos

-- Modificación:        hector.gonzalez
-- Fecha de Creacion: 	2017-03-15 Team ERGON - Sprint VERGON
...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_RE_ASIGN_TASKS_TO_EVERYONE]
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_RE_ASIGN_TASKS_TO_EVERYONE] (@LOGIN VARCHAR(50))
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  BEGIN TRY
    ...
    EXEC [FERCO].[OP_WMS_SP_GET_OPERATORS_ASSIGNED_TO_DISTRIBUTION_CENTER_BY_USER_FOR_REASIGN_TASK] @LOGIN

    -- ------------------------------------------------------------------------------------
    -- Obtiene los usuarios activos
    -- ------------------------------------------------------------------------------------
    SELECT
      [L].[LOGIN_ID] INTO #LOGIN
    FROM @TB_USERS [U]
    INNER JOIN [FERCO].[OP_WMS_LOGINS] [L]
      ON [U].[LOGIN_ID] = [L].[LOGIN_ID]
    --
    SELECT
