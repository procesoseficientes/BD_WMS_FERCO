...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_LABEL_DETAIL]
					@TYPE = 'CAJA'
					,@BARCODE = 'P-4881-1/2'
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_LABEL_DETAIL] (
		@TYPE VARCHAR(50)
		,@BARCODE VARCHAR(50)
		,@WAVE_PICKING_ID INT
	)
...
BEGIN
	...
	BEGIN
		...
		FROM
			[#BOXES] [B]
		INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [FERCO].[OP_WMS_FN_SPLIT_COLUMNS]([B].[ERP_DOC],
											2, '-') = [T].[WAVE_PICKING_ID]
		WHERE
			[B].[BOX_ID] = @BARCODE
