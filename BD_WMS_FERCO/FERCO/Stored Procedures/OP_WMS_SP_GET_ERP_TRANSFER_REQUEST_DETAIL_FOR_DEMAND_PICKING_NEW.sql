...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_ERP_TRANSFER_REQUEST_DETAIL_FOR_DEMAND_PICKING_NEW] ( @XML XML )
AS
    BEGIN
        SET NOCOUNT ON;
	--
        DECLARE @SOURCE_NAME VARCHAR(50) ,
            ...
            @QUERY NVARCHAR(MAX) ,
            @SALE_ORDER_ID NUMERIC(18, 0) ,
            @EXTERNAL_SOURCE_ID INT;
	--
        CREATE TABLE [#PICKING_DOCUMENT]
            (
              [EXTERNAL_SOURCE_ID] INT ,
              [PICKING_DOCUMENT_ID] NUMERIC(18, 0) ,
              [OWNER] VARCHAR(50) ,
	...
	--
        CREATE TABLE [#PICKING_DOCUMENT_DETAIL]
            (
              [SALES_ORDER_ID] NUMERIC(18, 0) NOT NULL ,
              [SKU] VARCHAR(200) ,
              [LINE_SEQ] INT NULL ,
	...
	--
		...
		--
			...
			--
                    ...
                    WHERE   [H].[EXTERNAL_SOURCE_ID] = 1--@EXTERNAL_SOURCE_ID
                            AND [H].[IS_FROM_ERP] = 1
			...
			SELECT * FROM #PICKING_DEMAND_DETAIL
		  -- ------------------------------------------------------------------------------------
		  -- Obtiene el detalle de la ordenes de venta de la fuente externa
		  -- ------------------------------------------------------------------------------------
				--SELECT * FROM [#PICKING_DEMAND_DETAIL]
                    SELECT  @QUERY = N'INSERT INTO [#PICKING_DOCUMENT_DETAIL]
					SELECT
						[TRD].[DOC_ENTRY]
						,[TRD].[MATERIAL_ID]
						,[TRD].[LINE_NUM]
					...
					INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([M].[MATERIAL_ID] COLLATE DATABASE_DEFAULT = [TRD].[MATERIAL_ID] COLLATE DATABASE_DEFAULT)
					LEFT JOIN [#PICKING_DEMAND_DETAIL] DD ON ([TRD].[DOC_ENTRY]	= DD.[DOC_ENTRY]  AND DD.[LINE_SEQ]= [TRD].[LINE_NUM])


					--LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH] ON (
      ...
      -- ------------------------------------------------------------------------------------
		...
		-- ------------------------------------------------------------------------------------
            ...
            FROM    [#PICKING_DOCUMENT_DETAIL] [PDD]
                    LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [UMM].[MATERIAL_ID] COLLATE DATABASE_DEFAULT = [PDD].[SKU] COLLATE DATABASE_DEFAULT
