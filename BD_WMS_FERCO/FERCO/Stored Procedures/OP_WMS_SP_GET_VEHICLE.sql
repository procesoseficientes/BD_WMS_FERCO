...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].OP_WMS_SP_GET_VEHICLE @VEHICLE_CODE = NULL
			--
			EXEC  [FERCO].OP_WMS_SP_GET_VEHICLE @VEHICLE_CODE = 18
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_VEHICLE] (
...
BEGIN
  ...
  --
	...
	LEFT JOIN [FERCO].[OP_WMS_TRANSPORT_COMPANY] AS [TC] ON ([V].[TRANSPORT_COMPANY_CODE] = [TC].[TRANSPORT_COMPANY_CODE])
	LEFT JOIN [FERCO].[OP_WMS_PILOT] AS [P] ON ([V].[PILOT_CODE] = [P].[PILOT_CODE])
	LEFT JOIN [FERCO].[OP_WMS_INSURANCE_DOCS] [I] ON ([I].[DOC_ID] = [V].[INSURANCE_DOC_ID])
	WHERE
		(
