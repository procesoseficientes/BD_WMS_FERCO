...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_NEXT_PICKING_DEMAND_DETAIL_BKGW] (@PICKING_DEMAND_HEADER_ID INT
, @OWNER VARCHAR(50))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  EXEC [FERCO].OP_WMS_SP_GET_MATERIALS_WITH_BATCH_FROM_DEMAND_DISPATCH @PICKING_DEMAND_HEADER_ID = ' + CAST(@PICKING_DEMAND_HEADER_ID AS VARCHAR) + '
	...
	FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH]
		left outer JOIN SAPSERVER.SBOFERCO.DBO.RDR1 RDR1 ON  ( '''+@OWNER+'''=''ferco'' and rdr1.DocEntry=[PDH].DOC_ENTRY  COLLATE DATABASE_DEFAULT)
		left outer JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [PDD] ON [PDD].[PICKING_DEMAND_HEADER_ID] = [PDH].[PICKING_DEMAND_HEADER_ID]  and ('''+@OWNER+'''+''/''+rdr1.itemCode=[PDD].material_id collate database_default or '''+@OWNER+'''<> ''ferco'' ) 
