...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_GET_TASK_DEMAND_DETAIL]
					@WAVE_PICKING_ID = 339
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_TASK_DEMAND_DETAIL] (@WAVE_PICKING_ID INT)
AS
BEGIN
    SET NOCOUNT ON;
	--
    ...
    SELECT DISTINCT
       ...
       ,CASE WHEN COALESCE([PDH].[IS_POSTED_ERP], [PD].[IS_POSTED_ERP], 0) = -1
             THEN 'Fallido'
             WHEN COALESCE([PDH].[IS_POSTED_ERP], [PD].[IS_POSTED_ERP], 0) = 0
                  AND ISNULL([PDH].[IS_AUTHORIZED], 0) = 1 THEN 'Autorizada'
             WHEN COALESCE([PDH].[IS_POSTED_ERP], [PD].[IS_POSTED_ERP], 0) = 0
                  AND COALESCE([PDH].[IS_AUTHORIZED], [PD].[IS_AUTHORIZED], 0) = 0
             THEN 'Pendiente de Autorización'
             WHEN COALESCE([PDH].[IS_POSTED_ERP], [PD].[IS_POSTED_ERP], 0) = 1
             THEN 'Enviado'
       ...
       ,CAST(ISNULL([PDH].[ERP_REFERENCE_DOC_NUM], [PD].[ERP_REFERENCE_DOC_NUM]) AS VARCHAR) [ERP_REFERENCE]
    ...
    INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] ON [TL].[WAREHOUSE_SOURCE] = [W].[WAREHOUSE_ID]
    LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] ON [PDH].[WAVE_PICKING_ID] = [TL].[WAVE_PICKING_ID]
    LEFT JOIN [FERCO].[OP_WMS_PICKING_ERP_DOCUMENT] [PD] ON [PD].[WAVE_PICKING_ID] = [TL].[WAVE_PICKING_ID]
    WHERE
