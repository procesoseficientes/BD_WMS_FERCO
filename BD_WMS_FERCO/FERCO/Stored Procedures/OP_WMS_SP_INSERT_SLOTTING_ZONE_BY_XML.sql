...
EXECUTE [FERCO].[OP_WMS_SP_INSERT_SLOTTING_ZONE_BY_XML] @XML = '<ArrayOfZonaDePosicionamiento>
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_INSERT_SLOTTING_ZONE_BY_XML] ( @XML AS XML )
AS
    BEGIN TRY
  ...
  ---------------------------------------------------------------------------------
        INSERT  INTO @SLOTTING_ZONE_TABLE
                ...
                FROM    @TEMPLATE_TABLE [TT]
                        LEFT JOIN [FERCO].[OP_WMS_ZONE] [Z] ON ( [TT].[ZONE] = [Z].[ZONE]
  ...
  ---------------------------------------------------------------------------------
                ...
                INSERT  INTO @SLOTTING_ZONE_TABLE
                        ...
                        SELECT  [TT].[WAREHOUSE_ID] ,
                                [TT].[CLASS_ID]
                        FROM    @TEMPLATE_TABLE [TT]
                                LEFT JOIN [FERCO].[OP_WMS_CLASS] [C] ON [TT].[CLASS_ID] = [C].[CLASS_ID]
  ...
  ---------------------------------------------------------------------------------
                ...
                INSERT  INTO @SLOTTING_ZONE_TABLE
                        ...
                        SELECT  [SZ].[ID] ,
                                ...
                                [TT].[MANDATORY]
                        FROM    @TEMPLATE_TABLE [TT]
                                LEFT JOIN [FERCO].[OP_WMS_SLOTTING_ZONE] [SZ] ON ( [TT].[ZONE] = [SZ].[ZONE]
  ...
  ---------------------------------------------------------------------------------
                ...
                WHILE ( EXISTS ( SELECT 1
                    ...
                    BEGIN
                        ...
                        FROM    [FERCO].[OP_WMS_ZONE]
                        WHERE   [ZONE] = @ZONE;

                        INSERT  INTO [FERCO].[OP_WMS_SLOTTING_ZONE]
                                ( [ID] ,
                                  [WAREHOUSE_CODE] ,
                                  [ZONE_ID] ,
                                  [ZONE] ,
  ...
  ---------------------------------------------------------------------------------
                ...
                FROM    [FERCO].[OP_WMS_SLOTTING_ZONE] [SZ]
  ...
  ---------------------------------------------------------------------------------
                ...
                INSERT  INTO [FERCO].[OP_WMS_SLOTTING_ZONE_BY_CLASS]
                        ...
                        FROM    @SLOTTING_ZONE_TABLE AS [SZT]
                                INNER JOIN [FERCO].[OP_WMS_SLOTTING_ZONE] [SZ] ON ( [SZT].[SLOTTING_ID] = [SZ].[ID] )
                                INNER JOIN [FERCO].[OP_WMS_CLASS] [C] ON [SZT].[CLASS_ID] = [C].[CLASS_ID];
  ...
  ---------------------------------------------------------------------------------
                ...
                INSERT  INTO @SLOTTING_ZONE_TABLE
                        ...
                        SELECT  [TT].[WAREHOUSE_ID] ,
                                [TT].[CLASS_ID]
                        FROM    @TEMPLATE_TABLE [TT]
                                LEFT JOIN [FERCO].[OP_WMS_SUB_CLASS] [C] ON [TT].[CLASS_ID] = [C].[SUB_CLASS_ID]
  ...
  ---------------------------------------------------------------------------------
                ...
                DELETE  [SZT]
                FROM    [FERCO].[OP_WMS_SLOTTING_ZONE_BY_SUB_CLASS] [SZBC]
                        INNER JOIN @SLOTTING_ZONE_TABLE [SZT] ON ( [SZBC].[SUB_CLASS_ID] = [SZT].[CLASS_ID]
                                                              AND [SZBC].[ID_SLOTTING_ZONE] = [SZT].[SLOTTING_ID]
                                                              );

                INSERT  INTO [FERCO].[OP_WMS_SLOTTING_ZONE_BY_SUB_CLASS]
                        ( [ID_SLOTTING_ZONE] ,
                          [SUB_CLASS_ID] ,
                          [SUB_CLASS_NAME]
                        )
                        ...
                        FROM    @SLOTTING_ZONE_TABLE AS [SZT]
                                INNER JOIN [FERCO].[OP_WMS_SLOTTING_ZONE] [SZ] ON ( [SZT].[SLOTTING_ID] = [SZ].[ID] )
                                INNER JOIN [FERCO].[OP_WMS_SUB_CLASS] [C] ON [SZT].[CLASS_ID] = [C].[SUB_CLASS_ID];
