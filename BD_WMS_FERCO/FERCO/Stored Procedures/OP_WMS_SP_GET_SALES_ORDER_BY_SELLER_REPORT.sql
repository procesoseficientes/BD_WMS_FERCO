...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_SALES_ORDER_BY_SELLER_REPORT]
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_SALES_ORDER_BY_SELLER_REPORT (@START_DATETIME DATETIME
...
BEGIN
  ...
  EXEC [FERCO].[OP_WMS_SP_GET_SONDA_SALES_ORDER_REPORT] @START_DATETIME = @START_DATETIME
                                                       , -- datetime
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO @SALES_ORDER
  EXEC [FERCO].[OP_WMS_SP_GET_ERP_SALES_ORDER_REPORT] @START_DATETIME = @START_DATETIME
                                                     , -- datetime
                                                      @END_DATETIME = @END_DATETIME
                                                     , -- datetime
                                                      @CODE_WAREHOUSE = @CODE_WAREHOUSE -- varchar(50)
  ...
  SELECT
   ...
   ,[DH].[WAVE_PICKING_ID]
   ,CASE
      WHEN [DH].[PICKING_DEMAND_HEADER_ID] IS NULL THEN 0
      ELSE 1
    END [IN_PICKING_DEMAND]
  ...
  LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH]
    ON [DH].[DOC_NUM] = [SO].[SALES_ORDER_ID]
      AND [DH].[EXTERNAL_SOURCE_ID] = [SO].[EXTERNAL_SOURCE_ID]
  LEFT JOIN [FERCO].[OP_WMS_TASK_LIST] [TL]
    ON [TL].[WAVE_PICKING_ID] = [DH].[WAVE_PICKING_ID]
