...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_LOAD_CLASSES_BY_XML]
					@XML = N'
<Data>
	<Clase>
...
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_LOAD_CLASSES_BY_XML (@XML XML
, @LOGIN VARCHAR(50))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  BEGIN TRY
    ...
    USING (SELECT
      ...
      FROM @CLASS) AS [TC]
    ON [C].[CLASS_ID] > 0
      AND [TC].[CLASS_NAME] = [C].[CLASS_NAME]
    WHEN MATCHED
      THEN UPDATE
        SET [C].[CLASS_NAME] = [TC].[CLASS_NAME]
    ...
    WHEN NOT MATCHED
      THEN INSERT ([CLASS_NAME]
        ...
        , [LAST_UPDATED]
        , [PRIORITY])
          VALUES ([TC].[CLASS_NAME], [TC].[CLASS_DESCRIPTION], [TC].[CLASS_TYPE], [TC].[CREATED_BY], [TC].[CREATED_DATETIME], [TC].[LAST_UPDATED_BY], [TC].[LAST_UPDATED], [TC].[PRIORITY]);
    -- ------------------------------------------------------------------------------------
    -- Muestra resultado
    -- ------------------------------------------------------------------------------------
    COMMIT;
