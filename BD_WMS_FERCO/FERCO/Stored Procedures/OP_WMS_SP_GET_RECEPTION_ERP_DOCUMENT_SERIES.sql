...
-- Ejemplo de Ejecucion:
		EXEC [FERCO].[OP_WMS_SP_GET_RECEPTION_ERP_DOCUMENT_SERIES] @SERIAL_NUMBER = 237
		
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_RECEPTION_ERP_DOCUMENT_SERIES] (@SERIAL_NUMBER INT)
AS
BEGIN
	...
	IF EXISTS ( SELECT
				...
				INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [H] ON [H].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [MSN].[RECEPTION_HEADER_ID]
				WHERE
	...
	BEGIN
		...
		FROM
			[FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MSN]
		INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [H] ON [H].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [MSN].[RECEPTION_HEADER_ID]
		LEFT JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [L].[LICENSE_ID] = [MSN].[LICENSE_ID]
		INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [M].[MATERIAL_ID] = [MSN].[MATERIAL_ID]
		WHERE
			[H].[TASK_ID] = @SERIAL_NUMBER;
	...
	LEFT JOIN [FERCO].[OP_WMS_CLASS] [CL] ON [CL].[CLASS_NAME] = [C].[SPARE1]
	LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [CL].[CLASS_ID] = [M].[MATERIAL_CLASS]
	LEFT JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MS] ON [MS].[MATERIAL_ID] = [M].[MATERIAL_ID]
	...
	LEFT JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [L].[CODIGO_POLIZA] = [T].[CODIGO_POLIZA]
	INNER JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MSN] ON [T].[MATERIAL_CODE] = [MSN].[MATERIAL_ID]
											AND [MSN].[LICENSE_ID] = [L].[LICENSE_ID]
											AND [MSN].[SERIAL] = [T].[SERIAL]
	INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [M].[MATERIAL_ID] = [T].[MATERIAL_CODE]
