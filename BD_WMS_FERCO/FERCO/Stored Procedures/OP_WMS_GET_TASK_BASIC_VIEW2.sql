...
*/
-- =============================================
create PROCEDURE [FERCO].[OP_WMS_GET_TASK_BASIC_VIEW2] (
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
	...
	SELECT
		...
		,[W].[USE_PICKING_LINE]
	FROM
		[FERCO].[OP_WMS_WAREHOUSE_BY_USER] [WU]
	LEFT JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] ON [WU].[WAREHOUSE_ID] = [W].[WAREHOUSE_ID]
	WHERE
		[WU].[LOGIN_ID] = @LOGIN;
  -- ------------------------------------------------------------------------------------
  ...
  -- ------------------------------------------------------------------------------------
	...
	LEFT JOIN [FERCO].[OP_WMS_TRANS] [T] ON [TL].[SERIAL_NUMBER] = [T].[TASK_ID]
  ...
  -- ------------------------------------------------------------------------------------
	...
	FROM
		[FERCO].[OP_WMS_TASK_LIST] [TL]
	LEFT JOIN [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER] [TH] ON [TH].[TRANSFER_REQUEST_ID] = [TL].[TRANSFER_REQUEST_ID]
	WHERE
  ...
  -- ------------------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------------------
			...
			FROM
				[FERCO].[OP_WMS_VIEW_TASK] [VT]
			INNER JOIN [#LOCATION_TARGET_RECEPTION] [LTR] ON [LTR].[SERIAL_NUMBER] = [VT].[SERIAL_NUMBER]
			LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RDH] ON ([VT].[SERIAL_NUMBER] = [RDH].[TASK_ID])
    ...
    -- ------------------------------------------------------------------------------------
			SELECT DISTINCT
				...
				,[VT].[PROJECT_SHORT_NAME]
			FROM
				[FERCO].[OP_WMS_VIEW_TASK_PICKING_HEADER] [VT]
			...
			LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] ON ([VT].[WAVE_PICKING_ID] = [PDH].[WAVE_PICKING_ID])
			LEFT JOIN [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER] [TRH] ON ([PDH].[TRANSFER_REQUEST_ID] = [TRH].[TRANSFER_REQUEST_ID])
			LEFT JOIN [FERCO].[OP_WMS_PICKING_ERP_DOCUMENT] [PD] ON [PD].[WAVE_PICKING_ID] = [VT].[WAVE_PICKING_ID]
			LEFT JOIN [#USERS] [U] ON [U].[login] = [VT].[TASK_ASSIGNEDTO]
    ...
    -- ------------------------------------------------------------------------------------
			...
			FROM
				[FERCO].[OP_WMS_TASK] [T]
			INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [CH] ON [T].[TASK_ID] = [CH].[TASK_ID]
			INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [CD] ON [CH].[PHYSICAL_COUNT_HEADER_ID] = [CD].[PHYSICAL_COUNT_HEADER_ID]
