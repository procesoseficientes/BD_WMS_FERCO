...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GENERATE_RECEPTION_DOCUMENT_FROM_CARGO_MANIFEST_BY_SALE]
					@MANIFEST_ID = 2210
					,@LOGIN = 'ACAMACHO'
				--
				EXEC [FERCO].[OP_WMS_SP_GENERATE_RECEPTION_DOCUMENT_FROM_CARGO_MANIFEST_BY_SALE]
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GENERATE_RECEPTION_DOCUMENT_FROM_CARGO_MANIFEST_BY_SALE](
	@MANIFEST_ID INT
	,@LOGIN VARCHAR(50)
)
AS
BEGIN
	...
	BEGIN TRY
		...
		SELECT TOP 1 @IN_PROGRESS = 1  FROM 
		 [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RD] 
		INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [TL] ON [TL].[SERIAL_NUMBER] = [RD].[TASK_ID]
		...
		FROM [FERCO].[OP_SETUP_COMPANY] [C]
		LEFT JOIN [FERCO].[OP_WMS_ACUERDOS_X_CLIENTE] [AC] ON ([AC].[CLIENT_ID] = [C].[COMPANY_CODE])
		...
		BEGIN
		    ...
		    FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [PD]
			INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH] ON [DH].[PICKING_DEMAND_HEADER_ID] = [PD].[PICKING_DEMAND_HEADER_ID]
		    INNER JOIN [FERCO].[OP_WMS_MANIFEST_DETAIL] [MD] ON (
		...
		INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [D] ON [H].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [D].[ERP_RECEPTION_DOCUMENT_HEADER_ID]
		INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [H].[TASK_ID] = [T].[SERIAL_NUMBER]
		LEFT JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [L].[CODIGO_POLIZA] = [T].[CODIGO_POLIZA_SOURCE]
		...
		BEGIN
			...
			BEGIN
				...
				INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] ON ([PDH].[PICKING_DEMAND_HEADER_ID] = [PD].[PICKING_DEMAND_HEADER_ID])
				WHERE [PD].[PICKING_DEMAND_HEADER_ID] > 0
				--
				...
				EXEC [FERCO].[OP_WMS_SP_INSERT_RECEPTION_GENERAL_POLICY] 
				...
				EXEC [FERCO].[OP_WMS_SP_INSERT_TASK_RECEPTION_ERP]
				...
				SELECT
					...
					,@MANIFEST_ID
					,@PLATE_NUMBER
				FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH]
				INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] ON ([W].[WAREHOUSE_ID] = [DH].[CODE_WAREHOUSE])
