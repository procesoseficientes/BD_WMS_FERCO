...
-- Ejemplo de Ejecucion:
			  SELECT * FROM [FERCO].[OP_WMS_INV_X_LICENSE] [OWIXL] INNER JOIN [FERCO].[OP_WMS_LICENSES] [OWL] ON [OWIXL].[LICENSE_ID] = [OWL].[LICENSE_ID] WHERE [OWIXL].[QTY] > 0  AND [OWL].[LICENSE_ID] = 45008
  SELECT * FROM [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [OWPCD] INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [OWPCH] ON [OWPCD].[PHYSICAL_COUNT_HEADER_ID] = [OWPCH].[PHYSICAL_COUNT_HEADER_ID]
  
  EXEC [FERCO].[OP_WMS_SP_INSERT_COUNT_EXECUTION] @LOGIN = 'ACAMACHO'
                                                    ,@TASK_ID = 9
  ...
  select  * from [FERCO].[OP_WMS_PHYSICAL_COUNTS_EXECUTION]
  select  * from [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER]
  select  * from [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL]
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_INSERT_COUNT_EXECUTION] (
		@LOGIN VARCHAR(25)
		,@TASK_ID INT
		,@LOCATION VARCHAR(25)
		,@LICENSE_ID INT
...
BEGIN
	...
	IF EXISTS ( SELECT TOP 1
					1
				FROM
					[FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [H]
				WHERE
					[H].[STATUS] = 'CANCELED'
	...
	IF NOT EXISTS ( SELECT TOP 1
						1
					FROM
						[FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [D]
					INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [H] ON [D].[PHYSICAL_COUNT_HEADER_ID] = [H].[PHYSICAL_COUNT_HEADER_ID]
					WHERE
						[D].[LOCATION] = @LOCATION
	...
	BEGIN
		...
		FROM
			[FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [D]
		INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [H] ON [H].[PHYSICAL_COUNT_HEADER_ID] = [D].[PHYSICAL_COUNT_HEADER_ID]
		WHERE
			[H].[TASK_ID] = @TASK_ID
		...
		FROM
			[FERCO].[OP_WMS_PHYSICAL_COUNTS_EXECUTION]
		WHERE
	...
	VALUES
			(
				...
				,(SELECT TOP 1
						[LOGIN_NAME]
					FROM
						[FERCO].[OP_WMS_FUNC_GETLOGIN_NAME](@LOGIN))
				,'CONTEO_FISICO'
				,'CONTEO UBICACION'
				,@BARCODE_ID
				,@MATERIAL_ID
				,@MATERIAL_DESCRIPTION
				,[FERCO].[OP_WMS_FN_GET_MATERIAL_COST_BY_MATERIAL](@MATERIAL_ID,
											@CLIENT_OWNER)
				,@LICENSE_ID
				,@LOCATION
				,@CLIENT_OWNER
	...
	DELETE
		[E]
	FROM
		[FERCO].[OP_WMS_PHYSICAL_COUNTS_EXECUTION] [E]
	INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [D] ON [D].[PHYSICAL_COUNT_DETAIL_ID] = [E].[PHYSICAL_COUNT_DETAIL_ID]
	INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [H] ON [H].[PHYSICAL_COUNT_HEADER_ID] = [D].[PHYSICAL_COUNT_HEADER_ID]
	WHERE
	...
	INSERT	INTO [FERCO].[OP_WMS_PHYSICAL_COUNTS_EXECUTION]
	SELECT TOP 1
	...
	FROM
		[FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [D]
	INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [H] ON [D].[PHYSICAL_COUNT_HEADER_ID] = [H].[PHYSICAL_COUNT_HEADER_ID]
	LEFT JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [L].[CURRENT_LOCATION] = [D].[LOCATION]
											AND [L].[LICENSE_ID] = @LICENSE_ID
	LEFT JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IL] ON (
	...
	LEFT JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [S] ON (
											[S].[STATUS] = 1
