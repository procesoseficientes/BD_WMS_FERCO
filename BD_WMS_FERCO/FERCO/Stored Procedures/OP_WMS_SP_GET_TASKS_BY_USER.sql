...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_TASKS_BY_USER] @LOGIN_ID='MARVIN'
					
   */
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TASKS_BY_USER]
(@LOGIN_ID VARCHAR(50))
AS
BEGIN
    ...
    SELECT [TaskList].[SERIAL_NUMBER] AS [id],
           CASE [TaskList].[TASK_SUBTYPE]
				WHEN 'RECEPCION_RT' THEN NULL
				ELSE [TaskList].[WAVE_PICKING_ID]
		   END AS [wavePickingId],
    ...
    FROM [FERCO].[OP_WMS_TASK_LIST] AS [TaskList]
		...
		LEFT OUTER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH]
			ON [TaskList].[WAVE_PICKING_ID] = [PDH].[WAVE_PICKING_ID]
