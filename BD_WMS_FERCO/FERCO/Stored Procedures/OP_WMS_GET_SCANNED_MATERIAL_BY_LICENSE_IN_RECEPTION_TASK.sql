...
-- Fecha de Creacion: 		2021-03-25 @ProcesosEficientes
-- Description:			    Se habilita la validacion de no permitir producto extra para traslados

/*
-- Ejemplo de Ejecucion:
               EXEC [FERCO].[OP_WMS_GET_SCANNED_BARCODE_FOR_LICENSE_IN_RECEPTION_TASK] @BARCODE_ID = 'autovanguard/VAA1001'
                                                                         ,@CLIENT_OWNER = 'FERCO_ALMACENADORA'
                                                                         ,@LICENSE_ID = 367930
                                                                         ,@TASK_ID = 476464
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_SCANNED_MATERIAL_BY_LICENSE_IN_RECEPTION_TASK] (
...
BEGIN
	...
	BEGIN
		...
		LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [UMM].[MATERIAL_ID] = [M].[MATERIAL_ID]
		WHERE
			(
	...
	BEGIN
		IF NOT EXISTS ( SELECT TOP 1
							1
						FROM
							[FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [RD]
						INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [M].[MATERIAL_ID] = [RD].[MATERIAL_ID]
						LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [UMM].[MATERIAL_ID] = [M].[MATERIAL_ID]
						WHERE
							(
								[M].[BARCODE_ID] = @BARCODE_ID
							...
							AND EXISTS ( SELECT
											1
											FROM
											[FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RH]
											WHERE
