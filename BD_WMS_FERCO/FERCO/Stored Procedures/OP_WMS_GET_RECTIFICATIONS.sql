...
/*
 --Ejemplo de ejecucion
      EXEC [FERCO].OP_WMS_GET_RECTIFICATIONS  
        @START_DATE = '20160701'
        ,@END_DATE = '20160715'
...
CREATE PROCEDURE FERCO.OP_WMS_GET_RECTIFICATIONS @START_DATE DATETIME
, @END_DATE DATETIME
AS
  SELECT DISTINCT
    RC.DOC_ID_RECTIFADA
   ...
   ,VC.CLIENT_CODE
   ,VC.CLIENT_NAME
  FROM [FERCO].OP_WMS_RECTIFICATION_HISTORY RC
  INNER JOIN [FERCO].OP_WMS_POLIZA_HEADER PH
    ON (RC.DOC_ID_RECTIFICACION = PH.DOC_ID)
  INNER JOIN [FERCO].OP_WMS_VIEW_CLIENTS VC
    ON (PH.CLIENT_CODE = VC.CLIENT_CODE)
  WHERE RC.DOC_ID_RECTIFICACION IS NOT NULL
