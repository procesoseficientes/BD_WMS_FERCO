...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_BLOCK_EXPIRED_POLIZAS]
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_BLOCK_EXPIRED_POLIZAS
AS
BEGIN
  ...
  --
  SELECT
    @BLOCKED_STATUS = [FERCO].[OP_WMS_FN_GET_PARAMETER_VALUE]('STATUS', 'FULL_BLOCK')
  --
  UPDATE [PH]
  SET [PH].[IS_BLOCKED] = 1
     ,[PH].[BLOCKED_STATUS] = @BLOCKED_STATUS
  FROM [FERCO].[OP_WMS_POLIZA_HEADER] [PH]
  INNER JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [C]
    ON ([C].[PARAM_GROUP] = @PARAM_GROUP
    AND [C].[PARAM_TYPE] = @PARAM_TYPE
    AND [PH].[REGIMEN] = [C].[PARAM_CAPTION])
  WHERE [PH].[WAREHOUSE_REGIMEN] = 'FISCAL'
  AND DATEDIFF(DAY, GETDATE(), [FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([CODIGO_POLIZA])) < 1
  AND [C].[SPARE2] = 'BLOQUEA_INVENTARIO_VENCIDO'
  --
  UPDATE [IXL]
  SET [IXL].[IS_BLOCKED] = 1
     ,[IXL].[BLOCKED_STATUS] = @BLOCKED_STATUS
  ...
  INNER JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [C]
    ...
    AND [PH].[REGIMEN] = [C].[PARAM_CAPTION])
  WHERE [PH].[WAREHOUSE_REGIMEN] = 'FISCAL'
  AND DATEDIFF(DAY, GETDATE(), [FERCO].[OP_WMS_FN_GET_EXPIRATION_DATE_FOR_POLIZA]([PH].[CODIGO_POLIZA])) < 1
  AND [C].[SPARE2] = 'BLOQUEA_INVENTARIO_VENCIDO'
END
