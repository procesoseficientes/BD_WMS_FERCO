...
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_DETAIL_BY_DOC_FOR_CERTIFICATE_ALMGEN] @DOC_ID INT
AS
BEGIN
  SET NOCOUNT ON;
  SELECT
   ...
   ,[PH].[DOC_ID]
   ,CASE
      WHEN [CH].[STATUS] = 'ANULAR' THEN 'DISPONIBLE'
      WHEN [CD].[CERTIFICATE_DEPOSIT_ID_DETAIL] IS NULL THEN 'DISPONIBLE'
      ELSE 'ASOCIADO'
    END AS [STATUS]
  ...
  LEFT JOIN [FERCO].[OP_WMS_CERTIFICATE_DEPOSIT_DETAIL] [CD]
    ON (
    [PH].[DOC_ID] = [CD].[DOC_ID]
    AND [T].[MATERIAL_CODE] = [CD].[MATERIAL_CODE]
    )
  LEFT JOIN [FERCO].[OP_WMS_CERTIFICATE_DEPOSIT_HEADER] [CH]
    ON ([CD].[CERTIFICATE_DEPOSIT_ID_HEADER] = [CH].[CERTIFICATE_DEPOSIT_ID_HEADER])
  WHERE [PH].[WAREHOUSE_REGIMEN] = 'GENERAL'
  AND [PH].[TIPO] = 'INGRESO'
  AND [T].[STATUS] = 'PROCESSED'
  AND [T].[TRANS_TYPE] = 'INGRESO_GENERAL'
  AND [PH].[DOC_ID] = @DOC_ID
  GROUP BY [T].[MATERIAL_DESCRIPTION]
          ,[T].[TARGET_WAREHOUSE]
          ,[PH].[DOC_ID]
          ,[T].[MATERIAL_CODE]
