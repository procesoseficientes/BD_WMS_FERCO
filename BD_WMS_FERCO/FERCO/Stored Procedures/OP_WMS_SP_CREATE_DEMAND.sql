...
-- Ejemplo de Ejecucion:
        EXEC [FERCO].[OP_WMS_SP_CREATE_DEMAND]
			@LOGIN = 'BETO'
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_CREATE_DEMAND] (
		@LOGIN VARCHAR(50)
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
	...
	SELECT
		...
		,[x].[Rec].[query]('./MIN_DAYS_EXPIRATION_DATE').[value]('.',
											'int')
		,(SELECT SERIES FROM [SAPSERVER].[SBOFERCO].[dbo].[ORDR] WHERE DocEntry = [x].[Rec].[query]('./DOC_ENTRY').[value]('.', 'VARCHAR(50)'))
	FROM
		@DEMAND.[nodes]('/ArrayOfOrdenDeVentaEncabezado/OrdenDeVentaEncabezado')
		AS [x] ([Rec]);

	IF NOT EXISTS (SELECT TOP 1 1 FROM @HEADER H
		LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [P] ON (
										H.DOC_ENTRY COLLATE DATABASE_DEFAULT = [P].[DOC_ENTRY]
										AND H.EXTERNAL_SOURCE_ID = [P].[EXTERNAL_SOURCE_ID]
										AND [P].[OWNER] = H.OWNER COLLATE DATABASE_DEFAULT
										)
	GROUP BY
		[H].[DOC_ENTRY]
		,[H].[CLIENT_ID]
	...
	BEGIN 
		RAISERROR ('Error: Documento ya fue operado en WMS.', 16, 1);
		RETURN
  ...
  -- ------------------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------------------
		...
		FROM
			@HEADER [H]
		INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PH] ON (
    ...
    -- ------------------------------------------------------------------------------------
		INSERT	INTO [FERCO].[OP_WMS_POLIZA_HEADER]
				(
					[FECHA_LLEGADA]
					,[LAST_UPDATED_BY]
					,[LAST_UPDATED]
  ...
  -- ------------------------------------------------------------------------------------
	BEGIN
      ...
      -- ------------------------------------------------------------------------------------
			...
			LEFT JOIN [FERCO].[OP_WMS_COMPANY] [C] ON [C].[CLIENT_CODE] = [D].[MATERIAL_OWNER]
			WHERE
				[H].[HEADER_ID] > 0
				AND [D].[DETAIL_ID] > 0
				AND [D].[USED] = 0
      ...
      -- ------------------------------------------------------------------------------------
			...
			BEGIN
        --
				INSERT	INTO [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER]
						(
							[REQUEST_TYPE]
							,[WAREHOUSE_FROM]
							,[WAREHOUSE_TO]
        ...
        --
				...
				INSERT	INTO [FERCO].[OP_WMS_TRANSFER_REQUEST_DETAIL]
						(
							[TRANSFER_REQUEST_ID]
							,[MATERIAL_ID]
							,[MATERIAL_NAME]
				...
				FROM
					@HEADER [H]
				INNER JOIN @DETAIL [D] ON ([D].[HEADER_ID] = [H].[HEADER_ID])
				INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([D].[MATERIAL_OWNER]
											+ '/'
      ...
      -- ------------------------------------------------------------------------------------
      -- NON_STORAGE SE UTILIZA PARA EL MATERIAL DE FERCO DE FLETE, HABILITARLO DE SER NECESARIO
      -- ------------------------------------------------------------------------------------
			...
			BEGIN
				...
				INSERT	INTO @OPERACION
						...
						EXEC [FERCO].[OP_WMS_SP_INSERT_TASKS_GENERAL_PICKING_DEMAND] @TASK_OWNER = @LOGIN, -- varchar(25)
      ...
      -- ------------------------------------------------------------------------------------
			SELECT
				...
				,@Mensaje = [O].[Mensaje]
				,@WAVE_PICKING_ID = CAST([FERCO].[OP_WMS_FN_SPLIT_COLUMNS]([O].[DbData],
											1, '|') AS INT)
				,@ASSEMBLED_QTY = ISNULL(CAST([FERCO].[OP_WMS_FN_SPLIT_COLUMNS]([O].[DbData],
											2, '|') AS INT),
											0)
			FROM
				@OPERACION [O];
  ...
  -- ------------------------------------------------------------------------------------
	BEGIN
		INSERT	INTO [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER]
				(
					[DOC_NUM]
					,[CLIENT_CODE]
					,[CODE_ROUTE]
    ...
    -- ------------------------------------------------------------------------------------
		INSERT	INTO [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL]
				(
  ...
  --
    ...
    -- ------------------------------------------------------------------------------------
		INSERT	INTO @OPERACION
				...
				EXEC [FERCO].[OP_WMS_SP_INSERT_PICKING_LINE_TASK] @WAVE_PICKING_ID = @WAVE_PICKING_ID, -- int
					@IS_CONSOLIDATED = @IS_CONSOLIDATED, -- int
    ...
    -- ------------------------------------------------------------------------------------
		SELECT
			@Resultado = [O].[Resultado]
			,@Mensaje = [O].[Mensaje]
			,@LINE_DOC = [FERCO].[OP_WMS_FN_SPLIT_COLUMNS]([O].[DbData],
											1, '|')
			,@BOX_QTY = ISNULL(CAST([FERCO].[OP_WMS_FN_SPLIT_COLUMNS]([O].[DbData],
											2, '|') AS INT),
								0)
		FROM
			@OPERACION [O];
