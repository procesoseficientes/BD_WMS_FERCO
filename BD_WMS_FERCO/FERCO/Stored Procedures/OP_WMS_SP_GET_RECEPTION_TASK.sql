...
/*
	Ejemplo Ejecucion: 
		EXEC	[FERCO].[OP_WMS_SP_GET_RECEPTION_TASK]
			@REGIMEN = 'GENERAL',
				@TASK_ASSIGNEDTO = 'ACAMACHO'
 */
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_RECEPTION_TASK]
	@REGIMEN VARCHAR(50)
...
BEGIN
	...
	INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PH] ON [PH].[DOC_ID] = [TL].[DOC_ID_SOURCE]
	LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [R] ON [R].[TASK_ID] = [TL].[SERIAL_NUMBER]
	LEFT JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [owc] ON (
