...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_VALIDATE_LOCATION_MAX_WEIGTH] @LOCATION_SPOT = 'B01-R01-C01-NC'
                                                          , @LICENCE_ID = 257730
                                                          

*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_VALIDATE_LOCATION_MAX_WEIGTH (@LOCATION_SPOT VARCHAR(25)
, @LICENCE_ID NUMERIC)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  FROM [FERCO].[OP_WMS_VW_GET_LOCATIONS_WITH_WEIGHT]
  WHERE [LOCATION_SPOT] = @LOCATION_SPOT

  SELECT
    [FERCO].[OP_WMS_FN_CONVERT_FROM_WEIGHT_MEASURE_UNIT_TO_DEFAULT](SUM([IL].[QTY]), MAX([M].[WEIGHT_MEASUREMENT])) PESO_MATERIAL INTO #PESOS_LICENCIA
