...
-- Ejemplo de Ejecucion:
  SELECT * FROM [FERCO].[OP_WMS_INV_X_LICENSE] [OWIXL] WHERE [OWIXL].LICENSE_ID = 22717
   DECLARE @RES INT
   EXEC  [FERCO].OP_WMS_SP_VALIDATE_MASTER_PACK_MATERIAL_IS_IN_LICENCE @LICENCE_ID = 22717, @MATERIAL_ID = 'C00030/LECHAUST' , @RESULT = @RES OUTPUT
   SELECT @RES  
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_VALIDATE_MASTER_PACK_MATERIAL_IS_IN_LICENCE (@LICENCE_ID NUMERIC, @MATERIAL_ID VARCHAR(50), @RESULT INT OUTPUT)
AS
BEGIN
  ---------------------------------------------------------------------------------
  -- VALIDAR PADRES
  ...
  FROM [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [C]
  ...
  WHILE @COUNT_PADRES <> (SELECT
      @COUNT_PADRES + COUNT(*)
    FROM [#PADRES] [P]
    INNER JOIN [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [C]
