...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_MATERIAL_PRINT_FORMAT]
					@MATERIAL_ID = 'viscosa/VDE1001'
					,@LOGIN = 'ACAMACHO'
					,@BARCODE_ID = 'viscosa/VDE1001C'<
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_MATERIAL_PRINT_FORMAT] (
...
BEGIN
	...
	BEGIN                        
		...
		LEFT JOIN [FERCO].[OP_WMS_COMPANY] [C] ON [C].[CLIENT_CODE] = [M].[CLIENT_OWNER]
		WHERE
			(
	...
	BEGIN
		...
		FROM
			[FERCO].[OP_WMS_MATERIALS] [M]
		INNER JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [UMM].[MATERIAL_ID] = [M].[MATERIAL_ID]
		INNER JOIN [FERCO].[OP_WMS_COMPANY] [C] ON [C].[CLIENT_CODE] = [M].[CLIENT_OWNER]
		WHERE
...
LINE 0 50 570 50 2'
			+ CASE	WHEN LEN(@MATERIAL_NAME) <= 35 THEN '
