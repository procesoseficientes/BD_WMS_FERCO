...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_REMOVE_QTY_IN_PICKING_DEMAND_DOCUMENT]
...
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_REMOVE_QTY_IN_PICKING_DEMAND_DOCUMENT (@WAVE_PICKING_ID NUMERIC(18, 0)
...
BEGIN
  ...
  --
  DECLARE @PICKING_DEMAND_HEADER_ID INT = 0
  ...
  BEGIN TRY
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO @DOCUMENT ([PICKING_DEMAND_DETAIL_ID]
    , [PRIORITY]
    , [QTY])
      SELECT
      ...
      INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [PD]
        ON ([PD].[PICKING_DEMAND_HEADER_ID] = [PH].[PICKING_DEMAND_HEADER_ID])
    ...
    INNER JOIN @DOCUMENT [D]
      ON ([D].[PICKING_DEMAND_DETAIL_ID] = [PD].[PICKING_DEMAND_DETAIL_ID])
