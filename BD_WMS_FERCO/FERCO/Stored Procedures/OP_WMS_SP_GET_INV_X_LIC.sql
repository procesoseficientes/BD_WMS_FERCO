...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_INV_X_LIC]
					@LICENSE_ID = 44570
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_INV_X_LIC] (
		@LICENSE_ID INT
...
BEGIN
	...
	LEFT JOIN [FERCO].[OP_WMS_SHELF_SPOTS] [S] ON [S].[LOCATION_SPOT] = [B].[CURRENT_LOCATION]
	LEFT JOIN [FERCO].[OP_WMS_WAREHOUSE_BY_USER] [W] ON [W].[WAREHOUSE_ID] = [S].[WAREHOUSE_PARENT]
											AND @LOGIN = [W].[LOGIN_ID]
	LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([M].[MATERIAL_ID] = [A].[MATERIAL_ID])
	WHERE
