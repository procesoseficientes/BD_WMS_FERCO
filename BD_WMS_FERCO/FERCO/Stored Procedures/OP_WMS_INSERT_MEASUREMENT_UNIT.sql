...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_INSERT_MEASUREMENT_UNIT]
					@CLIENT_ID = 'C00015'
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_INSERT_MEASUREMENT_UNIT] (
...
BEGIN
	BEGIN TRY
		...
		IF EXISTS ( SELECT TOP 1
						1
					FROM
						[FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM]
					INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [UMM].[MATERIAL_ID] = [M].[MATERIAL_ID]
					WHERE
						(
							[UMM].[BARCODE] = @BARCODE
		...
		------------------------------------------------------------------
		INSERT	INTO [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL]
				(
					[CLIENT_ID]
					,[MATERIAL_ID]
					,[MEASUREMENT_UNIT]
