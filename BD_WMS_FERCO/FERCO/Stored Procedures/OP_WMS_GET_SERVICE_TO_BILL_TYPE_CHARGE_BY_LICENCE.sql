...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_GET_SERVICE_TO_BILL_TYPE_CHARGE]  @INICIAL_DATE = '2016-11-02 00:00:00.000'
    , @END_DATE = '2016-11-02 23:59:59.850', @LAST_UPDATED_BY = 'PAGUILAR'
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_GET_SERVICE_TO_BILL_TYPE_CHARGE_BY_LICENCE (@INICIAL_DATE DATETIME
, @END_DATE DATETIME
...
BEGIN
  ...
  INNER JOIN [FERCO].[OP_WMS_VIEW_SERVICES] [S]
    ON [S].[SERVICE_CODE] = [TC].[SERVICE_CODE]
  WHERE [TC].[SERVICE_CODE] NOT IN (SELECT
      [SV].[SERVICE_CODE]
    FROM [FERCO].[OP_WMS_SERVICE] [SV])
  ...
  SELECT
   ...
   ,[C].[ACUERDO_COMERCIAL]
  FROM [FERCO].[OP_WMS_TYPE_CHARGE_X_LICENSE] TCL
  INNER JOIN [FERCO].[OP_WMS_LICENSES] [L]
    ON (
      [TCL].[LICENCESE_ID] = [L].[LICENSE_ID]
      )
  INNER JOIN [#CLIENT_TO_BILL] [C]
    ON (
      [L].[CLIENT_OWNER] = [C].[CLIENT_CODE]
      )
  ...
  AND ([TCL].[LAST_UPDATED] BETWEEN @INICIAL_DATE AND @END_DATE)
