...
-- Gustavo.Garcia 06-10-2021
-- Se agrega validación en el outter para que solo tome los registros de SAP que sean fletes cuando no existan en WMS, de lo contrario hace la entrega completa
...
-- Ejemplo de Ejecucion:
			...
			EXEC [FERCO].[OP_WMS_SP_GET_NEXT_PICKING_DEMAND_DETAIL] 
				@PICKING_DEMAND_HEADER_ID = 199946
				,@OWNER = 'ferco'
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_NEXT_PICKING_DEMAND_DETAIL] (@PICKING_DEMAND_HEADER_ID INT
, @OWNER VARCHAR(50))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  EXEC [FERCO].OP_WMS_SP_GET_MATERIALS_WITH_BATCH_FROM_DEMAND_DISPATCH @PICKING_DEMAND_HEADER_ID = ' + CAST(@PICKING_DEMAND_HEADER_ID AS VARCHAR) + '
	...
	FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] WITH (NOLOCK)
		left outer JOIN SAPSERVER.SBOFERCO.DBO.RDR1 RDR1 WITH (NOLOCK) ON  ( '''+@OWNER+'''=''ferco'' and rdr1.DocEntry=[PDH].DOC_ENTRY  COLLATE DATABASE_DEFAULT)
		left outer JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [PDD] WITH (NOLOCK) ON [PDD].[PICKING_DEMAND_HEADER_ID] = [PDH].[PICKING_DEMAND_HEADER_ID]  and ('''+@OWNER+'''+''/''+rdr1.itemCode=[PDD].material_id collate database_default or '''+@OWNER+'''<> ''ferco'' ) 
		left JOIN [FERCO].[OP_WMS_MATERIALS] [M] WITH (NOLOCK) ON ([M].[MATERIAL_ID] = [PDD].[MATERIAL_ID])
		--INNER JOIN [FERCO].[OP_WMS_MATERIAL_INTERCOMPANY] [MI] ON ([MI].[SOURCE] = ''' + @OWNER + ''' AND [PDD].[MASTER_ID_MATERIAL] = [MI].[MASTER_ID])
		LEFT JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] WITH (NOLOCK) ON [PDH].[CODE_WAREHOUSE] = [W].[WAREHOUSE_ID]
		LEFT JOIN @MATERIALS_WITH_BATCH [MWB] ON  ([MWB].[MATERIAL_ID] = [PDD].[MATERIAL_ID])
		LEFT JOIN [FERCO].[OP_WMS_MATERIALS_WITH_CATEGORY]  [MWC] WITH (NOLOCK) ON ([MWC].ITEM_CODE = [M].[MATERIAL_ID] )
		--LEFT JOIN SWIFT_INTERFACES_SV.ferco.ERP_VIEW_SALES_ORDER_DETAIL_CHANNEL_MODERN ODCM ON (ODCM.docentry = pdh.doc_num and ODCM.U_MasterIdSKU = pdd.MASTER_ID_MATERIAL COLLATE DATABASE_DEFAULT) 
	WHERE
		(PDD.QTY > 0 OR RDR1.Dscription like ''%Flete%'') AND 
