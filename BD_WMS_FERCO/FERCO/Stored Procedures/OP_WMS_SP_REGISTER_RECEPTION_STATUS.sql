...
                                                        ,@pTASK_ID = 295037
                                                        ,@pSTATUS = 'COMPLETED'    
SELECT * FROM [FERCO].[OP_WMS_TRANS] [owt] ORDER BY [owt].[SERIAL_NUMBER] DESC 
SELECT * FROM [FERCO].OP_WMS_TASK_LIST WHERE SERIAL_NUMBER = '234882'
                                                    
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_RECEPTION_STATUS] (
...
                            
				...
				SELECT
					...
					,(SELECT TOP 1
							[LOGIN_NAME]
						FROM
							[FERCO].[OP_WMS_FUNC_GETLOGIN_NAME](@pLOGIN_ID)) AS [LOGIN_NAME]
...
                                
					...
					INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PH] ON ([PH].[CODIGO_POLIZA] = [L].[CODIGO_POLIZA])
					LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [H] ON [H].[DOC_ID_POLIZA] = [PH].[DOC_ID]
					LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [D] ON (
...
                        -- ------------------------------------------------------------------------------------
					...
					SET	
						...
						,[TD].[STATUS] = CASE
											...
											END
					FROM
						[FERCO].[OP_WMS_TRANSFER_REQUEST_DETAIL] [TD]
...
                        --
					IF NOT EXISTS ( SELECT TOP 1
										1
									FROM
										[FERCO].[OP_WMS_TRANSFER_REQUEST_DETAIL]
									WHERE
										[TRANSFER_REQUEST_ID] = @TRANSFER_REQUEST_ID
										AND [STATUS] = @TRANSFER_REQUEST_OPEN_STATUS )
					BEGIN
						UPDATE
							[FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER]
						SET	
