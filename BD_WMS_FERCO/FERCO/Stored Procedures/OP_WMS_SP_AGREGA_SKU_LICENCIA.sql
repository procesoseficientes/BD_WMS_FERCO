...
-- Descripcion:			Se habilita la validacion de no permitir producto extra del mismo material en una orden de recepcion para traslados

-- Autor:				Elder Lucas
-- Fecha de Creacion: 	10 de septiembre 2021 @ProcesosEficientes
-- Descripcion:			Se agrega el manejo de recepciones de tipo RECTIFICATION_TASK
...
DECLARE
	...
	,@pResult VARCHAR(500);
EXEC [FERCO].[OP_WMS_SP_AGREGA_SKU_LICENCIA] @pLICENSE_ID = NULL , -- numeric(18, 0)
	@pBARCODE = '' , -- varchar(25)
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_AGREGA_SKU_LICENCIA]
(
	...
	@TASK_ID_RECTIFICATION NUMERIC(20,0) = 0,
...
BEGIN
    ...
    DECLARE @ErrorMessage NVARCHAR(4000),
			...
			@PK_LINE_DISPATCH_LICENSE INT = 0,
			@ACTUAL_DISPACH_LICENSE_QTY NUMERIC(18, 4) = 0,
			@IS_RECTIFICATION NUMERIC = 0,
			@DISPATCH_LICENSE_SOURCE NUMERIC(18,0);
    --,@ERROR_CODE INT = -1;
    ...
    BEGIN TRY
        BEGIN
            BEGIN TRAN;
			--SE VALIDA QUE LA RECEPCIÓN NO SEA DE RECTIFICACIÓN Y SI NO LO ES ENTONCES BORRAMOS LA LICENCIA DE ORIGEN QUE NO ES UTIL EN EL RESTO DE RECEPCIONES
			SET @DISPATCH_LICENSE_SOURCE = @SOURCE_LICENSE_ID
			SET @SOURCE_LICENSE_ID = 0
			IF EXISTS(SELECT TOP 1 1 FROM FERCO.OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER WHERE TASK_ID = @TASK_ID_RECTIFICATION AND TYPE = 'RECEPCION_RT')
			BEGIN
				SELECT
					@ACTUAL_DISPACH_LICENSE_QTY = IXL.QTY,
					@PK_LINE_DISPATCH_LICENSE = IXL.PK_LINE,
					@IS_RECTIFICATION = 1
				FROM FERCO.OP_WMS_INV_X_LICENSE IXL 
				 INNER JOIN FERCO.OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL RDD 
					ON IXL.LICENSE_ID = RDD.LICENSE_ID_SOURCE
				 INNER JOIN FERCO.OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER RDH
					ON RDH.ERP_RECEPTION_DOCUMENT_HEADER_ID = RDH.ERP_RECEPTION_DOCUMENT_HEADER_ID
				 INNER JOIN FERCO.OP_WMS_MATERIALS M ON M.MATERIAL_ID = RDD.MATERIAL_ID
				 WHERE RDH.TASK_ID = @TASK_ID_RECTIFICATION
					   AND RDD.LICENSE_ID_SOURCE = @DISPATCH_LICENSE_SOURCE
					   AND (M.BARCODE_ID = @pBARCODE OR M.ALTERNATE_BARCODE = @pBARCODE)
					   AND IXL.MATERIAL_ID = M.MATERIAL_ID
			END
			print @DISPATCH_LICENSE_SOURCE
			print @TASK_ID_RECTIFICATION
			print @ACTUAL_DISPACH_LICENSE_QTY
			PRINT @PK_LINE_DISPATCH_LICENSE
			PRINT @IS_RECTIFICATION
			...
			DECLARE @SOURCE_RECTIFICATION VARCHAR(20) = 'RECTIFICATION_TASK';
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    -- ------------------------------------------------------------------------------------
					print @QTY_TOTAL_IN_TRANS
					print  @MAX_QTY
					print @ALLOWS_TO_STORE_MORE_QUANTITY
            ...
            (
                ...
                FROM [FERCO].[OP_WMS_LICENSES] [L]
                    INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PH]
                        ON [PH].[CODIGO_POLIZA] = [L].[CODIGO_POLIZA]
                    INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RH]
                ...
                WHERE [L].[LICENSE_ID] = @pLICENSE_ID
                      ...
                      (
						  ...
						  OR [RH].[SOURCE] = @SOURCE_RECTIFICATION
                      )
            ...
            BEGIN
                ...
                FROM [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RH]
                    INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [RD]
                        ON [RD].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [RH].[ERP_RECEPTION_DOCUMENT_HEADER_ID]
                    LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM]
            ...
            SELECT [CLASS_ID],
                   ...
                   [LAST_UPDATED],
                   [PRIORITY]
            FROM [FERCO].[OP_WMS_FN_GET_CLASSES_BY_LICENSE](@pLICENSE_ID);
            --
            INSERT INTO @COMPATIBLE_CLASSES
            SELECT [CLASS_ID]
            FROM [FERCO].[OP_WMS_CLASS];
            --
            ...
            BEGIN
                ...
                FROM @COMPATIBLE_CLASSES [CC]
                    LEFT JOIN [FERCO].[OP_WMS_CLASS_ASSOCIATION] [CA]
            ...
            BEGIN
                ...
                EXEC [FERCO].[OP_WMS_SP_ADD_TONE_AND_CALIBER_BY_MATERIAL] @MATERIAL_ID = @pMATERIAL_ID,
                                                                            @TONE = @TONE,
                                                                            @CALIBER = @CALIBER;
			...
			-- ----------------------------------------------------------------------------------
            -- Se valida si es tarea de rectificación RECEPCION_RT y se actualiza en consecuencia
            -- ----------------------------------------------------------------------------------

			IF (@IS_RECTIFICATION = 1)
			DECLARE @ACTUAL_RECEPTION_LICENSE_QTY NUMERIC(18,0) = (SELECT QTY FROM FERCO.OP_WMS_INV_X_LICENSE WHERE LICENSE_ID = @pLICENSE_ID AND [MATERIAL_ID] = @pMATERIAL_ID)
			BEGIN
			PRINT 'SE TRATA DE ACTUALIZAR LA LICENCIA DE DESPACHO'
				UPDATE FERCO.OP_WMS_INV_X_LICENSE SET
					QTY = CASE @ACTION
					WHEN 'UPDATE' THEN @ACTUAL_DISPACH_LICENSE_QTY - @pQTY + @ACTUAL_RECEPTION_LICENSE_QTY
					ELSE @ACTUAL_DISPACH_LICENSE_QTY - @pQTY
					END
				WHERE PK_LINE = @PK_LINE_DISPATCH_LICENSE
			END
            ...
            BEGIN
                ...
                SET [STATUS_CODE] = @STATUS_CODE,
                    ...
                    [COLOR] = @COLOR
                FROM [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [S]
                    INNER JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IXL]
            ...
            DECLARE @RES INT;
            EXEC [FERCO].[OP_WMS_SP_VALIDATE_MASTER_PACK_MATERIAL_IS_IN_LICENCE] @LICENCE_ID = @pLICENSE_ID,
            ...
            (
                SELECT CASE
                           ...
                           ELSE
                               [M].[IS_CAR]
                       END AS [IS_CAR]
                FROM [FERCO].[OP_WMS_MATERIALS] AS [M]
                WHERE (
                          [BARCODE_ID] = @pBARCODE
                          OR [ALTERNATE_BARCODE] = @pBARCODE
                      )
            ...
            BEGIN
                ...
                (
                    SELECT TOP 1
                        ISNULL([AUDIT_ID], 0)
                    FROM [FERCO].[OP_WMS_AUDIT_RECEPTION_CONTROL] AS [AUDIT_ID]
                    WHERE [CODIGO_POLIZA] =
                (
            ...
            (
                SELECT 1
                FROM [FERCO].[OP_WMS_INV_X_LICENSE] [IXL]
                    INNER JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [S]
            ...
            BEGIN
                ...
                EXEC [FERCO].[OP_WMS_SP_ADD_STATUS_OF_MATERIAL_BY_LICENSE] @STATUS_CODE = @STATUS_CODE,
                                                                             @STATUS_NAME = @STATUS_NAME,
                                                                             @BLOCKS_INVENTORY = @BLOCKS_INVENTORY,
                                                                             @ALLOW_REALLOC = @ALLOW_REALLOC,
                                                                             @TARGET_LOCATION = @TARGET_LOCATION,
            ...
            SELECT @LOCKED_BY_INTERFACES = 1,
					@BLOCKED_BY = CONCAT('Tarea de recepción: ', rdh.ERP_RECEPTION_DOCUMENT_HEADER_ID)
            FROM [FERCO].[OP_WMS_LICENSES] [L]
            ...
            BEGIN
                SELECT @pQTY = COUNT([MSN].[LICENSE_ID]),
                       ...
                       @BATCH = MAX([MSN].[BATCH])
                FROM [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MSN]
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    BEGIN
                        UPDATE [FERCO].[OP_WMS_INVENTORY_RESERVED_BY_PROJECT]
            ...
            BEGIN
                ...
                (   @pLICENSE_ID, @pMATERIAL_ID, @pQTY, @pLAST_LOGIN, CURRENT_TIMESTAMP,
                    (
                        SELECT *
                        FROM [FERCO].[OP_WMS_FUNC_GETMATERIAL_DESC](   @pBARCODE,
                             (
                                 SELECT [CLIENT_OWNER]
                                 FROM [FERCO].[OP_WMS_LICENSES]
                                 WHERE [LICENSE_ID] = @pLICENSE_ID
                    ...
                    @ENTERED_MEASUREMENT_UNIT_CONVERSION_FACTOR, @CODE_SUPPLIER, @NAME_SUPPLIER, @IDLE, @PROJECT_ID, @PK_LINE_DISPATCH_LICENSE, @BLOCKED_BY);
                ...
                BEGIN
                    ...
                    FROM [FERCO].[OP_WMS_INVENTORY_RESERVED_BY_PROJECT]
                    WHERE [PROJECT_ID] = @PROJECT_ID
                    ...
                    INSERT INTO [FERCO].[OP_WMS_INVENTORY_RESERVED_BY_PROJECT]
                    (
                        [PROJECT_ID],
                        [PK_LINE],
                        [LICENSE_ID],
                    ...
                    INSERT INTO [FERCO].[OP_WMS_LOG_INVENTORY_RESERVED_BY_PROJECT]
                    (
                        [TYPE_LOG],
                        [PROJECT_ID],
                        [PK_LINE],
