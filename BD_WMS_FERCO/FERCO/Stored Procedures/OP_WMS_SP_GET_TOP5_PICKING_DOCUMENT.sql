...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_GET_TOP5_PICKING_DOCUMENT] @OWNER = 'ferco', @IS_INVOICE = 0
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TOP5_PICKING_DOCUMENT]
    (
     @OWNER VARCHAR(50)
    ,@IS_INVOICE INT = 0
    )
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
    ...
    SELECT
        @SENT_STATUS = [FERCO].[OP_WMS_FN_GET_PARAMETER_VALUE]('PICKING_DETAIL_STATUS',
                                                             'SENT')
       ,@ERROR_STATUS = [FERCO].[OP_WMS_FN_GET_PARAMETER_VALUE]('PICKING_DETAIL_STATUS',
                                                              'ERROR');
  ...
  --
    SELECT
       ...
       ,CASE WHEN [ISC].[VALUE] = @OWNER THEN 1
             ELSE 0
        END [INTERNAL_SALE_INTERFACE]
       ,CASE WHEN [ISC].[VALUE] IS NOT NULL
                  AND [ISC].[VALUE] != @OWNER THEN 1
             ELSE 0
    ...
    FROM
        [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] WITH (NOLOCK)
    ...
    WHERE
	   ...
	   AND [PDH].[CREATED_DATE] >= GETDATE() - 250
    ORDER BY
        [PDH].[PICKING_DEMAND_HEADER_ID] DESC;
		Print 'OWNER ' + @OWNER
		PRINT 'INTERNAL ' 
		PRINT @INTERNAL_SALE_COMPANIES
  -- ------------------------------------------------------------------------------------
  ...
  -- ------------------------------------------------------------------------------------
    SELECT
		...
		FROM
			[FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [D] WITH (NOLOCK)
  ...
  -- ------------------------------------------------------------------------------------
    ...
    SELECT
        @SALE_INVOICE_STATUS = [FERCO].[OP_WMS_FN_GET_PARAMETER_VALUE]('INNER_SALE_STATUS',
                                                              'SALE_INVOICE');
  ...
  -- ------------------------------------------------------------------------------------
    ...
    LEFT JOIN [FERCO].[OP_WMS_CUSTOMER_INTERCOMPANY] [CI] WITH (NOLOCK) ON [CI].[MASTER_ID] = [PDH].[CLIENT_CODE]
                                                           AND [CI].[SOURCE] = @OWNER
    LEFT JOIN [FERCO].[OP_WMS_SELLER_INTERCOMPANY] [SI] WITH (NOLOCK) ON [SI].[MASTER_ID] = [PDH].[MASTER_ID_SELLER]
                                                         AND [SI].[SOURCE] = @OWNER
    LEFT JOIN [FERCO].[OP_WMS_SELLER_INTERCOMPANY] [SI2] WITH (NOLOCK) ON [SI2].[SLP_CODE] = [PDH].[CODE_SELLER]
                                                          AND [SI2].[SOURCE] = @OWNER
    INNER JOIN [#PERFORMS_INTERNAL_SALE] [PIS] ON [PIS].[PICKING_DEMAND_HEADER_ID] = [PDH].[PICKING_DEMAND_HEADER_ID]
    INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] WITH (NOLOCK) ON [W].[WAREHOUSE_ID] = [PDH].[CODE_WAREHOUSE]
    WHERE
        (
         [PIS].[INTERNAL_SALE_COMPANY] = @OWNER
		...
		AND PDH.SOURCE_TYPE <>  'WT - ERP'
		ORDER BY PDH.PICKING_DEMAND_HEADER_ID DESC
