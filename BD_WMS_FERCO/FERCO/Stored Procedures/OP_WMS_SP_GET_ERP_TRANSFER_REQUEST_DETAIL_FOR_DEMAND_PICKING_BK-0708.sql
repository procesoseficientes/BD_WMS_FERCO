...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_ERP_TRANSFER_REQUEST_DETAIL_FOR_DEMAND_PICKING_BK-0708] (@XML XML)
AS
BEGIN
    SET NOCOUNT ON;
	--
    ...
    CREATE TABLE [#PICKING_DOCUMENT]
        ...
        ,[DATA_BASE_NAME] VARCHAR(50)
        ,[SCHEMA_NAME] VARCHAR(50)
        ,PRIMARY KEY ([EXTERNAL_SOURCE_ID], [PICKING_DOCUMENT_ID], [OWNER])
        );
	--
    ...
    BEGIN TRY
        ...
        SELECT
            [x].[Rec].[query]('./ExternalSourceId').[value]('.', 'int')
           ,[x].[Rec].[query]('./DocumentId').[value]('.', 'NUMERIC(18,0)')
