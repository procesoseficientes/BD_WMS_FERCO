...
-- Modificacion 10/11/2017 @ NEXUS-Team Sprint eFERCO
					-- rodrigo.gomez
...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_ERP_RECEPTION_DOCUMENT_DETAIL] 
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_ERP_RECEPTION_DOCUMENT_DETAIL] (
		@DOC_ID VARCHAR(MAX)
...
BEGIN
	...
	BEGIN TRY
		...
		INTO
			[#RECEPTION_DOCUMENTS]
		FROM
			[FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RDH]
		INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [RDD] ON [RDD].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [RDH].[ERP_RECEPTION_DOCUMENT_HEADER_ID]
		INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](@DOC_ID,
											@DELIMITER) [DOCS] ON [DOCS].[VALUE] = [RDH].[DOC_ID]
		WHERE
			[RDD].[IS_CONFIRMED] = 1
		...
		USING
			(SELECT
					...
					,MAX([RDD].[QTY]) [QTY]
				FROM
					[FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RH] --ON [RH].[DOC_ID] = [RP].[DOC_ID] AND [RH].[OWNER] = [RP].[OWNER]
				INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [RDD] ON [RDD].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [RH].[ERP_RECEPTION_DOCUMENT_HEADER_ID]
				INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [TL] ON [TL].[SERIAL_NUMBER] = [RH].[TASK_ID]
				INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](@DOC_ID,
        ...
        INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](''' + @DOC_ID
			+ ''', ''' + @DELIMITER
			+ ''') SO ON SO.VALUE = evrdd.SAP_RECEPTION_ID
		INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ( [M].[CLIENT_OWNER] = [evrdd].[OWNER_SKU] AND [evrdd].[OWNER_SKU] +'
			+ '''/'' + evrdd.[SKU] = [M].[MATERIAL_ID] COLLATE SQL_Latin1_General_CP1_CI_AS)
		LEFT JOIN #RECEPTION_DOCUMENTS [R] ON ( [R].[DOC_ID] = evrdd.SAP_RECEPTION_ID AND evrdd.LINE_NUM = [R].LINE_NUM AND  [M].[MATERIAL_ID] = [R].[MATERIAL_ID] 
													AND [R].[UNIT] = [evrdd].[UNIT] COLLATE DATABASE_DEFAULT )
		WHERE [evrdd].[OWNER] = ''' + @CLIENT_CODE + '''
		';
		--
		PRINT '--> @QUERY: ' + @QUERY;
		--
