...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].OP_WMS_SP_DELETE_PILOT @PILOT_CODE = 1
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_DELETE_PILOT (@PILOT_CODE INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  BEGIN TRY
    ...
    DELETE [FERCO].[OP_WMS_USER_X_PILOT]
    WHERE [PILOT_CODE] = @PILOT_CODE;

    DELETE [FERCO].[OP_WMS_PILOT]
    WHERE [PILOT_CODE] = @PILOT_CODE;
  ...
  BEGIN CATCH
    SELECT
      -1 AS [Resultado]
     ,CASE @@error
        WHEN 547 THEN 'No se puede eliminar el piloto debido a que se encuentra asociado al vehiculo con placas: ' + (SELECT
              V.[PLATE_NUMBER]
            FROM [FERCO].[OP_WMS_VEHICLE] AS V
            WHERE V.[PILOT_CODE] = @PILOT_CODE
