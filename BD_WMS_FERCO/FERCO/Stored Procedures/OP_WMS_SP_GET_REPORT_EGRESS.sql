...
/*
--	Ejemplo Ejecucion: 
			EXEC [FERCO].[OP_WMS_SP_GET_REPORT_EGRESS] 
 ...
 */
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_REPORT_EGRESS (@FECHA_INICIO DATETIME
...
BEGIN
  SET NOCOUNT ON;
  --
  DECLARE @WAREHOUSE_REGIMEN_GENERAL VARCHAR(25) = 'GENERAL'
  ...
  -- ------------------------------------------------------------------------------------
  SELECT
    [C].[VALUE] [CLIENT_CODE] INTO [#CLIENTS]
  FROM [FERCO].[OP_WMS_FUNC_SPLIT](@CLIENT_CODE, '|') [C];
  ...
  INNER JOIN [FERCO].[OP_WMS_TRANS] [T]
    ON (
  ...
  LEFT JOIN [FERCO].[OP_WMS_VIEW_VALORIZACION_FISCAL] [VF]
    ON (
        [TL].[LICENSE_ID_SOURCE] = [VF].[LICENSE_ID]
        AND [VF].[BARCODE_ID] = [TL].[BARCODE_ID]
      )
  LEFT JOIN [FERCO].[OP_WMS_VIEW_VALORIZACION_ALMGEN] [VG]
    ON (
        [TL].[LICENSE_ID_SOURCE] = [VG].[LICENSE_ID]
        AND [VG].[BARCODE_ID] = [TL].[BARCODE_ID]
      )
  LEFT JOIN [FERCO].[OP_WMS_POLIZA_DETAIL] [PD]
    ON (
        [PH].[DOC_ID] = [PD].[DOC_ID]
        AND [TL].[LINE_NUMBER_POLIZA_TARGET] = [PD].[LINE_NUMBER]
      )
  LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [CLP]
    ON [PH].[CLIENT_CODE] = [CLP].[CLIENT_CODE]
  LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [CLCONS]
    ON [PD].[CLIENT_CODE] = [CLCONS].[CLIENT_CODE]
  ...
  INNER JOIN @WAREHOUSE [W]
    ON [W].[WAREHOUSE_ID] = [TL].[WAREHOUSE_SOURCE]
  INNER JOIN [#CLIENTS] [CL]
    ON [CL].[CLIENT_CODE] = [TL].[CLIENT_OWNER]
