...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_LABEL_INFORMATION]
					
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_LABEL_INFORMATION](
	@TYPE VARCHAR(50)
	,@BARCODE VARCHAR(50)
)
AS
BEGIN
	...
	BEGIN
	    ...
	    SELECT TOP 1
			...
			, '' [STATE_CODE]
		FROM @BOXES [B]
		INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [FERCO].[OP_WMS_FN_SPLIT_COLUMNS]([B].[ERP_DOC],2,'-') = [T].[WAVE_PICKING_ID]		
    ...
    BEGIN
		...
		INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [T].[WAVE_PICKING_ID] = [PL].[WAVE_PICKING_ID]		
