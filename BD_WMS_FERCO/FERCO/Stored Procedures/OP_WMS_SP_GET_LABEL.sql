...
-- Ejemplo de Ejecucion:
                EXEC [FERCO].OP_WMS_SP_GET_LABEL @LABEL_ID = 1
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_LABEL (@LABEL_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  LEFT JOIN [FERCO].[OP_WMS_LOGINS] [L]
    ON (
    [L].[LOGIN_ID] = [PL].[LOGIN_ID]
    )
  LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH]
    ON (
    [PL].[WAVE_PICKING_ID] = [PDH].[WAVE_PICKING_ID] AND [PDH].[IS_CONSOLIDATED] = 0
    )
