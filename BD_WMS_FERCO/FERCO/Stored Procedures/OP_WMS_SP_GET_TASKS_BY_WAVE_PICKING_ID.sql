...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_TASKS_BY_WAVE_PICKING_ID] @LOGIN_ID='MARVIN',@WAVE_PICKING_ID = 10306
					
   */
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TASKS_BY_WAVE_PICKING_ID]
(
    @LOGIN_ID VARCHAR(50),
...
AS
BEGIN
	DECLARE @clientNameSource varchar(50) = NULL;
	IF EXISTS (SELECT TOP 1 1 FROM FERCO.OP_WMS_NEXT_PICKING_DEMAND_HEADER WHERE WAVE_PICKING_ID = @WAVE_PICKING_ID AND SERIES = 362) SET @clientNameSource = 'ECommerce';
    ...
    SELECT [TaskList].[SERIAL_NUMBER] AS [id],
           ...
           CASE @clientNameSource
				when 'ECommerce' then @clientNameSource
				ELSE [TaskList].[CLIENT_NAME]
				END AS [clientName],
           ...
           [TaskList].[STATUS_CODE] [STATUS_CODE],
           ...
           CASE
               ...
               ELSE
                   ''
           END [reference]
    FROM [FERCO].[OP_WMS_TASK_LIST] AS [TaskList]
        LEFT OUTER JOIN [FERCO].[OP_WMS_MATERIALS] AS [Material]
            ON [TaskList].[MATERIAL_ID] = [Material].[MATERIAL_ID]
        INNER JOIN [FERCO].[OP_WMS_CONFIGURATIONS] AS [Configuration]
        ...
        LEFT OUTER JOIN [FERCO].[OP_WMS_LICENSES] AS [License]
            ON [TaskList].[LICENSE_ID_SOURCE] = [License].[LICENSE_ID]
        LEFT OUTER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PH]
            ON [TaskList].[CODIGO_POLIZA_TARGET] = [PH].[CODIGO_POLIZA]
        LEFT OUTER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PHR]
