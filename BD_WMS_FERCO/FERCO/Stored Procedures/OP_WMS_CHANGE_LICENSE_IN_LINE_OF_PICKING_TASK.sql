...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_CHANGE_LICENSE_IN_LINE_OF_PICKING_TASK] (
...
BEGIN
    ...
    -- ------------------------------------------------------------------------------------
	SELECT
		@TASK_ASSIGNEDTO = [TL].[TASK_ASSIGNEDTO]
	...
	SELECT
		@IS_CONSOLIDATED = MAX([IS_CONSOLIDATED])
		,@DOC_NUM = CASE MAX([IS_CONSOLIDATED])
						WHEN 1 THEN 'CONSOLIDADO'
						WHEN 0 THEN MAX([DOC_NUM])
					END
	...
	SELECT
		@TOTAL_PENDING_BEFORE = SUM([QUANTITY_PENDING])
	FROM
	...
	BEGIN
		...
		EXEC [FERCO].[OP_WMS_SP_INSERT_TASKS_GENERAL] @TASK_OWNER = @LOGIN,                          -- varchar(25)
			@TASK_ASSIGNEDTO = @TASK_ASSIGNEDTO,           -- varchar(25)
			@QUANTITY_ASSIGNED = @QUANTITY_PENDING,        -- numeric
			@CODIGO_POLIZA_TARGET = @CODIGO_POLIZA_TARGET, -- varchar(25)
    ...
    -- ------------------------------------------------------------------------------------
	SELECT
		@TOTAL_PENDING_AFTER = SUM([QUANTITY_PENDING])
	FROM
		[FERCO].[OP_WMS_TASK_LIST]
	WHERE
		[WAVE_PICKING_ID] = @WAVE_PICKING_ID
	...
	BEGIN
		SET @Mensaje = 'El proceso generó una tarea o varias por más cantidad que la cantidad original de la línea = '
