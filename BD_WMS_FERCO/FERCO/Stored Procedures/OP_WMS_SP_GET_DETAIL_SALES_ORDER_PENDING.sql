...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_DETAIL_SALES_ORDER_PENDING]	@START_DATETIME = '2015-02-15 08:34:17.976'
														,@END_DATETIME = '2017-02-15 08:34:17.976'
														,@SOURCE_CODE_ROUTE = '1|'
														,@CODE_ROUTE = '001'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_DETAIL_SALES_ORDER_PENDING] (
  ...
  --
    ...
    -- ------------------------------------------------------------------------------------
		...
		INTO
			[#ROUTE]
		FROM
			[FERCO].[OP_WMS_FN_SPLIT](@SOURCE_CODE_ROUTE,
										@DELIMITER) [SCR]
		INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](@CODE_ROUTE,
...
,'SONDA' ORIGIN
,[D].[LINE_SEQ]
FROM [SWIFT_EXPRESS].[FERCO].[SONDA_SALES_ORDER_DETAIL] [D]
INNER JOIN [SWIFT_EXPRESS].[FERCO].[SONDA_SALES_ORDER_HEADER] [SOH]
 ON [SOH].[SALES_ORDER_ID] = [D].[SALES_ORDER_ID]
INNER JOIN #SALES_ORDERS [S]
 ON [D].[SALES_ORDER_ID] = [S].[DOC_NUM]
INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M]
 ON [M].[MATERIAL_ID] = 'FERCO/' + [D].[SKU]
LEFT JOIN [#PICKING_DEMAND_DETAIL] [PD]
 ON [D].[SALES_ORDER_ID] = [PD].[DOC_NUM]
 AND [PD].[MATERIAL_ID] = [M].[MATERIAL_ID]
 AND [D].[LINE_SEQ] = [PD].[LINE_NUM]
