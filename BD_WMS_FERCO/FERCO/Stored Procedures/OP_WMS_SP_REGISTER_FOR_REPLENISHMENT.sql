...
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_FOR_REPLENISHMENT] (
		@LOGIN VARCHAR(50)
		,@LICENSE_ID INT
...
BEGIN
    ...
    -- -----------------------------------
		...
		INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON ([L].[WAVE_PICKING_ID] = [L].[WAVE_PICKING_ID])
		WHERE
    ...
    -- -----------------------------------
		...
		IF EXISTS ( SELECT TOP 1
					...
					FROM
						[FERCO].[OP_WMS_SHELF_SPOTS] [SS]
					INNER JOIN [FERCO].[OP_WMS_ZONE] [Z] ON ([SS].[ZONE] = [Z].[ZONE])
					WHERE
						[SS].[LOCATION_SPOT] = @LOCATION_ID
    ...
    -- -----------------------------------
      ...
      -- -----------------------------------
        ...
        -- -----------------------------------
				...
				BEGIN
					EXEC [FERCO].[OP_WMS_SP_UPDATE_PARTIAL_LICENSE_FAST_PICKING] @LOGIN_ID = @LOGIN,
						@LICENSE_ID = @LICENSE_ID,
    ...
    -- -----------------------------------
		...
		LEFT JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MSN] ON (
