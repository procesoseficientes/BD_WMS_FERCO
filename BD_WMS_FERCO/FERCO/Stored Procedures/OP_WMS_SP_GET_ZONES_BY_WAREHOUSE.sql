...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_ZONES_BY_WAREHOUSE]  @WAREHOUSE = 'BODEGA_01|BODEGA_02'
   SELECT * FROM [FERCO].[OP_WMS_SHELF_SPOTS] [S]
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_ZONES_BY_WAREHOUSE (@WAREHOUSE VARCHAR(MAX))
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  INNER JOIN [FERCO].[OP_WMS_ZONE] [Z]
    ON Z.ZONE = S.ZONE
  GROUP BY [S].[ZONE]
          ,Z.DESCRIPTION
