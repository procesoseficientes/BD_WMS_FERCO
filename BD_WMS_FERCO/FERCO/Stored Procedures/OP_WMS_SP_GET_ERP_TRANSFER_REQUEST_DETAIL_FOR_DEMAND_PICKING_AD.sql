...
-- Modificacion 10/4/2017 @ NEXUS-Team Sprint enave
-- rodrigo.gomez
...
-- Descripcion:				Se agregan cambios aplicados de FERCO.
...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_ERP_TRANSFER_REQUEST_DETAIL_FOR_DEMAND_PICKING]
...
*/
-- =============================================
create PROCEDURE [FERCO].[OP_WMS_SP_GET_ERP_TRANSFER_REQUEST_DETAIL_FOR_DEMAND_PICKING_AD] (@XML XML)
AS
BEGIN
	SET NOCOUNT ON;
    --
	...
	CREATE TABLE [#PICKING_DOCUMENT] (
		...
		,[OWNER] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[SOURCE_NAME] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[DATA_BASE_NAME] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[SCHEMA_NAME] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,PRIMARY KEY
	...
	CREATE TABLE [#PICKING_DOCUMENT_DETAIL] (
		...
		,[SKU] VARCHAR(200) COLLATE DATABASE_DEFAULT
		,[LINE_SEQ] INT NULL
		...
		,[SERIE_2] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[REQUERIES_SERIE] INT
		,[COMBO_REFERENCE] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[PARENT_SEQ] INT
		,[IS_ACTIVE_ROUTE] INT
		,[CODE_PACK_UNIT] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[IS_BONUS] INT
		,[DESCRIPTION_SKU] VARCHAR(200) COLLATE DATABASE_DEFAULT
		,[BARCODE_ID] VARCHAR(25) COLLATE DATABASE_DEFAULT
		,[ALTERNATE_BARCODE] VARCHAR(25) COLLATE DATABASE_DEFAULT
		,[EXTERNAL_SOURCE_ID] INT
		,[SOURCE_NAME] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[ERP_OBJECT_TYPE] INT
		,[IS_MASTER_PACK] INT
		,[MATERIAL_OWNER] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[MASTER_ID_MATERIAL] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[SOURCE] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[USE_PICKING_LINE] INT
		,[unitMsr] VARCHAR(100)  COLLATE DATABASE_DEFAULT
		,[STATUS_CODE] VARCHAR(50) COLLATE DATABASE_DEFAULT
	);
    --
	...
	CREATE TABLE [#PICKING_DEMAND_DETAIL] (
		...
		,[DOC_ENTRY] VARCHAR(50)  COLLATE DATABASE_DEFAULT
		,[MATERIAL_ID] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[QTY] DECIMAL(16, 6)
		,[OWNER] VARCHAR(50) COLLATE DATABASE_DEFAULT
		,[LINE_SEQ] INT
	);
	...
	BEGIN TRY
		...
		SELECT
			[x].[Rec].[query]('./ExternalSourceId').[value]('.',
											'int')
		...
		INNER JOIN [FERCO].[OP_SETUP_EXTERNAL_SOURCE] [SES] ON ([x].[Rec].[query] ('./ExternalSourceId').[value] ('.',
											'int')   = [SES].[EXTERNAL_SOURCE_ID])
		WHERE
			[SES].[EXTERNAL_SOURCE_ID] > 0
			AND [SES].[READ_ERP] = 1;
		...
		BEGIN
			...
			SELECT
				...
				+ N''' [SOURCE_NAME]
						...
						,[TRD].unitMsr
						, TRD.STATUS_CODE
					FROM ' + @DATA_BASE_NAME + N'.'
				+ @SCHEMA_NAME
				+ N'.[ERP_VW_TRANSFER_REQUEST_DETAIL] [TRD]
					INNER JOIN #PICKING_DOCUMENT [PD] ON ([PD].[PICKING_DOCUMENT_ID]   = convert(varchar,[TRD].[DOC_ENTRY]) COLLATE DATABASE_DEFAULT  AND [PD].[OWNER]  = [TRD].[SOURCE] )
					
					INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([M].[MATERIAL_ID] = [TRD].[MATERIAL_ID])

					LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH] 
					ON (
						[DH].[DOC_NUM] = [TRD].[DOC_ENTRY]  				
            AND [DH].[SOURCE_TYPE]  = ''WT - ERP'' 
					)
					LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [DD] ON (
						[DD].[PICKING_DEMAND_HEADER_ID]  = [DH].[PICKING_DEMAND_HEADER_ID]  
						AND [DD].[MATERIAL_ID]  = [TRD].[MATERIAL_ID]  
						AND [DD].[LINE_NUM]  = [TRD].[LINE_NUM]
					)
					LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON (
						[UMM].[MEASUREMENT_UNIT] = [TRD].unitMsr 
						AND [M].MATERIAL_ID = [UMM].MATERIAL_ID 
					)
		...
		LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [UMM].[MATERIAL_ID]  = [PDD].[SKU] 
											AND [UMM].[MEASUREMENT_UNIT]   = [PDD].[unitMsr] 
		WHERE
