...
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_DISPATCH_BY_REGIME_GENERAL1]
(
...
BEGIN
    ...
    DECLARE @ErrorMessage NVARCHAR(4000),
            ...
            @VIN VARCHAR(40),
            @QTY_ASSIGNED_BEGIN DECIMAL(18, 4),
            @QTY_ASSIGNED_END DECIMAL(18, 4);
    ...
    BEGIN TRY
        ...
        FROM [FERCO].[OP_WMS_FUNC_GETLOGIN_NAME](@pLOGIN_ID) [D];
        ...
        SELECT @QTY_ASSIGNED_BEGIN = SUM([QUANTITY_ASSIGNED])
        FROM [FERCO].[OP_WMS_TASK_LIST]
        WHERE [WAVE_PICKING_ID] = @pWAVE_PICKING_ID
              AND [MATERIAL_ID] = @pMATERIAL_ID_LOCAL;

        SELECT TOP 1
               @WAREHOUSE_PARENT = ISNULL([WAREHOUSE_PARENT], 'BODEGA_DEF')
        FROM [FERCO].[OP_WMS_SHELF_SPOTS]
        WHERE [LOCATION_SPOT] = @pSOURCE_LOCATION;


        -- ------------------------------------------------------------------------------------
        -- Validación para evitar duplicidad en picking 
        -- ------------------------------------------------------------------------------------
        IF NOT EXISTS
        (
            SELECT TOP 1
            ...
            WHERE [TL].[WAVE_PICKING_ID] = @pWAVE_PICKING_ID
                  AND [TL].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL
                  AND [TL].[IS_COMPLETED] = 0
        )
        BEGIN
            SELECT @pRESULT = 'ERROR, Tarea ha sido completada, verifique.',
                   @ErrorCode = 1202;
        ...
        IF @pQUANTITY_UNITS >
        (
            SELECT SUM(ISNULL([TL].[QUANTITY_PENDING], 0))
            FROM [FERCO].[OP_WMS_TASK_LIST] [TL]
            WHERE [TL].[WAVE_PICKING_ID] = @pWAVE_PICKING_ID
                  AND [TL].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL
        )
        BEGIN
            SELECT @pRESULT = 'ERROR, La cantidad sobrepasa la tarea.',
                   @ErrorCode = 1202;
            RAISERROR(@pRESULT, 16, 1);
        END;
        ...
        BEGIN
            ...
            EXEC [FERCO].[OP_WMS_SP_VALIDATE_IF_PICKING_LICENSE_IS_AVAILABLE] @WAVE_PICKING_ID = @pWAVE_PICKING_ID,
            ...
            SELECT TOP 1
                   @QTY_AVAILABLE_NEW_LICENSE = ISNULL([QTY_AVAILABLE], 0)
            FROM @AVAILABLE_PICKING_LICENSE;
            ...
            BEGIN
                SELECT @pRESULT
                    = 'ERROR, Inventario insuficiente[' + CONVERT(VARCHAR(20), @QTY_AVAILABLE_NEW_LICENSE)
                      + '] en licencia origen: [' + CONVERT(VARCHAR(20), @pSOURCE_LICENSE) + '] verifique.',
                       @ErrorCode = 1201;
                RAISERROR(@pRESULT, 16, 1);
            END;

            DECLARE @QTY_TOTAL NUMERIC(18, 4) = 0,
                    @QTY_TOTAL_PENDING NUMERIC(18, 4) = 0;

            SELECT @QTY_TOTAL = SUM([TL].[QUANTITY_ASSIGNED]),
            ...
            BEGIN
                ...
                SET [QUANTITY_PENDING] += @pQUANTITY_UNITS - [QUANTITY_PENDING],
                    [QUANTITY_ASSIGNED] += @pQUANTITY_UNITS - [QUANTITY_PENDING]
                WHERE [WAVE_PICKING_ID] = @pWAVE_PICKING_ID
                      AND [MATERIAL_ID] = @pMATERIAL_ID_LOCAL
        ...
        BEGIN
            SELECT @pRESULT = 'ERROR, SKU Invalido: [' + @pMATERIAL_BARCODE + '/' + @pCLIENT_ID_LOCAL + '] verifique.',
                   @ErrorCode = 1203;
            RAISERROR(@pRESULT, 16, 1);
        END;
        ...
        BEGIN
            SELECT @pRESULT
                = 'ERROR, Inventario insuficiente[' + CONVERT(VARCHAR(20), @pINV_AVAILABLE) + '] en licencia origen: ['
                  + CONVERT(VARCHAR(20), @pSOURCE_LICENSE) + '] verifique.',
                   @ErrorCode = 1201;
            RAISERROR(@pRESULT, 16, 1);
        END;
        ...
        BEGIN
            ...
            SELECT @TERMS_OF_TRADE,
                   ...
                   (
                       SELECT TOP 1
                              [PARAM_CAPTION]
                       FROM [FERCO].[OP_WMS_FUNC_GETTRANS_DESC]('DESPACHO_GENERAL')
                   ),
                   'DESPACHO GENERAL'
                         ),
                   NULL,
                   ...
                   (
                       SELECT TOP 1
                              *
                       FROM [FERCO].[OP_WMS_FUNC_GETMATERIAL_DESC](@pMATERIAL_BARCODE, @pCLIENT_ID_LOCAL)
                   ),
                   NULL,
                   [FERCO].[OP_WMS_FN_GET_MATERIAL_COST_BY_MATERIAL](@pMATERIAL_ID_LOCAL, @pCLIENT_ID_LOCAL),
                   @pSOURCE_LICENSE,
                   NULL,
                   @pSOURCE_LOCATION,
                   'PUERTA_1',
                   ...
                   (
                       SELECT TOP 1
                              *
                       FROM [FERCO].[OP_WMS_FUNC_GETCLIENT_NAME](@pCLIENT_ID_LOCAL)
                   ),
                   -1,
                   @WAREHOUSE_PARENT,
                   NULL,
        ...
        BEGIN
            ...
            VALUES
            (   @TERMS_OF_TRADE, GETDATE(), @pLOGIN_ID, @LOGIN_NAME, 'DESPACHO_GENERAL',
                ISNULL(
                (
                    SELECT TOP 1
                           [PARAM_CAPTION]
                ...
                (
                    SELECT TOP 1
                           *
                    FROM [FERCO].[OP_WMS_FUNC_GETMATERIAL_DESC](@pMATERIAL_BARCODE, @pCLIENT_ID_LOCAL)
                ), NULL, [FERCO].[OP_WMS_FN_GET_MATERIAL_COST_BY_MATERIAL](@pMATERIAL_ID_LOCAL, @pCLIENT_ID_LOCAL),
                @pSOURCE_LICENSE, NULL, @pSOURCE_LOCATION, 'PUERTA_1', @pCLIENT_ID_LOCAL, @CLIENT_NAME,
                (@pQUANTITY_UNITS * -1), @WAREHOUSE_PARENT, NULL, @TRANS_SUBTYPE, @pCODIGO_POLIZA, @pSOURCE_LICENSE,
                'PROCESSED', @pWAVE_PICKING_ID, @pWAVE_PICKING_ID, @IS_FROM_SONDA, @BATCH, @DATE_EXPIRATION,
                @SOURCE_TYPE, @TRANSFER_REQUEST_ID, @VIN);
        END;
        ...
        BEGIN
            UPDATE [FERCO].[OP_WMS_LICENSES]
            SET [USED_MT2] = [USED_MT2] - @pMt2
            WHERE [LICENSE_ID] = @pSOURCE_LICENSE;
        END;

        IF @IS_MASTER_PACK = 1
        BEGIN
            EXEC [FERCO].[OP_WMS_SP_DISPATCH_MASTER_PACK] @MATERIAL_ID = @pMATERIAL_ID_LOCAL,
        ...
        (
            SELECT TOP 1
                   1
            FROM [FERCO].[OP_WMS_TASK_LIST] [T]
            WHERE [T].[WAVE_PICKING_ID] = @pWAVE_PICKING_ID
        ...
        BEGIN
            ...
            SELECT @QTY_COMPLETED = SUM([TL].[QUANTITY_ASSIGNED])
            FROM [FERCO].[OP_WMS_TASK_LIST] [TL]
            WHERE [TL].[WAVE_PICKING_ID] = @pWAVE_PICKING_ID
                  AND [TL].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL
            ...
            BEGIN
                ...
                DECLARE @QUANTITY_PENDING NUMERIC(18, 4) = 0,
                ...
                SET @QTY_PENDING = CASE
                                       WHEN (@QTY_TOTAL_PENDING - @pQUANTITY_UNITS) = 0 THEN
                                           @pQUANTITY_UNITS
                                       ELSE
                (@QTY_TOTAL_PENDING - @pQUANTITY_UNITS)
                                   END;
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        IF ((@QTY_TOTAL_PENDING - @pQUANTITY_UNITS)) = 0
                        BEGIN
                            UPDATE [FERCO].[OP_WMS_TASK_LIST]
                            SET [QUANTITY_PENDING] -= @QUANTITY_PENDING,
                    ...
                    BEGIN
                        UPDATE [FERCO].[OP_WMS_TASK_LIST]
                        SET [QUANTITY_PENDING] = @QTY_PENDING,
                            [QUANTITY_ASSIGNED] = CASE
                                                      WHEN [QUANTITY_PENDING] = [QUANTITY_ASSIGNED] THEN
                                                          @QTY_PENDING
                                                      ELSE
                            (([QUANTITY_ASSIGNED] - [QUANTITY_PENDING]) + @QTY_PENDING)
                                                  END
                        WHERE [WAVE_PICKING_ID] = @pWAVE_PICKING_ID
                              AND [MATERIAL_ID] = @pMATERIAL_ID_LOCAL
                              AND [LICENSE_ID_SOURCE] = @LICENSE_ID_SOURCE;
                    ...
                    DELETE @LICENSES
                    WHERE [LICENSE_ID] = @LICENSE_ID_SOURCE;

                END;

                UPDATE [TL]
                SET [TL].[QUANTITY_ASSIGNED] = ([TL].[QUANTITY_ASSIGNED] - [TL].[QUANTITY_PENDING]),
                    [TL].[QUANTITY_PENDING] = 0
                FROM [FERCO].[OP_WMS_TASK_LIST] [TL]
        ...
        BEGIN
            ...
            (
                SELECT TOP 1
                       1
                FROM [FERCO].[OP_WMS_LICENSES] [L]
                WHERE [L].[LICENSE_ID] = @LICENSE_DISPATCH_ID
            )
            BEGIN
                ...
                ----
                SELECT @TONE = [TCM].[TONE],
                       @CALIBER = [TCM].[CALIBER]
                FROM [FERCO].[OP_WMS_INV_X_LICENSE] [L]
                    INNER JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCM]
                        ON ([L].[TONE_AND_CALIBER_ID] = [TCM].[TONE_AND_CALIBER_ID])
                WHERE [L].[LICENSE_ID] = @pSOURCE_LICENSE
                ...
                (
                    SELECT TOP 1
                           1
                    FROM [FERCO].[OP_WMS_INV_X_LICENSE] [IL]
                    WHERE [IL].[LICENSE_ID] = @LICENSE_DISPATCH_ID
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        (
                            ...
                            FROM [FERCO].[OP_WMS_INV_X_LICENSE] [IL]
                                INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M]
                                    ON ([IL].[MATERIAL_ID] = [M].[MATERIAL_ID])
                                INNER JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCM]
                                    ON ([IL].[TONE_AND_CALIBER_ID] = [TCM].[TONE_AND_CALIBER_ID])
                            WHERE [IL].[LICENSE_ID] = @LICENSE_DISPATCH_ID
                                  AND [IL].[MATERIAL_ID] = @pMATERIAL_ID
                                  AND
                                  (
                        ...
                        BEGIN
                            SELECT @ErrorCode = 3002;
                            RAISERROR(
                                         'Ya existe un producto con ese tono o calibre, debe de crear otra licencia.',
                                         16,
                                         1
                                     );
                            RETURN;
                    ...
                    UPDATE [FERCO].[OP_WMS_INV_X_LICENSE]
                    SET [QTY] = [QTY] + @pQUANTITY_UNITS
                    WHERE [LICENSE_ID] = @LICENSE_DISPATCH_ID
                ...
                BEGIN
                    ...
                    INSERT [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE]
                    (
                        [STATUS_CODE],
                        [STATUS_NAME],
                        [BLOCKS_INVENTORY],
                    ...
                    VALUES
                    (@STATUS_CODE, @STATUS_NAME, @BLOCKS_INVENTORY, @ALLOW_REALLOC, @TARGET_LOCATION, @DESCRIPTION,
                     @COLOR, @LICENSE_DISPATCH_ID);
                    ...
                    BEGIN
                        ...
                        FROM [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCM]
                    ...
                    INSERT INTO [FERCO].[OP_WMS_INV_X_LICENSE]
                    (
                        [LICENSE_ID],
                        [MATERIAL_ID],
                        [MATERIAL_NAME],
                    ...
                    VALUES
                    (@LICENSE_DISPATCH_ID, @pMATERIAL_ID, @MATERIAL_NAME, @pQUANTITY_UNITS, @VOLUME_FACTOR, 0, 'N/A',
                     'N/A', GETDATE(), @pLOGIN_ID, @BARCODE_ID, @TERMS_OF_TRADE, '', GETDATE(), @DATE_EXPIRATION,
                     @BATCH, @pQUANTITY_UNITS, @VIN, @HANDLE_SERIAL, 0, 0, NULL, @STATUS_ID, @TONE_AND_CALIBER_ID, 1);
                END;
                ...
                BEGIN
                    ...
                    ---------------------------------------------------------------------------------    
                    DECLARE @MASTER_PACK_HEADER_ID_SOURCE INT,
                    ...
                    FROM [FERCO].[OP_WMS_MASTER_PACK_HEADER] [MPH]
                    WHERE [MPH].[LICENSE_ID] = @pSOURCE_LICENSE;
                    ---------------------------------------------------------------------------------
                    ...
                    (
                        SELECT TOP 1
                               1
                        FROM [FERCO].[OP_WMS_MASTER_PACK_HEADER] [MPH]
                        WHERE [MPH].[LICENSE_ID] = @LICENSE_DISPATCH_ID
                    )
                    BEGIN
                        ---------------------------------------------------------------------------------
                        ...
                        ---------------------------------------------------------------------------------    
                        UPDATE [MPH]
                        SET [MPH].[QTY] = [MPH].[QTY] + @pQUANTITY_UNITS
                        FROM [FERCO].[OP_WMS_MASTER_PACK_HEADER] [MPH]
                        WHERE [MPH].[LICENSE_ID] = @LICENSE_DISPATCH_ID;
                    END;
                    ELSE
                    BEGIN
                        ...
                        ---------------------------------------------------------------------------------    
                        INSERT [FERCO].[OP_WMS_MASTER_PACK_HEADER]
                        (
                            [LICENSE_ID],
                            [MATERIAL_ID],
                            [POLICY_HEADER_ID],
                        ...
                        ---------------------------------------------------------------------------------    
                        SELECT @MASTER_PACK_HEADER_ID = SCOPE_IDENTITY();
                        ...
                        SELECT @MASTER_PACK_HEADER_ID,
                               [MPDSS].[MATERIAL_ID],
                               [MPDSS].[QTY],
                               [MPDSS].[BATCH],
                               [MPDSS].[DATE_EXPIRATION]
                        FROM [FERCO].[OP_WMS_MASTER_PACK_DETAIL] [MPDSS]
                        WHERE [MPDSS].[MASTER_PACK_HEADER_ID] = @MASTER_PACK_HEADER_ID_SOURCE;
                    END;
                END;
            END;
        END;
        ...
        SELECT @QTY_ASSIGNED_END = SUM([QUANTITY_ASSIGNED])
        FROM [FERCO].[OP_WMS_TASK_LIST]
        WHERE [WAVE_PICKING_ID] = @pWAVE_PICKING_ID
              AND [MATERIAL_ID] = @pMATERIAL_ID_LOCAL;


        IF @QTY_ASSIGNED_BEGIN <> @QTY_ASSIGNED_END
        BEGIN
            SELECT @pRESULT
                = 'ERROR, La tarea tenia una cantidad asignada y termino con una cantidad asignada diferente al final del proceso, verifique.',
                   @ErrorCode = 1202;
            RAISERROR(@pRESULT, 16, 1);
        END;
        ...
        COMMIT TRANSACTION;
        EXEC [FERCO].[OP_WMS_SP_PICKING_HAS_BEEN_COMPLETED] @WAVE_PICKING_ID = @pWAVE_PICKING_ID, -- int
