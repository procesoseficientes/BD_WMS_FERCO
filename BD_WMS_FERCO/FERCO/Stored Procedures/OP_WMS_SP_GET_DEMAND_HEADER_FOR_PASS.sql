...
EXEC [FERCO].[OP_WMS_SP_GET_DEMAND_HEADER_FOR_PASS] @INITIAL_DATE = '2022-05-12 17:46:48.080'
                                      , @END_DATE = '2022-05-12 20:46:48.080'
...
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_DEMAND_HEADER_FOR_PASS] (
  ...
  --
	...
	BEGIN
		...
		SELECT
			...
			,SUM([PD].[QTY]) [QTY]
		FROM
			[FERCO].[OP_WMS3PL_PASSES] [PH]
		INNER JOIN [FERCO].[OP_WMS_PASS_DETAIL] [PD] ON ([PH].[PASS_ID] = [PD].[PASS_HEADER_ID])
		WHERE
			[PH].[STATUS] <> 'CANCELED'
		GROUP BY
			[PD].[PICKING_DEMAND_HEADER_ID]
		...
		FROM
			...
			WHERE [TL].[ASSIGNED_DATE] BETWEEN @INITIAL_DATE
									AND		@END_DATE
		GROUP BY
			[TL].[WAVE_PICKING_ID]
		HAVING
		...
		INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON ([L].[LICENSE_ID] = [TL].[LICENSE_ID_SOURCE])
		INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PH] ON ([PH].[CODIGO_POLIZA] = [TL].[CODIGO_POLIZA_TARGET])
	...
	---INSERTAMOS LAS OLAS COMPLETAS---
		...
		FROM
			[FERCO].[OP_WMS_TASK_LIST] [TL]
		WHERE
			[TL].[ASSIGNED_DATE] BETWEEN @INITIAL_DATE
									AND		@END_DATE
			-- AND [TL].[QUANTITY_PENDING] <> [TL].[QUANTITY_ASSIGNED]
		GROUP BY
			[TL].[WAVE_PICKING_ID]
		...
		FROM
			@TASK_LIST_TEMP [TLP]
		INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] ON ([TLP].[WAVE_PICKING_ID] = [PDH].[WAVE_PICKING_ID])
