...
/*
	Ejemplo Ejecucion: 
    EXEC	[FERCO].[OP_WMS_SP_GET_LOCATION_FOR_RECEPTION] @DISTRIBUTION_CENTER_ID= 'CTR_SUR' 
		
 */
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_LOCATION_FOR_RECEPTION] (
		@DISTRIBUTION_CENTER_ID VARCHAR(25) = NULL
	)
AS
BEGIN
	...
	FROM
		[FERCO].[OP_WMS_SHELF_SPOTS] [owss]
	INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] ON [W].[WAREHOUSE_ID] = [owss].[WAREHOUSE_PARENT]
	WHERE
		(
			[owss].[SPOT_TYPE] = 'RAMPA'
			OR [owss].[SPOT_TYPE] = 'PUERTA'
			OR [owss].[SPOT_TYPE] = 'RECEPCION'
		)
		AND (
				@DISTRIBUTION_CENTER_ID IS NULL
