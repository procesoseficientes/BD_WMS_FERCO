...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_ERP_RECEPTION_DOCUMENT_HEADER] 
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_ERP_RECEPTION_DOCUMENT_HEADER] (
...
BEGIN
	...
	-- ------------------------------------------------------------------------------------
	--IF @CLIENT_CODE = 'ferco'
	--BEGIN
		
		--DECLARE 
		--@COMPCODE VARCHAR(25) = @CLIENT_CODE;
	...
	SELECT
		...
		LEFT JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [D] ON ([D].[DOC_ID] = CAST([R].[SAP_REFERENCE] AS VARCHAR(50)) AND [D].[OWNER] = ''' + @CLIENT_CODE + ''')
		...
		GROUP BY 
			...
			,[R].DocNum 
			,[R].[OWNER]
			HAVING MAX(ISNULL(D.[IS_COMPLETE],0)) = 0 
		AND [R].[OWNER] = '''	+ @CLIENT_CODE + ''''
