...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_GET_NOT_ASSOCIATED_CLASSES]
					@CLASS_ID = 3
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_GET_NOT_ASSOCIATED_CLASSES (@CLASS_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  LEFT JOIN [FERCO].[OP_WMS_CLASS_ASSOCIATION] [CA]
    ON [CA].[CLASS_ASSOCIATED_ID] = [C].[CLASS_ID]
      AND [CA].[CLASS_ID] = @CLASS_ID
  INNER JOIN [FERCO].[OP_WMS_CONFIGURATIONS] [CONF]
    ON [CONF].[PARAM_NAME] = [C].[CLASS_TYPE]
      AND [CONF].[PARAM_GROUP] = @PARAM_GROUP
