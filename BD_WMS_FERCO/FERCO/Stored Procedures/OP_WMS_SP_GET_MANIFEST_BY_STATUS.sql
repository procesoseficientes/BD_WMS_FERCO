...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_MANIFEST_BY_STATUS] 
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_MANIFEST_BY_STATUS] (
		@START_DATE DATETIME
		,@END_DATE DATETIME
...
BEGIN
	...
	LEFT JOIN [FERCO].[OP_WMS_VEHICLE] [V] ON [MH].[VEHICLE] = [V].[VEHICLE_CODE]
	LEFT JOIN [FERCO].[OP_WMS_PILOT] [P] ON [V].[PILOT_CODE] = [P].[PILOT_CODE]
	LEFT JOIN [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER] [T] ON [T].[TRANSFER_REQUEST_ID] = [MH].[TRANSFER_REQUEST_ID]
	WHERE
