...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_VALIDATE_STOCK_INVENTORY_BY_MATERIAL_FOR_DISPATCH]
					@XML = N'
<ArrayOfOrdenDeVentaDetalle>
  <OrdenDeVentaDetalle>
    <SKU>ferco/SKUPRUEBA</SKU>
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_VALIDATE_STOCK_INVENTORY_BY_MATERIAL_FOR_DISPATCH] (
		@XML XML
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
	...
	FROM
		[#XMLMATERIAL] [XM]
	INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([XM].[MATERIAL_ID] = [M].[MATERIAL_ID])
	WHERE
		(
  ...
  -- ------------------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------------------
		SELECT @CURRENTLY_AVAILABLE = FERCO.OP_WMS_FN_GET_AVAILABLE_INVENTORY_FOR_MASTERPACK(@MATERIAL_ID, @CODE_WAREHOUSE)
