...
-- Ejemplo de Ejecucion:
                EXEC [FERCO].[OP_WMS_SP_GET_PICKING_TAG] @WAVE_PICKING_ID = 4685 , @LOGIN = 'ACAMACHO'
          SELECT * FROM [FERCO].[OP_WMS_TASK_LIST] [OWTL] ORDER BY 1 DESC
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_PICKING_TAG (@WAVE_PICKING_ID NUMERIC
, @LOGIN VARCHAR(25))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  INNER JOIN [FERCO].[OP_WMS_LOGINS] [L]
    ON [L].[LOGIN_ID] = @LOGIN
  LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [H]
