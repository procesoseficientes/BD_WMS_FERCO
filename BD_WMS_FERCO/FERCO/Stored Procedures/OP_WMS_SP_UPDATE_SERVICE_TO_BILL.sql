...
-- Description:	 se pone el parametro de DOC_Num opcional por el Bug #9561: Error Servicios por cobrar Tarifario 
  ...
  /*  EJEMPLO DE EJECUCION:
	EXEC [FERCO].OP_WMS_SP_UPDATE_SERVICE_TO_BILL
	@SERVICES_TO_BILL_ID = 1
		...
		,@TRANSACTION_ID 5
	--
	SELECT * FROM [FERCO].[OP_WMS_SERVICES_TO_BILL] WHERE SERVICES_TO_BILL_ID = 1
=============================================*/
CREATE PROCEDURE [FERCO].[OP_WMS_SP_UPDATE_SERVICE_TO_BILL] (@SERVICES_TO_BILL_ID INT
, @QTY INT
...
BEGIN TRY
  --
  UPDATE [FERCO].[OP_WMS_SERVICES_TO_BILL]
  SET QTY =
           CASE @QTY
             WHEN NULL THEN QTY
             ELSE @QTY
