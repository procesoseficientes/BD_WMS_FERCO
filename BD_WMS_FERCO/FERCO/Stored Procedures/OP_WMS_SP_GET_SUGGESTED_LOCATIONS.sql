...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_SUGGESTED_LOCATIONS]
					@LICENSE_ID = 317851,
					@LOGIN_ID = 'MARVIN'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_SUGGESTED_LOCATIONS] (@LICENSE_ID INT
, @LOGIN_ID VARCHAR(25))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  INSERT INTO @MATERIAL ([MATERIAL_ID], [BATCH], [DATE_EXPIRATION], [VIN], [TONE], [CALIBER])
    SELECT
      [owixl].[MATERIAL_ID]
     ,[owixl].[BATCH]
     ...
     ,[owtacbm].[TONE]
     ,[owtacbm].[CALIBER]
    FROM [ferco].[OP_WMS_INV_X_LICENSE] [owixl]
    LEFT JOIN [ferco].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [owtacbm]
      ON ([owixl].[TONE_AND_CALIBER_ID] = [owtacbm].[TONE_AND_CALIBER_ID])
    WHERE [owixl].[LICENSE_ID] = @LICENSE_ID
  ...
  BEGIN
    ...
    -- ----------------------------------
    INSERT INTO @SUGGESTED_LOCATIONS ([LOCATION], [MATERIAL_ID], [MATERIAL_NAME], [QTY], [TONE], [CALIBER], [BATCH], [DATE_EXPIRATION])
      SELECT TOP 10
        [L].[CURRENT_LOCATION]
      ...
      INNER JOIN [ferco].[OP_WMS_SHELF_SPOTS] [SS]
        ON ([L].[CURRENT_LOCATION] = [SS].[LOCATION_SPOT])
      INNER JOIN [FERCO].[OP_WMS_WAREHOUSE_BY_USER] [WU]
        ON ([L].[CURRENT_WAREHOUSE] = [WU].[WAREHOUSE_ID]
        )
  ...
  SELECT
   ...
   ,[BATCH]
   ,[DATE_EXPIRATION]
  FROM @SUGGESTED_LOCATIONS [SL]
  WHERE [SL].[QTY] > 0
  ORDER BY [SL].[MATERIAL_ID], CASE @TIPO_ORDENAMIENTO
    WHEN 0 THEN -[SL].[QTY]
    ELSE [SL].[QTY]
  END;
