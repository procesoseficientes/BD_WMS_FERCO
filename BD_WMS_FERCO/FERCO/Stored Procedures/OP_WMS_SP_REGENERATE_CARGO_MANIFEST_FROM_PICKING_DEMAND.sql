...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_REGENERATE_CARGO_MANIFEST_FROM_PICKING_DEMAND]
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_REGENERATE_CARGO_MANIFEST_FROM_PICKING_DEMAND (@WAVE_PICKING_ID INT
, @LOGIN VARCHAR(50))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  --
  DECLARE @MANIFEST_HEADER_ID INT = 0
  ...
  BEGIN TRY
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO @RESULT
    EXEC [FERCO].[OP_WMS_SP_INSERT_MANIFEST_DETAIL] @MANIFEST_HEADER_ID = @MANIFEST_HEADER_ID
                                                   , -- int
    ...
    SET [STATUS] = 'CREATED'
    WHERE [MANIFEST_HEADER_ID] IN (SELECT
      ...
      INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [TL]
        ON [TL].[WAVE_PICKING_ID] = [MD].[WAVE_PICKING_ID]
      WHERE [MH].[STATUS] = 'IN_PICKING'
