...
/*
  -- Ejemplo de Ejecucion:
				-- 
				BEGIN TRAN 
				EXEC [FERCO].OP_WMS_DELETE_PICKING 
					@WAVE_PICKING_ID = 899              
					,@USER_ID = 'JMAZARIAGOS'
					

					SELECT * FROM FERCO.OP_WMS_TASK_LIST WHERE WAVE_PICKING_ID = 899
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_DELETE_PICKING]
...
BEGIN TRY
	...
	BEGIN
		...
		BEGIN
			...
			FROM
				...
				-- ------------------------------------------------------------------------------------
			EXEC [FERCO].[OP_WMS_SP_CANCEL_PICKING_DETAIL_LINE] @LOGIN_ID = @USER_ID,
				@WAVE_PICKING_ID = @WAVE_PICKING_ID,
				@MATERIAL_ID = @MATERIAL_ID;
			DELETE FROM
				@MATERIALS
	...
	BEGIN
		...
		FROM
			[FERCO].[OP_WMS_POLIZA_DETAIL] [PD]
		INNER JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PH] ON ([PD].[DOC_ID] = [PH].[DOC_ID])
		INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [TL] ON (
