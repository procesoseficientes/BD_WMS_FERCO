...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].OP_WMS_SP_GET_INVENTORY_FOR_PICKING_BY_STATUS_MATERIAL
					@XML = N'
<ArrayOfOrdenDeVentaDetalle>
  <OrdenDeVentaDetalle>
    ...
    <MATERIAL_OWNER>ferco</MATERIAL_OWNER>
  </OrdenDeVentaDetalle>
  <OrdenDeVentaDetalle>   
    <SKU>ferco/RD001</SKU>
  ...
  <OrdenDeVentaDetalle>
    ...
    <MATERIAL_OWNER>ferco</MATERIAL_OWNER>
					...
					SELECT * FROM [FERCO].[OP_WMS_MATERIALS] WHERE [MATERIAL_ID] = 'ferco/C00000123'

					
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_INVENTORY_FOR_PICKING_BY_STATUS_MATERIAL] (
		@XML XML
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
	...
	SELECT 
		...
		FERCO.OP_WMS_FN_GET_AVAILABLE_INVENTORY_FOR_MASTERPACK(SK.MATERIAL_ID, @CODE_WAREHOUSE) [AVAILABLE_QTY],
		MAX([IP].STATUS_CODE) STATUS_CODE,
		MAX([IP].STATUS_NAME) STATUS_NAME,
		MAX([IP].[COLOR]) [COLOR],
		MAX([IP].AVAILABLE_QTY)
	FROM @SKUS SK
	INNER JOIN FERCO.OP_WMS_MATERIALS OWM
	ON SK.MATERIAL_ID = OWM.MATERIAL_ID
	INNER JOIN FERCO.[OP_WMS_VIEW_INVENTORY_FOR_PICKING_BY_STATUS_MATERIAL] [IP]
	ON SK.MATERIAL_ID = [IP].MATERIAL_ID
	WHERE FERCO.OP_WMS_FN_GET_AVAILABLE_INVENTORY_FOR_MASTERPACK(SK.MATERIAL_ID, @CODE_WAREHOUSE) > 0
	GROUP BY
		SK.[MATERIAL_ID],
		[IP].[STATUS_CODE];
END;