...
-- Ejemplo de Ejecucion:
	select * 	  FROM [FERCO].[OP_WMS_TASK_LIST] [TL]
    WHERE [TL].[TASK_TYPE] = 'TAREA_REUBICACION'
	SELECT * FROM [FERCO].OP_WMS_LOGINS
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_REALLOC_DISTRIBUTE_TASKS_TO_OPERS
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  DECLARE @WAREHOUSE_TASK VARCHAR(25)
  ...
  LEFT JOIN [FERCO].[OP_WMS_TASK_LIST] [T]
    ON [T].[TASK_ASSIGNEDTO] = [L].[LOGIN_ID]
  ...
  INNER JOIN [FERCO].[OP_WMS_WAREHOUSE_BY_USER] [WU]
    ON [WU].[LOGIN_ID] = [L].[LOGIN_ID]
  WHERE [WU].[WAREHOUSE_ID] = @WAREHOUSE_TASK
  AND [L].[LOGIN_TYPE] <> 'PC'
  AND [L].[CAN_RELOCATE] = 1
  GROUP BY [L].[LOGIN_ID]
  ORDER BY COUNT(ISNULL([T].[TASK_ASSIGNEDTO], 0)) ASC;

  IF @OPERADOR IS NOT NULL
  BEGIN
    UPDATE [TL]
    SET [TL].[TASK_ASSIGNEDTO] = @OPERADOR
