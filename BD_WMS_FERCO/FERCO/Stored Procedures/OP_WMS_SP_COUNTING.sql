...
CREATE PROCEDURE FERCO.OP_WMS_SP_COUNTING
...
BEGIN
  ...
  BEGIN TRY
    ...
    IF NOT EXISTS (SELECT
          *
        FROM FERCO.OP_WMS_COUNTING
        WHERE COUNT_ID = @pCOUNT_ID)
    BEGIN
      INSERT INTO [FERCO].[OP_WMS_COUNTING] ([CREATED_BY]
      , [CREATED_DATE]
      , [COUNT_TYPE]
      , [ACCURACY])
        VALUES (@pCREATED_BY, CURRENT_TIMESTAMP, CASE @pCOUNT_TYPE WHEN 'MATERIAL_ID' THEN 'PRODUCTO' WHEN 'CLIENT_CODE' THEN 'CLIENTE' WHEN 'CODIGO_POLIZA' THEN 'POLIZA' WHEN 'REGIMEN' THEN 'REGIMEN' WHEN 'WAREHOUSE_ID' THEN 'BODEGA' WHEN 'COMPLETO' THEN 'COMPLETO' END, 0);

      SET @pCOUNT_ID = SCOPE_IDENTITY()
    END
    ELSE
    BEGIN
      UPDATE [FERCO].[OP_WMS_COUNTING]
      ...
      WHERE COUNT_ID = @pCOUNT_ID;
    END
    DECLARE TASK_CURSOR CURSOR FOR SELECT
      CLIENT_CODE
     ,MATERIAL_ID
     ,BARCODE_ID
     ,MATERIAL_NAME
     ...
     ,CODIGO_POLIZA
     ,CURRENT_LOCATION
    FROM FERCO.OP_WMS_TEMP_TOCOUNT
    ...
    INSERT INTO [FERCO].[OP_WMS_TASK_LIST_COUNTING] ([COUNT_ID], [COUNT_TYPE], [COUNT_REGIMEN], [COUNT_CLIENT_OWNER]
    , [CREATED_DATE], [EXPECTED_LOCATION_SPOT], [EXPECTED_MATERIAL_ID]
    , [MATERIAL_NAME], COUNT_CLIENT_NAME)
      VALUES (@pCOUNT_ID, CASE @pCOUNT_TYPE WHEN 'MATERIAL_ID' THEN 'PRODUCTO' WHEN 'CLIENT_CODE' THEN 'CLIENTE' WHEN 'CODIGO_POLIZA' THEN 'POLIZA' WHEN 'REGIMEN' THEN 'REGIMEN' WHEN 'WAREHOUSE_ID' THEN 'BODEGA' WHEN 'COMPLETO' THEN 'COMPLETO' END, @REGIMEN, @CLIENT_CODE, GETDATE(), @CURRENT_LOCATION, @MATERIAL_ID, @MATERIAL_NAME, @CLIENT_NAME);
    ...
    DELETE FERCO.OP_WMS_TEMP_TOCOUNT;
    SELECT
      @PRESULT = 'OK';
  END TRY
  BEGIN CATCH
