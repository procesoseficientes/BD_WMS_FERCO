...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_INSERT_MANIFEST_DETAIL] 
				@MANIFEST_HEADER_ID = 10000
				,@PICKING_DEMAND_HEADER_ID = 1
				,@LAST_UPDATE_BY = 'YO'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_INSERT_MANIFEST_DETAIL]
(
...
BEGIN
    ...
    BEGIN TRY
        ...
        (
            SELECT TOP 1
                1
            FROM [FERCO].[OP_WMS_MANIFEST_DETAIL]
        ...
        FROM [FERCO].[OP_WMS_MANIFEST_DETAIL] [D]
            INNER JOIN [FERCO].[OP_WMS_MANIFEST_HEADER] [H]
        ...
        INSERT [FERCO].[OP_WMS_MANIFEST_DETAIL]
        (
            [MANIFEST_HEADER_ID],
            [CODE_ROUTE],
            [CLIENT_CODE],
        ...
        SELECT @MANIFEST_HEADER_ID [MANIFEST_HEADER_ID],
               ...
               ISNULL([M].[ERP_AVERAGE_PRICE], 0)
        FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [DD]
            INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH]
		...
		--
        -- ------------------------------------------------------------------------------------
        -- Se autoriza el picking desde el manifiesto de carga
        -- ------------------------------------------------------------------------------------

		UPDATE [PH]
        SET 
			[PH].[IS_AUTHORIZED] = 1
        FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PH]
        WHERE
			[PH].[IS_POSTED_ERP] <= 0
            AND [PH].[IS_AUTHORIZED] = 0
			AND [PH].[PICKING_DEMAND_HEADER_ID] = @PICKING_DEMAND_HEADER_ID;
		--
        ...
        BEGIN
            ...
            SELECT [COMPONENT_MATERIAL],
                   1
            FROM [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK]
            ...
            FROM [FERCO].[OP_WMS_PICKING_LABELS] [PL]
                INNER JOIN @COMPONENTS [C]
                    ON [C].[MATERIAL_ID] = [PL].[MATERIAL_ID]
                LEFT JOIN [FERCO].[OP_WMS_PICKING_LABEL_BY_MANIFEST] [LBM]
                    ON (
            ...
            BEGIN
                ...
                BEGIN
                    INSERT [FERCO].[OP_WMS_PICKING_LABEL_BY_MANIFEST]
                    (
                        [LABEL_ID],
                        [MANIFEST_DETAIL_ID],
                        [MATERIAL_ID],
        ...
        FROM [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDD]
        ...
        BEGIN
            ...
            BEGIN
                UPDATE [FERCO].[OP_WMS_MANIFEST_HEADER]
