...
-- Ejemplo de Ejecucion:
		...
		--
		EXEC [FERCO].[OP_WMS_SP_GET_ACUERDO_COMERCIAL]
			@AcuerdoComercialId = 12
			,@pResult = @pResult OUTPUT
		--
		SELECT @pResult [pResult]
		--
		SELECT * FROM  [FERCO].[OP_WMS_TARIFICADOR_HEADER] WHERE [ACUERDO_COMERCIAL_ID] = 12
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_ACUERDO_COMERCIAL (@pClientId VARCHAR(50)
...
BEGIN
  ...
  BEGIN
    ...
    FROM [FERCO].[OP_WMS_TARIFICADOR_HEADER] [th]
    INNER JOIN [FERCO].[OP_WMS_TARIFICADOR_DETAIL] [td]
      ON (
