...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_MERGE_LICENSE_IN_LOCATION] (
		@LOCATION VARCHAR(25)
...
BEGIN
	...
	LEFT JOIN [FERCO].[OP_WMS_FN_GET_COMMITED_INVENTORY_BY_LICENCE]() [C] ON [C].[MATERIAL_ID] = [IL].[MATERIAL_ID]
	...
	BEGIN
		...
		LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] ON [PDH].[PICKING_DEMAND_HEADER_ID] = [IL].[PICKING_DEMAND_HEADER_ID]
		WHERE
	...
	BEGIN TRY
		...
		BEGIN
			...
			FROM
				[FERCO].[OP_WMS_ACUERDOS_X_CLIENTE] [AC]
			LEFT JOIN [FERCO].[OP_WMS_TARIFICADOR_HEADER] [T] ON [T].[ACUERDO_COMERCIAL_ID] = [AC].[ACUERDO_COMERCIAL]
			WHERE
			...
			SELECT
				...
				,[IL].[MATERIAL_ID]
				,[IL].[MATERIAL_NAME]
				,[FERCO].[OP_WMS_FN_GET_MATERIAL_COST_BY_MATERIAL]([IL].[MATERIAL_ID],
											[IL].[CLIENT_OWNER])
				,[IL].[LICENSE_ID]
				,@LICENSE_ID
				,@LOCATION
			...
			FROM
				[#OLD_LICENSE] [IL]
			INNER JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [MS] ON [MS].[LICENSE_ID] = [IL].[LICENSE_ID]
			...
			FROM
				[#OLD_LICENSE] [L]
			INNER JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IL] ON [IL].[PK_LINE] = [L].[PK_LINE]
			WHERE
