...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_GET_SALE_ORDER_DETAIL_BY_ORDERS_FROM_EXTERNAL]  @EXTERNAL_SOURCES_ID = '1|1|1'
	                                                                      				,@SALES_ORDER_IDS = '49264|49265|49267'
																						,@CODE_WAREHOUSE = 'C002'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_SALE_ORDER_DETAIL_BY_ORDERS_FROM_EXTERNAL]
    (
...
BEGIN
    ...
    BEGIN TRY
        ...
        INTO
            [#TB_SALES_ORDER_ID]
        FROM
            [FERCO].[OP_WMS_FN_SPLIT](@EXTERNAL_SOURCES_ID, @DELIMITER) [ES]
        INNER JOIN [FERCO].[OP_WMS_FN_SPLIT](@SALES_ORDER_IDS, @DELIMITER) [SO] ON ([SO].[ID] = [ES].[ID])
      ...
      -- ------------------------------------------------------------------------------------
            SELECT
				...
				SELECT DISTINCT
					[SOD].[SALES_ORDER_ID]
					,[OWC].[CLIENT_CODE] +' + '''/'' + [VAS].[OWNER_ID] [SKU]
					,[SOD].[LINE_SEQ]
                ...
                + ''' [SOURCE_NAME]
					...
					,[C].[OWNER] [SOURCE]
					,[SOD].[DISCOUNT_TYPE] 
					,[FERCO].[OP_WMS_FN_CONVERT_FROM_WEIGHT_MEASURE_UNIT_TO_DEFAULT]([M].[WEIGTH], [M].[WEIGHT_MEASUREMENT]) [MATERIAL_WEIGHT]
				...
				INNER JOIN [FERCO].[OP_WMS_COMPANY] [OWC] ON [OWC].[COMPANY_NAME] = [VAS].[OWNER]
															AND [OWC].[EXTERNAL_SOURCE_ID] = '
                + CAST(@EXTERNAL_SOURCE_ID AS VARCHAR)
                + '
				INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [OWC].[CLIENT_CODE] +'
      ...
      -- ------------------------------------------------------------------------------------
        ...
        LEFT JOIN [FERCO].[OP_WMS_VIEW_INVENTORY_FOR_PICKING] [A] ON [SOD].[SKU] = [A].[MATERIAL_ID]
                                                              AND [A].[CURRENT_WAREHOUSE] = @CODE_WAREHOUSE
        LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] ON [UMM].[MATERIAL_ID] = [SOD].[SKU]
