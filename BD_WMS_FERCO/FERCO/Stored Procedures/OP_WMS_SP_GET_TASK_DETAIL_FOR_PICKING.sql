...
-- Modificacion 10/6/2017 @ NEXUS-Team Sprint eFERCO
-- rodrigo.gomez
...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_TASK_DETAIL_FOR_PICKING]
					@WAVE_PICKING_ID=4883
					,@LOGIN=N'ADMIN'
					,@CLASS = '46' 
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TASK_DETAIL_FOR_PICKING]
    (
     @WAVE_PICKING_ID INT
    ,@LOGIN VARCHAR(25)
...
INSERT  INTO @TB_USERS
        ...
        EXEC [FERCO].[OP_WMS_SP_GET_OPERATORS_ASSIGNED_TO_DISTRIBUTION_CENTER_BY_USER] @LOGIN;
...
SELECT DISTINCT
   ...
   ,[A].FROM_MASTERPACK
FROM
    [FERCO].[OP_WMS_TASK_LIST] AS [A]
INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [M].[MATERIAL_ID] = [A].[MATERIAL_ID]
INNER JOIN [#CLASS] [C] ON [C].[CLASS_ID] = CAST([M].[MATERIAL_CLASS] AS INT)
INNER JOIN [FERCO].[OP_WMS_CLASS] [CL] ON [CL].[CLASS_ID] = [C].[CLASS_ID]
LEFT JOIN @TB_USERS [U] ON ([U].[LOGIN_ID] = [A].[TASK_ASSIGNEDTO])
LEFT JOIN [FERCO].[OP_WMS_POLIZA_HEADER] [PHT] ON (
                                                   [PHT].[DOC_ID] = [A].[DOC_ID_TARGET]
                                                   AND [PHT].[WAREHOUSE_REGIMEN] = [A].[REGIMEN]
                                                  )
LEFT JOIN [FERCO].[OP_WMS_WAREHOUSES] [W] ON [A].[WAREHOUSE_SOURCE] = [W].[WAREHOUSE_ID]
WHERE
