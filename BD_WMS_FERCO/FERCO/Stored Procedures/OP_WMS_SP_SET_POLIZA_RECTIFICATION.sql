...
/*
  EXEC [FERCO].OP_WMS_SP_SET_POLIZA_RECTIFICATION  
...
CREATE PROCEDURE FERCO.OP_WMS_SP_SET_POLIZA_RECTIFICATION @DOC_ID_RECTIFADA NUMERIC
...
BEGIN
  BEGIN TRY
    DECLARE @CODDIGO_POLIZA_RECTIFICACION AS VARCHAR(15)
    ...
    WHERE PH.DOC_ID = @DOC_ID_RECTIFICACION
    ...
    FROM [FERCO].OP_WMS_POLIZA_HEADER PH
    INNER JOIN [FERCO].OP_WMS_RECTIFICATION_HISTORY RH
      ON (PH.DOC_ID = RH.DOC_ID_RECTIFADA)
    WHERE PH.DOC_ID = @DOC_ID_RECTIFADA
    AND RH.DOC_ID_RECTIFICACION IS NULL

    PRINT ('Insertar en la tabla de historial')

    INSERT INTO [FERCO].OP_WMS_RECTIFICATION_HISTORY (DOC_ID_RECTIFADA
    ...
    , OPERADOR_RECTIFICADA)
      ...
      INNER JOIN [FERCO].OP_WMS_POLIZA_HEADER PH
        ON (L.CODIGO_POLIZA_RECTIFICACION = PH.CODIGO_POLIZA)
      WHERE PH.DOC_ID = @DOC_ID_RECTIFADA


    PRINT ('Actualiza la poliza de rectificacion')
    UPDATE [FERCO].OP_WMS_POLIZA_HEADER
    SET DOC_ID_RECTIFICACION = @DOC_ID_RECTIFADA
    ...
    UPDATE [FERCO].OP_WMS_POLIZA_HEADER
    SET CODIGO_POLIZA_RECTIFICACION = @CODDIGO_POLIZA_RECTIFICACION
    ...
    SET CODIGO_POLIZA_RECTIFICACION = NULL
    ...
    , VIN)
      ...
      WHERE CODIGO_POLIZA = @CODDIGO_POLIZA_RECTIFADA
