...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_RECEPTION_RETURN_BY_TASK]
					@TASK_ID = 518217
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_RECEPTION_RETURN_BY_TASK (@TASK_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  INSERT INTO @INV_ON_LICENSES
    ...
    LEFT JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IL]
      ON [IL].[LICENSE_ID] = [L].[LICENSE_ID]
        AND [RD].[MATERIAL_ID] = [IL].[MATERIAL_ID]
  ...
  LEFT JOIN @INV_ON_LICENSES [IL]
    ON [IL].[MATERIAL_ID] = [RD].[MATERIAL_ID]
