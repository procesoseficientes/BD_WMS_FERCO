...
-- Modificacion 8/17/2020
-- gustavo.garcia
-- Se cambia el estatus de la tabla temporal
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_MARK_PICKING_AS_SEND_TO_ERP_ADMIN_TASK]
    (
...
BEGIN
  ...
  --
    ...
    --
		...
		DECLARE @EXECQUERY VARCHAR(250) = '
		UPDATE [SAPSERVER].SBOPruebas.dbo.odrf
		SET
			u_estado2 = ''01'', DocStatus   = ''C''
		WHERE DocNum = ' + CAST(@DOC_NUM AS VARCHAR(250))
		PRINT(@EXECQUERY)
		EXEC(@EXECQUERY)
