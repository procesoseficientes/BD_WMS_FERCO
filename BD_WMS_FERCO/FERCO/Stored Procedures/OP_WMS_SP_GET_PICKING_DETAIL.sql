...
-- Modificacion 10/19/2017 @ NEXUS-Team Sprint enave
-- rodrigo.gomez
...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_PICKING_DETAIL] @PICKING_HEADER_ID = '6371|6372|6373|6374' 
			--
			EXEC [FERCO].[OP_WMS_SP_GET_PICKING_DETAIL] @PICKING_HEADER_ID = '67500'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_PICKING_DETAIL] (
...
BEGIN
  ...
  --
	...
	FROM
		[FERCO].[OP_WMS_MANIFEST_DETAIL] [D]
	INNER JOIN [FERCO].[OP_WMS_MANIFEST_HEADER] [H] ON [H].[MANIFEST_HEADER_ID] = [D].[MANIFEST_HEADER_ID]
	WHERE
		[H].[STATUS] <> ''CANCELED'';
    --
		...
		SELECT DISTINCT ISNULL([CMP].[COMPONENT_MATERIAL],
				...
				[FERCO].[OP_WMS_FN_CONVERT_FROM_WEIGHT_MEASURE_UNIT_TO_DEFAULT](ISNULL(( ( ISNULL(MC.[WEIGTH],[M].[WEIGTH]) ) * ( CASE WHEN [MD].[QTY_PENDING_DELIVERY] > 0
				THEN [MD].[QTY_PENDING_DELIVERY]
				ELSE  ISNULL( [CMP].[QTY] , 1 ) * [DD].[QTY] 
		...
		FROM    [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [DD]
				INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH] ON [DH].[PICKING_DEMAND_HEADER_ID] = [DD].[PICKING_DEMAND_HEADER_ID] 
			
	
				INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([M].[MATERIAL_ID] = [DD].[MATERIAL_ID] AND M.MATERIAL_ID > '''' )
				LEFT JOIN [FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK] [CMP] ON [CMP].[MASTER_PACK_CODE] = [M].[MATERIAL_ID] AND M.IS_MASTER_PACK = 1
				LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [MC] ON [CMP].[COMPONENT_MATERIAL] = [MC].[MATERIAL_ID]
