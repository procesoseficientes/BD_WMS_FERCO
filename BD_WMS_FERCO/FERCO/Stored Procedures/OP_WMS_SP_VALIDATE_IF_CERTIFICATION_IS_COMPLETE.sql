...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].OP_WMS_SP_VALIDATE_IF_CERTIFICATION_IS_COMPLETE @CERTIFICATION_HEADER_ID = 1
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_VALIDATE_IF_CERTIFICATION_IS_COMPLETE (@CERTIFICATION_HEADER_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  BEGIN TRY
    ...
    INNER JOIN [FERCO].[OP_WMS_PICKING_LABEL_BY_MANIFEST] [PLB]
      ON [PLB].[MANIFEST_DETAIL_ID] = [MD].[MANIFEST_DETAIL_ID]
    INNER JOIN [FERCO].[OP_WMS_CERTIFICATION_HEADER] [CH]
    ...
    FROM [FERCO].[OP_WMS_CERTIFICATION_DETAIL] [CD]
    INNER JOIN [FERCO].[OP_WMS_CERTIFICATION_HEADER] [CH]
