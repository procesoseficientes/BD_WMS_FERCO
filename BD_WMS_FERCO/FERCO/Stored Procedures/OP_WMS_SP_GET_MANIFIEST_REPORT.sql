...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_GET_MANIFIEST_REPORT] @MANIFEST_HEADER_ID = 1198
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_MANIFIEST_REPORT] (
...
BEGIN
  ...
  --
	...
	INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH] ON ([MD].[PICKING_DEMAND_HEADER_ID] = [DH].[PICKING_DEMAND_HEADER_ID])
	INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [DD] ON (
											[DH].[PICKING_DEMAND_HEADER_ID] = [DD].[PICKING_DEMAND_HEADER_ID]
											AND [MD].[MATERIAL_ID] = [DD].[MATERIAL_ID]
											AND [MD].[LINE_NUM] = [DD].[LINE_NUM]
											)
	LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [MD].[MATERIAL_ID] = [M].[MATERIAL_ID]
	LEFT JOIN [FERCO].[OP_WMS_VIEW_CLIENTS] [C] ON [MD].[CLIENT_CODE] = [C].[CLIENT_CODE]
	LEFT JOIN [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER] [T] ON [T].[TRANSFER_REQUEST_ID] = [DH].[TRANSFER_REQUEST_ID]
	LEFT JOIN [FERCO].[OP_WMS_PILOT] [P] ON [P].[PILOT_CODE] = [MH].[DRIVER]
	WHERE
