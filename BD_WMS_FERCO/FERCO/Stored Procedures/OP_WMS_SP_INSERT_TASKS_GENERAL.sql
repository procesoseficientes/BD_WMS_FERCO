...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN
				SELECT
					@ASSEMBLY_QTY = [FERCO].[OP_WMS_FN_GET_AVAILABLE_INVENTORY_TO_ASSAMBLE_FOR_MASTERPACK](@MATERIAL_ID,
											@WAREHOUSE);
		...
		IF @SEND_ERP = 1
			AND NOT EXISTS ( SELECT TOP 1
									1
								FROM
									[FERCO].[OP_WMS_PICKING_ERP_DOCUMENT]
								WHERE
									[WAVE_PICKING_ID] = @WAVE_PICKING_ID )
		BEGIN
			INSERT	INTO [FERCO].[OP_WMS_PICKING_ERP_DOCUMENT]
					(
						[WAVE_PICKING_ID]
						,[CODE_WAREHOUSE]
		...
		BEGIN
			...
			BEGIN
				...
				EXEC [FERCO].[OP_WMS_SP_INSERT_TASKS_GENERAL] @TASK_OWNER = @TASK_OWNER,                     -- varchar(25)
