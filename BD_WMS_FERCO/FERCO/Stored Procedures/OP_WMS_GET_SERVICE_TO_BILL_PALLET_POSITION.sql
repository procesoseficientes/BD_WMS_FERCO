...
-- Ejemplo de Ejecucion:
      EXEC [FERCO].OP_WMS_GET_SERVICE_TO_BILL_PALLET_POSITION 
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_SERVICE_TO_BILL_PALLET_POSITION] (
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
	...
	SELECT
		...
		,[C].[BILLING_FRECUENCY]
	FROM
		[FERCO].[OP_WMS_FN_GET_CUSTOMERS_TO_BILL_BY_TRADE_AGREEMENT](1,
											@TYPE,
											@PROCESS_DATE,
											@SERVICE_CODE, 0) [C];
  ...
  -- ------------------------------------------------------------------------------------
	SELECT
		...
		,[C].[UNIT_PRICE]
		* [FERCO].[OP_WMS_FN_GET_SERVICE_TO_BILL_PERCENTAGE](@TYPE,
											@PROCESS_DATE,
		...
		* COUNT([PBC].[CURRENT_WAREHOUSE]) [TOTAL_AMOUNT]
		...
		,CASE CAST([FERCO].[OP_WMS_FN_GET_SERVICE_TO_BILL_PERCENTAGE](@TYPE,
											@PROCESS_DATE,
											MIN([PBC].[LAST_UPDATED]),
  ...
  -- ------------------------------------------------------------------------------------
	...
	FROM
		[#PRE_RESULT] [R]
	INNER JOIN [FERCO].[OP_WMS_SERVICES_TO_BILL] [S] ON (
	...
	WHERE
		...
		AND [S].[PROCESS_DATE] > [FERCO].[OP_WMS_FN_GET_FIRST_DAY_OF_MONTH]([R].[PROCESS_DATE])
		AND [S].[IS_CHARGED] = 1;
