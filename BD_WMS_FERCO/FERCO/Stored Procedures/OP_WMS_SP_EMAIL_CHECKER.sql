...
CREATE PROCEDURE FERCO.OP_WMS_SP_EMAIL_CHECKER @CLIENT_CODE NVARCHAR(15),
...
BEGIN TRY
  ...
  SELECT
    @FACTOR = FACTOR3
  FROM [FERCO].[OP_WMS_VIEW_CLIENT_FACTORS]
  WHERE CLIENT_CODE = @CLIENT_CODE
  AND TASK_LIST_ID = @TASK_LIST_ID
  SELECT
    @pFactor = @FACTOR
  ...
  BEGIN
    ...
    BEGIN
      ...
      SET [IS_lOCKED] = '1'
      WHERE [WAVE_PICKING_ID] = @TASK_LIST_ID
      IF @@ERROR = 0
      BEGIN
        SELECT
          @pResult = 'PICKING DESBLOQUEADO, PERMITE PASAR'
      END
      ELSE
      BEGIN
        SELECT
    ...
    BEGIN
      ...
      SET [IS_lOCKED] = '0'
      WHERE [IS_lOCKED] = '1'
      AND [WAVE_PICKING_ID] = @TASK_LIST_ID
      IF @@ERROR = 0
      BEGIN
        SELECT
          @pResult = 'QUEDA MENOS DEL 30% DE MERCADERIA'
      END
      ELSE
      BEGIN
        SELECT
  ...
  BEGIN
    ...
    BEGIN
      SELECT
        @pResult = 'PICKING BLOQUEADO, ENVIO DE CORREO'
    END
    ELSE
    BEGIN
      SELECT
  ...
  BEGIN
    ...
    BEGIN
      SELECT
        @pResult = '0 DEUDA, PICKING DESBLOQUEADO'
    END
    ELSE
    BEGIN
      SELECT
