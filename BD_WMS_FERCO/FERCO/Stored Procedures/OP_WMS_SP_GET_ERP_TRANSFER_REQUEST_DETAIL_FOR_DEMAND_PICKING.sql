...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_ERP_TRANSFER_REQUEST_DETAIL_FOR_DEMAND_PICKING] ( @XML XML )
AS
    BEGIN
        SET NOCOUNT ON;
	--
	...
	--
		...
		-- ------------------------------------------------------------------------------------
            ...
            INSERT  [#PICKING_DEMAND_DETAIL]
                    SELECT  [H].[DOC_NUM] ,
                            ...
                            [H].[OWNER] ,
                            [D].[LINE_NUM] [LINE_SEQ]
                    FROM    [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [D] WITH (NOLOCK)
                            INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [H] WITH (NOLOCK) ON [D].[PICKING_DEMAND_HEADER_ID] = [H].[PICKING_DEMAND_HEADER_ID]
			...
			SELECT
				@QUERY = N'INSERT INTO [#PICKING_DOCUMENT_DETAIL]
					...
					FROM ' + @DATA_BASE_NAME + N'.' + @SCHEMA_NAME
                            + '.[ERP_VW_TRANSFER_REQUEST_DETAIL] [TRD] WITH (NOLOCK)
					...
					INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] WITH (NOLOCK) ON ([M].[MATERIAL_ID] COLLATE DATABASE_DEFAULT = [TRD].[MATERIAL_ID] COLLATE DATABASE_DEFAULT)
     ...
     --       AND [DH].[SOURCE_TYPE] = ''WT - ERP''
					...
					--)
					LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] WITH (NOLOCK) ON (
      ...
      -- ------------------------------------------------------------------------------------
		...
		-- ------------------------------------------------------------------------------------
            SELECT  [PDD].[SALES_ORDER_ID] ,
                    ...
                    [PDD].[SOURCE] ,
					[PDD].[MATERIAL_WEIGHT] ,
					[PDD].[MATERIAL_WEIGHT]*[PDD].[QTY] AS TOTAL_WEIGHT,
            ...
            FROM    [#PICKING_DOCUMENT_DETAIL] [PDD]
                    LEFT JOIN [FERCO].[OP_WMS_UNIT_MEASUREMENT_BY_MATERIAL] [UMM] WITH (NOLOCK) ON [UMM].[MATERIAL_ID] COLLATE DATABASE_DEFAULT = [PDD].[SKU] COLLATE DATABASE_DEFAULT
