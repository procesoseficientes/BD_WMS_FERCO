...
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_OCUPPACY_LEVEL]
...
BEGIN
	...
	SELECT @pWAREHOUSE_PARENT = (SELECT WAREHOUSE_PARENT FROM [FERCO].OP_WMS_SHELF_SPOTS 
				WHERE LOCATION_SPOT = @pSOURCE_LOCATION)
	
	SELECT @pZONE = (SELECT ZONE FROM [FERCO].OP_WMS_SHELF_SPOTS 
				WHERE LOCATION_SPOT = @pSOURCE_LOCATION)

	SELECT @pSPOT_TYPE = (SELECT SPOT_TYPE FROM [FERCO].OP_WMS_SHELF_SPOTS 
				WHERE LOCATION_SPOT = @pSOURCE_LOCATION)
	...
	IF (@pOCCUPANCY_LEVEL = 0) AND 
		EXISTS(SELECT 1 FROM [FERCO].OP_WMS_VIEW_INVENTORY_DETAIL WHERE CURRENT_LOCATION = @pSOURCE_LOCATION AND QTY >0 )
	BEGIN
	...
	--	NOT EXISTS(SELECT 1 FROM FERCO.OP_WMS_VIEW_INVENTORY_DETAIL WHERE CURRENT_LOCATION = @pSOURCE_LOCATION  AND QTY >0)
	--BEGIN
	...
	IF NOT EXISTS(
		SELECT 1 FROM [FERCO].OP_WMS_OCCUPANCY_LEVEL
		WHERE  INPUT_DATE = @pINPUT_DATE AND LOCATION_SPOT= UPPER(@pSOURCE_LOCATION)) 
	BEGIN
		INSERT INTO [FERCO].[OP_WMS_OCCUPANCY_LEVEL]
		   ([INPUT_DATE]
		   ,[LOCATION_SPOT]
		   ,[WAREHOUSE_PARENT]
		   ,[SPOT_TYPE]
	...
	ELSE 
		BEGIN
			...
			UPDATE [FERCO].OP_WMS_OCCUPANCY_LEVEL 
