...
-- Ejemplo de Ejecucion:
                EXEC [FERCO].[OP_WMS_SP_ROLLBACK_IMPLOSION_IN_PROGRESS]  @LICENSE_ID = 378262 , @MASTER_PACK_CODE = 'autovanguard/VAD1001', @LOGIN = 'ADMIN'
                SELECT * FROM [FERCO].[OP_WMS_MASTER_PACK_HEADER] [H] WHERE     [H].[IS_IMPLOSION] = 1
                SELECT * FROM [FERCO].[OP_WMS_TASK_LIST]
                    WHERE [LICENSE_ID_TARGET] IS NOT NULL
                SELECT * FROM [FERCO].OP_WMS_LICENSES WHERE LICENSE_ID = 378262
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_ROLLBACK_IMPLOSION_IN_PROGRESS (@LICENSE_ID NUMERIC
...
BEGIN
  ...
  BEGIN TRY
    ...
    FROM [FERCO].[OP_WMS_MASTER_PACK_HEADER] [H]
    ...
    DELETE [FERCO].[OP_WMS_MASTER_PACK_DETAIL]
    WHERE [MASTER_PACK_HEADER_ID] = @MASTER_PACK_HEADER_ID

    DELETE [FERCO].[OP_WMS_MASTER_PACK_HEADER]
    WHERE [MASTER_PACK_HEADER_ID] = @MASTER_PACK_HEADER_ID
