...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_GET_SERVICE_TO_BILL_MT2] @PROCESS_DATE = '2017-03-29 00:00:000'
  ...
  --B04-TA-C08-NU	1	MT2	Metros cuadrados en piso	GENERAL	49362
    ...
    SELECT
     *
    FROM [FERCO].[OP_WMS_FN_GET_CUSTOMERS_TO_BILL_BY_TRADE_AGREEMENT](1, 'ON_DEMAND', '2017-03-29 00:00:000', 'MT2', 0) [C]

*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_GET_SERVICE_TO_BILL_MT2 (@PROCESS_DATE DATETIME
...
BEGIN
  ...
  INSERT INTO [#CLIENT_TO_BILL] ([CLIENT_CODE], [CLIENT_NAME], [ACUERDO_COMERCIAL], [TYPE_CHARGE_ID], [TYPE_CHARGE_DESCRIPTION], [SERVICE_ID], [SERVICE_CODE], [SERVICE_DESCRIPTION], [UNIT_PRICE], [REGIMEN], [BILLING_FRECUENCY])
    SELECT
     ...
     ,[C].[REGIMEN]
     ,[C].[BILLING_FRECUENCY]
    FROM [FERCO].[OP_WMS_FN_GET_CUSTOMERS_TO_BILL_BY_TRADE_AGREEMENT](1, @TYPE, @PROCESS_DATE, 'MT2', 0) [C]
  -- 

  SELECT
    [L].[USED_MT2] [QTY]
   ,[C].[SERVICE_CODE] [TRANSACTION_TYPE]
   ,[C].[UNIT_PRICE] * [FERCO].[OP_WMS_FN_GET_SERVICE_TO_BILL_PERCENTAGE](@TYPE, @PROCESS_DATE, [L].[LAST_UPDATED], [C].[BILLING_FRECUENCY]) [PRICE]
   ,[C].[UNIT_PRICE] * [L].[USED_MT2] * [FERCO].[OP_WMS_FN_GET_SERVICE_TO_BILL_PERCENTAGE](@TYPE, @PROCESS_DATE, [L].[LAST_UPDATED], [C].[BILLING_FRECUENCY]) [TOTAL_AMOUNT]
   ...
   ,CASE CAST([FERCO].[OP_WMS_FN_GET_SERVICE_TO_BILL_PERCENTAGE](@TYPE, @PROCESS_DATE, [L].[LAST_UPDATED], [C].[BILLING_FRECUENCY]) AS INT)
      WHEN 1 THEN 0
      ELSE 1
    END [HAS_ADJUST]
   ,[C].[BILLING_FRECUENCY]
  ...
  DELETE [R]
    FROM [#PRE_RESULT] [R]
    INNER JOIN [FERCO].[OP_WMS_SERVICES_TO_BILL] [S]
  ...
  WHERE [R].[BILLING_FRECUENCY] = 15
    ...
    AND [S].[PROCESS_DATE] BETWEEN [FERCO].[OP_WMS_FN_GET_FIRST_DAY_OF_MONTH]([R].[PROCESS_DATE])
    AND [FERCO].[OP_WMS_FN_GET_LAST_DAY_OF_FIRST_FORTNIGHT]([R].[PROCESS_DATE])
  ...
  WHERE [R].[BILLING_FRECUENCY] = 15
    AND DATEPART(DAY, [R].[PROCESS_DATE]) > 15
    AND [S].[PROCESS_DATE] BETWEEN [FERCO].[OP_WMS_FN_GET_LAST_DAY_OF_FIRST_FORTNIGHT]([R].[PROCESS_DATE])
    AND [FERCO].[OP_WMS_FN_GET_LAST_DAY_OF_MONTH]([R].[PROCESS_DATE])
    AND [S].[IS_CHARGED] = 1
