...
-- Ejemplo de Ejecucion:
			EXEC  [FERCO].[OP_WMS_SP_MARK_RECEPTION_AS_SEND_TO_ERP]
				@RECEPTION_DOCUMENT_ID = 4045
				,@POSTED_RESPONSE = 'Exito al guardar en sap'
				,@ERP_REFERENCE = '3088'
				--
			select * from [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] where [ERP_RECEPTION_DOCUMENT_HEADER_ID] = 4045
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_MARK_RECEPTION_AS_SEND_TO_ERP]
    (
...
BEGIN
  ...
  --
    ...
    -- ------------------------------------------------------------------------------------
        ...
        LEFT JOIN [FERCO].[OP_WMS_WAREHOUSES] [WH] ON [H].[ERP_WAREHOUSE_CODE] = [WH].[ERP_WAREHOUSE]
        LEFT JOIN [FERCO].[OP_WMS_MATERIAL_PROPERTY_BY_WAREHOUSE] [MW] ON (
