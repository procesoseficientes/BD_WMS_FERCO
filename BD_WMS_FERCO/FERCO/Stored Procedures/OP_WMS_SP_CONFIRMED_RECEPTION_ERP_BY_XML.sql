...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_CONFIRMED_RECEPTION_ERP_BY_XML]
					

*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_CONFIRMED_RECEPTION_ERP_BY_XML] (
		@XML XML
		,@XML_SERIES XML
		,@LOGIN VARCHAR(50)
		,@TASK_ID INT
...
BEGIN
	...
	BEGIN TRY
		...
		FROM
			...
			AS [x] ([Rec])
		INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [x].[Rec].[query]('./MATERIAL_ID').[value]('.',
		...
		INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [RDD] ON [RDD].[ERP_RECEPTION_DOCUMENT_DETAIL_ID] = [DRE].[ERP_RECEPTION_DOCUMENT_DETAIL_ID];
		...
		-- ------------------------------------------------------------------------------------
			MERGE [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL]
				AS [RD]
			USING
				(SELECT
						[ERP_RECEPTION_DOCUMENT_DETAIL_ID]
		...
		-- ------------------------------------------------------------------------------------
			INSERT	INTO @OPERACION
					EXEC [FERCO].[OP_WMS_SP_AUTHORIZE_ERP_RECEPTION_DOCUMENT] @ERP_RECEPTION_DOCUMENT_HEADER_ID = @RECEPTION_HEADER_ID, -- int
		...
		-- ------------------------------------------------------------------------------------
			IF EXISTS ( SELECT TOP 1
							1
						FROM
							[FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL]
						WHERE
							(
								[IS_CONFIRMED] = 0
