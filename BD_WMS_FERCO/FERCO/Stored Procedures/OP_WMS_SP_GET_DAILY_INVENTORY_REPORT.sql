...
-- Ejemplo de Ejecucion:
        exec [FERCO].[OP_WMS_SP_GET_DAILY_INVENTORY_REPORT] @REPORT_DATETIME = '2017-11-16 14:38:02.607', @WAREHOUSE = 'BODEGA_01|BODEGA_02', @LOGIN = 'PABS'
		SELECT * 	FROM
		[FERCO].[OP_WMS_TRANS] [T]
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_DAILY_INVENTORY_REPORT (@REPORT_DATETIME DATETIME
...
BEGIN
  ...
  INSERT INTO @WAREHOUSES ([WAREHOUSE_ID])
    ...
    FROM [FERCO].[OP_WMS_FN_SPLIT](@WAREHOUSE, @DELIMITER) [W];
  -- ------------------------------------------------------------------------------------
  -- Despachos
  -- ------------------------------------------------------------------------------------
  INSERT INTO @TRANSACTIONS ([MATERIAL_ID]
  , [WAREHOUSE]
  , [QTY_INCOME]
  , [QTY_OUTPUT]
  , [TYPE])
    ...
    INNER JOIN @WAREHOUSES [W]
      ON [T].[SOURCE_WAREHOUSE] = [W].[WAREHOUSE_ID]
    WHERE [T].[TRANS_TYPE] IN ('DESPACHO_ALMGEN',
    'DESPACHO_FISCAL',
    'DESPACHO_GENERAL',
    'EXPLODE_OUT')
  ...
  , [TYPE])
    ...
    FROM [FERCO].[OP_WMS_TRANS] [T]
    INNER JOIN @WAREHOUSES [W]
      ON [T].[TARGET_WAREHOUSE] = [W].[WAREHOUSE_ID]
    WHERE [T].[TRANS_TYPE] IN ('INGRESO_FISCAL',
    'INGRESO_GENERAL',
  ...
  , [TYPE])
    SELECT
     ...
     ,[W].[WAREHOUSE_ID]
     ,SUM(CASE
        WHEN [W].[WAREHOUSE_ID] = [T].[TARGET_WAREHOUSE] THEN [T].[QUANTITY_UNITS]
        ELSE 0
      END) [QTY_INCOME]
     ,SUM(CASE
        WHEN [W].[WAREHOUSE_ID] = [T].[SOURCE_WAREHOUSE] THEN [T].[QUANTITY_UNITS] * -1
        ELSE 0
      END) [QTY_OUTPUT]
     ,'REUBICACION PARCIAL' [TYPE]
    FROM [FERCO].[OP_WMS_TRANS] [T]
    INNER JOIN [FERCO].[OP_WMS_WAREHOUSES] [W]
      ON [W].[WAREHOUSE_ID] = [T].[SOURCE_WAREHOUSE]
        OR [W].[WAREHOUSE_ID] = [T].[TARGET_WAREHOUSE]
    INNER JOIN @WAREHOUSES [WP]
  ...
  , [TYPE])
    ...
    INNER JOIN @WAREHOUSES [WP]
      ON [W].[WAREHOUSE_ID] = [WP].[WAREHOUSE_ID]
    WHERE [T].[TRANS_TYPE] = 'REUBICACION_COMPLETA'
    AND CONVERT(DATE, [T].[TRANS_DATE]) = CONVERT(DATE, @REPORT_DATETIME)
    AND [T].[SOURCE_WAREHOUSE] <> [T].[TARGET_WAREHOUSE]
    AND [T].[STATUS] = 'PROCESSED'
    AND [T].[MATERIAL_CODE] <> ''
  ...
  SELECT
   ...
   ,SUM([QTY_OUTPUT]) [QTY_OUTPUT] INTO [#RESUME_TRANSACTIONS]
  FROM @TRANSACTIONS [T]
  ...
  SELECT
   ...
   ,[S].[WAREHOUSE_PARENT] [WAREHOUSE] INTO [#INVENTORY]
  FROM [FERCO].[OP_WMS_INV_HISTORY] [I]
  INNER JOIN [FERCO].[OP_WMS_SHELF_SPOTS] [S]
    ON [I].[CURRENT_LOCATION] = [S].[LOCATION_SPOT]
  INNER JOIN @WAREHOUSES [W]
    ON [W].[WAREHOUSE_ID] = [S].[WAREHOUSE_PARENT]
  WHERE CONVERT(DATE, [I].[SNAPSHOT_DATE]) = CONVERT(DATE, DATEADD(DAY,
  -1,
  @REPORT_DATETIME))
  GROUP BY [S].[WAREHOUSE_PARENT]
          ,[I].[MATERIAL_ID];
  ...
  SELECT
   ...
   ,[M].[IS_MASTER_PACK]
  FROM [#INVENTORY] [I]
  FULL OUTER JOIN [#RESUME_TRANSACTIONS] [T]
    ON [T].[MATERIAL_ID] = [I].[MATERIAL_ID]
      AND [T].[WAREHOUSE] = [I].[WAREHOUSE]
  LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [M]
    ON ISNULL([I].[MATERIAL_ID],
      [T].[MATERIAL_ID]) = [M].[MATERIAL_ID];



  INSERT INTO [FERCO].[OP_WMS_LOG_REPORT] ([LOG_DATETIME]
  , [REPORT_NAME]
  , [PARAMETER_LOGIN]
  , [PARAMETER_WAREHOUSE]
