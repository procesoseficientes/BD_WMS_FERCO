...
-- Ejemplo de Ejecucion:				
				--
				exec [FERCO].[OP_WMS_SP_UPDATE_OR_INSERT_OP_WMS_INV_X_LICENSE_EXT] 
						@CUSTOMER ='C00330'
						,@USER  ='ADMIN'
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_UPDATE_OR_INSERT_OP_WMS_INV_X_LICENSE_EXT]
  ...
  -- BEGIN
	...
	USING
		(SELECT
				*
			FROM
				[FERCO].[OP_WMS_CHARGE_EXTERNAL_INVENTORY])
		AS [EXT]
	...
	WHEN NOT MATCHED THEN
		...
		VALUES	(
					...
					,(SELECT
							*
						FROM
							[FERCO].[OP_WMS_FUNC_GETLOGIN_NAME](@USER))
					,'INGRESO_GENERAL'
					,ISNULL((SELECT
									*
								FROM
									[FERCO].[OP_WMS_FUNC_GETTRANS_DESC]('INGRESO_GENERAL')),
							'INGRESO_GENERAL')
					,'N/A'
					,[EXT].[CODIGO]
					,@CUSTOMER + '/' + [EXT].[CODIGO]
					,[EXT].[DESCRIPCION]
					,NULL
					,[FERCO].[OP_WMS_FN_GET_MATERIAL_COST_BY_MATERIAL](@CUSTOMER + '/' + [EXT].[CODIGO],
											@CUSTOMER)
					,@LICENSE_ID
					,@LICENSE_ID
					,@LOCATION
					...
					,(SELECT
							*
						FROM
							[FERCO].[OP_WMS_FUNC_GETCLIENT_NAME](@CUSTOMER))
					,[EXT].[QTY]
					,@WAREHOUSE
					,@WAREHOUSE
					,''
