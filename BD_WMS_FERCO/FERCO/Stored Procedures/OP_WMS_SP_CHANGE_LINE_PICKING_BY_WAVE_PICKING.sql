...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_CHANGE_LINE_PICKING_BY_WAVE_PICKING]
					@WAVE_PICKING_ID = 4894
					,@PICKING_LINE_ID = 'LINEA_PICKING_1'
					,@LOGIN = 'ADMIN'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_CHANGE_LINE_PICKING_BY_WAVE_PICKING](
	@WAVE_PICKING_ID INT
...
BEGIN
	...
	BEGIN TRY
		...
		-- ------------------------------------------------------------------------------------
		IF EXISTS(SELECT TOP 1 1 FROM [FERCO].[OP_WMS_TASK_LIST] WHERE ([IS_ACCEPTED] = 1 OR [IS_CANCELED] = 1 OR [IS_COMPLETED] = 1) AND [WAVE_PICKING_ID] = @WAVE_PICKING_ID)
		BEGIN
