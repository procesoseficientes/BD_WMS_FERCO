...
-- Modificacion 04-Oct-17 @ Nexus Team Sprint eFERCO
-- pablo.aguilar
...
-- Ejemplo de Ejecucion:
	EXEC [FERCO].[OP_WMS_SP_INSERT_TASKS_GENERAL_PICKING_DEMAND] 
...
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_INSERT_TASKS_GENERAL_PICKING_DEMAND] (
	...
	BEGIN TRY
		...
		BEGIN
			...
			SELECT
				...
				,[IP].[CURRENT_LOCATION]
			FROM
				[FERCO].[OP_WMS_FN_GET_INVENTORY_FROM_PROYECT](@PROJECT_ID) [IP]
			WHERE
		...
		FROM
			[FERCO].[OP_WMS_VIEW_INVENTORY_FOR_PICKING]
		WHERE
			[MATERIAL_ID] = @MATERIAL_ID
			AND [CURRENT_WAREHOUSE] = @CODE_WAREHOUSE;

		print 'avav inv: ' + CAST(@AVAILABLE_QTY AS VARCHAR)
		print ''
		print 'avav inv: ' + CAST(@QUANTITY_ASSIGNED AS VARCHAR)
		print ''
		...
		PRINT '@AVAILABLE_QTY: ' + CAST (@AVAILABLE_QTY AS VARCHAR)
		PRINT '@IS_MASTER_PACK: ' + CAST (@IS_MASTER_PACK AS VARCHAR)
		PRINT '@FROM_MASTERPACK: ' + CAST (@FROM_MASTERPACK AS VARCHAR)
		...
		BEGIN
			...
			BEGIN
				SELECT
					@ASSEMBLY_QTY = [FERCO].[OP_WMS_FN_GET_AVAILABLE_INVENTORY_FOR_MASTERPACK_ON_DISPATCH_DEMAND](@MATERIAL_ID,
											@CODE_WAREHOUSE,
											@STATUS_CODE);


				PRINT '@ASSEMBLY_QTY: ' + CAST (@ASSEMBLY_QTY AS VARCHAR)
		...
		BEGIN
			...
			SELECT
				...
				,1
			FROM
				[FERCO].[OP_WMS_FN_GET_LICENSE_TO_PICK_FOR_PROYECT](@MATERIAL_ID,
											@CODE_WAREHOUSE,
											@QUANTITY_ASSIGNED,
											@HAVBATCH, NULL,
											@PICKING_HEADER_ID,
			...
			SELECT
				...
				,2
			FROM
				[FERCO].[OP_WMS_FN_GET_LICENSE_TO_PICK](@MATERIAL_ID,
											@CODE_WAREHOUSE,
											@QUANTITY_ASSIGNED,
											@HAVBATCH, NULL,
											@PICKING_HEADER_ID,
		...
		BEGIN
			...
			INTO
				[#COMPONENTES]
			FROM
				[FERCO].[OP_WMS_COMPONENTS_BY_MASTER_PACK]
			WHERE
			...
			BEGIN
				...
				FROM
					[#COMPONENTES] [C]
				INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [C].[COMPONENT_MATERIAL] = [M].[MATERIAL_ID];
