...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_PREPARED_SALES_ORDER_TO_DISPATCH_ERP]
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_PREPARED_SALES_ORDER_TO_DISPATCH_ERP]
	(
...
AS
	...
	-- ------------------------------------------------------------------------------------
		...
		INTO
			[#CLIENT]
		FROM
			[FERCO].[OP_WMS_FN_SPLIT](@CLIENTS ,@DELIMITER) [C];
	...
	-- ------------------------------------------------------------------------------------
		...
		INNER JOIN [#CLIENT] [C]
		ON	[C].[CLIENT_ID] = [PDH].[CLIENT_CODE]
		...
		WHERE
			...
			GROUP BY [PDH].[PICKING_DEMAND_HEADER_ID]
			, [PDH].[DOC_NUM]
			,[PDH].[DOC_ENTRY]
