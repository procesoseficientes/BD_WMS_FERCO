...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_INVENTORY_BY_MATERIAL_AND_LICENSE]
...
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_INVENTORY_BY_MATERIAL_AND_LICENSE (@LICENSE_ID_SOURCE NUMERIC(18, 0)
...
BEGIN
  ...
  BEGIN TRY
    ...
    SELECT
      ...
      END
    FROM [FERCO].[OP_WMS_MASTER_PACK_HEADER] [PH]
    INNER JOIN [FERCO].[OP_WMS_MASTER_PACK_DETAIL] [PD]
      ON ([PD].[MASTER_PACK_HEADER_ID] = [PH].[MASTER_PACK_HEADER_ID])
    LEFT JOIN [FERCO].[OP_WMS_TASK_LIST] [T]
      ON (
    ...
    LEFT JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [I]
      ON (
