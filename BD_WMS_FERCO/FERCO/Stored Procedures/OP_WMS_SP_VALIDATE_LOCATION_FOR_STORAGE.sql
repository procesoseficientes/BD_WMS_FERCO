...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_VALIDATE_LOCATION_FOR_STORAGE] 
				@LOGIN = 'ACAMACHO'
...
--*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_VALIDATE_LOCATION_FOR_STORAGE] (
		@LOGIN VARCHAR(50)
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
	INSERT	INTO @WAREHOUSE
			EXEC [FERCO].[OP_WMS_SP_GET_WAREHOUSE_BY_USER_CD] @LOGIN = @LOGIN;
  ...
  -- ------------------------------------------------------------------------------------
	...
	INNER JOIN [FERCO].[OP_WMS_TRANSFER_REQUEST_HEADER] [TH] ON ([TH].[TRANSFER_REQUEST_ID] = [T].[TRANSFER_REQUEST_ID])
	WHERE
    ...
    -- ------------------------------------------------------------------------------------
		...
		FROM
			[FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [H]
		WHERE
			[H].[TASK_ID] = @TASK_ID
  ...
  -- ------------------------------------------------------------------------------------
		...
		SELECT
			...
			,[PRIORITY]
		FROM
			[FERCO].[OP_WMS_FN_GET_CLASSES_BY_LICENSE](@LICENSE_ID);
  ...
  -- ------------------------------------------------------------------------------------
		...
		SELECT
			...
			,[PRIORITY]
		FROM
			[FERCO].[OP_WMS_FN_GET_CLASSES_BY_LOCATION](@LOCATION_SPOT);
  ...
  -- ------------------------------------------------------------------------------------
	  ...
	  --
		...
		--
			...
			LEFT JOIN [FERCO].[OP_WMS_CLASS_ASSOCIATION] [CA] ON [CC].[CLASS_ID] = [CA].[CLASS_ASSOCIATED_ID]
  ...
  -- ------------------------------------------------------------------------------------
		...
		SELECT
			...
			,0
		FROM
			[FERCO].[OP_WMS_FN_GET_SUB_CLASSES_BY_LICENSE](@LICENSE_ID);
  ...
  -- ------------------------------------------------------------------------------------
		...
		SELECT
			...
			,0
		FROM
			[FERCO].[OP_WMS_FN_GET_SUB_CLASSES_BY_LOCATION](@LOCATION_SPOT);
	END
  -- ------------------------------------------------------------------------------------
  ...
  -- ------------------------------------------------------------------------------------
	...
	INNER JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [ML] ON [ML].[STATUS_ID] = [IL].[STATUS_ID]
	WHERE
  ...
  -- ------------------------------------------------------------------------------------
    ...
    -- ------------------------------------------------------------------------------------
		...
		INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [L].[MATERIAL_ID] = [M].[MATERIAL_ID]
		INNER JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [SML] ON [SML].[STATUS_ID] = [L].[STATUS_ID]
		LEFT JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCM] ON [L].[TONE_AND_CALIBER_ID] = [TCM].[TONE_AND_CALIBER_ID]
		WHERE
      ...
      -- ------------------------------------------------------------------------------------
			IF @HANDLE_TONE = 1
				AND EXISTS ( SELECT TOP 1
								...
								INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [L].[LICENSE_ID] = [IL].[LICENSE_ID]
								INNER JOIN [FERCO].[OP_WMS_SHELF_SPOTS] [S] ON [S].[LOCATION_SPOT] = [L].[CURRENT_LOCATION]
								INNER JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [TCM] ON [TCM].[TONE_AND_CALIBER_ID] = [IL].[TONE_AND_CALIBER_ID]
								WHERE
									[S].[LOCATION_SPOT] = @LOCATION_SPOT
									AND [IL].[MATERIAL_ID] = @MATERIAL_ID
									AND [TCM].[TONE] <> @TONE )
    ...
    -- ------------------------------------------------------------------------------------
			IF @PROJECT_ID IS NOT NULL
				AND EXISTS ( SELECT TOP 1
								...
								INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON ([IXL].[LICENSE_ID] = [L].[LICENSE_ID])
								INNER JOIN [FERCO].[OP_WMS_SHELF_SPOTS] [SS] ON ([SS].[LOCATION_SPOT] = [L].[CURRENT_LOCATION])
								WHERE
  ...
  -- ------------------------------------------------------------------------------------
		...
		FROM
			[FERCO].[OP_WMS_SLOTTING_ZONE] [SZ]
		INNER JOIN [FERCO].[OP_WMS_SLOTTING_ZONE_BY_CLASS] [SZC] ON ([SZ].[ID] = [SZC].[ID_SLOTTING_ZONE])
		WHERE
  ...
  -- ------------------------------------------------------------------------------------
		...
		FROM
			[FERCO].[OP_WMS_SLOTTING_ZONE] [SZ]
		INNER JOIN [FERCO].[OP_WMS_SLOTTING_ZONE_BY_SUB_CLASS] [SZC] ON ([SZ].[ID] = [SZC].[ID_SLOTTING_ZONE])
		WHERE
			[SZ].[WAREHOUSE_CODE] = @WAREHPUSE_CODE
			AND [SZ].[ZONE] = @ZONE;
