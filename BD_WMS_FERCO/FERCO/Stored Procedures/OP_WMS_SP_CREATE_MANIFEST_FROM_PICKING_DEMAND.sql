...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_CREATE_MANIFEST_FROM_PICKING_DEMAND]
					@VEHICLE_CODE = 32
					,@LOGIN = 'BETO'
...
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_CREATE_MANIFEST_FROM_PICKING_DEMAND (@VEHICLE_CODE INT
, @LOGIN VARCHAR(50)
, @XML XML)
AS
BEGIN
  ...
  BEGIN TRY
    ...
    -- ------------------------------------------------------------------------------------
    SELECT TOP 1
      @DISTRIBUTION_CENTER_ID = [W].[DISTRIBUTION_CENTER_ID]
    ...
    FROM [FERCO].[OP_WMS_VEHICLE] [V]
    WHERE [V].[VEHICLE_CODE] = @VEHICLE_CODE

    -- ------------------------------------------------------------------------------------
    -- Obtiene el detalle del manifiesto
    -- ------------------------------------------------------------------------------------
    INSERT INTO @MANIFEST_DETAIL ([WAVE_PICKING_ID]
    ...
    BEGIN
      ...
      -- ------------------------------------------------------------------------------------
      INSERT INTO [FERCO].[OP_WMS_MANIFEST_HEADER] ([DRIVER]
      , [VEHICLE]
      , [DISTRIBUTION_CENTER]
      , [CREATED_DATE]
      , [STATUS]
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO [FERCO].[OP_WMS_MANIFEST_DETAIL] ([MANIFEST_HEADER_ID]
    , [CODE_ROUTE]
    , [CLIENT_CODE]
    , [WAVE_PICKING_ID]
    , [MATERIAL_ID]
    ...
    , [CERTIFICATION_TYPE])
      ...
      INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [DH]
        ON ([DH].[PICKING_DEMAND_HEADER_ID] = [MD].[PICKING_DEMAND_HEADER_ID])
      INNER JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [DD]
        ON ([DD].[PICKING_DEMAND_HEADER_ID] = [DH].[PICKING_DEMAND_HEADER_ID])
