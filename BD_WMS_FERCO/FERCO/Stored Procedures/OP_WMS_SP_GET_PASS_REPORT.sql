...
-- Ejemplo de Ejecucion:
		...
		EXEC [FERCO].[OP_WMS_SP_GET_PASS_REPORT]
		...
		SELECT * FROM [FERCO].[OP_WMS3PL_PASSES] [owpp]
		SELECT * FROM [FERCO].[OP_WMS_PASS_DETAIL] [owpd]

*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_PASS_REPORT] (
		@START_DATE DATETIME
		,@END_DATE DATETIME
...
BEGIN
  ...
  --
	SELECT
		...
		,[PDH].[ERP_REFERENCE]
	FROM
		[FERCO].[OP_WMS3PL_PASSES] AS [P]
	INNER JOIN @LOGINS [L] ON (
								[L].[LOGIN_ID] = [P].[CREATED_BY]
								OR [L].[LOGIN_ID] = [P].[LAST_UPDATED_BY]
								)
	INNER JOIN [FERCO].[OP_WMS_PILOT] AS [PIL] ON ([P].[DRIVER_ID] = [PIL].[PILOT_CODE])
	LEFT JOIN [FERCO].[OP_WMS_PASS_DETAIL] AS [PD] ON ([PD].[PASS_HEADER_ID] = [P].[PASS_ID])
	LEFT JOIN [FERCO].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH] ON ([PDH].[PICKING_DEMAND_HEADER_ID] = [PD].[PICKING_DEMAND_HEADER_ID])
	WHERE
