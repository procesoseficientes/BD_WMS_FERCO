...
-- Ejemplo de Ejecucion:
         EXEC [FERCO].[OP_WMS_SP_GET_RECEPTION_DETAILS_BY_SAP_R3] @RECEPTION_HEADER = 104
*/
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_RECEPTION_DETAILS_BY_SAP_R3] (
...
BEGIN
	...
	SELECT
		@DOC_ID [EBELN]
		,[P].[BUKRS] [BUKRS]
		, --este campo esta pendiente de definir de donde se va a extraer y si es importante para esta rfc
		...
		,[P].[MSEHL] [MSEHL]
		,'101' [BSART]
		,CASE	WHEN [P].[RESWK] = '' THEN 'MAT-NUEVO'
				ELSE [P].[CHARG]
			END [CHARG]
		,'101' [MOVE_TYPE]
		,'OC' [INPUT_TYPE]
	...
	INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [T].[SERIAL_NUMBER] = [H].[TASK_ID]
	INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON [M].[MATERIAL_ID] = [D].[MATERIAL_ID]
	INNER JOIN [SWIFT_R3_INTER].[dbo].[RFC_PURCHASES] [P] ON [P].[EBELN] = [H].[DOC_ID]
