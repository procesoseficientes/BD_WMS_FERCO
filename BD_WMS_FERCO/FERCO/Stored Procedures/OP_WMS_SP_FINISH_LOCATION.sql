...
-- Ejemplo de Ejecucion:
  ...
  EXEC [FERCO].[OP_WMS_SP_FINISH_LOCATION] @LOGIN = 'ACAMACHO'
                                             ,@TASK_ID = 7
                                             ,@LOCATION = 'B02-P01-F01-NU'
                                             ,@RESULT = ''
  
			SELECT * FROM [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [D]
          INNER JOIN  [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [H] 
        ON [D].[PHYSICAL_COUNT_HEADER_ID] = [H].[PHYSICAL_COUNT_HEADER_ID] WHERE H.TASK_ID = 7
      SELECT * FROM [FERCO].OP_WMS_TASK
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_FINISH_LOCATION] (
		@LOGIN VARCHAR(25)
		,@TASK_ID INT
		,@LOCATION VARCHAR(25)
  ...
  --
	...
	BEGIN
      ...
      ---------------------------------------------------------------------------------  
			...
			INNER JOIN [FERCO].[OP_WMS_LICENSES] [L] ON [L].[CURRENT_LOCATION] = [D].[LOCATION]
			...
			LEFT JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_EXECUTION] [E] ON (
      ...
      ---------------------------------------------------------------------------------  
			...
			INNER JOIN [FERCO].[OP_WMS_MATERIAL_X_SERIAL_NUMBER] [S] ON (
      ...
      ---------------------------------------------------------------------------------  
			...
			IF NOT EXISTS ( SELECT TOP 1
								1
							FROM
								[FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [CD]
							INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER] [CH] ON [CD].[PHYSICAL_COUNT_HEADER_ID] = [CH].[PHYSICAL_COUNT_HEADER_ID]
							INNER JOIN [FERCO].[OP_WMS_TASK] [T] ON [CH].[TASK_ID] = [T].[TASK_ID]
							WHERE
								[CH].[TASK_ID] = @TASK_ID
								AND (
			...
			BEGIN
				UPDATE
					[FERCO].[OP_WMS_PHYSICAL_COUNTS_HEADER]
				SET	
					[STATUS] = 'COMPLETED'
				WHERE
					[TASK_ID] = @TASK_ID;
