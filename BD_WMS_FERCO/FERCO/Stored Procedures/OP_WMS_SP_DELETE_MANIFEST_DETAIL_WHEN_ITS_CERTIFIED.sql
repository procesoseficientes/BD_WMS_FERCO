...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_DELETE_MANIFEST_DETAIL_WHEN_ITS_CERTIFIED]
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_DELETE_MANIFEST_DETAIL_WHEN_ITS_CERTIFIED (@MANIFEST_HEADER_ID INT
...
BEGIN
  ...
  BEGIN TRY
    ...
    -- ------------------------------------------------------------------------------------
    SELECT
      [VALUE] [PICKING_DEMAND_HEADER_ID] INTO #DEMAND
    FROM [FERCO].[OP_WMS_FN_SPLIT](@DEMAND_HEADER_ID, '|')

    -- ------------------------------------------------------------------------------------
    -- Obtiene los IDs de manifiesto detalle a eliminar de picking labels x manifest
    -- ------------------------------------------------------------------------------------
    SELECT
      [MD].[MANIFEST_DETAIL_ID] INTO #MANIFEST_DETAIL
    FROM [FERCO].[OP_WMS_MANIFEST_DETAIL] [MD]
    INNER JOIN [#DEMAND] [D]
      ON [D].[PICKING_DEMAND_HEADER_ID] = [MD].[PICKING_DEMAND_HEADER_ID]
