...
-- Ejemplo de Ejecucion:
		EXEC	[FERCO].[OP_WMS_SP_VALIDATE_TASK_STATUS] @LOGIN = 'ACAMACHO',
            @SERIAL_NUMBER = 0,
            @TASK_ID  = 16,
            @WAVE_PICKING_ID = 0,
            @MATERIAL_ID = '',
...
SELECT *
          ...
          select * FROM [FERCO].[OP_WMS_TASK_LIST] [T]
          WHERE [T].[TASK_ASSIGNEDTO] = 'BCORADO'
          AND [T].[WAVE_PICKING_ID] = 4417
          AND [T].[MATERIAL_ID] = 'FERCO/100003'
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_VALIDATE_TASK_STATUS] (@LOGIN VARCHAR(50),
@SERIAL_NUMBER INT,
@TASK_ID INT,
@WAVE_PICKING_ID INT,
  ...
  BEGIN TRY
    ...
    BEGIN
      ...
      IF NOT EXISTS (SELECT TOP 1
          ...
          INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [D]
