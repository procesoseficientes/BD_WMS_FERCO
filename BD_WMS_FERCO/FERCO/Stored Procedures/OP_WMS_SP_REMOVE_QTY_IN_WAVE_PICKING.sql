...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_REMOVE_QTY_IN_WAVE_PICKING]
...
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_REMOVE_QTY_IN_WAVE_PICKING (@WAVE_PICKING_ID NUMERIC(18, 0)
...
BEGIN
  ...
  --
  DECLARE @SERIAL_NUMBER NUMERIC(18, 0) = 0
  ...
  BEGIN TRY
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO @TASK ([SERIAL_NUMBER]
    , [QUANTITY_PENDING]
    , [QUANTITY_ASSIGNED])
      SELECT
        [TL].[SERIAL_NUMBER]
    ...
    BEGIN
      ...
      --
      UPDATE @TASK
      SET [QUANTITY_PENDING] =
          CASE
            WHEN @REMOVE_QTY >= @QUANTITY_ASSIGNED THEN @QUANTITY_PENDING
            ELSE (@QUANTITY_PENDING - @REMOVE_QTY)
          END
         ,[QUANTITY_ASSIGNED] =
          CASE
            WHEN @REMOVE_QTY >= @QUANTITY_ASSIGNED THEN @QUANTITY_ASSIGNED
            ELSE (@QUANTITY_ASSIGNED - @REMOVE_QTY)
          END
         ,[IS_COMPLETED] =
          CASE
            WHEN @REMOVE_QTY >= @QUANTITY_ASSIGNED THEN 1
            ELSE 0
          END
         ,[IS_CANCELED] =
          CASE
         ...
         ,[CANCELED_DATETIME] =
          CASE
            WHEN @REMOVE_QTY >= @QUANTITY_ASSIGNED THEN GETDATE()
            ELSE NULL
          END
         ,[CANCELED_BY] =
          CASE
            WHEN @REMOVE_QTY >= @QUANTITY_ASSIGNED THEN @LOGIN
            ELSE NULL
          END
         ,[USED] = 1
    ...
    -- ------------------------------------------------------------------------------------
    UPDATE [TL]
    SET [TL].[QUANTITY_PENDING] = [T].[QUANTITY_PENDING]
    ...
    FROM [FERCO].[OP_WMS_TASK_LIST] [TL]
    INNER JOIN @TASK [T]
      ON ([T].[SERIAL_NUMBER] = [TL].[SERIAL_NUMBER])
