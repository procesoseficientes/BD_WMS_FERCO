...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_SP_GET_PHYSICAL_COUNTS] @START_DATETIME = '2016-02-24'
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_PHYSICAL_COUNTS] (
...
BEGIN
  ...
  --
	...
	INTO
		[#WAREHOUSE]
	FROM
		[FERCO].[OP_WMS_FN_SPLIT](@CODE_WAREHOUSE, '|') [W];
	...
	INTO
		[#LOGIN]
	FROM
		[FERCO].[OP_WMS_FN_SPLIT](@USERS_ASSIGNED_TO, '|') [L];
	...
	SELECT
		...
		/ CAST(COUNT([D].[PHYSICAL_COUNT_DETAIL_ID]) AS NUMERIC(9,
		...
		,COUNT([E].[PHYSICAL_COUNTS_EXECUTION_ID]) [TOTAL_CONTADOS]
		,SUM(CASE [E].[HIT_OR_MISS]
				WHEN 'H' THEN 1
				ELSE 0
				END) [TOTAL_HITS]
	...
	LEFT JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_EXECUTION] [E] ON [D].[PHYSICAL_COUNT_DETAIL_ID] = [E].[PHYSICAL_COUNT_DETAIL_ID]
	WHERE
	...
	FROM
		[#COUNTING_HEADER] [H]
	INNER JOIN [FERCO].[OP_WMS_PHYSICAL_COUNTS_DETAIL] [D] ON [D].[PHYSICAL_COUNT_HEADER_ID] = [H].[PHYSICAL_COUNT_HEADER_ID]
	INNER JOIN [FERCO].[OP_WMS_WAREHOUSE_BY_USER] [WU] ON [WU].[WAREHOUSE_ID] = [D].[WAREHOUSE_ID]
	...
	LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [MD] ON [MD].[MATERIAL_ID] = [D].[MATERIAL_ID]
	LEFT JOIN [FERCO].[OP_WMS_MATERIALS] [ME] ON [ME].[MATERIAL_ID] = [E].[MATERIAL_ID]
	WHERE
		(
