...
-- Modificacion 1/26/2018 @ Reborn-Team Sprint Trotzdem
					-- diego.as
					-- Se agregan campos por recepcion de ferco
...
-- Ejemplo de Ejecucion:
        EXEC [FERCO].[OP_WMS_SP_INSERT_ERP_RECEPTION_DOCUMENT_DETAIL]
			@ERP_RECEPTION_DOCUMENT_HEADER_ID = 3
      ,@MATERIAL_ID = 'ferco/CHOCOLATE'
      ,@QTY = 46      
--
		SELECT * FROM [FERCO].OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL 
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_INSERT_ERP_RECEPTION_DOCUMENT_DETAIL]
	(
...
AS
	...
	-------------------------------------------------------------------------------------------------------------
	--Si el tipo de recepcion es 'DEVOLUCION_FACTURA' entonces se obtiene la Bodega por defecto en configurations
	-------------------------------------------------------------------------------------------------------------

	IF((SELECT TYPE FROM [FERCO].OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER WHERE ERP_RECEPTION_DOCUMENT_HEADER_ID = @ERP_RECEPTION_DOCUMENT_HEADER_ID) = 'DEVOLUCION_FACTURA')
	
	SELECT
        @WAREHOUSE_CODE = ISNULL([C].[TEXT_VALUE], @WAREHOUSE_CODE)
    FROM
        [FERCO].[OP_WMS_CONFIGURATIONS] AS [C]
    WHERE
        [C].[PARAM_NAME] = 'ERP_WAREHOUSE_INVOICE_RETURN_ORDER';
