...
-- Ejemplo de Ejecucion:
        EXEC [FERCO].[OP_WMS_SP_GET_TASKS]
			@TASK_LIST = '
				
			'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_TASKS_BY_WAVE_PICKING] (@TASK_LIST XML)
AS
	BEGIN
		--
		SET NOCOUNT ON;
		...
		SELECT
		 ...
		 ,[x].[Rec].[query]('./MATERIAL_ID').[value]('.', 'VARCHAR(50)')     
		...
		INNER JOIN @TASK [T] ON(
			...
			AND ([T].[MATERIAL_ID] IS NULL OR [TL].[MATERIAL_ID] = [T].[MATERIAL_ID])
		)
	END;