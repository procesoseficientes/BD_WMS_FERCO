...
-- Description:			    Obtiene el slotting configurado
--/*
-- Ejemplo de Ejecucion:
--        EXEC [FERCO].[OP_WMS_GET_SLOTTING] @LOGIN = 'RUDI'
--*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_SLOTTING]
(
    @LOGIN VARCHAR(25),
...
BEGIN
    ...
    SELECT [Z].[WAREHOUSE_CODE],
           ...
           COUNT([SC].[ID]) AS [FAMILY]
    FROM [FERCO].[OP_WMS_ZONE] [Z]
        INNER JOIN @WAREHOUSE_TABLE [WT]
            ON ([Z].[WAREHOUSE_CODE] = [WT].[CODE_WAREHOUSE])
        LEFT JOIN [FERCO].[OP_WMS_SLOTTING_ZONE] [SZ]
            ON [Z].[ZONE_ID] = [SZ].[ZONE_ID]
        LEFT JOIN [FERCO].[OP_WMS_SLOTTING_ZONE_BY_CLASS] [SC]
            ON [SZ].[ID] = [SC].[ID_SLOTTING_ZONE]
