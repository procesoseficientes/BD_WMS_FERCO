...
-- Ejemplo de Ejecucion:
         EXEC [FERCO].[OP_WMS_SP_GET_RETURN_FOR_RESERVE_DETAILS_BY_SAP_R3] @RECEPTION_HEADER = 104
*/
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_RETURN_FOR_RESERVE_DETAILS_BY_SAP_R3] (
...
BEGIN
	...
	INTO
		[#TASK]
	FROM
		[FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [PDH]
	INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [PDH].[TASK_ID] = [T].[SERIAL_NUMBER]
	WHERE
	...
	INTO
		[#RESERVED]
	FROM
		[FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_DETAIL] [PDD]
	INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [PDH] ON [PDH].[ERP_RECEPTION_DOCUMENT_HEADER_ID] = [PDD].[ERP_RECEPTION_DOCUMENT_HEADER_ID]
	...
	SELECT
		...
		,(SELECT TOP 1
				[TRA].[STATUS_CODE]
			FROM
				[FERCO].[OP_WMS_TRANS] [TRA]
			INNER JOIN [FERCO].[OP_WMS_TASK_LIST] [T] ON [TRA].[TASK_ID] = [T].[SERIAL_NUMBER]
			INNER JOIN [FERCO].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] [RDH] ON [RDH].[TASK_ID] = [T].[SERIAL_NUMBER]
			WHERE
