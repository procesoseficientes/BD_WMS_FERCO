...
-- Ejemplo de Ejecucion:
		...
		--
		EXEC [FERCO].[OP_WMS_SP_REGISTER_REALLOC_FOR_NO_IMMEDIATE_PICKING] 
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_REALLOC_FOR_NO_IMMEDIATE_PICKING] (
		@LOGIN_ID VARCHAR(25)
...
BEGIN
    ...
    -- ----------------------------------------------------------------------------------
		...
		LEFT	 JOIN [FERCO].[OP_WMS_TONE_AND_CALIBER_BY_MATERIAL] [T] ON [T].[TONE_AND_CALIBER_ID] = [IL].[TONE_AND_CALIBER_ID]
		WHERE
			[IL].[LICENSE_ID] = @SOURCE_LICENSE
			AND [IL].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL;
		...
		VALUES
				(
					...
					,(SELECT TOP 1
							[LOGIN_NAME]
						FROM
							[FERCO].[OP_WMS_FUNC_GETLOGIN_NAME](@LOGIN_ID))
					,'TAREA_REUBICACION'
					,ISNULL((SELECT
									[PARAM_CAPTION]
								FROM
									[FERCO].[OP_WMS_FUNC_GETTRANS_DESC]('TAREA_REUBICACION')),
							'TAREA_REUBICACION')
					,NULL
					,@MATERIAL_BARCODE
					,@pMATERIAL_ID_LOCAL
					,(SELECT TOP 1
							[MATERIAL_NAME]
						FROM
							[FERCO].[OP_WMS_FUNC_GETMATERIAL_DESC](@MATERIAL_BARCODE,
											@pCLIENT_ID_LOCAL))
					,NULL
    ...
    ---------------------------------------------------------------------------------  
      ...
      ---------------------------------------------------------------------------------
			INSERT	INTO @AVAILABLE_PICKING_LICENSE
					...
					EXEC [FERCO].[OP_WMS_SP_VALIDATE_IF_PICKING_LICENSE_IS_AVAILABLE] @WAVE_PICKING_ID = @WAVE_PICKING_ID,
