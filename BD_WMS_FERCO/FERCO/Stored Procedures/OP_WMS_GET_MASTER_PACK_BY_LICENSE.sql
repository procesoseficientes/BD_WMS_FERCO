...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].[OP_WMS_GET_MASTER_PACK_BY_LICENSE] @LICENSE_ID = 177679 , @MATERIAL_ID = 'FERCO/SKUPRUEBA'
    SELECT * FROM [FERCO].OP_WMS_MASTER_PACK_HEADER [MPH]

  ARREGLAR MASTERPACK ANTERIORES 
  SELECT * FROM [FERCO].[OP_WMS_MASTER_PACK_HEADER] [OWMPH]
--UPDATE [FERCO].[OP_WMS_MASTER_PACK_HEADER] SET [QTY] = 1 WHERE [EXPLODED] = 0 AND [QTY] IS NULL 
--UPDATE [FERCO].[OP_WMS_MASTER_PACK_HEADER] SET [QTY] = 0 WHERE [EXPLODED] = 1 AND [QTY] IS NULL  

*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_GET_MASTER_PACK_BY_LICENSE] (
		@LICENSE_ID NUMERIC
...
BEGIN
  ...
  --
	...
	INNER JOIN [FERCO].[OP_WMS_MATERIALS] [M] ON ([M].[MATERIAL_ID] = [MPH].[MATERIAL_ID])
