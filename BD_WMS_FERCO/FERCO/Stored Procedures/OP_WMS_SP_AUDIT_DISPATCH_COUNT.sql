...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_AUDIT_DISPATCH_COUNT] @pAUDIT_ID = 3901
                                             ,@pMETHOD = 'INPUT'
...
CREATE PROCEDURE FERCO.OP_WMS_SP_AUDIT_DISPATCH_COUNT @pAUDIT_ID NUMERIC(18, 0),
@pMETHOD VARCHAR(25),
  ...
  BEGIN
    BEGIN TRY
      BEGIN
        SELECT
          @pCLIENT_ID = (SELECT TOP 1
            ...
            WHERE NUMERO_ORDEN = (SELECT
                NUMERO_ORDEN
              FROM [FERCO].OP_WMS_AUDIT_DISPATCH_CONTROL
        ...
        IF EXISTS (SELECT
              1
            FROM [FERCO].OP_WMS_AUDIT_DISPATCH_SERIES
            WHERE AUDIT_ID = @pAUDIT_ID
        ...
        BEGIN
          ...
          BEGIN
            ...
            --                INNER JOIN [FERCO].OP_WMS_INV_X_LICENSE IV
            ...
            FROM [FERCO].OP_WMS_AUDIT_DISPATCH_SKUS S
            ...
            FROM [FERCO].OP_WMS_AUDIT_DISPATCH_SKUS
            WHERE CODIGO_POLIZA = @pCODIGO_POLIZA
            AND MATERIAL_ID = @pMATERIAL_ID
            AND BATCH = @BATCH
            AND DATE_EXPIRATION = @DATE_EXPIRATION
        ...
        INSERT INTO [FERCO].[OP_WMS_AUDIT_DISPATCH_SKUS] ([AUDIT_ID]
        , [CODIGO_POLIZA]
        , [MATERIAL_ID]
        , [BARCODE_ID]
        , [MATERIAL_NAME]
        ...
        , BATCH
        , DATE_EXPIRATION)
          VALUES (@pAUDIT_ID, @pCODIGO_POLIZA, @pMATERIAL_ID, @pBARCODE_ID, (SELECT MATERIAL_NAME FROM [FERCO].OP_WMS_MATERIALS WHERE MATERIAL_ID = @pMATERIAL_ID), @pSCANNED, @pINPUTED, CURRENT_TIMESTAMP, @pLOGIN_ID, @BATCH, @DATE_EXPIRATION)

        IF (@pMETHOD = 'SCANNING')
        BEGIN
        BEGIN TRY
          BEGIN
            INSERT INTO [FERCO].[OP_WMS_AUDIT_DISPATCH_SERIES] ([AUDIT_ID]
            , [SERIAL_NUMBER]
            , [MATERIAL_ID]
            , [LAST_UPDATED])
    ...
    BEGIN CATCH
      ...
      IF (@pMETHOD = 'INPUT')
      BEGIN
        UPDATE [FERCO].[OP_WMS_AUDIT_DISPATCH_SKUS]
      ...
      BEGIN TRY
        BEGIN
          --SELECT @pMATERIAL_ID = ISNULL((SELECT TOP 1 MATERIAL_ID FROM [FERCO].OP_WMS_MATERIALS 
