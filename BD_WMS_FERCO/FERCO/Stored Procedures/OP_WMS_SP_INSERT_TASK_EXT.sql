...
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_INSERT_TASK_EXT (@WAVE_PICKING_ID DECIMAL
, @QTY NUMERIC(18, 2)
, @LICENCE_ID NUMERIC
...
BEGIN
  ...
  , [MATERIAL_SHORT_NAME]
  , [IS_lOCKED])
    VALUES (@WAVE_PICKING_ID, 0, 'TAREA_PICKING', 'DESPACHO_GENERAL', @LOGIN_ID, '', 'OLA DE PICKING #' + CONVERT(VARCHAR(25), @WAVE_PICKING_ID), GETDATE(), 0, @QTY, '', @CODIGIO_POLIZA_TARGET, @LICENCE_ID, 'GENERAL', 1, 0, 0, 0, @MATERIAL_ID, @BARCODE_ID, @ALTERNATE_BARCODE, @MATERIAL_NAME, @CURRENT_WAREHOUSE, @CURRENT_LOCATION, @CLIENT_OWNER, @CLIENT_NAME, GETDATE(), GETDATE(), @MATERIAL_NAME, 0);
END
