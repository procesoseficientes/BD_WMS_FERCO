...
-- Ejemplo de Ejecucion:
		...
		--
		EXEC [FERCO].[OP_WMS_SP_REGISTER_DISPATCH_GENERAL] 
...
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_REGISTER_REALLOC_FOR_REPLENISHMENT] (
     @LOGIN_ID VARCHAR(25)
...
BEGIN
  ...
  --DECLARE @IS_FROM_SONDA INT;
    ...
    -- ----------------------------------------------------------------------------------
        SELECT
            @TERMS_OF_TRADE = (SELECT TOP 1
                               ...
                               FROM
                                [FERCO].[OP_WMS_LICENSES] [L]
                               INNER JOIN [FERCO].[OP_WMS_INV_X_LICENSE] [IL] ON ([IL].[LICENSE_ID] = [IL].[LICENSE_ID])
                               WHERE
									[L].[LICENSE_ID] = @SOURCE_LICENSE);
    ...
    -- ----------------------------------------------------------------------------------
        ...
        FROM
            [FERCO].[OP_WMS_INV_X_LICENSE] [IL]
		LEFT JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] [S] ON [S].[STATUS_ID] = [IL].[STATUS_ID]
        WHERE
            [IL].[LICENSE_ID] = @SOURCE_LICENSE
            AND [IL].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL;
    ...
    -- ----------------------------------------------------------------------------------
      ...
      -- ----------------------------------------------------------------------------------
            ...
            EXEC [FERCO].[OP_WMS_SP_AGREGA_SKU_LICENCIA] @pLICENSE_ID = @NEW_LICENSE_ID,
      ...
      -- ----------------------------------------------------------------------------------
        ...
        -- ----------------------------------------------------------------------------------
                ...
                EXEC [FERCO].[OP_WMS_SP_CREA_LICENCIA] @pCODIGO_POLIZA = @pCODIGO_POLIZA,
                    @pLOGIN = @LOGIN_ID,
			...
			SELECT @STATUS_NAME = [S].[STATUS_CODE]
			 FROM
				[FERCO].[OP_WMS_INV_X_LICENSE] [il]
				INNER JOIN [FERCO].[OP_WMS_STATUS_OF_MATERIAL_BY_LICENSE] S ON [S].[STATUS_ID] = [il].[STATUS_ID]
			WHERE
				[il].[LICENSE_ID] = @NEW_LICENSE_ID
				AND [il].[MATERIAL_ID] = @pMATERIAL_ID_LOCAL;  
    ...
    -- ----------------------------------------------------------------------------------
        EXEC [FERCO].[OP_WMS_SP_REGISTER_INV_TRANS] @pTRADE_AGREEMENT = @TERMS_OF_TRADE,
			@pLOGIN_ID = @LOGIN_ID,
    ...
    --    -- ----------------------------------------------------------------------------------
    --    INSERT INTO [FERCO].[OP_WMS_TRANS] ([TERMS_OF_TRADE]
    --    , [TRANS_DATE]
    --    , [LOGIN_ID]
    --    , [LOGIN_NAME]
    --    , [TRANS_TYPE]
    ...
    --    , [BATCH]
    --    , DATE_EXPIRATION)
    --      VALUES ((SELECT TOP 1 TERMS_OF_TRADE FROM [FERCO].OP_WMS_INV_X_LICENSE WHERE LICENSE_ID = @SOURCE_LICENSE AND MATERIAL_ID = @pMATERIAL_ID_LOCAL), GETDATE(), @LOGIN_ID, (SELECT TOP 1 * FROM [FERCO].[OP_WMS_FUNC_GETLOGIN_NAME](@LOGIN_ID)), 'TAREA_REABASTECIMIENTO', ISNULL((SELECT PARAM_CAPTION FROM [FERCO].OP_WMS_FUNC_GETTRANS_DESC('TAREA_REABASTECIMIENTO')), 'TAREA_REABASTECIMIENTO'), NULL, @MATERIAL_BARCODE, @pMATERIAL_ID_LOCAL, (SELECT * FROM [FERCO].OP_WMS_FUNC_GETMATERIAL_DESC(@MATERIAL_BARCODE, @pCLIENT_ID_LOCAL)), NULL, NULL, @SOURCE_LICENSE, NULL, @SOURCE_LOCATION, 'PUERTA_1', @pCLIENT_ID_LOCAL, (SELECT * FROM [FERCO].OP_WMS_FUNC_GETCLIENT_NAME(@pCLIENT_ID_LOCAL)), (@QUANTITY_UNITS * -1), ISNULL((SELECT ISNULL([WAREHOUSE_PARENT], 'BODEGA_DEF') FROM [FERCO].OP_WMS_SHELF_SPOTS WHERE LOCATION_SPOT = @SOURCE_LOCATION), 'BODEGA_DEF'), NULL, 'TAREA_REABASTECIMIENTO', @pCODIGO_POLIZA, @SOURCE_LICENSE, 'PROCESSED', @WAVE_PICKING_ID, @WAVE_PICKING_ID, 0, @BATCH, @DATE_EXPIRATION)
    ...
    -- ----------------------------------------------------------------------------------
        ...
        BEGIN
            EXEC [FERCO].[OP_WMS_INSERT_MASTER_PACK_BY_REALLOC_PARTIAL] @SOURCE_LICENSE = @SOURCE_LICENSE,
    ...
    -- ----------------------------------------------------------------------------------    
        ...
        EXEC [FERCO].[OP_WMS_SP_MASTER_PACK_CASCADE_EXPLODE_IN_REPLENISHMENT] @MATERIAL_ID = @pMATERIAL_ID_LOCAL,
