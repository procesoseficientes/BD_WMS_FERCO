...
-- Ejemplo de Ejecucion:
			EXEC [FERCO].OP_WMS_SP_GET_DELIVERY_COMPLIANCE_REPORT @VEHICLE_CODES = '17|19|21|25' ,@PILOT_CODES = '20', @START_DATE = '2017-10-19 6:50:05 PM' , @END_DATE = '2017-11-10 11:09:53 AM'
*/
-- =============================================
CREATE PROCEDURE [FERCO].[OP_WMS_SP_GET_DELIVERY_COMPLIANCE_REPORT] (
...
BEGIN
  ...
  --
	...
	SELECT DISTINCT
		...
		,[V].[PLATE_NUMBER]
	FROM
		[FERCO].[OP_WMS_FUNC_SPLIT_3](@VEHICLE_CODES, '|') [VS]
	INNER JOIN [FERCO].[OP_WMS_VEHICLE] [V] ON [VS].[VALUE] = [V].[VEHICLE_CODE];
	...
	SELECT DISTINCT
		...
		,[P].[LAST_NAME]
	FROM
		[FERCO].[OP_WMS_FUNC_SPLIT_3](@PILOT_CODES, '|') [PS]
	INNER JOIN [FERCO].[OP_WMS_PILOT] [P] ON [PS].[VALUE] = [P].[PILOT_CODE];
	...
	INSERT	INTO [#MANIFEST_STATUS_AND_PERCENTAGE]
			EXEC [FERCO].[OP_WMS_SP_GET_MANIFEST_PERCENTAGE_AND_STATUS_BY_DEMAND_PICKING] @VEHICLE_CODES = @VEHICLE_CODES,
...
INSERT INTO #DELIVERY_COMPLIANCE_REPORT
    SELECT
     ...
     ,[T].[EXPECTED_GPS]
     ,[T].[POSTED_GPS]
     ,[FERCO].[OP_WMS_FN_CALCULATE_DISTANCE]([T].[EXPECTED_GPS], T.[POSTED_GPS]) AS [DISTANCE]
     ,[T].[ACCEPTED_STAMP]
     ,[T].[COMPLETED_STAMP]
