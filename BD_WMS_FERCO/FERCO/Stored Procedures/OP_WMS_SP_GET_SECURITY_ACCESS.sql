...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_GET_SECURITY_ACCESS]
					@PARENT = 'POLIZAS_EXPIRADAS'
					,@CATEGORY = 'SCREEN_SECURITY'
					,@LOGIN = 'BETO'
*/
-- =============================================
CREATE PROCEDURE FERCO.OP_WMS_SP_GET_SECURITY_ACCESS (@PARENT VARCHAR(25)
, @CATEGORY VARCHAR(25)
, @LOGIN VARCHAR(25))
AS
BEGIN
  ...
  SELECT
   ...
   ,[C].[MPC_4]
   ,[C].[MPC_5]
  FROM [FERCO].[OP_WMS_CHECKPOINTS] [C]
  INNER JOIN [FERCO].[OP_WMS_ROLES_JOIN_CHECKPOINTS] [JC]
    ON ([JC].[CHECK_ID] = [C].[CHECK_ID])
  INNER JOIN [FERCO].[OP_WMS_LOGINS] [L]
    ON ([L].[ROLE_ID] = [JC].[ROLE_ID])
