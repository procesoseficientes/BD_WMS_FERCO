...
-- Ejemplo de Ejecucion:
				EXEC [FERCO].[OP_WMS_SP_CREATE_TICKET]
*/
-- =============================================
CREATE PROCEDURE [OP_WMS_SP_CREATE_TICKET]
AS
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        ...
        ---------------------------------------------------------------------------------
        SELECT @DATE_FORMAT = FORMAT(@ADD_DATE, 'dd/MM/yyyy hh:mm:ss');

        ---------------------------------------------------------------------------------
        -- INSERTAMOS EL NUEVO TICKET Y OBTENEMOS EL CORRELATIVO
        ---------------------------------------------------------------------------------
        INSERT INTO [FERCO].[OP_WMS_TICKETS]
        (
            [POLIZA_DOC_ID],
            [CREATED_DATE],
            [STATUS]
