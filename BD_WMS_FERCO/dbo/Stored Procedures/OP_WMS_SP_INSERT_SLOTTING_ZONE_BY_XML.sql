...
CREATE PROCEDURE [OP_WMS_SP_INSERT_SLOTTING_ZONE_BY_XML] ( @XML AS
                                                              XML )
AS
    BEGIN TRY
  ...
  ---------------------------------------------------------------------------------
        ...
        IF ( EXISTS ( SELECT    1
            ...
            BEGIN
                ...
                SELECT  -1 AS [Resultado] ,
                        CONCAT('No se encontro la familia: ', [F].[CLASS_ID],
                               ', bodega: ', [F].[WAREHOUSE_ID]) AS [Mensaje] ,
                        -1 AS [Codigo]
                FROM    @SLOTTING_ZONE_TABLE [F];
                RETURN;
            END;
  ...
  ---------------------------------------------------------------------------------
        ...
        FROM    [FERCO].[OP_WMS_SLOTTING_ZONE_BY_CLASS] [SZBC]
                INNER JOIN @SLOTTING_ZONE_TABLE [SZT] ON ([SZBC].[CLASS_ID] = [SZT].[CLASS_ID] AND [SZBC].[ID_SLOTTING_ZONE] = [SZT].[SLOTTING_ID]);
