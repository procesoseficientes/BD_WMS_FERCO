CREATE TABLE [dbo].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER] (
    ...
    PRIMARY KEY CLUSTERED ([ERP_RECEPTION_DOCUMENT_HEADER_ID] ASC) WITH (FILLFACTOR = 80, STATISTICS_NORECOMPUTE = ON)
);


GO
CREATE NONCLUSTERED INDEX [IN_OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER_DOC_NUM_IS_FROM_WAREHOUSE_TRANSFER]
    ON [dbo].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER]([DOC_NUM] ASC, [IS_FROM_WAREHOUSE_TRANSFER] ASC)
    INCLUDE([LAST_UPDATE]) WITH (FILLFACTOR = 80, STATISTICS_NORECOMPUTE = ON);


GO
CREATE NONCLUSTERED INDEX [IN_OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER_DOC_NUM_IS_FROM_WAREHOUSE_TRANSFER_TASK_ID]
    ON [dbo].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER]([DOC_NUM] ASC, [IS_FROM_WAREHOUSE_TRANSFER] ASC, [TASK_ID] ASC) WITH (FILLFACTOR = 80, STATISTICS_NORECOMPUTE = ON);


GO
CREATE NONCLUSTERED INDEX [IN_OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER_TASK_ID]
    ON [dbo].[OP_WMS_ERP_RECEPTION_DOCUMENT_HEADER]([TASK_ID] ASC)
    INCLUDE([CODE_CLIENT], [CODE_SUPPLIER], [DOC_NUM], [EXTERNAL_SOURCE_ID], [NAME_SUPPLIER], [OWNER]) WITH (FILLFACTOR = 80, STATISTICS_NORECOMPUTE = ON);
