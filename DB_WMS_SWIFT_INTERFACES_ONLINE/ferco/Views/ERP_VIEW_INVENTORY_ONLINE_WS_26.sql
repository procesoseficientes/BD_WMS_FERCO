...
-- Ejemplo de Ejecucion:
        SELECT * FROM [ferco].[ERP_VIEW_INVENTORY_ONLINE_WS_26]  
*/
-- =============================================
CREATE VIEW [ferco].[ERP_VIEW_INVENTORY_ONLINE_WS_26]
AS (
	...
	FROM OPENQUERY([SAPSERVER], '
		SELECT 
			...
			,SUM(case when whsCode = ''26'' then  OITW.OnHand else 0 end ) ON_HAND_26
		...
		FROM [SBOFERCO].[dbo].[OITW] OITW
			...
			--INNER JOIN [SBOFERCO].[dbo].[OUOM] OUOM ON OUOM.UomEntry = UGP1.UomEntry
				where whsCode IN ( ''26'') AND OITW.OnHand > 0
				GROUP BY 
