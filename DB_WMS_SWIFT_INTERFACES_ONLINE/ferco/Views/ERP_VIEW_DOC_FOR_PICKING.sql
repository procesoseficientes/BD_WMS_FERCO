...
-- Ejemplo de Ejecucion:
				SELECT * FROM [ferco].[ERP_VIEW_DOC_FOR_PICKING]
*/
-- =============================================
CREATE VIEW ferco.ERP_VIEW_DOC_FOR_PICKING
AS
SELECT
  [SAP_REFERENCE]
...
'
  SELECT
   ...
   ,NULL WAREHOUSE_NAME
   ,'''' CODE_OPER --o.U_oper  CODE_OPER
   ,o.SlpCode CODE_SELLER
   ,o.DocDate DOC_DATE
   ,o.DocTotal TOTAL_AMOUNT
  FROM SBOFERCO.dbo.ORDR o
  WHERE o.DocStatus = ''O''
  UNION ALL
  SELECT 
    o.DocNum SAP_REFERENCE
   ...
   ,NULL DOC_DATE
   ,0 TOTAL_AMOUNT
  FROM SBOFERCO.dbo.OWTQ o
  INNER JOIN SBOFERCO.dbo.OWHS o1
    ON o1.WhsCode = (SELECT TOP 1 w1.WhsCode FROM SBOFERCO.dbo.WTQ1 w1 where w1.DocEntry = o.DocEntry)
  WHERE o.DocStatus = ''O''
    ');
