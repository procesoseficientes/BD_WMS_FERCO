...
-- Ejemplo de Ejecucion:
				EXEC [ferco].[ERP_SP_GENERATE_BONUS_LIST_BY_CUSTOMER_AND_SKU]
					@BONUS_TYPE = 3
					,@ORDER = 1
*/
-- =============================================
CREATE PROCEDURE ferco.ERP_SP_GENERATE_BONUS_LIST_BY_CUSTOMER_AND_SKU (@BONUS_TYPE INT
, @ORDER INT)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  IF @ORDER = 1
  BEGIN
    TRUNCATE TABLE [ferco].[ERP_TB_BONUS]
    --
    PRINT '----> Limpio tabla'
  END
  ...
  INSERT INTO #BONUS
    ...
    LEFT JOIN ferco.ERP_TB_BONUS B
      ON (
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO ferco.ERP_TB_BONUS
    SELECT
      [CODE_ROUTE]
     ,[CODE_CUSTOMER]
     ,[SKU]
