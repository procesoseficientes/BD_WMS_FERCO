...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [ferco].[ERP_SP_GENERATE_DISCOUNT_LIST_BY_CUSTOMER]
					@DISCOUNT_TYPE = 2
					,@ORDER = 3
*/
-- =============================================
CREATE PROCEDURE ferco.ERP_SP_GENERATE_DISCOUNT_LIST_BY_CUSTOMER (@DISCOUNT_TYPE INT
, @ORDER INT)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  SELECT
   ...
   ,C.Z_RUTA CODE_ROUTE INTO #CUSTOMER_BY_ROUTE
  ...
  IF @ORDER = 1
  BEGIN
    TRUNCATE TABLE ferco.ERP_TB_DISCOUNT
    --
    PRINT '----> Limpio tabla'
  END
  ...
  BEGIN
    INSERT INTO #DISCOUNT
      SELECT
       ...
       ,S.CODE_SKU SKU
       ,@DISCOUNT DISCOUNT
      FROM ferco.ERP_VIEW_SKU S
      LEFT JOIN ferco.ERP_TB_DISCOUNT D
        ON (
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO ferco.ERP_TB_DISCOUNT
    SELECT
      [CODE_ROUTE]
     ,[CODE_CUSTOMER]
     ,[CODE_SKU]
