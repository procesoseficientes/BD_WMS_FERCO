...
-- Ejemplo de Ejecucion:
			EXEC [ferco].[BULK_DATA_SP_GENERATE_PRICE_LIST_FOR_ROUTE]
*/
-- =============================================
CREATE PROCEDURE ferco.BULK_DATA_SP_GENERATE_PRICE_LIST_FOR_ROUTE
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  IF EXISTS (SELECT
        *
      FROM [SWIFT_EXPRESS].[ferco].[SWIFT_PARAMETER] [SP]
  ...
  BEGIN
    ...
    -- ------------------------------------------------------------------------------------
    TRUNCATE TABLE [SWIFT_EXPRESS].[ferco].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE]
    ...
    INSERT INTO @ROUTES ([CODE_ROUTE])
      SELECT DISTINCT
        [SR].[CODE_ROUTE]
      FROM [SWIFT_EXPRESS].[ferco].[SWIFT_ROUTES] [SR]
      INNER JOIN [SWIFT_EXPRESS].[ferco].[USERS] [U]
    ...
    EXEC [SWIFT_EXPRESS].[ferco].[SWIFT_SP_GENERATE_PRICE_LIST_FOR_ROUTE] @CODE_ROUTE = @CODE_ROUTE
