...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [ferco].[BULK_DATA_SP_IMPORT_PRICE_LIST]
*/
-- =============================================
CREATE PROCEDURE ferco.BULK_DATA_SP_IMPORT_PRICE_LIST
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  WHEN NOT MATCHED
    THEN INSERT ([CODE_PRICE_LIST]
      ...
      , [OWNER]
      , [OWNER_ID])
        VALUES ([SRC].[CODE_PRICE_LIST], [SRC].[NAME_PRICE_LIST], [SRC].[COMMENT], [SRC].[LAST_UPDATE], [SRC].[LAST_UPDATE_BY], [SRC].[OWNER], [SRC].[OWNER_ID]);
END
