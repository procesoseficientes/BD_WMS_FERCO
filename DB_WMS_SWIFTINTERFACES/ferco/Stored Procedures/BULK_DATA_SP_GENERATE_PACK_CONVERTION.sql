...
-- Ejemplo de Ejecucion:
				EXEC [ferco].[BULK_DATA_SP_GENERATE_PACK_CONVERTION]
*/
-- =============================================
CREATE PROCEDURE ferco.BULK_DATA_SP_GENERATE_PACK_CONVERTION
AS
BEGIN
  SET NOCOUNT ON;
  ...
  -- ------------------------------------------------------------------------------------
  MERGE [SWIFT_EXPRESS].[ferco].[SONDA_PACK_UNIT] AS [SPU]
  USING (SELECT
      'Manual' [CODE_PACK_UNIT]) AS [EPU]
  ON [SPU].[CODE_PACK_UNIT] COLLATE DATABASE_DEFAULT = [EPU].[CODE_PACK_UNIT]
  WHEN MATCHED
    THEN UPDATE
      SET [DESCRIPTION_PACK_UNIT] = 'Manual'
         ,[LAST_UPDATE] = GETDATE()
  ...
  WHEN NOT MATCHED
    THEN INSERT ([CODE_PACK_UNIT]
      ...
      , [LAST_UPDATE_BY]
      , [UM_ENTRY])
        VALUES ('Manual', 'Manual', GETDATE(), 'BULK_DATA', -1);

  -- ------------------------------------------------------------------------------------
  --	Genera las unidades de medida
  -- ------------------------------------------------------------------------------------
  MERGE [SWIFT_EXPRESS].[ferco].[SONDA_PACK_CONVERSION] AS [TRG]
  USING (SELECT
      [epc].[CODE_SKU]
     ...
     ,1 [ORDER]
    FROM [SWIFT_EXPRESS].[ferco].[SWIFT_VIEW_ALL_SKU] [epc]) AS [SRC]
