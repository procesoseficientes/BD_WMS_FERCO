...
-- Ejemplo de Ejecucion:
        EXEC [FERCO].[BULK_DATA_SP_IMPORT_SELLER_INTERCOMPANY_FOR_OP_WMS]
		--
		SELECT * FROM [OP_WMS_FERCO].[FERCO].[OP_WMS_SELLER_INTERCOMPANY]
*/
-- =============================================
CREATE PROCEDURE [ferco].[BULK_DATA_SP_IMPORT_SELLER_INTERCOMPANY_FOR_OP_WMS]
AS
BEGIN
	SET NOCOUNT ON;
	--
	...
	SELECT
		...
		,[SOURCE]
		,ISNULL([SERIE], '')
	FROM [SWIFT_INTERFACES_ONLINE].[ferco].[ERP_VIEW_SELLER_SOURCE]
	--
	DELETE FROM [OP_WMS_FERCO].[FERCO].[OP_WMS_SELLER_INTERCOMPANY]
	--
	INSERT INTO [OP_WMS_FERCO].[FERCO].[OP_WMS_SELLER_INTERCOMPANY]
			(
				[MASTER_ID]
				,[SLP_CODE]
				,[SOURCE]
