...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [ferco].[BULK_DATA_SP_IMPORT_SELLER]
*/
-- =============================================
CREATE PROCEDURE ferco.BULK_DATA_SP_IMPORT_SELLER
AS
BEGIN
  SET NOCOUNT ON;
  --
  MERGE [SWIFT_EXPRESS].ferco.[SWIFT_SELLER] [TRG]
  USING (SELECT
      *
  ...
  WHEN NOT MATCHED
    THEN INSERT ([SELLER_CODE]
      ...
      , [SOURCE]
      , [STATUS])
        VALUES ([SRC].[SELLER_CODE], [SRC].[SELLER_NAME], [SRC].[OWNER], [SRC].[OWNER_ID], [SRC].[GPS], [SRC].[SOURCE], 'ACTIVE');
END
