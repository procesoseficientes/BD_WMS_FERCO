...
-- Ejemplo de Ejecucion:
				...
				--
				EXEC [ferco].[SWIFT_SP_GET_INFORMATION_OF_SELLER]				
					@DOC_NUM = '19295, 162'
				--
				SELECT @DOC_NUM
*/
-- =============================================
CREATE PROCEDURE [ferco].SWIFT_SP_GET_INFORMATION_OF_SELLER (@DOC_NUM VARCHAR(MAX))
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  SELECT
    ...
    FROM [SBOFERCO].[dbo].ORDR a
    INNER JOIN [SBOFERCO].[dbo].rdr1 a1
      ON a.docentry = a1.docentry
    INNER JOIN [SBOFERCO].[dbo].oslp b
      ON a.slpcode = b.slpcode
    INNER JOIN [SBOFERCO].[dbo].oshp c
      ON a.trnspcode = c.trnspcode
    INNER JOIN [SBOFERCO].[dbo].ocrd d
      ON a.cardcode = d.cardcode
    INNER JOIN [SBOFERCO].[dbo].octg e
      ON d.groupnum = e.groupnum
    INNER JOIN [SBOFERCO].[dbo].[@SUCURSAL_CCOSTO] f
