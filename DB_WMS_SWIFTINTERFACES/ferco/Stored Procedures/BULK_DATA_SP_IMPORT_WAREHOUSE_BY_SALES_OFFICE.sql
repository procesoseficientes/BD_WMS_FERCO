...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [ferco].[BULK_DATA_SP_IMPORT_WAREHOUSE_BY_SALES_OFFICE]
*/
-- =============================================
CREATE PROCEDURE ferco.BULK_DATA_SP_IMPORT_WAREHOUSE_BY_SALES_OFFICE
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  FROM [SWIFT_EXPRESS].[ferco].[SWIFT_SALES_OFFICE] [O]
  INNER JOIN [SWIFT_INTERFACES_ONLINE].[ferco].[ERP_VIEW_WAREHOUSE_BY_SALES_OFFICE] [WSO]
    ON (
  ...
  FROM [SWIFT_EXPRESS].[ferco].[SWIFT_WAREHOUSES] [W]
