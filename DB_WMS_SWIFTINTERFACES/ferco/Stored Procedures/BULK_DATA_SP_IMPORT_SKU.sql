...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [ferco].[BULK_DATA_SP_IMPORT_SKU]
				--
				SELECT * FROM [ferco].[SWIFT_ERP_SKU]
*/
-- =============================================
...
BEGIN
  ...
  -- ------------------------------------------------------------------------------------
  SELECT
    @CODEE_PRICE_LIST = SWIFT_EXPRESS.ferco.[SWIFT_FN_GET_PARAMETER]('ERP_HARDCODE_VALUES', 'PRICE_LIST')
  --
  SELECT
    @NAME_PRICE_LIST = epl.NAME_PRICE_LIST
  FROM SWIFT_INTERFACES_ONLINE.ferco.ERP_PRICE_LIST epl

  -- ------------------------------------------------------------------------------------
  -- Obtiene coloca el nombre de la lista de preicos
  -- ------------------------------------------------------------------------------------
  UPDATE ferco.SWIFT_ERP_SKU
