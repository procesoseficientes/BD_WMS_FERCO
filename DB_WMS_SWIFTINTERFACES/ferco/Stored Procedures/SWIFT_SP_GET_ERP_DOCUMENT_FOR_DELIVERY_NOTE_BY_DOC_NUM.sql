...
-- Ejemplo de Ejecucion:
				EXEC ferco.SWIFT_SP_GET_ERP_DOCUMENT_FOR_DELIVERY_NOTE_BY_DOC_NUM
					@DATABASE = 'SBOFERCO'
					,@DOC_NUM = 50
...
*/
-- =============================================
CREATE PROCEDURE [ferco].[SWIFT_SP_GET_ERP_DOCUMENT_FOR_DELIVERY_NOTE_BY_DOC_NUM]
(
    @DATABASE VARCHAR(50),
    @DOC_NUM INT,
    ...
    SELECT @QUERY
		...
		FROM (SELECT * FROM OPENQUERY([SAPSERVER],''
          ...
          + N' AS HAS_MASTERPACK
				...
				,' + CAST(@POSTED_STATUS AS VARCHAR) + N' AS POSTED_STATUS 

				'
          + CASE
                WHEN @DATABASE = 'DETALLES'
                     OR @DATABASE = 'CASA_OAKLAND' THEN
                    '
		, cast (NULL as datetime ) AS U_FacFecha 
        ,'''''''' AS U_FacNit 
        ,'''''''' AS U_FacNom 
        ,'''''''' AS U_Sucursal
        ,'''''''' AS U_MotivoAuto
        ...
        ,ISNULL(SOERP.NumAtCard, ''''N/A'''') AS NumAtCard '
			...
			FROM ' + @DATABASE + N'.dbo.ORDR SOERP 
				INNER JOIN ' + @DATABASE
          + N'.dbo.RDR12 R12 ON R12.DocEntry = SOERP.DocEntry
			WHERE SOERP.DocNum = ''''' + CAST(@DOC_NUM AS VARCHAR) + N''''' '')) A';
    --
    PRINT @QUERY;
    --
