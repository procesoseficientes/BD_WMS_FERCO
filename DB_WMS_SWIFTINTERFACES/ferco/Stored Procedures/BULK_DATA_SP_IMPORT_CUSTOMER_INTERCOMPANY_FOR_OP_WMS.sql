...
-- Ejemplo de Ejecucion:
        EXEC [ferco].[BULK_DATA_SP_IMPORT_CUSTOMER_INTERCOMPANY_FOR_OP_WMS]
		--
		SELECT * FROM [OP_WMS_FERCO].[FERCO].[OP_WMS_CUSTOMER_INTERCOMPANY]
*/
-- =============================================
CREATE PROCEDURE [ferco].[BULK_DATA_SP_IMPORT_CUSTOMER_INTERCOMPANY_FOR_OP_WMS]
AS
BEGIN
	SET NOCOUNT ON;
	--
	...
	SELECT
		...
		,[CARD_NAME]
		,[TAX_ID]
	FROM [SWIFT_INTERFACES_ONLINE].[ferco].[ERP_VIEW_CUSTOMER_SOURCE]
	--
	DELETE FROM [OP_WMS_FERCO].[FERCO].[OP_WMS_CUSTOMER_INTERCOMPANY]
	--
	INSERT INTO [OP_WMS_FERCO].[FERCO].[OP_WMS_CUSTOMER_INTERCOMPANY]
			(
				[MASTER_ID]
				,[CARD_CODE]
				,[SOURCE]
